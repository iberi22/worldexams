...
class BundleValidator:
    ...
    def validate_file(self, file_path: str) -> ValidationResult:
        ...
        for i, (header, block) in enumerate(zip(headers, question_blocks), 1):
            ...
            # 2. Check metadata
            competency_tags = ["ICFES", "BNCC", "Aristas", "SNEPE", "APRENDER", "NAP", "EBAU", "PAA", "UNGE", "Competencia"]
            ...
            if not found_competency:
                issues.append(
                    ValidationIssue("HIGH", f"Question {i} missing competency metadata field (e.g., BNCC, ICFES, etc.)", file_path)
                )
