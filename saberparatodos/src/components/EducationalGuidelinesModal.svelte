<script lang="ts">
  ...
  // Future: import MexGuidelinesContent from './MexGuidelinesContent.svelte';

  let { onClose, countryCode = 'CO', grade = 11, subject = 'Matemáticas', period = 1 } = $props();
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-6 pt-0 relative z-10">
      ...
      {:else if isMexico}
        <div class="p-8 text-center">
          ...
          <h3 class="text-xl font-bold text-white mb-2">Lineamientos EXANI-II</h3>
          <p class="text-white/40 text-xs">Contenido localizado para México en preparación...</p>
        </div>
      {:else}
        <div class="p-8 text-center">
          <span class="text-4xl mb-4 block">🌎</span>
          <h3 class="text-xl font-bold text-white mb-2">Lineamientos Regionales</h3>
          <p class="text-white/40 text-xs">La base de datos de lineamientos para esta región se está actualizando.</p>
        </div>
      {/if}
    </div>
