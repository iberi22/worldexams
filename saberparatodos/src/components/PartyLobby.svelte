<script lang="ts">
  ...
  onMount(() => {
    ...
    channel = supabase.channel(`party_lobby:${partyCode}`)
      .on('postgres_changes', {
         event: 'UPDATE',
         schema: 'public',
...
<div class="min-h-screen flex flex-col items-center justify-center p-6 text-[#F5F5DC] animate-fade-in-up">
  <div class="w-full max-w-4xl text-center">
    ...
    <div class="mb-12">
      <h2 class="text-sm uppercase tracking-[0.3em] opacity-60 mb-4">Código de la Party</h2>
      <button
         type="button"
         ...
         onkeydown={(e) => e.key === 'Enter' && navigator.clipboard.writeText(partyCode)}
      >
