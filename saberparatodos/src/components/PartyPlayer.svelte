<script lang="ts">
  ...
  interface Props {
    // Si viene desde Host, ya tiene estos datos
    partyCode: string;
    studentName?: string;
    studentId?: string;
    autoJoin?: boolean;
  ...
  let { partyCode, studentName = '', studentId = '', autoJoin = false, isHost = false }: Props = $props();
    ...
    channel = supabase.channel(`party:${partyCode}`)
      .on('broadcast', { event: 'game_state_update' }, (payload) => {
         ...
         if (data.question_data) {
             // Map ExamView Question format to PartyPlayer format
             const q = data.question_data;
             currentQuestion = {
                 question_index: (data.current_question_index || 0) + 1,
                 question_id: q.id,
                 question_text: q.text,
                 // Ensure options have text property (ExamView uses 'text', PartyPlayer used 'text')
  ...
  function submitAnswer(answer: string) {
    ...
    /*
       Si soy HOST, no necesito enviar mi respuesta por broadcast si solo yo la consumiré,
       pero para mantener consistencia y que aparezca en el dashboard del host (que soy yo mismo en otra vista),
       la enviamos igual.
    */
    channel.send({
      type: 'broadcast',
  ...
  <div class="w-full max-w-lg animate-fade-in-up">
    ...
    {#if viewState === 'joining'}
      ...
      <div class="text-center">
        <h1 class="text-5xl font-black text-emerald-500 mb-2">{partyCode}</h1>
        <p class="text-sm uppercase tracking-widest opacity-60 mb-8">Únete a la party</p>
