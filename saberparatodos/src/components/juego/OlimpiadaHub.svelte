...
<div class="w-full space-y-4" aria-label="Olimpiada trimestral">
  <div class="flex items-center justify-between gap-2">
    <h3 class="text-sm font-bold text-white/90">Olimpiada {season}</h3>
    <span class="text-[10px] font-mono px-2 py-1 rounded-full border border-white/10 text-white/60">
  ...
  {#if elegibilidad.elegible}
    <p class="text-xs text-emerald-300 font-semibold">✓ Clasificas: Oro+ en {elegibilidad.semanasOro} semanas + simulacro válido.</p>
  {:else}
  ...
  {#if elegibilidad.elegible && estado !== 'cerrada'}
    {#if optIn}
      <p class="text-xs text-white/70">Inscrito como competidor anónimo. Tu alias y país aparecerán si ganas — nunca tus notas.</p>
    {:else}
      <button
        type="button"
        onclick={onOptIn}
