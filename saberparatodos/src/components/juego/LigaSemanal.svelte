...
<div class="w-full space-y-3" aria-label="Liga semanal">
  <div class="flex items-center justify-between gap-2">
    <h3 class="text-sm font-bold text-white/90" data-testid="liga-title">Liga {liga.tier} · {liga.weekKey}</h3>
    <span class="text-xs font-mono text-white/60" data-testid="liga-puesto">Puesto {liga.puestoJugador}/30</span>
  </div>

  {#if liga.movimiento === 1}
    <p class="text-xs font-semibold text-emerald-300" data-testid="liga-movimiento">▲ Zona de ascenso — ¡sigue así!</p>
  {:else if liga.movimiento === -1}
    <p class="text-xs font-semibold text-red-300" data-testid="liga-movimiento">▼ Zona de descenso — suma XP para salir.</p>
  {/if}
