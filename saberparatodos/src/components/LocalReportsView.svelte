<script lang="ts">
  ...
  function generateStudyPrompt() {
    ...
    generatedStudyPrompt = generateImprovementPrompt(
      profileData,
      runtimeCountry,
    );
    notebookLMPrompt = generateNotebookLMPrompt(profileData, runtimeCountry);
    notebookLMUpdatePrompt = generateNotebookLMUpdatePrompt(
      profileData,
      runtimeCountry,
    );
    chatgptStudyPrompt = generateChatGPTStudyPrompt(
      profileData,
      runtimeCountry,
    );
  ...
  async function openQuestionModal(questionId: string | number) {
    ...
    for (const record of historyResults) {
      if (record.details && Array.isArray(record.details)) {
        const foundInHistory = record.details.find(
          (d) => String(d.questionId) === qid,
        );
    ...
    try {
      ...
      if (!selectedQuestionData) {
        ...
        console.warn(
          "💡 This question may be from an older weekly rotation that is no longer available.",
        );
      }
    } catch (err) {
      ...
      loadingQuestion = false;
      console.log(
        `🏁 Final state - selectedQuestionData:`,
        selectedQuestionData ? `ID: ${selectedQuestionData.id}` : "NULL",
      );
    }
  }
  ...
  let mmrHistory = $derived(
    historyResults.length >= 2
      ? historyResults
          .slice(0, 10)
          .reverse()
          .map(
            (r, i) =>
              250 +
              i * 5 +
              ((r.correctCount / r.totalQuestions) * 100 - 50) * 0.5,
          )
      : [],
  );

  let accuracyHistory = $derived(
    historyResults.length >= 2
      ? historyResults
          .slice(0, 10)
          .reverse()
  ...
  let metricsProgress = $derived({
    ...
    remainingTotal: Math.max(
      0,
      MIN_TOTAL_FOR_METRICS -
        ((userProfile as UserProfile | null)?.totalQuestions || 0),
    ),
    isUnlocked:
      ((userProfile as UserProfile | null)?.totalQuestions || 0) >=
      MIN_TOTAL_FOR_METRICS,
  ...
  let weakAreas = $derived(
    (() => {
      ...
      const getWeightedScore = (correct: number, seen: number) =>
        (correct + 1) / (seen + 2);
        ...
        return Object.values(profile.subjects)
          ...
          .map((s) => {
            ...
            return {
              ...
              mmr: subj.mmr,
              weightedScore: getWeightedScore(
                Math.round(subj.accuracy * subj.questionsAnswered),
    ...
    userProfile?.subjects
      ? Object.values(userProfile.subjects)
          ...
          .map((s) => ({
            ...
            seen: s.questionsAnswered,
            weightedScore: getWeightedScore(
              Math.round(s.accuracy * s.questionsAnswered),
              s.questionsAnswered,
            ),
          }))
      : [],
...
<div
  class="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center bg-black/90 backdrop-blur-md"
  transition:fade
>
  <div
    class="bg-[#121212] border-t sm:border border-white/10 rounded-t-[2.5rem] sm:rounded-2xl w-full max-w-5xl h-[94vh] sm:h-[90vh] flex flex-col shadow-2xl overflow-hidden relative mb-[env(safe-area-inset-bottom)]"
  >
    <!-- Mobile Drag Handle -->
    <div
      class="w-12 h-1.5 bg-white/10 rounded-full mx-auto mt-3 mb-1 sm:hidden"
    ></div>

    <!-- Header -->
    <div
      class="px-5 py-4 sm:p-6 border-b border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0 bg-gradient-to-b from-white/5 to-transparent"
    >
      <div class="flex items-center gap-4 w-full sm:w-auto">
        <div
          class="w-12 h-12 bg-gradient-to-tr from-emerald-600 to-emerald-400 rounded-2xl flex items-center justify-center text-2xl shadow-lg shadow-emerald-500/20"
        >
          🧠
        </div>
        <div>
          <h2 class="text-xl font-black text-white flex items-center gap-2">
            <span
              class="bg-clip-text text-transparent bg-gradient-to-r from-white to-white/60"
              >Inteligencia Local</span
            >
            <span
              class="text-[10px] bg-emerald-500 text-black font-black px-1.5 py-0.5 rounded-sm uppercase tracking-tighter"
              >BETA</span
            >
          </h2>
          <p
            class="text-[10px] sm:text-xs text-white/40 font-medium uppercase tracking-widest mt-0.5"
          >
        ...
        >
          ...
          >
            <path
              ...
              stroke-width="2"
              d="M3 21v-8a2 2 0 012-2h14a2 2 0 012 2v8l-6-3l-6 3l-6-3z"
            />
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
              d="M3 13V5a2 2 0 012-2h14a2 2 0 012 2v8"
            />
          </svg>
        </button>
      ...
      <div class="flex items-center gap-3 w-full sm:w-auto">
        <div
          class="flex bg-white/5 p-1 rounded-2xl border border-white/5 flex-1 sm:flex-none backdrop-blur-xl"
        >
          <button
        ...
        >
          ...
          <span
            class="text-xs font-bold uppercase tracking-widest hidden lg:inline"
            >Reportar</span
          >
        </button>
    ...
    <!-- Content -->
    <div
      class="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent"
    >
      {#if loading}
        <div class="flex flex-col items-center justify-center py-20 gap-4">
          ...
          ></div>
          <p class="text-sm text-white/40 animate-pulse">
            Analizando redes neuronales locales...
          </p>
        </div>
      {:else}
        <!-- DASHBOARD VIEW -->
        {#if activeTab === "dashboard"}
          {#if !userProfile || userProfile.totalQuestions < 5}
            <div class="text-center py-16 space-y-8 max-w-lg mx-auto">
              <div class="relative">
                <div
                  class="w-24 h-24 bg-white/5 rounded-[2rem] flex items-center justify-center mx-auto text-4xl shadow-2xl animate-pulse"
                >
                  🎓
                </div>
                <div
              ...
              <div class="relative z-10">
                <h3 class="text-2xl font-black text-white mb-2 leading-tight">
                  Forjando tu Inteligencia
                </h3>
                <p class="text-white/40 text-sm leading-relaxed px-6">
                  Estamos analizando tus patrones. Completa 5 exámenes para
                  desbloquear tu <span class="text-emerald-400 font-bold"
                    >MMR</span
                  > y perfil detallado.
                </p>
              </div>
              <div class="px-10">
                <div
                  class="w-full bg-white/5 h-3 rounded-full overflow-hidden border border-white/5 p-0.5"
                >
                  <div
                    class="bg-gradient-to-r from-emerald-600 to-emerald-400 h-full rounded-full transition-all duration-1000 shadow-[0_0_15px_rgba(52,211,153,0.3)]"
                    style="width: {((userProfile?.totalQuestions || 0) / 5) *
                      100}%"
                  ></div>
                </div>
                <p
                  class="text-[10px] text-emerald-400 font-black uppercase tracking-widest mt-3"
                >
          ...
          {:else}
            <div class="space-y-6 pb-10" in:fade={{ duration: 400 }}>
              <!-- 0. Activity Heatmap -->
              <div
                class="bg-white/5 border border-white/10 rounded-3xl p-5 sm:p-6 backdrop-blur-md"
              >
                <h3
                  class="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-5 flex items-center justify-between"
                >
                  ...
                  <span
                    class="bg-emerald-500/10 text-emerald-500 px-2 py-1 rounded-lg text-[9px]"
                    >Últimos 60 días</span
                  >
                </h3>
                <div
                ...
                >
                  {#each activityDays as day}
                    ...
                    >
                      <div
                        class="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 bg-white text-black text-[10px] font-black px-3 py-2 rounded-xl whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-20 transition-all scale-75 group-hover:scale-100 shadow-2xl"
                      >
                        {day.count} <span class="font-medium">preguntas</span>
                        <div class="text-[8px] opacity-40 font-medium">
                          {day.date}
                        </div>
                        <div
                          class="absolute top-full left-1/2 -translate-x-1/2 border-8 border-transparent border-t-white"
                        ></div>
                      </div>
                    </div>
                  {/each}
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <!-- 1. Stats Card -->
                <div
                  class="bg-gradient-to-br from-white/10 to-transparent border border-white/10 rounded-[2.5rem] p-8 relative overflow-hidden group shadow-2xl"
                >
                  <div
                    class="absolute -top-10 -right-10 w-40 h-40 bg-emerald-500/5 rounded-full blur-[60px] group-hover:bg-emerald-500/10 transition-colors"
                  ></div>
                  ...
                  <div class="flex items-center justify-between mb-8">
                    ...
                    >
                      ESTIMADO {runtimeCountry.examName}
                    </div>
                  ...
                  <div class="flex items-end gap-3 mb-2">
                    <div
                      class="text-7xl font-black tracking-tighter text-white drop-shadow-2xl"
                    >
                      {userProfile.icfesEstimate.score}
                    </div>
                    <div
                      class="text-xl font-black text-white/20 mb-3 tracking-widest"
                    >
                      /500
                    </div>
                  </div>
                  ...
                  <div class="mb-8">
                    <div
                      class="text-[10px] text-white/40 font-medium uppercase tracking-[0.1em] leading-relaxed"
                    >
                      Lectura proxy con precision {Math.round(
                        userProfile.globalAccuracy * 100,
                      )}% y {userProfile.icfesEstimate.evidenceCount} preguntas observadas.
                    </div>
                    <div
                      class="mt-2 text-[10px] text-white/30 font-medium uppercase tracking-[0.1em] leading-relaxed"
                    >
                      Benchmark local activo: {benchmarkConfig.label}
                      {benchmarkConfig.benchmarkScore}/500.
                    </div>
                    <div
                      class="mt-2 text-[10px] text-amber-200/70 font-medium uppercase tracking-[0.1em] leading-relaxed"
                    >
                      {userProfile.icfesEstimate.disclaimer} Confianza: {userProfile
                        .icfesEstimate.label}. Metodo: {userProfile
                        .icfesEstimate.methodologyVersion}.
                    </div>
                  </div>
                  ...
                  <div class="space-y-4 pt-6 border-t border-white/5">
                    <div class="flex justify-between items-center group/item">
                      <span
                        class="text-[10px] font-bold text-white/30 uppercase tracking-[0.1em]"
                        >Knowledge Base</span
                      >
                      <span class="text-sm font-black text-white"
                        >{userProfile.totalQuestions}
                        <span class="text-[10px] opacity-40">Q</span></span
                      >
                    </div>
                    <div class="flex justify-between items-center group/item">
                      <span
                        class="text-[10px] font-bold text-white/30 uppercase tracking-[0.1em]"
                        >Global Accuracy</span
                      >
                      <span class="text-sm font-black text-white"
                        >{Math.round(userProfile.globalAccuracy * 100)}%</span
                      >
                    </div>
                    <div class="flex justify-between items-center group/item">
                      <span
                      ...
                      >
                      <span class="text-sm font-black text-white"
                        >{userProfile.advancedMetrics.consistencyScore}<span
                          class="text-[10px] opacity-40">/100</span
                        ></span
                      >
                    </div>
                  </div>
                  ...
                  <div class="mt-8 pt-8 border-t border-white/5">
                    <PercentileRank
                      percentile={Math.min(
                        ...
                        Math.max(
                          1,
                          ((userProfile.globalMMR - 800) / 800) * 100,
                        ),
                      )}
                    />
                  </div>
                ...
                <div class="md:col-span-2 flex flex-col gap-6">
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div
                      class="bg-white/5 border border-white/5 rounded-3xl p-6 flex flex-col items-center justify-center text-center group hover:bg-white/[0.07] transition-all"
                    >
                      <div
                        class="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 mb-2"
                      >
                        Consistencia
                      </div>
                      <div
                        class="text-3xl font-black text-emerald-400 drop-shadow-lg"
                      >
                        {userProfile.advancedMetrics.consistencyScore}
                      </div>
                      <div
                        class="w-12 h-1 bg-white/10 rounded-full mt-3 overflow-hidden"
                      >
                        <div
                          class="h-full bg-emerald-500"
                          style="width: {userProfile.advancedMetrics
                            .consistencyScore}%"
                        ></div>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      {#if userProfile.advancedMetrics.avgTimeCorrect > 0}
                        {@const seconds = Math.round(
                          userProfile.advancedMetrics.avgTimeCorrect / 1000,
                        )}
                        <div
                          class="text-3xl font-black {seconds > 120
                            ? 'text-red-400'
                      ...
                      {/if}
                      <div
                        class="text-[9px] font-bold text-white/20 mt-2 uppercase"
                      >
                        Promedio p/ Pregunta
                      </div>
                    </div>
                  ...
                  <!-- Analysis List -->
                  <div
                    class="flex-1 bg-gradient-to-br from-indigo-500/10 to-transparent border border-indigo-500/20 rounded-[2.5rem] p-8 relative overflow-hidden group"
                  >
                    <div
                      class="absolute -bottom-10 -left-10 w-40 h-40 bg-indigo-500/10 rounded-full blur-[60px]"
                    ></div>

                    <h3
                      class="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 mb-6 flex items-center gap-3"
                    >
                      <span
                    ...
                    <div class="space-y-4">
                      {#each insights as insight}
                        <div
                          class="flex gap-4 items-start bg-white/5 hover:bg-white/[0.08] p-5 rounded-2xl border border-white/5 transition-all transform hover:translate-x-1"
                        >
                          <div
                            class="w-10 h-10 bg-indigo-500/20 rounded-xl flex items-center justify-center text-lg shrink-0"
                          >
                            💡
                          </div>
                          <p
                            class="text-[13px] text-indigo-50/70 leading-relaxed font-medium"
                          >
                            {@html insight}
                          </p>
                        </div>
                ...
                <!-- 3. Progress Charts -->
                <div
                  class="md:col-span-3 bg-white/5 border border-white/10 rounded-[2.5rem] p-8"
                >
                  <h3
                    class="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-8 flex items-center gap-3"
                  >
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <!-- MMR Progression -->
                    <div
                      class="bg-black/40 rounded-3xl p-6 border border-white/5 backdrop-blur-xl"
                    >
                      <div
                        class="text-[10px] font-black text-white/20 uppercase tracking-[0.15em] mb-6"
                      >
                        Tendencia MMR
                      </div>
                      {#if mmrHistory.length >= 2}
                        ...
                        <div class="flex justify-between items-center mt-6">
                          ...
                          <div class="flex flex-col items-end">
                            ...
                            >
                            <span
                              class={`text-lg font-black ${mmrHistory[mmrHistory.length - 1] >= mmrHistory[0] ? "text-emerald-400" : "text-red-400"}`}
                            >
                              {Math.round(mmrHistory[mmrHistory.length - 1])}
                              <span class="text-xs"
                                >{mmrHistory[mmrHistory.length - 1] >=
                                mmrHistory[0]
                                  ? "▲"
                                  : "▼"}</span
                              >
                      ...
                      {:else}
                        <div
                          class="h-40 flex flex-col items-center justify-center gap-4 opacity-20"
                        >
                          <div class="text-3xl">📉</div>
                          <div
                            class="text-[10px] font-black uppercase tracking-widest text-center px-10 leading-relaxed"
                          >
                            Secuencia interrumpida. Completa 2 exámenes
                            continuos.
                          </div>
                        </div>
                      {/if}
                      ...
                      {:else}
                        ...
                        >
                          ...
                          <div
                            class="text-[10px] font-black uppercase tracking-widest text-center px-10 leading-relaxed"
                          >
                            No hay ráfagas de datos suficientes.
                          </div>
                        </div>
                      {/if}
                    </div>
                ...
                >
                  <h3
                    class="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-8 flex items-center justify-between"
                  >
                    <span>Cluster de Asignaturas</span>
                    <span class="text-[8px] bg-white/5 px-2 py-1 rounded"
                      >Ordenado por Dominio</span
                    >
                  </h3>
                  <div
                  ...
                  >
                    {#each Object.values(userProfile.subjects).sort((a, b) => b.mmr - a.mmr) as subj}
                      <div
                        class="bg-black/30 rounded-3xl p-5 border border-white/5 hover:border-emerald-500/30 transition-all group overflow-hidden relative"
                      >
                        <div
                          class="absolute -right-4 -bottom-4 text-6xl opacity-[0.03] grayscale transition-all group-hover:scale-110 group-hover:opacity-[0.08] pointer-events-none"
                        >
                        ...
                        <div class="flex justify-between items-center mb-4">
                          <span
                            class="text-[11px] font-black text-white truncate max-w-[60%] uppercase tracking-tight"
                            >{subj.name}</span
                          >
                          <span
                            class="text-[10px] font-black bg-emerald-500 text-black px-2 py-0.5 rounded-sm"
                            >{Math.round(subj.mmr)}</span
                          >
                        </div>
                        ...
                        <div class="space-y-4">
                          ...
                          <div
                            class="text-[9px] font-bold text-white/20 flex justify-between"
                          >
                            <span>RECORRIDO</span>
                ...
                {#if competencyStats.length >= 3}
                  <div
                    class="md:col-span-3 bg-white/5 border border-white/10 rounded-[2.5rem] p-8 flex flex-col md:flex-row items-center gap-8"
                  >
                    <div class="flex-1">
                      <h3
                        class="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400 mb-2 flex items-center gap-3"
                      >
                        <span
                          class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"
                        ></span>
                      ...
                      <p class="text-xs text-white/50 leading-relaxed mb-6">
                        Visualización holística de tus habilidades cognitivas.
                        Un gráfico equilibrado indica un aprendizaje integral.
                      </p>
                      ...
                      <div class="grid grid-cols-2 gap-4">
                        ...
                        >
                          ...
                          <div class="text-sm font-black text-emerald-400">
                            {competencyStats.sort(
                              (a, b) => b.correct / b.seen - a.correct / a.seen,
                            )[0]?.name || "-"}
                          </div>
                        </div>
                        <div
                ...
                >
                  <!-- Strongest -->
                  <div
                    class="bg-emerald-900/10 border border-emerald-500/20 rounded-xl p-5"
                  >
                    <h3
                      class="text-xs font-bold uppercase tracking-widest text-emerald-400 mb-4"
                    >
                      Fortalezas (Top 3)
                    </h3>
                    <div class="space-y-2">
                      {#if topStrengths.length > 0}
                        {#each topStrengths as item}
                          <div
                            class="flex items-center justify-between text-sm py-2 border-b border-emerald-500/10 last:border-0 pl-2"
                          >
                            <span class="text-emerald-100/80">{item.name}</span>
                            <span class="font-mono text-emerald-400"
                              >{Math.round(
                                (item.correct / item.seen) * 100,
                              )}%</span
                            >
                          </div>
                        {/each}
                      {:else}
                        <div class="space-y-3">
                          <p class="text-xs text-white/40 italic">
                            Datos insuficientes para determinar fortalezas.
                          </p>
                          {#if !metricsProgress.isUnlocked}
                            <div
                              class="bg-emerald-900/20 rounded-lg p-3 border border-emerald-500/10"
                            >
                              <div class="flex items-center gap-2 mb-2">
                                <span class="text-lg">🔓</span>
                                <span
                                  class="text-xs font-bold text-emerald-400 uppercase tracking-widest"
                                  >Desbloquear Métricas</span
                                >
                              </div>
                              <div class="text-xs text-white/60 mb-2">
                                Completa <span
                                  class="font-bold text-emerald-400"
                                  >{metricsProgress.remainingTotal}</span
                                > preguntas más para ver tus fortalezas.
                              </div>
                              <div
                                class="w-full bg-black/40 h-2 rounded-full overflow-hidden"
                              >
                                <div
                                  class="bg-gradient-to-r from-emerald-600 to-emerald-400 h-full transition-all duration-500"
                                  style="width: {Math.min(
                                    100,
                                    (metricsProgress.totalQuestions /
                                      metricsProgress.neededTotal) *
                                      100,
                                  )}%"
                                ></div>
                              </div>
                          ...
                          {:else}
                            <div
                              class="bg-emerald-900/20 rounded-lg p-4 border border-emerald-500/10 text-[10px] text-white/50 leading-relaxed italic text-center"
                            >
                              Análisis en curso... Sigue practicando para que el
                              sistema identifique tus fortalezas por
                              competencia.
                            </div>
                          {/if}
                        </div>
                  ...
                  <!-- Weakest -->
                  <div
                    class="bg-red-900/10 border border-red-500/20 rounded-xl p-5"
                  >
                    <h3
                      class="text-xs font-bold uppercase tracking-widest text-red-400 mb-4"
                    >
                    ...
                    <div class="space-y-2">
                      {#if topWeaknesses.length > 0}
                        {#each topWeaknesses as item}
                          <div
                            class="flex items-center justify-between text-sm py-2 border-b border-red-500/10 last:border-0 pl-2"
                          >
                      ...
                      {:else}
                        <div class="space-y-3">
                          <p class="text-xs text-white/40 italic">
                            Datos insuficientes para determinar debilidades.
                          </p>
                          {#if !metricsProgress.isUnlocked}
                            <div
                              class="bg-red-900/20 rounded-lg p-3 border border-red-500/10"
                            >
                              <div class="flex items-center gap-2 mb-2">
                                <span class="text-lg">🎯</span>
                                <span
                                  class="text-xs font-bold text-red-400 uppercase tracking-widest"
                                  >Identificar Áreas de Mejora</span
                                >
                              </div>
                              <div class="text-xs text-white/60 mb-2">
                                Responde <span class="font-bold text-red-400"
                                  >{metricsProgress.remainingTotal}</span
                                > preguntas más para identificar dónde mejorar.
                              </div>
                              <div
                                class="w-full bg-black/40 h-2 rounded-full overflow-hidden"
                              >
                          ...
                          {:else}
                            <div
                              class="bg-red-900/20 rounded-lg p-4 border border-red-500/10 text-[10px] text-white/50 leading-relaxed italic text-center"
                            >
                              Diversifica tus prácticas para permitirle a la IA
                              mapear tus áreas de mejora con precisión.
                            </div>
                          {/if}
                        </div>
                      {/if}
                ...
                {#if userProfile.topics}
                  {@const critTopics = Object.values(userProfile.topics)
                    .filter((t) => t.seen >= 3 && t.accuracy < 0.6)
                    .sort((a, b) => a.accuracy - b.accuracy)
                    .slice(0, 5)}
                  {#if critTopics.length > 0}
                    <div
                      class="md:col-span-3 bg-gradient-to-r from-red-900/10 to-transparent border border-red-500/20 rounded-xl p-6"
                    >
                      <h3
                        class="text-sm font-bold uppercase tracking-widest text-red-400 mb-6 flex items-center gap-2"
                      >
                        <svg
                          class="w-4 h-4"
                          fill="none"
                          ...
                          <div>
                            ...
                            >
                              ...
                              <span class="text-red-400 font-mono"
                                >{Math.round(topic.accuracy * 100)}%</span
                              >
                            </div>
                            <div
                              class="h-2 bg-black/40 rounded-full overflow-hidden flex"
                            >
                              <!-- Correct part -->
                              <div
                            ...
                            <div
                              class="text-[10px] text-white/30 text-right mt-0.5"
                            >
                ...
                <div
                  class="md:col-span-3 bg-gradient-to-br from-blue-900/10 to-transparent border border-blue-500/20 rounded-xl p-6 relative overflow-hidden"
                >
                  <div
                    class="absolute top-0 right-0 w-64 h-64 bg-blue-500/5 rounded-full blur-3xl"
                  ></div>

                  <h3
                    class="text-sm font-bold uppercase tracking-widest text-blue-400 mb-6 flex items-center gap-2 relative z-10"
                  >
                    <svg
                      class="w-4 h-4"
                      fill="none"
                  ...
                  {#if weakAreas.length > 0}
                    ...
                    >
                      <!-- Step 1: Identificar -->
                      <div
                        class="bg-black/20 rounded-lg p-5 border border-blue-500/10 relative group hover:border-blue-500/30 transition-all"
                      >
                        <div
                          class="absolute -top-3 -left-3 w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg"
                        >
                          1
                        </div>
                        <h4 class="text-sm font-bold text-blue-300 mb-3 mt-2">
                        ...
                        <div class="space-y-2">
                          {#each weakAreas as area}
                            <div class="flex items-center gap-2 text-xs">
                              ...
                              <span class="ml-auto text-red-400 font-mono"
                                >{Math.round(
                                  (area.correct / area.seen) * 100,
                                )}%</span
                              >
                            </div>
                          {/each}
                        ...
                        <div class="space-y-3">
                          <div class="flex items-start gap-2 text-xs">
                            ...
                            <span class="text-white/70"
                              >Realiza 5 exámenes enfocados en tus áreas débiles</span
                            >
                          </div>
                          <div class="flex items-start gap-2 text-xs">
                            <svg
                            ...
                            >
                            <span class="text-white/70"
                              >Revisa las explicaciones de cada pregunta fallada</span
                            >
                          </div>
                          <div class="flex items-start gap-2 text-xs">
                            <svg
                            ...
                            >
                            <span class="text-white/70"
                              >Toma notas de conceptos clave</span
                            >
                          </div>
                        </div>
                      </div>
                            ...
                            <div class="space-y-1">
                              <div class="flex justify-between text-[10px]">
                                <span class="text-white/50"
                                  >{area.name.substring(0, 15)}...</span
                                >
                                <span class="text-blue-400 font-mono"
                                  >{currentAcc}% → {targetAcc}%</span
                                >
                              </div>
                              <div
                                class="h-1 bg-black/40 rounded-full overflow-hidden"
                              >
                                <div
                                  class="h-full bg-gradient-to-r from-red-500 to-emerald-500"
                                  style="width: {(currentAcc / targetAcc) *
                                    100}%"
                                ></div>
                              </div>
                            </div>
                    ...
                    <!-- Action Buttons -->
                    <div
                      class="mt-6 flex flex-wrap justify-center gap-3 relative z-10"
                    >
                    ...
                    {#if runtimeCountry.features?.blog}
                      ...
                      <div
                        class="mt-4 flex flex-wrap justify-center gap-2 relative z-10"
                      >
                        {#each weakAreas as area}
                          <button
                            onclick={() => {
                  ...
                  {:else}
                    <div
                      class="text-center py-8 text-white/30 text-sm relative z-10"
                    >
                      Completa más exámenes para generar tu plan de mejora
                      personalizado
                    </div>
                  {/if}
                </div>
                ...
                <div
                  class="md:col-span-3 mt-12 py-8 border-t border-white/5 text-center px-6"
                >
                  <div
                    class="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full text-[10px] font-black text-amber-500 uppercase tracking-widest mb-4"
                  >
                    <svg
                      class="w-3 h-3"
                      fill="none"
                  ...
                  <p
                    class="text-xs text-white/40 leading-relaxed max-w-md mx-auto italic"
                  >
                    Este análisis y las preguntas utilizadas son generadas por
                    modelos de IA. Aunque buscamos la máxima precisión, pueden
                    contener errores o desajustes.
                  </p>
                  <button
                    onclick={() =>
                      window.open("https://t.me/sabercol_bot", "_blank")}
        ...
        {:else}
          {#if historyResults.length === 0}
            <div class="text-center py-12 space-y-4">
              <div
                class="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mx-auto text-3xl opacity-30"
              >
                📭
              </div>
          ...
          {:else}
            <div class="space-y-3" in:fade={{ duration: 200 }}>
              {#each historyResults as result (result.id || result.timestamp)}
                <div
                  class="bg-white/5 rounded-xl border border-white/5 hover:border-white/10 hover:bg-white/[0.07] transition-all overflow-hidden"
                >
                  <!-- Exam Entry Row -->
                  <button
                  ...
                  >
                    <div class="min-w-0">
                      <div class="flex items-center gap-2 mb-1">
                        <span
                          class="text-xs font-bold uppercase tracking-widest bg-white/10 px-2 py-0.5 rounded text-white/70 group-hover:text-white transition-colors"
                        >
                          {result.subject || "General"}
                        </span>
                        {#if result.grade}
                          <span
                            class="text-[10px] opacity-50 border border-white/10 px-1.5 rounded"
                          >
                            Grado {result.grade}°
                          </span>
                        {/if}
                      ...
                      <div class="text-xs text-white/40 font-mono">
                        {formatDate(result.timestamp)} • {Math.round(
                    ...
                    <div class="flex items-center gap-3">
                      <div class="text-right shrink-0">
                        <div
                          class={`text-xl font-bold ${getGradeColor(result.score)}`}
                        >
                          {result.score}%
                        </div>
                        <div
                      ...
                      <svg
                        class="w-4 h-4 text-white/20 group-hover:text-emerald-400 transition-colors"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
    ...
    <!-- Footer -->
    <div
      class="p-4 border-t border-white/5 bg-black/20 text-center shrink-0 flex justify-between items-center text-[10px] text-white/20"
    >
...
{#if selectedExam}
  ...
  >
    <!-- Top bar -->
    <div
      class="flex items-center gap-3 px-4 sm:px-6 py-4 border-b border-white/10 bg-[#121212]/80 backdrop-blur-sm shrink-0"
    >
      <button
      ...
      >
        ...
        <span
          class="text-xs font-bold uppercase tracking-widest hidden sm:inline"
          >Historial</span
        >
      </button>
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          <span class="text-sm font-black text-white uppercase tracking-tight"
            >{selectedExam.subject || "Examen General"}</span
          >
          {#if selectedExam.grade}
            <span
              class="text-[10px] text-white/30 border border-white/10 px-1.5 py-0.5 rounded"
              >Grado {selectedExam.grade}°</span
            >
          {/if}
        </div>
        <div class="text-[10px] text-white/30 font-mono">
          {formatDate(selectedExam.timestamp)}
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-8 pb-16">
        ...
        <div class="text-center space-y-4">
          <div class="relative w-36 h-36 mx-auto">
            ...
            >
              ...
              ></circle>
              <circle
                class="{selectedExam.score >= 60
                  ? 'text-emerald-500'
                  : selectedExam.score >= 40
                    ? 'text-yellow-500'
                    : 'text-red-500'} transition-all duration-1000 ease-out stroke-current"
                stroke-width="7"
                stroke-linecap="round"
                cx="50"
                cy="50"
          ...
          <p class="text-xs uppercase tracking-widest opacity-50">
            {selectedExam.correctCount} de {selectedExam.totalQuestions} Correctas
            • {Math.round(selectedExam.timeSpentSeconds / 60)} min
          </p>
        </div>
        ...
        {#if selectedExam.details && selectedExam.details.length > 0}
          <div class="space-y-4">
            <h3
              class="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 flex items-center gap-2"
            >
            ...
            {#each selectedExam.details as q, i}
              ...
              {@const userAnswerId = selectedExam.answers
                ? selectedExam.answers[String(q.questionId)]
                : undefined}
              {@const isCorrect = q.isCorrect}

              <div
                class="border rounded-xl overflow-hidden {isCorrect
                  ? 'border-emerald-500/25 bg-emerald-900/5'
              ...
              >
                ...
                <div class="p-4 sm:p-5 border-b border-white/5">
                  <div class="flex items-center gap-2 mb-2">
                    ...
                    >
                    <div
                      class="px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest border rounded {isCorrect
                    ...
                    {#if q.difficulty}
                      <span
                        class="text-[10px] text-white/20 border border-white/10 px-1.5 rounded"
                        >Nivel {q.difficulty}</span
                      >
                    {/if}
                  </div>
                  ...
                  {#if question?.context}
                    <div
                      class="bg-emerald-900/10 border border-emerald-500/20 rounded-lg p-3 sm:p-4 mb-3 overflow-y-auto max-h-[25vh] scrollbar-thin scrollbar-thumb-emerald-500/20 scrollbar-track-transparent"
                    >
                      <div
                        class="text-[10px] font-bold text-emerald-400 mb-1.5 uppercase tracking-wider flex items-center gap-2"
                      >
                        <svg
                          class="w-3 h-3"
                          fill="none"
                      ...
                      <div
                        class="text-xs sm:text-sm text-gray-300 font-serif leading-relaxed space-y-2"
                      >
                  ...
                  {#if question?.text}
                    <div
                      class="text-sm sm:text-base text-white/90 leading-relaxed"
                    >
                      <MathRenderer content={question.text} />
                    </div>
                  {:else}
                    <div class="font-mono text-xs text-white/30 italic">
                      {q.questionId}
                    </div>
                  {/if}
                </div>
                ...
                <div class="p-4 sm:p-5 space-y-3">
                  {#if question?.options && question.options.length > 0}
                    ...
                    <div class="space-y-2">
                      {#each question.options as opt}
                        ...
                        >
                          <span
                            class="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-full text-sm font-bold
                            {isTheCorrect
                          ...
                          <div
                            class="flex-1 text-sm {isTheCorrect
                              ? 'text-emerald-100'
                              : isUserAnswer && !isCorrect
                                ? 'text-red-100/80'
                                : 'text-white/50'}"
                          ...
                          {#if isTheCorrect}
                            <span class="text-emerald-400 text-sm shrink-0"
                              >✓</span
                            >
                  ...
                  {:else}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div
                        class="p-3 bg-white/5 border border-white/5 rounded-lg"
                      >
                        <span
                          class="block text-[10px] uppercase tracking-widest opacity-40 mb-1"
                          >Tu Respuesta</span
                        >
                        <span
                          class="{isCorrect
                      ...
                      {#if !isCorrect && q.correctOptionId}
                        <div
                          class="p-3 bg-white/5 border border-emerald-500/20 rounded-lg"
                        >
                          <span
                            class="block text-[10px] uppercase tracking-widest opacity-40 mb-1"
                  ...
                  {#if question?.explanation}
                    <div
                      class="mt-2 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg"
                    >
                      <div
                        class="text-xs font-bold uppercase tracking-widest text-blue-400 mb-2"
                      >
                        Explicación
                      </div>
        ...
        {:else}
          <div class="text-center py-16 space-y-4">
            ...
            <p class="text-white/30 text-sm">
              No hay detalles de preguntas guardados para este examen.
            </p>
            <p class="text-white/20 text-xs max-w-sm mx-auto">
              Los exámenes nuevos guardarán los detalles automáticamente para
              revisión completa.
            </p>
          </div>
        {/if}
...
{#if selectedQuestionId}
  ...
  >
    ...
    >
      <!-- Modal Header -->
      <div
        class="p-4 border-b border-white/5 flex items-center justify-between bg-[#121212]/50"
      >
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {:else if selectedQuestionData}
          ...
          <div class="bg-white/5 rounded-xl p-6 border border-white/10">
            ...
            <div class="flex flex-wrap justify-between items-start gap-2 mb-4">
              ...
              <div
                class="flex gap-2 text-[10px] uppercase tracking-widest text-white/40"
              >
                <span
                  class="px-2 py-0.5 bg-white/5 rounded border border-white/10"
                  >Grado {selectedQuestionData.grade}°</span
                >
                <span
                  class="px-2 py-0.5 bg-white/5 rounded border border-white/10"
                  >Nivel {selectedQuestionData.difficulty}</span
                >
              </div>
            </div>
            ...
            <div class="space-y-3">
              {#each selectedQuestionData.options as option, i}
                {@const isCorrect =
                  option.id === selectedQuestionData.correctOptionId}
                <div
                  class="flex items-start gap-3 p-3 rounded-lg {isCorrect
                    ? 'bg-emerald-500/10 border border-emerald-500/30'
                    : 'bg-white/5 border border-white/10'}"
                >
                  <span
                    class="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-full {isCorrect
                      ? 'bg-emerald-500 text-white'
                      : 'bg-white/10 text-white/50'} text-sm font-bold"
                  >
                    {option.id}
                  </span>
                  <div
                    class="flex-1 text-sm {isCorrect
                      ? 'text-emerald-100'
                      : 'text-white/70'}"
                  >
          ...
          {#if selectedQuestionData.context}
            ...
            >
              <div
                class="text-xs font-bold uppercase tracking-widest text-amber-400 mb-2 flex items-center gap-2"
              >
                <svg
                  class="w-4 h-4"
                  fill="none"
            ...
            <div class="grid grid-cols-2 gap-3 text-xs">
              {#if selectedQuestionData.topic}
                <div class="bg-white/5 rounded-lg p-2">
                  ...
                  <span class="text-emerald-400 font-bold ml-1"
                    >{selectedQuestionData.topic}</span
                  >
                </div>
              {/if}
              {#if selectedQuestionData.bundleId}
                <div class="bg-white/5 rounded-lg p-2">
                  ...
                  <span class="text-white/60 font-mono ml-1"
                    >{selectedQuestionData.bundleId}</span
                  >
                </div>
              {/if}
              <div class="bg-white/5 rounded-lg p-2">
                <span class="text-white/40">ID:</span>
                <span class="text-white/60 font-mono ml-1 truncate"
                  >{selectedQuestionData.id}</span
                >
              </div>
              <div class="bg-white/5 rounded-lg p-2">
                <span class="text-white/40">Variante:</span>
                <span class="text-white/60 font-mono ml-1"
                  >{selectedQuestionData.variant || "v1"}</span
                >
              </div>
            </div>
          </div>
        {:else}
          ...
          <div class="text-center py-12 px-6">
            ...
            <h4 class="text-lg font-bold text-white/60 mb-3">
              Pregunta no disponible
            </h4>
            <div class="text-sm text-white/40 space-y-2 max-w-md mx-auto">
              <p>
                Esta pregunta es de un examen anterior y ya no está disponible
                en la rotación actual de preguntas.
              </p>
              <p class="text-xs text-white/30 mt-4">
                💡 <strong>Buenas noticias:</strong> A partir de ahora, todos los
                exámenes nuevos guardarán las preguntas completas localmente, así
                siempre podrás revisarlas.
              </p>
            </div>
          </div>
        {/if}
...
{#if showHelpModal}
  ...
  >
    ...
    >
      <!-- Modal Header -->
      <div
        class="p-4 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-emerald-900/30 to-transparent"
      >
        <div class="flex items-center gap-2">
          <span class="text-2xl">📊</span>
          <div>
            <h3 id="help-modal-title" class="font-bold text-white text-lg">
              Sistema de Calificaciones MMR
            </h3>
            <span class="text-xs text-white/40"
              >Matchmaking Rating adaptado para educación</span
            >
          </div>
        </div>
        <button
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        >
          ...
          <p class="text-sm text-white/70 leading-relaxed">
            <strong>MMR (Matchmaking Rating)</strong> es un sistema de
            puntuación adaptado del algoritmo <strong>ELO</strong>
            usado en ajedrez y videojuegos competitivos. Mide tu nivel de habilidad
            basándose en tu rendimiento real, no solo en el porcentaje de respuestas
            correctas.
          </p>
        </div>
        ...
        <div class="space-y-3">
          ...
          <div class="grid gap-3">
            <div
              class="bg-white/5 border border-white/10 rounded-lg p-3 flex gap-3"
            >
              <span class="text-xl">📈</span>
              <div>
                <div class="font-bold text-white text-sm">Ganar puntos</div>
                <p class="text-xs text-white/50">
                  Responde correctamente para subir tu rating. Preguntas más
                  difíciles dan más puntos.
                </p>
              </div>
            </div>
            <div
            ...
            >
              ...
              <div>
                ...
                <p class="text-xs text-white/50">
                  Al equivocarte pierdes puntos, pero menos si la pregunta era
                  muy difícil para tu nivel.
                </p>
              </div>
            </div>
            <div
            ...
            >
              ...
              <div>
                ...
                <p class="text-xs text-white/50">
                  Si tu rating es alto, se espera que aciertes más. Si es bajo,
                  equivocarte penaliza menos.
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-3">
          ...
          <div
            class="bg-black/30 rounded-xl border border-white/10 overflow-hidden"
          >
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-white/10 bg-white/5">
                  <th
                    class="text-left p-3 text-white/40 font-normal uppercase text-xs"
                    >Rango</th
                  >
                  <th
                    class="text-center p-3 text-white/40 font-normal uppercase text-xs"
                    >MMR</th
                  >
                  <th
                    class="text-left p-3 text-white/40 font-normal uppercase text-xs"
              ...
              <tbody class="divide-y divide-white/5">
                <tr class="hover:bg-white/5">
                  ...
                  <td class="p-3 text-white/50 text-xs"
                    >Comenzando el camino de aprendizaje</td
                  >
                </tr>
                <tr class="hover:bg-white/5">
                  <td class="p-3 text-orange-400 font-bold">Aprendiz</td>
                  <td class="p-3 text-center font-mono text-white/70"
                    >600-799</td
                  >
                  <td class="p-3 text-white/50 text-xs"
                    >Construyendo bases sólidas</td
                  >
                </tr>
                <tr class="hover:bg-white/5">
                ...
                <tr class="hover:bg-white/5">
                  ...
                  >
                  <td class="p-3 text-white/50 text-xs"
                    >Dominio sólido de conceptos</td
                  >
                </tr>
                <tr class="hover:bg-white/5">
                  ...
                  >
                  <td class="p-3 text-white/50 text-xs"
                    >Rendimiento superior al promedio</td
                  >
                </tr>
                <tr class="hover:bg-white/5">
        ...
        <div class="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
          ...
          <ul class="text-xs text-white/60 space-y-1">
            <li>
              • Practica constantemente - la consistencia importa más que las
              rachas
            </li>
            <li>
              • No te desanimes por las pérdidas - son oportunidades de
              aprendizaje
            </li>
            <li>
              • Enfrenta preguntas difíciles - aunque pierdas, el sistema
              recompensa el intento
            </li>
            <li>
              • Revisa tus errores - entender por qué fallaste mejora más que
...
{#if showStudyPromptModal}
  ...
  >
    ...
    >
      <!-- Modal Header -->
      <div
        class="p-4 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-purple-900/30 to-transparent"
      >
        <div class="flex items-center gap-2">
          <span class="text-2xl">🎓</span>
          <div>
            <h3 id="study-prompt-title" class="font-bold text-white text-lg">
              Guía de Estudio Personalizada
            </h3>
            <span class="text-xs text-white/40"
              >Copia este prompt y úsalo en ChatGPT, Gemini o Claude</span
            >
          </div>
        </div>
        <button
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4">
        <!-- Instructions -->
        <div
          class="bg-purple-500/10 border border-purple-500/20 rounded-xl p-4"
        >
          ...
          <ol class="text-sm text-white/70 space-y-1 list-decimal list-inside">
            ...
            <li>
              Abre <a
                ...
                target="_blank"
                class="text-purple-400 hover:underline">ChatGPT</a
              >,
              <a
                href="https://gemini.google.com"
                target="_blank"
                class="text-purple-400 hover:underline">Gemini</a
              >
              o
              <a
                href="https://claude.ai"
                target="_blank"
                class="text-purple-400 hover:underline">Claude</a
              >
            </li>
        ...
        <div class="bg-black/40 rounded-xl border border-white/10 p-4">
          ...
          <pre
            class="text-sm text-white/80 whitespace-pre-wrap font-mono leading-relaxed max-h-64 overflow-y-auto">{generatedStudyPrompt}</pre>
        </div>
        ...
        >
          ...
          <p class="text-xs text-white/60 mb-4">
            Ideal para sesiones interactivas de preguntas y respuestas. ChatGPT
            actuará como un tutor socrático.
          </p>

          <div class="bg-black/30 rounded-lg p-3">
            <div class="flex justify-between items-center mb-2">
              <span
                class="text-[10px] text-emerald-400 font-black uppercase tracking-widest"
                >Prompt de Configuración</span
              >
              <button
                onclick={async () => {
                  try {
                    await navigator.clipboard.writeText(chatgptStudyPrompt);
                  } catch (err) {}
                }}
            ...
            <pre
              class="text-[10px] text-white/70 whitespace-pre-wrap font-mono leading-relaxed max-h-32 overflow-y-auto">{chatgptStudyPrompt}</pre>
          </div>
        </div>
        ...
        <div class="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
          ...
          <p class="text-xs text-white/60 mb-4">
            Mejora tu "Mentalidad de Examen". Este prompt analiza tu velocidad y
            consistencia para darte tácticas psicológicas.
          </p>

          <div class="bg-black/30 rounded-lg p-3">
            <div class="flex justify-between items-center mb-2">
              <span
                class="text-[10px] text-blue-400 font-black uppercase tracking-widest"
                >Prompt de Mentoría</span
              >
              <button
                onclick={async () => {
                  try {
                    await navigator.clipboard.writeText(studyTipsPrompt);
                  } catch (err) {}
                }}
            ...
            <pre
              class="text-[10px] text-white/70 whitespace-pre-wrap font-mono leading-relaxed max-h-32 overflow-y-auto">{studyTipsPrompt}</pre>
          </div>
        </div>
        ...
        <div class="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
          <h4
            class="text-amber-400 font-bold mb-3 flex items-center justify-between gap-2"
          >
            <div class="flex items-center gap-2">
              <span>📓</span> NotebookLM
              <span class="text-[9px] bg-amber-500 text-black px-1 rounded ml-1"
                >RECOMENDADO</span
              >
            </div>
            <!-- Toggle -->
            <div
              class="flex bg-black/40 rounded-lg p-0.5 border border-white/10 text-[10px] font-mono"
            >
              <button
          ...
          {#if notebookLMPromptType === "setup"}
            <ol
              class="text-sm text-white/70 space-y-3 list-decimal list-inside mb-4"
            >
              <li>
                Abre <a
                  href="https://notebooklm.google.com"
                  target="_blank"
                  class="text-amber-400 hover:underline">NotebookLM</a
                > y crea un cuaderno.
              </li>
              <li>
                ...
                <div class="mt-2 flex items-center gap-2">
                  <code
                    class="bg-black/40 px-3 py-2 rounded text-amber-300 text-xs flex-1 truncate"
                    >https://saberparatodos.space/notebooklm</code
                  >
                  <button
                    onclick={async () => {
                      try {
                        await navigator.clipboard.writeText(
                          "https://saberparatodos.space/notebooklm",
                        );
                      } catch (err) {}
                    }}
          ...
          {:else}
            ...
            >
              ...
              <li>
                Copia el texto de abajo y pégalo <strong>en el chat</strong> (o crea
                una nueva nota).
              </li>
              <li>
                El asistente actualizará tu plan de estudio con tus nuevos
                fallos.
              </li>
            </ol>
          {/if}
          ...
          <div class="bg-black/30 rounded-lg p-3">
            <div class="flex justify-between items-center mb-2">
              <span
                class="text-xs text-amber-400 font-bold uppercase tracking-widest"
              >
                {notebookLMPromptType === "setup"
                  ? "Prompt de Inicio"
            ...
            <pre
              class="text-xs text-white/70 whitespace-pre-wrap font-mono leading-relaxed max-h-32 overflow-y-auto">{notebookLMPromptType ===
              "setup"
                ? notebookLMPrompt
                : notebookLMUpdatePrompt}</pre>
          </div>
      ...
      <!-- Modal Footer -->
      <div
        class="p-4 border-t border-white/5 bg-black/20 flex justify-between items-center"
      >
        <div class="flex gap-2">
          <a
            href="https://chat.openai.com"
            target="_blank"
            class="px-3 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs text-white/60 hover:text-white transition-colors"
          >
            ChatGPT →
          </a>
          <a
...
{#if showHelpModal}
  ...
  >
    ...
    >
      <div
        class="p-6 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-emerald-600/10 to-transparent"
      >
        <h3
          class="text-lg font-black text-white uppercase tracking-widest flex items-center gap-2"
        >
      ...
      <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
        <section class="space-y-3">
          ...
          <p class="text-sm text-white/70 leading-relaxed">
            El <span class="text-white font-bold">Rating Técnico (MMR)</span> es un
            sistema de puntuación dinámico (estilo ELO) que mide tu habilidad real
            basándose en la dificultad de las preguntas que respondes.
          </p>
        </section>
        ...
        <section class="space-y-3">
          ...
          <ul class="text-xs text-white/60 space-y-2 list-disc list-inside">
            <li>
              Subes puntos al responder correctamente <span
                class="text-emerald-400 font-bold">preguntas difíciles</span
              >.
            </li>
            <li>
              Bajas puntos si fallas <span class="text-red-400 font-bold"
                >preguntas fáciles</span
              >.
            </li>
            <li>
              El sistema se estabiliza después de <span
                class="text-white font-bold">20-30 preguntas</span
              >.
            </li>
          </ul>
        </section>
        ...
        <section class="space-y-3">
          <h4
            class="text-xs font-black text-emerald-400 uppercase tracking-tighter"
          >
            Estimado {runtimeCountry.examName} Proxy
          </h4>
          <p class="text-xs text-amber-200/80 leading-relaxed">
            {userProfile?.icfesEstimate?.disclaimer ||
              `Estimacion de practica; no reemplaza el reporte oficial de ${runtimeCountry.institutionName}.`}
          </p>
          <p class="text-sm text-white/70 leading-relaxed">
            Usamos una metodologia proxy en escala <span
              class="text-white font-bold">0 a 500 puntos</span
            >
            para orientar tu practica. No reemplaza el reporte oficial de {runtimeCountry.institutionName}
            ni replica su metodologia psicometrica.
          </p>
        </section>

        <div
          class="bg-emerald-500/10 rounded-xl p-4 border border-emerald-500/20"
        >
