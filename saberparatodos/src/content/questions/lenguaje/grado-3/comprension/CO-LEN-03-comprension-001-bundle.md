---
# === METADATA GLOBAL ===
id: "CO-LEN-03-comprension-001"
country: "CO"
grado: 3
asignatura: "Lenguaje"
tema: "Comprensión Lectora"
dificultad: 2
estado: "draft"
creador: "Copilot"
source_lang: "es-CO"
llm_model: "Copilot"
agent: "Cascade"
ide: "VS Code"
bundle_version: "2.1"
total_questions: 7
difficulty_distribution: "1 original (3) + 2 fácil (1-2) + 2 media (3) + 2 difícil (4-5)"

# === SOURCE ATTRIBUTION ===
source: "ICFES-Curriculum"
source_url: "https://www.icfes.gov.co"
source_license: "Educational Use - Colombian Curriculum"
source_id: "ICFES-LEN-003-001"
original_concept: "Comprensión literal e inferencial de textos narrativos"
competencia_icfes: "Competencia Lectora"
componente: "Semántico"
generation_date: "2025-12-04"
---

# Bundle: Comprensión de Textos Narrativos

> **Fuente:** Currículo ICFES Saber 3° - Lenguaje
> **Competencia:** Comprensión literal e inferencial
> **Contexto:** Colombia - Cuentos con elementos de la cultura colombiana

---

## Texto Base (para preguntas 1-5)

### La Aventura de Tomás en el Mercado

Tomás vivía con su abuela en un pueblo cerca de Medellín. Cada sábado, iban juntos al mercado a comprar frutas y verduras. Un día, mientras la abuela compraba tomates, Tomás vio un perrito callejero que parecía tener hambre.

—Abuela, ¿podemos darle algo de comer? —preguntó Tomás con tristeza.

La abuela sonrió y le dio un pan que llevaba en su bolsa. Tomás se acercó despacio al perrito y le ofreció el pan. El animal movió la cola con alegría y comió rápidamente.

—Tienes un corazón muy bueno, Tomás —dijo la abuela mientras le acariciaba la cabeza.

Desde ese día, cada sábado Tomás llevaba algo especial para el perrito, a quien llamó "Manchas" por las manchas negras en su pelaje blanco.

---

## Pregunta 1 (Original - Dificultad 3)

**ID:** `CO-LEN-03-comprension-001-v1`

### Enunciado

¿Por qué la abuela le dijo a Tomás que tenía "un corazón muy bueno"?

### Opciones

- [ ] A) Porque Tomás siempre la ayudaba a cargar las bolsas
- [x] B) Porque Tomás se preocupó por un perrito que tenía hambre
- [ ] C) Porque Tomás compró las frutas más baratas
- [ ] D) Porque Tomás caminaba despacio en el mercado

### Explicación Pedagógica

**¿Por qué B es correcta?**
La abuela dijo esto justo después de que Tomás le diera de comer al perrito hambriento. El texto dice claramente que Tomás vio al perrito "que parecía tener hambre" y pidió ayuda para darle comida. Esta acción muestra bondad, que es tener "un corazón bueno".

**¿Por qué las otras son incorrectas?**
- **A)** El texto no menciona que Tomás ayudaba a cargar bolsas.
- **C)** No se habla de que Tomás comprara nada.
- **D)** Caminar despacio no tiene relación con tener buen corazón.

**Competencia evaluada:** Inferencia - comprender la razón de una acción

---

## Pregunta 2 (Fácil A - Dificultad 1)

**ID:** `CO-LEN-03-comprension-001-v2`

### Enunciado

¿Qué le dio la abuela a Tomás para el perrito?

### Opciones

- [ ] A) Un tomate
- [x] B) Un pan
- [ ] C) Una manzana
- [ ] D) Un hueso

### Explicación Pedagógica

El texto dice claramente: "La abuela sonrió y le dio un pan que llevaba en su bolsa."
Esta es información literal (exacta) que aparece en el texto.

**Competencia evaluada:** Comprensión literal - identificar información explícita

---

## Pregunta 3 (Fácil B - Dificultad 2)

**ID:** `CO-LEN-03-comprension-001-v3`

### Enunciado

¿Cuándo iban Tomás y su abuela al mercado?

### Opciones

- [x] A) Cada sábado
- [ ] B) Todos los días
- [ ] C) Los domingos
- [ ] D) Una vez al mes

### Explicación Pedagógica

El texto dice: "Cada sábado, iban juntos al mercado a comprar frutas y verduras."
La respuesta está escrita directamente en el primer párrafo.

**Competencia evaluada:** Comprensión literal - identificar información de tiempo

---

## Pregunta 4 (Media A - Dificultad 3)

**ID:** `CO-LEN-03-comprension-001-v4`

### Enunciado

¿Por qué Tomás llamó "Manchas" al perrito?

### Opciones

- [ ] A) Porque el perrito estaba sucio de barro
- [ ] B) Porque era el nombre de su perro anterior
- [x] C) Porque el perrito tenía manchas negras en su pelaje blanco
- [ ] D) Porque la abuela le sugirió ese nombre

### Explicación Pedagógica

El texto dice: "...a quien llamó 'Manchas' por las manchas negras en su pelaje blanco."
La explicación del nombre está directamente en el texto, conectando la descripción física del perrito con su nombre.

**Competencia evaluada:** Comprensión literal - identificar causa y efecto

---

## Pregunta 5 (Media B - Dificultad 3)

**ID:** `CO-LEN-03-comprension-001-v5`

### Enunciado

¿Cómo sabemos que el perrito estaba feliz cuando Tomás le dio el pan?

### Opciones

- [ ] A) Porque ladró muy fuerte
- [x] B) Porque movió la cola con alegría
- [ ] C) Porque saltó sobre Tomás
- [ ] D) Porque lamió la mano de la abuela

### Explicación Pedagógica

El texto dice: "El animal movió la cola con alegría y comió rápidamente."
Mover la cola es una forma en que los perros muestran que están contentos.

**Competencia evaluada:** Comprensión literal - identificar emociones de los personajes

---

## Pregunta 6 (Difícil A - Dificultad 4)

**ID:** `CO-LEN-03-comprension-001-v6`

### Enunciado

¿Qué tipo de persona es Tomás según el texto?

### Opciones

- [ ] A) Una persona miedosa que no le gustan los animales
- [ ] B) Una persona egoísta que solo piensa en sí mismo
- [x] C) Una persona bondadosa que se preocupa por los demás
- [ ] D) Una persona distraída que siempre se pierde

### Explicación Pedagógica

**Análisis del personaje:**
- Tomás vio al perrito con hambre y se preocupó por él
- Pidió a su abuela ayuda para alimentarlo
- Se acercó "despacio" (con cuidado) al perrito
- La abuela dijo que tenía "un corazón muy bueno"
- Siguió llevando comida al perrito cada sábado

Todas estas acciones muestran que Tomás es bondadoso y se preocupa por los demás, incluso por un perrito callejero.

**Competencia evaluada:** Inferencia - caracterización de personajes

---

## Pregunta 7 (Difícil B - Dificultad 5)

**ID:** `CO-LEN-03-comprension-001-v7`

### Enunciado

Lee el siguiente fragmento:

*"—Abuela, ¿podemos darle algo de comer? —preguntó Tomás con tristeza."*

¿Qué nos dice la palabra "tristeza" sobre cómo se sentía Tomás?

### Opciones

- [ ] A) Tomás estaba enojado porque no quería ir al mercado
- [ ] B) Tomás estaba cansado de caminar tanto
- [x] C) Tomás se sentía mal al ver al perrito con hambre
- [ ] D) Tomás tenía miedo del perrito

### Explicación Pedagógica

**Análisis de la emoción:**
La palabra "tristeza" nos dice que Tomás se sintió triste al ver al perrito con hambre. No estaba enojado, ni cansado, ni tenía miedo. Ver a un animal sufriendo le causó tristeza, lo que muestra su sensibilidad y empatía.

Este tipo de pregunta requiere:
1. Identificar la emoción mencionada (tristeza)
2. Conectarla con la situación (ver al perrito hambriento)
3. Descartar emociones que no corresponden al contexto

**Competencia evaluada:** Inferencia - comprensión de emociones y vocabulario emocional

---

## 📊 Metadata de Validación

| Pregunta | ID | Dificultad | Tipo | Validado |
|----------|-----|------------|------|----------|
| 1 | CO-LEN-03-comprension-001-v1 | 3 | Inferencia | ⬜ |
| 2 | CO-LEN-03-comprension-001-v2 | 1 | Literal | ⬜ |
| 3 | CO-LEN-03-comprension-001-v3 | 2 | Literal | ⬜ |
| 4 | CO-LEN-03-comprension-001-v4 | 3 | Literal | ⬜ |
| 5 | CO-LEN-03-comprension-001-v5 | 3 | Literal | ⬜ |
| 6 | CO-LEN-03-comprension-001-v6 | 4 | Inferencia | ⬜ |
| 7 | CO-LEN-03-comprension-001-v7 | 5 | Inferencia | ⬜ |
