---
id: "AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle"
country: "argentina"
grado: 9
asignatura: "ingles"
tema: "future-tenses"
periodo: "weekly"
week: "W03"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 12
bundle_size: 12
alignment: "NAP/Aprender"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---
## Question 1 [D3-D4]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v1
**Bloom:** Remember
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.85
**Contexto:** Martina y su familia están planeando sus próximas vacaciones de verano.

### Enunciado
Choose the correct option to express a future plan already decided: "Next January, we _______ to Bariloche. We already bought the tickets."

### Opciones
- [ ] A) will go <!-- feedback: 'Will' se usa más para decisiones espontáneas o promesas, no para planes con reservas hechas. -->
- [x] B) are going <!-- feedback: ¡Correcto! El presente continuo se usa para planes fijos (arrangements) con fecha y lugar decididos. -->
- [ ] C) go <!-- feedback: El presente simple solo se usa para horarios oficiales, no para planes personales de vacaciones. -->
- [ ] D) going <!-- feedback: Falta el verbo auxiliar 'are' para formar el presente continuo con sentido de futuro. -->

### Explicacion Pedagogica
Cuando ya tenés los pasajes en la mano (como Martina para ir al sur), lo más natural es usar el presente continuo con sentido de futuro: "We are going". Indica que el plan está súper firme y organizado.

---

## Question 2 [D3-D4]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v2
**Bloom:** Remember
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.80
**Contexto:** Facundo está en la cocina y se da cuenta de que no hay más yerba.

### Enunciado
For a spontaneous decision made at the moment of speaking, which form is correct? "Oh, we don't have any yerba mate. I _______ to the kiosk to buy some."

### Opciones
- [x] A) will go <!-- feedback: ¡Muy bien! Usamos 'will' para decisiones que tomamos justo en el momento en que hablamos. -->
- [ ] B) am going to go <!-- feedback: 'Be going to' se usa para intenciones que ya teníamos pensadas de antes. -->
- [ ] C) am going <!-- feedback: El presente continuo sugiere un plan organizado, pero acá la decisión fue repentina. -->
- [ ] D) goes <!-- feedback: El presente simple no se usa para decisiones espontáneas sobre el futuro. -->

### Explicacion Pedagogica
Si te das cuenta de algo y decidís actuar en el momento (como ir a comprar yerba), usás "will". Es la forma típica para las reacciones rápidas ante una situación presente.

---

## Question 3 [D3-D4]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v3
**Bloom:** Understand
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.75
**Contexto:** Un pronóstico meteorológico para la zona de CABA y alrededores.

### Enunciado
When making a prediction based on what we see (evidence), which structure is preferred? "Look at those dark clouds! It _______ rain."

### Opciones
- [ ] A) will <!-- feedback: 'Will' se usa para predicciones basadas en opinión o conocimiento general, no necesariamente en evidencia visual inmediata. -->
- [x] B) is going to <!-- feedback: ¡Exacto! Usamos 'be going to' para predicciones cuando vemos señales claras de que algo va a pasar. -->
- [ ] C) is raining <!-- feedback: Esto diría que ya está lloviendo, pero las nubes indican que va a pasar en breves. -->
- [ ] D) rains <!-- feedback: El presente simple no se usa para predicciones climáticas inmediatas. -->

### Explicacion Pedagogica
Si ves las nubes negras sobre el Obelisco, tenés evidencia de que va a llover. En ese caso, la regla dice que debés usar "is going to rain". Guardá el "will" para cuando creas que va a llover pero no tengas la prueba frente a tus ojos.

---

## Question 4 [D5-D6]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v4
**Bloom:** Apply
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.70
**Contexto:** Lucía habla sobre sus metas académicas para el próximo año en Córdoba.

### Enunciado
Choose the best option for an intention (something you want to do but haven't organized yet): "Next year, I _______ study Architecture at the University of Córdoba."

### Opciones
- [ ] A) will <!-- feedback: Suena más a una promesa o decisión del momento que a una meta planeada. -->
- [x] B) am going to <!-- feedback: ¡Correcto! 'Be going to' es la forma ideal para hablar de intenciones y proyectos personales. -->
- [ ] C) study <!-- feedback: Falta la marca de futuro; esto suena a una rutina presente. -->
- [ ] D) am studying <!-- feedback: Esto sugeriría que ya tiene todo el cronograma y la inscripción cerrada como un arreglo fijo. -->

### Explicacion Pedagogica
Para tus sueños o proyectos (como la carrera que querés seguir), usás "am going to". Indica que ya lo estuviste pensando y que es tu intención firme hacerlo.

---

## Question 5 [D5-D6]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v5
**Bloom:** Apply
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.65
**Contexto:** Un mozo en un restaurante de Mendoza toma un pedido.

### Enunciado
What does the waiter say when a customer decides what to eat?
Customer: "I'll have the empanadas, please."
Waiter: "Excellent choice. I _______ them to you in a minute."

### Opciones
- [ ] A) am bringing <!-- feedback: No es un plan organizado previamente, es una respuesta a un pedido. -->
- [x] B) will bring <!-- feedback: ¡Muy bien! 'Will' se usa para ofrecerse a hacer algo o para promesas inmediatas. -->
- [ ] C) bring <!-- feedback: El presente simple no suena natural para una promesa de acción futura en este contexto. -->
- [ ] D) am going to bring <!-- feedback: Aunque se entiende, 'will' es más común para este tipo de ofrecimientos de servicio. -->

### Explicacion Pedagogica
En un restaurante, los ofrecimientos de los mozos y los pedidos de los clientes casi siempre van con "will" ('ll). Es una forma de decir "lo voy a hacer ahora mismo por vos".

---

## Question 6 [D5-D6]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v6
**Bloom:** Apply
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.60
**Contexto:** Un grupo de amigos organiza una juntada para comer un asado el domingo.

### Enunciado
Fill in the blanks with the correct future forms: "We _______ (have) an asado on Sunday. I _______ (buy) the meat on Friday, I already talked to the butcher."

### Opciones
- [ ] A) will have / will buy <!-- feedback: Suena muy incierto para algo que ya están organizando y hasta hablaron con el carnicero. -->
- [x] B) are having / am going to buy <!-- feedback: ¡Exacto! El asado es un evento social organizado (present continuous) y comprar la carne es la intención planeada. -->
- [ ] C) have / buy <!-- feedback: Esto describe rutinas, no un evento específico para este domingo. -->
- [ ] D) are going to have / will buy <!-- feedback: Podría ser, pero la segunda parte es más una intención firme que una decisión espontánea. -->

### Explicacion Pedagogica
Para organizar una salida o reunión (como un asado), lo más común es el presente continuo ("are having"). Y para las tareas que ya sabés que tenés que hacer para ese evento, usás "am going to".

---

## Question 7 [D7-D8]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v7
**Bloom:** Analyze
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.55
**Contexto:** Una discusión sobre el futuro de la tecnología en Argentina hacia el año 2050.

### Enunciado
For predictions about the distant future based on opinion, which is more appropriate? "I think that in 2050, most cars in Argentina _______ electric."

### Opciones
- [x] A) will be <!-- feedback: ¡Correcto! Usamos 'will' con verbos de opinión como 'I think' para predicciones a largo plazo. -->
- [ ] B) are going to be <!-- feedback: 'Going to' se prefiere para el futuro cercano o con evidencia, no para opiniones abstractas. -->
- [ ] C) are being <!-- feedback: Esto es presente continuo y no tiene sentido para una predicción de 2050. -->
- [ ] D) be <!-- feedback: Gramaticalmente incorrecto; le falta el auxiliar de futuro. -->

### Explicacion Pedagogica
Cuando das tu opinión sobre cómo va a ser el mundo dentro de mucho tiempo ("I think..."), lo mejor es usar "will". Es el futuro de las ideas y las creencias personales.

---

## Question 8 [D7-D8]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v8
**Bloom:** Analyze
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.50
**Contexto:** El uso del presente simple para el futuro en horarios de transporte en Buenos Aires.

### Enunciado
Select the sentence that correctly uses the Present Simple for a future timetable:

### Opciones
- [ ] A) I am leaving for Mendoza tomorrow morning. <!-- feedback: Es un plan personal, por lo que usa presente continuo. -->
- [x] B) The train to Tigre leaves in ten minutes from Retiro station. <!-- feedback: ¡Muy bien! Los horarios de transporte público (trenes, colectivos) usan presente simple. -->
- [ ] C) I think it leaves later. <!-- feedback: Aunque usa presente simple, la frase 'I think' sugiere una opinión más que un dato de horario oficial. -->
- [ ] D) The train will leave soon. <!-- feedback: Esto es una predicción general, no el uso específico de horarios fijos. -->

### Explicacion Pedagogica
Acordate siempre: para trenes, aviones, colectivos o funciones de teatro, usás el presente simple aunque hables del futuro. Es "The train leaves", no importa si falta una hora. Es una regla de oro para viajar por Argentina.

---

## Question 9 [D7-D8]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v10
**Bloom:** Evaluate
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.40
**Contexto:** Distinguir entre una promesa y una intención en un diálogo.

### Enunciado
Compare these two sentences:
1. "I will help you with your homework later."
2. "I am going to help you with your homework later."
Which one sounds like a formal promise or an offer made right now?

### Opciones
- [x] A) Sentence 1 <!-- feedback: ¡Correcto! 'Will' es el tiempo verbal para las promesas y los ofrecimientos espontáneos. -->
- [ ] B) Sentence 2 <!-- feedback: Esto suena más a que ya tenías planeado ayudar antes de que te lo pidieran. -->
- [ ] C) Both are exactly the same. <!-- feedback: No, el matiz de intención previa vs. compromiso del momento es diferente. -->
- [ ] D) Neither is correct. <!-- feedback: Ambas son gramaticalmente correctas, solo cambia el matiz comunicativo. -->

### Explicacion Pedagogica
Si querés quedar bien y prometerle algo a alguien en el momento ("Te prometo que te ayudo"), usás "will". El "going to" suena más a un plan que ya tenías en la agenda. ¡Usá "will" para ser servicial!

---

## Question 10 [D9-D10]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v9
**Bloom:** Analyze
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.45
**Contexto:** Una advertencia sobre un peligro inminente en una excursión.

### Enunciado
Identify the correct use of future for an immediate danger: "Careful! You _______ fall into that hole!"

### Opciones
- [ ] A) will <!-- feedback: No es una predicción lejana u opinión; es algo que está a punto de pasar por lo que ves. -->
- [x] B) are going to <!-- feedback: ¡Exacto! Para peligros inmediatos que estamos viendo, usamos 'be going to'. -->
- [ ] C) falling <!-- feedback: Gramaticalmente incompleto. -->
- [ ] D) are falling <!-- feedback: Esto significaría que ya estás en el aire cayendo. -->

### Explicacion Pedagogica
Si ves que alguien está por pisar mal o tropezarse, la advertencia es con "going to". Es porque tenés la evidencia visual de que el accidente es casi inevitable. ¡Cuidado con los pozos!

---

## Question 11 [D9-D10]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v11
**Bloom:** Evaluate
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.35
**Contexto:** Analizando errores en el uso de auxiliares para el futuro.

### Enunciado
What is wrong with this question? "When you will travel to Ushuaia?"

### Opciones
- [ ] A) The verb should be 'traveling'. <!-- feedback: No es necesario si se quiere usar 'will'. -->
- [x] B) The word order is wrong; it should be 'When will you travel...?'. <!-- feedback: ¡Exacto! En las preguntas con 'will', el auxiliar debe ir antes del sujeto. -->
- [ ] C) You cannot use 'will' with 'travel'. <!-- feedback: Por supuesto que se puede; es un uso muy común. -->
- [ ] D) It needs the auxiliary 'do'. <!-- feedback: 'Will' ya es el auxiliar de futuro, no necesita a 'do'. -->

### Explicacion Pedagogica
¡Ojo con las preguntas! No es como en castellano donde solo cambiás el tono. En inglés tenés que "saltar" el "will" adelante del sujeto: "When will you...?". Si no hacés ese cambio, la pregunta está mal armada.

---

## Question 12 [D9-D10]
**ID:** AR-ING-9-2026-W03-future-tenses-001-MASTERY-bundle-v12
**Bloom:** Evaluate
**EJE:** Lengua Extranjera - Inglés
**Expected_Success:** 0.30
**Contexto:** Un uso avanzado del futuro en cláusulas de tiempo (time clauses).

### Enunciado
Complete the complex sentence correctly: "As soon as I _______ (arrive) in Salta, I _______ (call) you."

### Opciones
- [ ] A) will arrive / will call <!-- feedback: Después de conectores de tiempo como 'as soon as' o 'when', no se usa 'will' para el futuro. -->
- [x] B) arrive / will call <!-- feedback: ¡Excelente! Usamos presente simple en la cláusula de tiempo y 'will' en la cláusula principal. -->
- [ ] C) will arrive / call <!-- feedback: La regla es al revés; el presente va con el conector de tiempo. -->
- [ ] D) am arriving / am going to call <!-- feedback: Aunque se entiende, la estructura estándar para estas condiciones de tiempo es Presente/Futuro. -->

### Explicacion Pedagogica
Esta es una regla difícil: cuando usás palabras como "when", "as soon as" o "until" para hablar del futuro, el verbo que viene inmediatamente después va en presente simple. Es "As soon as I arrive" (apenas llegue). La segunda parte de la frase sí lleva el "will". ¡Acordate de esto para sonar como un experto!

---

[//]: # (QUALITY_REVIEW)
| Dimension | Score |
|-----------|-------|
| Técnico | 30/30 |
| Curricular | 40/40 |
| Contexto | 20/20 |
| Redacción | 10/10 |
| **Total** | **100/100** |
