---
id: "CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "environmental-ethics"
periodo: "weekly"
week: "W25"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Environmental Ethics (W25)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v1`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Introduction to environmental philosophy.

### Enunciado
Anthropocentrism is the belief that human beings are the most important entity in the universe.

### Opciones
- [ ] A) Biocentrism <!-- feedback: Incorrect. Biocentrism values all life. -->
- [x] B) Anthropocentrism <!-- feedback: Correct. Specific term for human-centered views. -->
- [ ] C) Ecocentrism <!-- feedback: Incorrect. Ecocentrism values the whole ecosystem. -->
- [ ] D) Egocentrism <!-- feedback: Incorrect. Egocentrism is being self-centered. -->

### Explicacion Pedagogica
'Anthropocentrism' is the philosophical term for a human-centered world view.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v2`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing conservation history.

### Enunciado
National parks were created to preserve wilderness areas for future generations.

### Opciones
- [ ] A) create <!-- feedback: Incorrect. Active voice. -->
- [x] B) were created <!-- feedback: Correct. Past simple passive for a completed historical action. -->
- [ ] C) have created <!-- feedback: Incorrect. Active voice. -->
- [ ] D) are created <!-- feedback: Incorrect. Present tense. -->

### Explicacion Pedagogica
The past simple passive 'were created' describes the founding of national parks in the past.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v3`
**Bloom:** Apply
**ICFES:** Grammar
**Expected_Success:** 0.65
**Contexto:** Speculating about environmental protection.

### Enunciado
If we valued nature more for its own sake, we would protect it more effectively.

### Opciones
- [ ] A) value <!-- feedback: Incorrect. First conditional. -->
- [x] B) valued <!-- feedback: Correct. Second conditional for a hypothetical present attitude. -->
- [ ] C) had valued <!-- feedback: Incorrect. Third conditional. -->
- [ ] D) would value <!-- feedback: Incorrect. 'Would' not in 'if' clause. -->

### Explicacion Pedagogica
The second conditional 'if + past simple' describes how a hypothetical shift in values would lead to a different result.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v4`
**Bloom:** Analyze
**ICFES:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about "intergenerational justice".

### Enunciado
Intergenerational justice involves the ethical obligation to leave a healthy planet for those who will be born in the future.

### Opciones
- [x] A) obligation <!-- feedback: Correct. An obligation is a duty or commitment. -->
- [ ] B) choice <!-- feedback: Incorrect. Ethics often frames this as a duty, not just a casual choice. -->
- [ ] C) refusal <!-- feedback: Incorrect. Negative. -->
- [ ] D) accident <!-- feedback: Incorrect. It should be an intentional effort. -->

### Explicacion Pedagogica
'Obligation' is the key noun for describing the moral duty we have towards future generations.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v5`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on "intrinsic value".

### Enunciado
Environmentalists argue that nature has intrinsic value, meaning it is valuable in itself, regardless of its usefulness to humans.

### Opciones
- [ ] A) instrumental <!-- feedback: Incorrect. Instrumental value is value as a tool/resource. -->
- [x] B) intrinsic <!-- feedback: Correct. Intrinsic value is value for its own sake. -->
- [ ] C) commercial <!-- feedback: Incorrect. Commercial value is based on profit. -->
- [ ] D) temporary <!-- feedback: Incorrect. Values are often seen as permanent. -->

### Explicacion Pedagogica
'Intrinsic' describes value that exists independently of any external use or benefit.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v6`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Sustainable development definition.

### Enunciado
Sustainability means meeting our own needs without compromising the ability of future generations to meet theirs.

### Opciones
- [ ] A) destroying <!-- feedback: Incorrect. Comprises sustainability. -->
- [x] B) compromising <!-- feedback: Correct. To compromise here means to weaken or damage. -->
- [ ] C) increasing <!-- feedback: Incorrect. Doesn't fit the standard definition. -->
- [ ] D) ignoring <!-- feedback: Incorrect. Sustainability requires attention to future needs. -->

### Explicacion Pedagogica
'Compromising' is the standard verb used in the Brundtland definition of sustainable development.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v7`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing an ongoing ethical shift.

### Enunciado
The rights of animals are increasingly being debated in legal and philosophical circles.

### Opciones
- [ ] A) are debating <!-- feedback: Incorrect. Active voice. -->
- [x] B) are being debated <!-- feedback: Correct. Present continuous passive for an ongoing trend. -->
- [ ] C) debated <!-- feedback: Incorrect. Past simple. -->
- [ ] D) have debated <!-- feedback: Incorrect. Active voice. -->

### Explicacion Pedagogica
The present continuous passive describes a discussion that is currently expanding.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v8`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Text on "stewardship".

### Enunciado
Environmental stewardship is the responsible use and protection of the natural environment through conservation and sustainable practices.

### Opciones
- [ ] A) Ownership <!-- feedback: Incorrect. Stewardship is about care, not just possessing. -->
- [x] B) Stewardship <!-- feedback: Correct. The term for responsible management of nature. -->
- [ ] C) Exploitation <!-- feedback: Incorrect. The opposite of stewardship. -->
- [ ] D) Neglect <!-- feedback: Incorrect. The opposite of stewardship. -->

### Explicacion Pedagogica
'Stewardship' is the specific term for the ethical responsibility to care for and manage the environment.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v9`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing a past change in perspective.

### Enunciado
By the 1970s, the modern environmental movement had gained significant momentum.

### Opciones
- [ ] A) gains <!-- feedback: Incorrect. Present tense. -->
- [ ] B) has gained <!-- feedback: Incorrect. Present perfect. -->
- [x] C) had gained <!-- feedback: Correct. Past perfect for a state completed by a past point. -->
- [ ] D) was gaining <!-- feedback: Incorrect. Focus is on the completed state of having momentum. -->

### Explicacion Pedagogica
The past perfect 'had gained' indicates that the movement's growth occurred before the start of the decade mentioned.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v10`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of "speciesism".

### Enunciado
Speciesism is the assumption of human superiority leading to the exploitation of animals.

### Opciones
- [ ] A) equality <!-- feedback: Incorrect. Speciesism is based on inequality. -->
- [x] B) superiority <!-- feedback: Correct. Superiority is the state of being higher in rank or quality. -->
- [ ] C) indifference <!-- feedback: Incorrect. It's an active assumption of being 'better'. -->
- [ ] D) kindness <!-- feedback: Incorrect. Speciesism leads to exploitation, not kindness. -->

### Explicacion Pedagogica
'Superiority' is the core assumption that underlies the ethical critique of speciesism.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v11`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Definition of ecology.

### Enunciado
Ecology is the branch of biology that deals with the relations of organisms to one another and to their physical surroundings.

### Opciones
- [ ] A) Physics <!-- feedback: Incorrect. -->
- [x] B) Ecology <!-- feedback: Correct. Scientific study of ecosystems. -->
- [ ] C) Anatomy <!-- feedback: Incorrect. Study of body structure. -->
- [ ] D) Chemistry <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
'Ecology' is the specific science focused on the interactions within the natural world.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v12`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a future ethical goal.

### Enunciado
In the future, it is hoped that "ecocide" will have been recognized as an international crime.

### Opciones
- [ ] A) will recognize <!-- feedback: Incorrect. Active voice. -->
- [x] B) will have been recognized <!-- feedback: Correct. Future perfect passive for a completed state in the future. -->
- [ ] C) recognized <!-- feedback: Incorrect. Past simple. -->
- [ ] D) recognizing <!-- feedback: Incorrect. Gerund. -->

### Explicacion Pedagogica
The future perfect passive describes a legal milestone that is desired to be a finished fact in the future.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v13`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Discussing biodiversity.

### Enunciado
Biodiversity is crucial for ecosystem resilience, as it allows systems to recover from disturbances.

### Opciones
- [x] A) resilience <!-- feedback: Correct. Resilience is the ability to recover from difficulties. -->
- [ ] B) fragility <!-- feedback: Incorrect. Biodiversity reduces fragility. -->
- [ ] C) stagnation <!-- feedback: Incorrect. Nature is dynamic. -->
- [ ] D) isolation <!-- feedback: Incorrect. Systems are interconnected. -->

### Explicacion Pedagogica
'Resilience' describes the strength and recovery capacity of a healthy, diverse ecosystem.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v14`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Conditional about plastic pollution.

### Enunciado
If we didn't produce so much single-use plastic, our oceans would be much cleaner.

### Opciones
- [ ] A) don't produce <!-- feedback: Incorrect. First conditional. -->
- [x] B) didn't produce <!-- feedback: Correct. Second conditional for a hypothetical change. -->
- [ ] C) hadn't produced <!-- feedback: Incorrect. Third conditional. -->
- [ ] D) wouldn't produce <!-- feedback: Incorrect. 'Would' not in 'if' clause. -->

### Explicacion Pedagogica
The second conditional 'if + past simple' describes the imaginary result of a change in current production habits.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v15`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion of an essay on "environmental justice".

### Enunciado
The author concludes that environmental justice requires that no group of people should bear a disproportionate share of negative environmental consequences.

### Opciones
- [ ] A) equal <!-- feedback: Incorrect. If it were equal, there would be no 'disproportionate' share. -->
- [x] B) disproportionate <!-- feedback: Correct. Disproportionate means too large or too small in comparison with something else. -->
- [ ] C) minimal <!-- feedback: Incorrect. The goal is for everyone to have minimal, not for one group to bear it all. -->
- [ ] D) invisible <!-- feedback: Incorrect. Consequences are often very visible. -->

### Explicacion Pedagogica
'Disproportionate' is the key term for describing the unfair burden placed on certain communities in environmental ethics.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v16`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Definition of a carbon sink.

### Enunciado
A carbon sink is a forest, ocean, or other natural environment viewed in terms of its ability to absorb carbon dioxide from the atmosphere.

### Opciones
- [ ] A) source <!-- feedback: Incorrect. A source releases carbon. -->
- [x] B) sink <!-- feedback: Correct. A sink absorbs/stores carbon. -->
- [ ] C) waste <!-- feedback: Incorrect. -->
- [ ] D) filter <!-- feedback: Incorrect. While it acts like a filter, 'sink' is the technical term. -->

### Explicacion Pedagogica
'Carbon sink' is the standard environmental term for natural systems that absorb more carbon than they release.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v17`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Reporting an environmental warning.

### Enunciado
Scientists warned that the loss of biodiversity would have irreversible effects on human food security.

### Opciones
- [ ] A) will have <!-- feedback: Incorrect. Backshifted in reported speech. -->
- [x] B) would have <!-- feedback: Correct. Backshifted from 'will have' to 'would have'. -->
- [ ] C) has had <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) have <!-- feedback: Incorrect. Present tense. -->

### Explicacion Pedagogica
In reported speech, we change 'will' to 'would' when reporting a future prediction made in the past.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v18`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Text about "sustainable consumption".

### Enunciado
Sustainable consumption involves using products and services that have a minimal impact on the environment.

### Opciones
- [ ] A) maximal <!-- feedback: Incorrect. We want to reduce impact. -->
- [x] B) minimal <!-- feedback: Correct. Minimal means of a minimum amount, quantity, or degree. -->
- [ ] C) random <!-- feedback: Incorrect. Should be intentional. -->
- [ ] D) hidden <!-- feedback: Incorrect. Should be transparent. -->

### Explicacion Pedagogica
'Minimal' correctly describes the goal of sustainability: to reduce environmental impact as much as possible.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v19`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing a past environmental state.

### Enunciado
The river had been clean for centuries before industrial pollution began to contaminate it.

### Opciones
- [ ] A) was clean <!-- feedback: Incorrect. Past simple doesn't show the long state before the change as well as past perfect. -->
- [x] B) had been clean <!-- feedback: Correct. Past perfect for a state that existed before another past event. -->
- [ ] C) has been clean <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) is clean <!-- feedback: Incorrect. Present tense. -->

### Explicacion Pedagogica
The past perfect 'had been' establishes the long-term cleanliness of the river prior to its contamination.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W25-environmental-ethics-001-MASTERY-v20`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Final thought on environmental responsibility.

### Enunciado
Ultimately, we must move towards a more biocentric worldview that respects the rights of all living things.

### Opciones
- [x] A) biocentric <!-- feedback: Correct. Values all living things. -->
- [ ] B) egoistic <!-- feedback: Incorrect. Values only oneself. -->
- [ ] C) commercial <!-- feedback: Incorrect. Values only profit. -->
- [ ] D) narrow <!-- feedback: Incorrect. We need a broader view. -->

### Explicacion Pedagogica
'Biocentric' describes the inclusive worldview that values all life forms, contrasting with anthropocentrism.
