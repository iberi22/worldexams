---
id: "CL-ING-11-2026-W01-global-issues-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "global-issues"
periodo: "weekly"
week: "W01"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Global Issues (W01)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v1`
**Bloom:** Remember
**EJE:** Reading Comprehension
**Expected_Success:** 0.85
**Contexto:** A news headline about climate change.

### Enunciado
According to recent reports, global temperatures have risen by 1.1 degrees Celsius since the late 19th century.

### Opciones
- [x] A) have risen <!-- feedback: Correct. The present perfect is used for actions starting in the past and continuing to the present. -->
- [ ] B) rose <!-- feedback: Incorrect. This would imply the rising finished at a specific point in the past. -->
- [ ] C) are rising <!-- feedback: Incorrect. While they are rising, the sentence specifies "since the late 19th century." -->
- [ ] D) had risen <!-- feedback: Incorrect. The past perfect is used for an action completed before another past action. -->

### Explicacion Pedagogica
The present perfect 'have risen' is used with 'since' to describe a situation that began in the past and persists in the present.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v2`
**Bloom:** Understand
**EJE:** Vocabulary
**Expected_Success:** 0.75
**Contexto:** Discussion about sustainable energy.

### Enunciado
Transitioning to renewable energy sources is crucial to mitigate the effects of greenhouse gas emissions.

### Opciones
- [ ] A) finite <!-- feedback: Incorrect. Finite means limited, which refers to fossil fuels. -->
- [x] B) renewable <!-- feedback: Correct. Renewable sources like solar and wind are key to sustainability. -->
- [ ] C) fossil <!-- feedback: Incorrect. Fossil fuels are the primary cause of the emissions being discussed. -->
- [ ] D) depleted <!-- feedback: Incorrect. Depleted means used up. -->

### Explicacion Pedagogica
'Renewable' is the appropriate term for energy sources that are not exhausted when used, such as solar or wind power.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v3`
**Bloom:** Apply
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Talking about future environmental goals.

### Enunciado
By the year 2050, many countries will have aimed to achieve net-zero carbon emissions.

### Opciones
- [ ] A) will hope <!-- feedback: Incorrect. This expresses a simple future hope rather than a projected completion. -->
- [x] B) will have aimed <!-- feedback: Correct. The future perfect is used to describe an action that will be completed by a certain point in the future. -->
- [ ] C) are aiming <!-- feedback: Incorrect. This describes a current action, not one relative to 2050. -->
- [ ] D) aimed <!-- feedback: Incorrect. This is the past tense. -->

### Explicacion Pedagogica
The future perfect 'will have aimed' is used to project the state of an action at a specific future deadline.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v4`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** An article about biodiversity loss.

### Enunciado
The extinction of certain species often leads to a domino effect, disrupting entire ecosystems and threatening human food security.

### Opciones
- [ ] A) static <!-- feedback: Incorrect. Static means unchanging. -->
- [x] B) domino <!-- feedback: Correct. A domino effect occurs when one event sets off a chain of similar events. -->
- [ ] C) isolated <!-- feedback: Incorrect. The text describes disruption across "entire ecosystems," not in isolation. -->
- [ ] D) reversible <!-- feedback: Incorrect. Extinction is usually permanent and the resulting disruptions are difficult to reverse. -->

### Explicacion Pedagogica
The term 'domino effect' metaphorically describes how the loss of one species triggers a sequence of negative impacts throughout an ecosystem.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v5`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Debate on international cooperation for global health.

### Enunciado
Critics argue that the global response to the pandemic was fragmented. Which of the following best summarizes this perspective?

### Opciones
- [ ] A) All nations worked in perfect harmony from the beginning. <!-- feedback: Incorrect. This contradicts the term 'fragmented'. -->
- [ ] B) The response was led by a single, powerful global entity. <!-- feedback: Incorrect. Fragmented implies lack of central leadership or unity. -->
- [x] C) Nations prioritized individual interests over a unified global strategy. <!-- feedback: Correct. A fragmented response implies lack of coordination and division. -->
- [ ] D) Technology was the only barrier to a successful outcome. <!-- feedback: Incorrect. The focus of the criticism is on the 'response', not just technology. -->

### Explicacion Pedagogica
'Fragmented' in this context means disjointed or lacking unity, indicating that countries did not cooperate effectively.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v6`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Urbanization and its impact.

### Enunciado
The rapid growth of cities, known as urbanization, presents challenges for infrastructure and waste management.

### Opciones
- [ ] A) ruralization <!-- feedback: Incorrect. This is the opposite of city growth. -->
- [x] B) urbanization <!-- feedback: Correct. Urbanization refers to the process of making an area more urban. -->
- [ ] C) migration <!-- feedback: Incorrect. While migration causes urbanization, the term for city growth itself is urbanization. -->
- [ ] D) congestion <!-- feedback: Incorrect. Congestion is a symptom of urbanization, not the name of the process. -->

### Explicacion Pedagogica
Urbanization is the standard sociological and geographical term for the expansion and development of cities.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v7`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Discussion on poverty reduction.

### Enunciado
If more developed countries increased their foreign aid, many developing nations could improve their healthcare systems faster.

### Opciones
- [ ] A) increase <!-- feedback: Incorrect. This would be used in a first conditional for a real possibility. -->
- [x] B) increased <!-- feedback: Correct. The second conditional (if + past simple, could + verb) is used for hypothetical situations. -->
- [ ] C) would increase <!-- feedback: Incorrect. 'Would' is not used in the 'if' clause of a conditional. -->
- [ ] D) have increased <!-- feedback: Incorrect. This does not fit the conditional structure. -->

### Explicacion Pedagogica
The second conditional describes a hypothetical situation in the present or future, using the past simple in the 'if' clause.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v8`
**Bloom:** Apply
**EJE:** Reading Comprehension
**Expected_Success:** 0.65
**Contexto:** Text about digital divide.

### Enunciado
The "digital divide" refers to the gap between those who have access to information and communication technology and those who do not.

### Opciones
- [ ] A) modern transportation <!-- feedback: Incorrect. The digital divide is about information technology. -->
- [x] B) information and communication technology <!-- feedback: Correct. This is the definition of the digital divide. -->
- [ ] C) free healthcare services <!-- feedback: Incorrect. This is a different type of social inequality. -->
- [ ] D) quality higher education <!-- feedback: Incorrect. While related, the primary focus is on digital access. -->

### Explicacion Pedagogica
The digital divide specifically concerns the inequality in access to computers, the internet, and related technologies.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v9`
**Bloom:** Analyze
**EJE:** Vocabulary
**Expected_Success:** 0.70
**Contexto:** Environmental advocacy.

### Enunciado
Reducing our carbon footprint is one of the most effective ways individuals can contribute to fighting climate change.

### Opciones
- [x] A) footprint <!-- feedback: Correct. A carbon footprint is the total amount of greenhouse gases produced by an individual or entity. -->
- [ ] B) track <!-- feedback: Incorrect. 'Track' is not the standard term used with 'carbon' in this context. -->
- [ ] C) impact <!-- feedback: Incorrect. While we reduce our 'impact', the specific idiomatic term is 'footprint'. -->
- [ ] D) shadow <!-- feedback: Incorrect. 'Shadow' is not used in this scientific context. -->

### Explicacion Pedagogica
'Carbon footprint' is a common environmental term for the measure of impact on the climate.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v10`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Excerpt on ethical consumerism.

### Enunciado
The text suggests that consumer choices potentially have the power to influence corporate behavior regarding labor rights.

### Opciones
- [ ] A) rarely <!-- feedback: Incorrect. Ethical consumerism is based on the idea that choices *do* have power. -->
- [ ] B) never <!-- feedback: Incorrect. This contradicts the premise of the text. -->
- [x] C) potentially <!-- feedback: Correct. The text implies a possibility of influence through collective consumer action. -->
- [ ] D) accidentally <!-- feedback: Incorrect. Consumer choices in this context are deliberate and intended to influence. -->

### Explicacion Pedagogica
'Potentially' captures the idea that while one choice might not change a company, the aggregate of many choices can.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Grammar
**Expected_Success:** 0.80
**Contexto:** General fact about the UN.

### Enunciado
The United Nations was established in 1945 to promote international cooperation.

### Opciones
- [ ] A) is <!-- feedback: Incorrect. The UN was established in the past. -->
- [x] B) was <!-- feedback: Correct. Use the past simple passive for a completed action in the past. -->
- [ ] C) has been <!-- feedback: Incorrect. This would imply it is still in the process of being established. -->
- [ ] D) were <!-- feedback: Incorrect. 'The United Nations' is treated as a singular organization. -->

### Explicacion Pedagogica
A specific past date (1945) requires the past simple.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v12`
**Bloom:** Understand
**EJE:** Vocabulary
**Expected_Success:** 0.75
**Contexto:** Discussion on food security.

### Enunciado
Food insecurity occurs when people lack regular access to enough safe and nutritious food for an active and healthy life.

### Opciones
- [ ] A) Malnutrition <!-- feedback: Incorrect. Malnutrition is the result, but the state of lacking access is food insecurity. -->
- [ ] B) Famine <!-- feedback: Incorrect. Famine is an extreme and widespread scarcity of food. -->
- [x] C) Food insecurity <!-- feedback: Correct. This is the formal term for lacking consistent access to food. -->
- [ ] D) Poverty <!-- feedback: Incorrect. Poverty is a cause, but not the specific definition of the food situation. -->

### Explicacion Pedagogica
Food insecurity is the technical term for the condition described.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v13`
**Bloom:** Apply
**EJE:** Grammar
**Expected_Success:** 0.65
**Contexto:** Talking about a past event with current relevance.

### Enunciado
Many refugees had left their homes due to conflict before the international community provided aid.

### Opciones
- [ ] A) leave <!-- feedback: Incorrect. Present tense. -->
- [ ] B) have left <!-- feedback: Incorrect. Present perfect is for past to present. -->
- [x] C) had left <!-- feedback: Correct. The past perfect is used for an action that happened before another past action. -->
- [ ] D) were leaving <!-- feedback: Incorrect. This describes an action in progress. -->

### Explicacion Pedagogica
The past perfect 'had left' establishes the sequence of two past events.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v14`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Text about deforestation.

### Enunciado
The primary driver of deforestation in the Amazon is the expansion of agricultural land for cattle ranching and soy production.

### Opciones
- [ ] A) consequence <!-- feedback: Incorrect. The text lists ranching as the cause. -->
- [x] B) driver <!-- feedback: Correct. In this context, 'driver' means the main cause or force behind a process. -->
- [ ] C) solution <!-- feedback: Incorrect. Agriculture is the problem. -->
- [ ] D) obstacle <!-- feedback: Incorrect. Deforestation is the process. -->

### Explicacion Pedagogica
'Driver' is a common term in social and environmental sciences to describe a primary cause.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v15`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Argument about the benefits and drawbacks of globalization.

### Enunciado
The author implies that while globalization has increased wealth globally, it has also exacerbated economic inequality within many countries.

### Opciones
- [ ] A) eliminated <!-- feedback: Incorrect. Inequality may have worsened. -->
- [x] B) exacerbated <!-- feedback: Correct. To exacerbate means to make a problem or bad situation worse. -->
- [ ] C) stabilized <!-- feedback: Incorrect. Stabilization would mean it stopped changing. -->
- [ ] D) ignored <!-- feedback: Incorrect. Globalization isn't a sentient being. -->

### Explicacion Pedagogica
'Exacerbated' correctly identifies the negative impact of globalization mentioned in the text.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v16`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Human rights discussion.

### Enunciado
The Universal Declaration of Human Rights states that everyone is entitled to life, liberty, and security of person.

### Opciones
- [ ] A) allowed <!-- feedback: Incorrect. 'Entitled' is stronger. -->
- [x] B) entitled <!-- feedback: Correct. To be entitled to something means to have a right to it. -->
- [ ] C) requested <!-- feedback: Incorrect. These are fundamental rights. -->
- [ ] D) supposed <!-- feedback: Incorrect. This implies a weak expectation. -->

### Explicacion Pedagogica
'Entitled' is the formal term used in legal and rights-based contexts.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v17`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Talking about sustainable habits.

### Enunciado
We must use less plastic if we want to protect our oceans from further pollution.

### Opciones
- [ ] A) might <!-- feedback: Incorrect. This expresses a weak possibility. -->
- [ ] B) would <!-- feedback: Incorrect. This is conditional. -->
- [x] C) must <!-- feedback: Correct. 'Must' expresses strong obligation or necessity. -->
- [ ] D) could <!-- feedback: Incorrect. This expresses ability or possibility. -->

### Explicacion Pedagogica
'Must' is used to express a strong necessity, which is appropriate for the urgent tone of environmental protection.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v18`
**Bloom:** Apply
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Text about the circular economy.

### Enunciado
A circular economy aims to eliminate waste by keeping products and materials in use for as long as possible.

### Opciones
- [ ] A) generate <!-- feedback: Incorrect. The goal is the opposite. -->
- [x] B) eliminate <!-- feedback: Correct. The goal of a circular economy is to design out waste. -->
- [ ] C) ignore <!-- feedback: Incorrect. The circular economy is a strategy to manage materials. -->
- [ ] D) export <!-- feedback: Incorrect. Exporting waste is not the same. -->

### Explicacion Pedagogica
The core principle of a circular economy is to minimize or eliminate waste through better design and reuse.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v19`
**Bloom:** Analyze
**EJE:** Vocabulary
**Expected_Success:** 0.70
**Contexto:** News report on water scarcity.

### Enunciado
Many regions are facing severe water shortages, leading to conflicts over access to clean drinking water.

### Opciones
- [ ] A) abundance <!-- feedback: Incorrect. Abundance means plenty. -->
- [ ] B) surplus <!-- feedback: Incorrect. Surplus means an extra amount. -->
- [x] C) shortages <!-- feedback: Correct. A shortage is a state where something needed cannot be obtained in sufficient amounts. -->
- [ ] D) flows <!-- feedback: Incorrect. 'Flows' refers to the movement of water. -->

### Explicacion Pedagogica
'Shortages' correctly describes the lack of water that causes the mentioned conflicts.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W01-global-issues-001-MASTERY-v20`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Conclusion of an essay on social justice.

### Enunciado
The author concludes that systemic change is required to address the root causes of global poverty.

### Opciones
- [ ] A) hide <!-- feedback: Incorrect. The author wants to solve the problem. -->
- [ ] B) maintain <!-- feedback: Incorrect. The author wants to change the system. -->
- [x] C) address <!-- feedback: Correct. In this context, to 'address' means to think about and begin to deal with an issue. -->
- [ ] D) witness <!-- feedback: Incorrect. 'Witnessing' is passive. -->

### Explicacion Pedagogica
'Address' is the appropriate verb for dealing with complex social issues.
