---
id: "CL-ING-11-2026-W31-creative-writing-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "creative-writing"
periodo: "weekly"
week: "W31"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Creative Writing (W31)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v1`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Introduction to figurative language.

### Enunciado
A metaphor is a figure of speech that describes an object or action in a way that isn’t literally true, but helps explain an idea or make a comparison.

### Opciones
- [ ] A) Fact <!-- feedback: Incorrect. A metaphor is not literally true. -->
- [x] B) Metaphor <!-- feedback: Correct. Specific term for this type of comparison. -->
- [ ] C) Slogan <!-- feedback: Incorrect. Used in advertising. -->
- [ ] D) Archive <!-- feedback: Incorrect. A collection of records. -->

### Explicacion Pedagogica
'Metaphor' is the foundational literary term for non-literal comparisons used to create imagery.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v2`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing narrative voice.

### Enunciado
First-person narration is a mode of storytelling in which the narrator is a character in the story and speaks for themselves.

### Opciones
- [ ] A) speak <!-- feedback: Incorrect. Subject-verb agreement ('narrator' is singular). -->
- [x] B) speaks <!-- feedback: Correct. Present simple for a definition. -->
- [ ] C) speaking <!-- feedback: Incorrect. Gerund. -->
- [ ] D) spoke <!-- feedback: Incorrect. Past tense. -->

### Explicacion Pedagogica
The present simple 'speaks' describes the constant function of a narrative style.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v3`
**Bloom:** Apply
**ICFES:** Grammar
**Expected_Success:** 0.65
**Contexto:** Speculating about a plot twist.

### Enunciado
If the author had revealed the secret in the first chapter, the ending wouldn't have been so shocking.

### Opciones
- [ ] A) revealed <!-- feedback: Incorrect. Second conditional. -->
- [x] B) had revealed <!-- feedback: Correct. Third conditional for hypothetical past. -->
- [ ] C) has revealed <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) would reveal <!-- feedback: Incorrect. 'Would' not used in 'if' clause. -->

### Explicacion Pedagogica
The third conditional is used to imagine how a different narrative choice would have affected the reader's experience.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v4`
**Bloom:** Analyze
**ICFES:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about "show, don't tell".

### Enunciado
"Show, don't tell" is a technique used in creative writing to allow the reader to experience the story through action and sensory details.

### Opciones
- [x] A) sensory <!-- feedback: Correct. Sensory details relate to the five senses. -->
- [ ] B) boring <!-- feedback: Incorrect. The technique aims to make writing more engaging. -->
- [ ] C) hidden <!-- feedback: Incorrect. Details should be present to 'show' the scene. -->
- [ ] D) numerical <!-- feedback: Incorrect. Unrelated to the creative technique. -->

### Explicacion Pedagogica
'Sensory' details are the sights, sounds, smells, etc., that writers use to create immersive scenes.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v5`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on "character development".

### Enunciado
Effective character development involves creating a protagonist who is complex and multi-dimensional.

### Opciones
- [ ] A) simple <!-- feedback: Incorrect. Good characters are usually complex. -->
- [x] B) complex <!-- feedback: Correct. Complex characters have many parts or aspects. -->
- [ ] C) invisible <!-- feedback: Incorrect. Characters need to be present. -->
- [ ] D) perfect <!-- feedback: Incorrect. Flaws often make characters more relatable and interesting. -->

### Explicacion Pedagogica
'Complex' refers to the richness and depth of a well-written character's personality and motivations.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v6`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Describing a draft.

### Enunciado
A draft is a preliminary version of a piece of writing.

### Opciones
- [ ] A) finale <!-- feedback: Incorrect. The final version. -->
- [x] B) draft <!-- feedback: Correct. An early or rough version of a text. -->
- [ ] C) slogan <!-- feedback: Incorrect. Advertising phrase. -->
- [ ] D) prompt <!-- feedback: Incorrect. A starting instruction for writing. -->

### Explicacion Pedagogica
'Draft' is the standard term for the various versions a writer produces before finishing a work.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v7`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a writer's routine.

### Enunciado
She is used to revising her work multiple times before she is satisfied with the result.

### Opciones
- [ ] A) use to <!-- feedback: Incorrect grammar. -->
- [x] B) is used to <!-- feedback: Correct. 'Be used to + -ing' means 'accustomed to'. -->
- [ ] C) used to <!-- feedback: Incorrect. Refers to a past habit. -->
- [ ] D) used <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
'Is used to + gerund' describes a current state of being accustomed to a specific professional habit.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v8`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Text on "alliteration".

### Enunciado
Alliteration is the occurrence of the same letter or sound at the beginning of adjacent or closely connected words.

### Opciones
- [ ] A) Rhyme <!-- feedback: Incorrect. Occurs at the end of words. -->
- [x] B) Alliteration <!-- feedback: Correct. Occurs at the beginning of words. -->
- [ ] C) Irony <!-- feedback: Incorrect. Contrast between expectation and reality. -->
- [ ] D) Imagery <!-- feedback: Incorrect. Use of vivid description. -->

### Explicacion Pedagogica
'Alliteration' is the technical term for the repetition of initial consonant sounds.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v9`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing an ongoing process of creative inspiration.

### Enunciado
New stories are being inspired by current events and personal experiences every day.

### Opciones
- [ ] A) are inspiring <!-- feedback: Incorrect. Active voice. -->
- [x] B) are being inspired <!-- feedback: Correct. Present continuous passive for an ongoing process. -->
- [ ] C) inspired <!-- feedback: Incorrect. Past simple. -->
- [ ] D) have inspired <!-- feedback: Incorrect. Active voice. -->

### Explicacion Pedagogica
The present continuous passive describes the active and constant influence of life on literature.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v10`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of "overused tropes".

### Enunciado
A cliché is a phrase or opinion that is overused and betrays a lack of original thought.

### Opciones
- [ ] A) masterpiece <!-- feedback: Incorrect. A masterpiece is original and great. -->
- [x] B) cliché <!-- feedback: Correct. Term for an unoriginal or overused idea. -->
- [ ] C) metaphor <!-- feedback: Incorrect. A tool for creative comparison. -->
- [ ] D) dialogue <!-- feedback: Incorrect. A tool for representing speech. -->

### Explicacion Pedagogica
'Cliché' describes the unoriginality that writers aim to avoid in creative work.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v11`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Definition of a genre.

### Enunciado
A genre is a category of artistic composition characterized by similarities in form, style, or subject matter.

### Opciones
- [ ] A) Script <!-- feedback: Incorrect. A written plan. -->
- [x] B) Genre <!-- feedback: Correct. A category like mystery, romance, or sci-fi. -->
- [ ] C) Prompt <!-- feedback: Incorrect. -->
- [ ] D) Review <!-- feedback: Incorrect. An evaluation. -->

### Explicacion Pedagogica
'Genre' is the standard term for classifying literary and artistic works.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v12`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a future publication milestone.

### Enunciado
By the end of the year, she will have completed her second novel.

### Opciones
- [ ] A) will complete <!-- feedback: Incorrect. Simple future. -->
- [x] B) will have completed <!-- feedback: Correct. Future perfect for a completed action by a future point. -->
- [ ] C) completes <!-- feedback: Incorrect. Present. -->
- [ ] D) completed <!-- feedback: Incorrect. Past. -->

### Explicacion Pedagogica
The future perfect describes a goal that will be a finished achievement by the stated deadline.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v13`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Discussing "voice" in writing.

### Enunciado
A writer's voice is the unique style or perspective that makes their work recognizable.

### Opciones
- [x] A) unique <!-- feedback: Correct. Unique means being the only one of its kind; unlike anything else. -->
- [ ] B) identical <!-- feedback: Incorrect. Opposite of unique. -->
- [ ] C) common <!-- feedback: Incorrect. 'Voice' should be distinct. -->
- [ ] D) invisible <!-- feedback: Incorrect. Voice should be felt by the reader. -->

### Explicacion Pedagogica
'Unique' captures the individuality that defines a writer's specific stylistic 'voice'.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v14`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Condition about creativity.

### Enunciado
If I had more time to daydream, I would come up with more interesting story ideas.

### Opciones
- [ ] A) have <!-- feedback: Incorrect. First conditional. -->
- [x] B) had <!-- feedback: Correct. Second conditional for a hypothetical change. -->
- [ ] C) have had <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) would have <!-- feedback: Incorrect. 'Would' not used in 'if' clause. -->

### Explicacion Pedagogica
The second conditional 'if + past simple' describes how a hypothetical change in lifestyle would affect creativity.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v15`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion of an essay on the importance of revision.

### Enunciado
The author concludes that "writing is rewriting," emphasizing the critical role of revision in the creative process.

### Opciones
- [ ] A) secondary <!-- feedback: Incorrect. The author says it's critical (essential). -->
- [x] B) critical <!-- feedback: Correct. Critical means expressing adverse or disapproving comments or judgments, but also (as here) essential or fundamental. -->
- [ ] C) optional <!-- feedback: Incorrect. Author says it's central to the process. -->
- [ ] D) boring <!-- feedback: Incorrect. While it might be, 'critical' is the professional evaluation of its role. -->

### Explicacion Pedagogica
'Critical' correctly identifies the essential and fundamental role that revision plays in high-quality writing.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v16`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Describing a "protagonist".

### Enunciado
The protagonist is the main character in a play, novel, or movie.

### Opciones
- [ ] A) Antagonist <!-- feedback: Incorrect. The opponent. -->
- [x] B) Protagonist <!-- feedback: Correct. The leading character. -->
- [ ] C) Narrator <!-- feedback: Incorrect. The one telling the story. -->
- [ ] D) Extra <!-- feedback: Incorrect. A character with no speaking part. -->

### Explicacion Pedagogica
'Protagonist' is the technical term for the central figure in a creative narrative.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v17`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Reporting a writer's advice.

### Enunciado
The instructor suggested that we experiment with different narrative perspectives.

### Opciones
- [x] A) experiment <!-- feedback: Correct. Subjunctive base form after 'suggested'. -->
- [ ] B) experiments <!-- feedback: Incorrect. -->
- [ ] C) to experiment <!-- feedback: Incorrect. Doesn't fit 'that' clause. -->
- [ ] D) experimenting <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
Verbs of suggestion take a 'that' clause with the base form (subjunctive mood) of the following verb.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v18`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Text about "poetic license".

### Enunciado
Poetic license is the freedom of a writer to depart from conventional rules of language or fact for artistic effect.

### Opciones
- [ ] A) Restriction <!-- feedback: Incorrect. It's about freedom. -->
- [x] B) Poetic license <!-- feedback: Correct. Term for artistic freedom from rules. -->
- [ ] C) Plagiarism <!-- feedback: Incorrect. This is illegal/unethical use of work. -->
- [ ] D) Grammar <!-- feedback: Incorrect. Refers to the rules themselves. -->

### Explicacion Pedagogica
'Poetic license' is the standard phrase for the artistic freedom allowed to writers and poets.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v19`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing a past struggle with writer's block.

### Enunciado
I had been struggling with writer's block for weeks before the inspiration finally struck.

### Opciones
- [ ] A) was struggling <!-- feedback: Incorrect. Past continuous. -->
- [x] B) had been struggling <!-- feedback: Correct. Past perfect continuous for duration before a past point. -->
- [ ] C) have been struggling <!-- feedback: Incorrect. Present perfect continuous. -->
- [ ] D) struggle <!-- feedback: Incorrect. Present simple. -->

### Explicacion Pedagogica
The past perfect continuous emphasizes the duration of the difficulty leading up to a specific past moment of success.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W31-creative-writing-001-MASTERY-v20`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Final thought on self-expression.

### Enunciado
Creative writing is a powerful means of self-expression, allowing us to share our unique vision of the world with others.

### Opciones
- [x] A) self-expression <!-- feedback: Correct. Expression of one's own personality, feelings, or ideas. -->
- [ ] B) isolation <!-- feedback: Incorrect. Writing aims to connect. -->
- [ ] C) confusion <!-- feedback: Incorrect. Writing aims for clarity of vision. -->
- [ ] D) greed <!-- feedback: Incorrect. Unrelated. -->

### Explicacion Pedagogica
'Self-expression' is the primary purpose and benefit of creative work described in the text.
