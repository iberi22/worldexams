---
id: "CL-ING-11-2026-W21-future-society-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "future-society"
periodo: "weekly"
week: "W21"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Future Society (W21)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3-D4]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v1`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Introduction to futuristic concepts.

### Enunciado
Automation is the use of technology to perform tasks that were previously done by humans.

### Opciones
- [ ] A) Tradition <!-- feedback: Incorrect. Tradition focuses on the past. -->
- [x] B) Automation <!-- feedback: Correct. Key concept for future labor markets. -->
- [ ] C) Manual labor <!-- feedback: Incorrect. Done by hand. -->
- [ ] D) Craftsmanship <!-- feedback: Incorrect. Skilled manual work. -->

### Explicacion Pedagogica
'Automation' is the standard term for the technological replacement of human labor.

---
## Question 2 [D3-D4]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v2`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Future prediction.

### Enunciado
By 2050, researchers hope that a cure for many common diseases will have been found.

### Opciones
- [ ] A) will find <!-- feedback: Incorrect. Active voice. -->
- [x] B) will have been found <!-- feedback: Correct. Future perfect passive for a completed state in the future. -->
- [ ] C) found <!-- feedback: Incorrect. Past tense. -->
- [ ] D) have been found <!-- feedback: Incorrect. Present perfect. -->

### Explicacion Pedagogica
The future perfect passive 'will have been found' describes an achievement projected to be finished by 2050.

---
## Question 3 [D3-D4]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v3`
**Bloom:** Apply
**EJE:** Grammar
**Expected_Success:** 0.65
**Contexto:** Speculating about a major societal shift.

### Enunciado
If we had prioritized sustainability earlier, we wouldn't be facing such severe climate crises now.

### Opciones
- [ ] A) prioritized <!-- feedback: Incorrect. Second conditional. -->
- [x] B) had prioritized <!-- feedback: Correct. Mixed conditional (past action, present result). -->
- [ ] C) have prioritized <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) would prioritize <!-- feedback: Incorrect. 'Would' not used in 'if' clause. -->

### Explicacion Pedagogica
The mixed conditional connects a past hypothetical failure with a present negative reality.

---
## Question 4 [D3-D4]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v4`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about "Universal Basic Income" (UBI).

### Enunciado
UBI is a model for providing all citizens of a country with a given sum of money, regardless of their income or employment status.

### Opciones
- [x] A) regardless of <!-- feedback: Correct. Means without paying attention to or being affected by. -->
- [ ] B) because of <!-- feedback: Incorrect. UBI is not dependent on these factors. -->
- [ ] C) in addition to <!-- feedback: Incorrect. While it is extra money, 'regardless of' is the key part of the definition. -->
- [ ] D) instead of <!-- feedback: Incorrect. UBI is usually proposed as a base, not necessarily a replacement for all other systems. -->

### Explicacion Pedagogica
'Regardless of' is the essential prepositional phrase for describing the unconditional nature of UBI.

---
## Question 5 [D5-D6]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v5`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on the ethics of "transhumanism".

### Enunciado
Transhumanism is the belief or theory that the human race can evolve beyond its current physical and mental limitations through science and technology.

### Opciones
- [ ] A) Stagnation <!-- feedback: Incorrect. Stagnation means no change. -->
- [x] B) Transhumanism <!-- feedback: Correct. Definition of the futuristic movement. -->
- [ ] C) Tradition <!-- feedback: Incorrect. Focuses on the past. -->
- [ ] D) Naturalism <!-- feedback: Incorrect. Focuses on nature without technological interference. -->

### Explicacion Pedagogica
'Transhumanism' is the specific philosophical term for the movement to enhance humans through technology.

---
## Question 6 [D5-D6]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v6`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Space colonization.

### Enunciado
To colonize another planet means to send people to live there and establish a permanent settlement.

### Opciones
- [ ] A) visit <!-- feedback: Incorrect. Visiting is temporary. -->
- [x] B) colonize <!-- feedback: Correct. To establish a permanent presence. -->
- [ ] C) destroy <!-- feedback: Incorrect. Opposite goal. -->
- [ ] D) ignore <!-- feedback: Incorrect. Opposite of exploration. -->

### Explicacion Pedagogica
'Colonize' is the standard term for establishing a new permanent human community in a new location.

---
## Question 7 [D5-D6]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v7`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a future trend in education.

### Enunciado
In the future, personalized learning paths are likely to be designed by AI to meet each student's specific needs.

### Opciones
- [ ] A) are designing <!-- feedback: Incorrect. Active voice. -->
- [x] B) to be designed <!-- feedback: Correct. Passive infinitive after 'likely to'. -->
- [ ] C) design <!-- feedback: Incorrect. Active voice. -->
- [ ] D) having been designed <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
The passive infinitive 'to be designed' follows the structure 'likely to' to describe a future expectation.

---
## Question 8 [D5-D6]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v8`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Text on "vertical farming".

### Enunciado
Vertical farming could provide a sustainable solution to food production in densely populated urban areas.

### Opciones
- [ ] A) rural <!-- feedback: Incorrect. Rural areas have plenty of land for traditional farming. -->
- [x] B) urban <!-- feedback: Correct. Vertical farming is designed for city environments. -->
- [ ] C) arctic <!-- feedback: Incorrect. Too cold. -->
- [ ] D) desert <!-- feedback: Incorrect. While possible, 'urban' is the primary context for vertical farming. -->

### Explicacion Pedagogica
'Urban' is the appropriate context for vertical farming, which aims to produce food where land is scarce.

---
## Question 9 [D5-D6]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v9`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Expressing a hope for the future.

### Enunciado
I wish we would solve the problem of world hunger once and for all.

### Opciones
- [ ] A) solve <!-- feedback: Incorrect. Need 'would' for a desired future change. -->
- [x] B) would solve <!-- feedback: Correct. 'Wish + would' for a future change or someone else's behavior. -->
- [ ] C) solved <!-- feedback: Incorrect. 'Wish + past simple' is for present state desires. -->
- [ ] D) have solved <!-- feedback: Incorrect. 'Wish + past perfect' is for past regret. -->

### Explicacion Pedagogica
'Wish + would' is used to express a strong desire for a future change in a situation.

---
## Question 10 [D5-D6]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v10`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of "technological utopianism".

### Enunciado
The author warns against technological utopianism, the belief that technology will automatically solve all of society's problems.

### Opciones
- [ ] A) realism <!-- feedback: Incorrect. Utopianism is the opposite of realism. -->
- [x] B) utopianism <!-- feedback: Correct. The belief in an ideal or perfect society. -->
- [ ] C) pessimism <!-- feedback: Incorrect. Pessimism is the belief that things will go wrong. -->
- [ ] D) minimalism <!-- feedback: Incorrect. Lifestyle choice. -->

### Explicacion Pedagogica
'Utopianism' refers to the belief in the possibility of creating an ideal world through specific means, in this case, technology.

---
## Question 11 [D7-D8]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Defining a term for the future.

### Enunciado
A futurist is a person who studies the future and makes predictions about it based on current trends.

### Opciones
- [ ] A) historian <!-- feedback: Incorrect. Studies the past. -->
- [x] B) futurist <!-- feedback: Correct. Standard term for future-studies professional. -->
- [ ] C) archaeologist <!-- feedback: Incorrect. Studies history via physical remains. -->
- [ ] D) linguist <!-- feedback: Incorrect. Studies language. -->

### Explicacion Pedagogica
'Futurist' (or futurologist) is the name for experts who analyze current trends to project future outcomes.

---
## Question 12 [D7-D8]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v12`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Passive voice in a futuristic scenario.

### Enunciado
By next century, many coastal cities will have been submerged by rising sea levels.

### Opciones
- [ ] A) will submerge <!-- feedback: Incorrect. Active voice. -->
- [x] B) will have been submerged <!-- feedback: Correct. Future perfect passive for a completed state. -->
- [ ] C) are submerged <!-- feedback: Incorrect. Present. -->
- [ ] D) submerging <!-- feedback: Incorrect. Gerund. -->

### Explicacion Pedagogica
The future perfect passive indicates that the state of being submerged will be a completed fact by the next century.

---
## Question 13 [D7-D8]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v13`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Discussing longevity.

### Enunciado
Scientific advancements in biotechnology could significantly increase human longevity.

### Opciones
- [x] A) longevity <!-- feedback: Correct. Longevity means long life. -->
- [ ] B) height <!-- feedback: Incorrect. Not a primary focus of biotechnology. -->
- [ ] C) confusion <!-- feedback: Incorrect. Science aims for clarity. -->
- [ ] D) scarcity <!-- feedback: Incorrect. Usually used for resources. -->

### Explicacion Pedagogica
'Longevity' is the specific term for length of life, a major area of future medical research.

---
## Question 14 [D7-D8]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v14`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Conditional about the energy transition.

### Enunciado
If the world shifted to 100% renewable energy, carbon emissions would plummet.

### Opciones
- [ ] A) shifts <!-- feedback: Incorrect. First conditional. -->
- [x] B) shifted <!-- feedback: Correct. Second conditional for a hypothetical change. -->
- [ ] C) has shifted <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) had shifted <!-- feedback: Incorrect. Third conditional. -->

### Explicacion Pedagogica
The second conditional 'if + past simple' describes the likely result of a hypothetical total energy shift.

---
## Question 15 [D7-D8]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v15`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion of an essay on future work.

### Enunciado
The author concludes that adaptability will be the most valuable skill in the future job market.

### Opciones
- [x] A) adaptability <!-- feedback: Correct. The ability to adjust to new conditions. -->
- [ ] B) resistance <!-- feedback: Incorrect. Resisting change is usually negative. -->
- [ ] C) stagnation <!-- feedback: Incorrect. No movement. -->
- [ ] D) boredom <!-- feedback: Incorrect. Negative. -->

### Explicacion Pedagogica
'Adaptability' correctly identifies the core skill needed for a rapidly changing professional landscape.

---
## Question 16 [D7-D8]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v16`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Digital currency.

### Enunciado
Cryptocurrency is a digital or virtual currency that is secured by cryptography.

### Opciones
- [ ] A) Cash <!-- feedback: Incorrect. Physical. -->
- [x] B) Cryptocurrency <!-- feedback: Correct. The specific digital term. -->
- [ ] C) Barter <!-- feedback: Incorrect. Trading goods directly. -->
- [ ] D) Credit <!-- feedback: Incorrect. Too broad. -->

### Explicacion Pedagogica
'Cryptocurrency' is the specific name for the decentralized digital payment systems.

---
## Question 17 [D9-D10]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v17`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Reporting a future prediction.

### Enunciado
Experts predicted that AI would eventually surpass human intelligence in many tasks.

### Opciones
- [ ] A) will <!-- feedback: Incorrect. Backshifted in reported speech. -->
- [x] B) would <!-- feedback: Correct. Backshifted from 'will' to 'would'. -->
- [ ] C) should <!-- feedback: Incorrect. Expresses advice or expectation. -->
- [ ] D) can <!-- feedback: Incorrect. Backshifted to 'could'. -->

### Explicacion Pedagogica
In reported speech, we change 'will' to 'would' when the reporting verb is in the past tense.

---
## Question 18 [D9-D10]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v18`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Text about "sustainable cities".

### Enunciado
Future cities must be designed to be self-sufficient in terms of energy and water resources.

### Opciones
- [ ] A) dependent <!-- feedback: Incorrect. Cities want to be less dependent. -->
- [x] B) self-sufficient <!-- feedback: Correct. Needing no outside help. -->
- [ ] C) wasteful <!-- feedback: Incorrect. Goal is the opposite. -->
- [ ] D) congested <!-- feedback: Incorrect. Goal is to reduce congestion. -->

### Explicacion Pedagogica
'Self-sufficient' is the goal for sustainable city planning where resources are produced locally.

---
## Question 19 [D9-D10]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v19`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing a long-term research project.

### Enunciado
By 2030, scientists will have been monitoring global temperatures for over a century.

### Opciones
- [ ] A) will monitor <!-- feedback: Incorrect. Simple future. -->
- [x] B) will have been monitoring <!-- feedback: Correct. Future perfect continuous for duration up to a future point. -->
- [ ] C) have been monitoring <!-- feedback: Incorrect. Present perfect continuous. -->
- [ ] D) monitor <!-- feedback: Incorrect. Present simple. -->

### Explicacion Pedagogica
The future perfect continuous 'will have been monitoring' describes the total duration of a study at a specific future time.

---
## Question 20 [D9-D10]
**ID:** `CL-ING-11-2026-W21-future-society-001-MASTERY-v20`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Final thought on future ethics.

### Enunciado
The greatest challenge for the future is to ensure that technological progress is guided by human ethics.

### Opciones
- [x] A) ethics <!-- feedback: Correct. Moral principles that govern behavior. -->
- [ ] B) speed <!-- feedback: Incorrect. Speed is part of technology, but not the 'guide'. -->
- [ ] C) profit <!-- feedback: Incorrect. Profit is a guide for business, but the 'challenge' is usually about higher principles. -->
- [ ] D) curiosity <!-- feedback: Incorrect. Curiosity drives the progress, but ethics must guide its application. -->

### Explicacion Pedagogica
'Ethics' refers to the moral framework necessary to ensure that technology is used for the benefit of humanity.
