---
id: "CL-ING-11-2026-W08-travel-tourism-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "travel-tourism"
periodo: "weekly"
week: "W08"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Travel and Tourism (W08)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v1`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Planning a trip.

### Enunciado
An itinerary is a detailed plan or route of a journey.

### Opciones
- [ ] A) accommodation <!-- feedback: Incorrect. Accommodation refers to where you stay. -->
- [x] B) itinerary <!-- feedback: Correct. An itinerary outlines the schedule and locations of a trip. -->
- [ ] C) souvenir <!-- feedback: Incorrect. A souvenir is a memento. -->
- [ ] D) customs <!-- feedback: Incorrect. Customs refers to border control. -->

### Explicacion Pedagogica
'Itinerary' is the standard travel term for a planned route or schedule.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v2`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a beautiful destination.

### Enunciado
The landscape was so breathtaking that I couldn't stop taking pictures.

### Opciones
- [ ] A) breathless <!-- feedback: Incorrect. Breathless means unable to breathe easily. -->
- [x] B) breathtaking <!-- feedback: Correct. Breathtaking means astonishing or magnificent. -->
- [ ] C) breathed <!-- feedback: Incorrect. Past tense of breathe. -->
- [ ] D) breathing <!-- feedback: Incorrect. The act of inhaling/exhaling. -->

### Explicacion Pedagogica
'Breathtaking' is a common adjective used to describe impressive scenery.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v3`
**Bloom:** Apply
**EJE:** Grammar
**Expected_Success:** 0.65
**Contexto:** Using advanced emphasis (Inversion).

### Enunciado
Never before had I seen such a stunning sunset as the one in the Atacama Desert.

### Opciones
- [ ] A) I had seen <!-- feedback: Incorrect. After 'Never before', we use inversion (had I seen). -->
- [x] B) had I seen <!-- feedback: Correct. Negative adverbials at the start of a sentence trigger inversion. -->
- [ ] C) I saw <!-- feedback: Incorrect. Doesn't follow the inversion rule. -->
- [ ] D) did I see <!-- feedback: Incorrect. Past simple inversion is less common with 'before'. -->

### Explicacion Pedagogica
Negative adverbials like 'Never' or 'Seldom' require inversion (verb before subject) when placed at the beginning of a sentence.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v4`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about sustainable travel.

### Enunciado
Ecotourism aims to minimize the impact of visitors on the environment and support local communities.

### Opciones
- [x] A) Ecotourism <!-- feedback: Correct. Ecotourism focuses on ecological preservation. -->
- [ ] B) Mass tourism <!-- feedback: Incorrect. Mass tourism often has a high environmental impact. -->
- [ ] C) Business travel <!-- feedback: Incorrect. Focuses on work, not environment. -->
- [ ] D) Luxury travel <!-- feedback: Incorrect. Focuses on comfort and high cost. -->

### Explicacion Pedagogica
'Ecotourism' is the specific term for travel that prioritizes environmental and cultural conservation.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v5`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on the "authentic" travel experience.

### Enunciado
Travelers often seek "off-the-beaten-path" destinations to avoid crowded tourist areas and find genuine local culture.

### Opciones
- [ ] A) mainstream <!-- feedback: Incorrect. Mainstream refers to popular, common choices. -->
- [x] B) off-the-beaten-path <!-- feedback: Correct. This idiom refers to places that are not frequently visited. -->
- [ ] C) metropolitan <!-- feedback: Incorrect. Metropolitan refers to large cities. -->
- [ ] D) commercialized <!-- feedback: Incorrect. Commercialized places are often the most crowded. -->

### Explicacion Pedagogica
'Off-the-beaten-path' is an idiomatic expression for remote or less popular locations.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v6`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Airport procedures.

### Enunciado
You must check in your luggage at least two hours before your flight departs.

### Opciones
- [ ] A) check out <!-- feedback: Incorrect. Check out is for leaving a hotel. -->
- [x] B) check in <!-- feedback: Correct. To register and hand over luggage at an airport. -->
- [ ] C) check up <!-- feedback: Incorrect. A medical examination. -->
- [ ] D) check through <!-- feedback: Incorrect. Less common in this specific context. -->

### Explicacion Pedagogica
'Check in' is the standard phrasal verb for registering for a flight or hotel.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v7`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Talking about a missed opportunity.

### Enunciado
If I had brought my camera, I would have taken a picture of the penguin colony.

### Opciones
- [ ] A) brought <!-- feedback: Incorrect. Second conditional. -->
- [x] B) had brought <!-- feedback: Correct. Third conditional for hypothetical past. -->
- [ ] C) bring <!-- feedback: Incorrect. Present tense. -->
- [ ] D) would bring <!-- feedback: Incorrect. 'Would' not used in 'if' clause. -->

### Explicacion Pedagogica
The third conditional (if + past perfect) is used for hypothetical situations in the past.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v8`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Describing a hotel.

### Enunciado
The hotel offers a wide range of amenities, such as a swimming pool, a gym, and free Wi-Fi.

### Opciones
- [ ] A) necessities <!-- feedback: Incorrect. Amenities are extra comforts, not just basics. -->
- [x] B) amenities <!-- feedback: Correct. Amenities are useful or desirable features of a place. -->
- [ ] C) obstacles <!-- feedback: Incorrect. Obstacles are things that block your way. -->
- [ ] D) chores <!-- feedback: Incorrect. Chores are routine tasks. -->

### Explicacion Pedagogica
'Amenities' is the professional term for features that increase the comfort of a hotel or apartment.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v9`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing an ongoing travel trend.

### Enunciado
More and more people are choosing to travel solo to gain independence and self-confidence.

### Opciones
- [ ] A) lonely <!-- feedback: Incorrect. Lonely is a feeling. -->
- [x] B) solo <!-- feedback: Correct. Solo means alone or unaccompanied. -->
- [ ] C) unique <!-- feedback: Incorrect. Unique means one of a kind. -->
- [ ] D) isolated <!-- feedback: Incorrect. Isolated implies being cut off, often negatively. -->

### Explicacion Pedagogica
'Solo travel' is the accepted term for traveling alone.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v10`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of "overtourism".

### Enunciado
The text suggests that overtourism can erode the very qualities that made a destination attractive in the first place.

### Opciones
- [ ] A) enhance <!-- feedback: Incorrect. Overtourism usually damages qualities. -->
- [x] B) erode <!-- feedback: Correct. Erode means to gradually wear away or destroy. -->
- [ ] C) preserve <!-- feedback: Incorrect. Preservation is the opposite of erosion. -->
- [ ] D) simplify <!-- feedback: Incorrect. It makes management more complex. -->

### Explicacion Pedagogica
'Erode' metaphorically describes the gradual destruction of a location's appeal due to too many visitors.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Lodging types.

### Enunciado
A hostel is an inexpensive lodging place for travelers, often with dormitory-style rooms.

### Opciones
- [ ] A) resort <!-- feedback: Incorrect. A resort is usually expensive and luxury. -->
- [x] B) hostel <!-- feedback: Correct. Hostels are budget-friendly and often communal. -->
- [ ] C) villa <!-- feedback: Incorrect. A villa is a large, private house. -->
- [ ] D) penthouse <!-- feedback: Incorrect. A penthouse is a luxury apartment. -->

### Explicacion Pedagogica
'Hostel' is the specific term for budget, communal traveler accommodation.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v12`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Passenger rights.

### Enunciado
Passengers whose flights are cancelled are entitled to a full refund or rebooking.

### Opciones
- [ ] A) who <!-- feedback: Incorrect. 'Who' refers to the subject, but we need possession (their flights). -->
- [x] B) whose <!-- feedback: Correct. 'Whose' is the possessive relative pronoun. -->
- [ ] C) which <!-- feedback: Incorrect. Refers to things. -->
- [ ] D) that <!-- feedback: Incorrect. Relative pronoun, but doesn't show possession. -->

### Explicacion Pedagogica
'Whose' is used to show that the flights belong to the passengers.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v13`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Describing a remote place.

### Enunciado
The mountain village was so secluded that it felt like a different world.

### Opciones
- [x] A) secluded <!-- feedback: Correct. Secluded means hidden from view or not frequented by many people. -->
- [ ] B) public <!-- feedback: Incorrect. Public means open to everyone. -->
- [ ] C) urban <!-- feedback: Incorrect. Urban refers to cities. -->
- [ ] D) accessible <!-- feedback: Incorrect. Accessible means easy to reach. -->

### Explicacion Pedagogica
'Secluded' is the perfect adjective for a place that is quiet, private, and far from others.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v14`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Advice on travel insurance.

### Enunciado
You should always take out travel insurance in case of medical emergencies or lost luggage.

### Opciones
- [ ] A) get on <!-- feedback: Incorrect. To board a vehicle. -->
- [x] B) take out <!-- feedback: Correct. To 'take out' insurance means to obtain it. -->
- [ ] C) set off <!-- feedback: Incorrect. To begin a journey. -->
- [ ] D) put up <!-- feedback: Incorrect. To stay somewhere. -->

### Explicacion Pedagogica
'Take out' is the standard phrasal verb for obtaining an insurance policy.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v15`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Impact of tourism on culture.

### Enunciado
While tourism can bring economic benefits, it can also lead to the commodification of local traditions.

### Opciones
- [ ] A) preservation <!-- feedback: Incorrect. Preservation keeps things as they are. -->
- [x] B) commodification <!-- feedback: Correct. Making something into an object for trade or profit. -->
- [ ] C) appreciation <!-- feedback: Incorrect. Appreciation is positive. -->
- [ ] D) elimination <!-- feedback: Incorrect. The traditions usually remain but change their meaning for profit. -->

### Explicacion Pedagogica
'Commodification' refers to turning cultural elements into products for sale, often losing their original meaning.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Exploring a new city.

### Enunciado
We spent the whole afternoon sightseeing around the historic center of the city.

### Opciones
- [ ] A) commuting <!-- feedback: Incorrect. Commuting is traveling to work. -->
- [x] B) sightseeing <!-- feedback: Correct. Visiting interesting places. -->
- [ ] C) migrating <!-- feedback: Incorrect. Moving to a new place to live. -->
- [ ] D) navigating <!-- feedback: Incorrect. Finding the way. -->

### Explicacion Pedagogica
'Sightseeing' is the standard term for visiting tourist attractions.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v17`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a past trip.

### Enunciado
By the time we reached the summit, it had been snowing for three hours.

### Opciones
- [ ] A) was snowing <!-- feedback: Incorrect. Past continuous. -->
- [x] B) had been snowing <!-- feedback: Correct. Past perfect continuous for a duration before a past point. -->
- [ ] C) has been snowing <!-- feedback: Incorrect. Present perfect continuous. -->
- [ ] D) snowed <!-- feedback: Incorrect. Past simple. -->

### Explicacion Pedagogica
The past perfect continuous describes an action that was ongoing up to a certain point in the past.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v18`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Describing a local guide.

### Enunciado
Our guide was incredibly knowledgeable and shared many fascinating stories about the ruins.

### Opciones
- [ ] A) ignorant <!-- feedback: Incorrect. Ignorant means not knowing. -->
- [x] B) knowledgeable <!-- feedback: Correct. Knowledgeable means intelligent and well-informed. -->
- [ ] C) suspicious <!-- feedback: Incorrect. Suspicious means showing distrust. -->
- [ ] D) terminal <!-- feedback: Incorrect. Terminal refers to an end point. -->

### Explicacion Pedagogica
'Knowledgeable' is a positive adjective for someone who has a lot of information.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v19`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Reporting travel advice.

### Enunciado
The travel agent recommended that we book our flights well in advance.

### Opciones
- [ ] A) booked <!-- feedback: Incorrect. Doesn't fit the 'that' clause. -->
- [x] B) book <!-- feedback: Correct. Subjunctive form after 'recommended'. -->
- [ ] C) to book <!-- feedback: Incorrect. Infinitive doesn't fit the 'that' clause. -->
- [ ] D) booking <!-- feedback: Incorrect. Gerund doesn't fit here. -->

### Explicacion Pedagogica
Verbs of recommendation like 'recommend' take the subjunctive (base form) in a 'that' clause.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W08-travel-tourism-001-MASTERY-v20`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Conclusion on the impact of travel.

### Enunciado
Ultimately, travel should be a means of broadening one's horizons and fostering mutual understanding.

### Opciones
- [x] A) broadening <!-- feedback: Correct. To broaden one's horizons means to expand one's experience or knowledge. -->
- [ ] B) narrowing <!-- feedback: Incorrect. Narrowing means making smaller. -->
- [ ] C) ignoring <!-- feedback: Incorrect. Travel involves paying attention to new things. -->
- [ ] D) restricting <!-- feedback: Incorrect. Restricting means limiting. -->

### Explicacion Pedagogica
'Broadening one's horizons' is a common idiom for expanding one's worldview through travel or learning.
