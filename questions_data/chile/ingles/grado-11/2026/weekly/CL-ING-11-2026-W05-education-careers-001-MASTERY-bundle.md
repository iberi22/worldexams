---
id: "CL-ING-11-2026-W05-education-careers-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "education-careers"
periodo: "weekly"
week: "W05"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Education and Careers (W05)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v1`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Choosing a university path.

### Enunciado
Students who want to specialize in a specific field often choose a ______ as their primary area of study.

### Opciones
- [ ] A) minor <!-- feedback: Incorrect. A minor is a secondary area of study. -->
- [x] B) major <!-- feedback: Correct. A major is the main subject studied by a college student. -->
- [ ] C) elective <!-- feedback: Incorrect. An elective is an optional course. -->
- [ ] D) syllabus <!-- feedback: Incorrect. A syllabus is an outline of a course. -->

### Explicacion Pedagogica
'Major' is the standard academic term for a student's principal subject of study in higher education.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v2`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing future job opportunities.

### Enunciado
By the time you graduate, you will have gained valuable experience through your internships.

### Opciones
- [ ] A) gain <!-- feedback: Incorrect. Present simple. -->
- [ ] B) will gain <!-- feedback: Incorrect. Simple future. -->
- [x] C) will have gained <!-- feedback: Correct. Future perfect for an action completed by a specific future point. -->
- [ ] D) have gained <!-- feedback: Incorrect. Present perfect. -->

### Explicacion Pedagogica
The future perfect 'will have gained' describes an achievement that will be a completed fact at the time of graduation.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v3`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Preparing for the workforce.

### Enunciado
A well-written curriculum vitae (CV) should highlight your skills and relevant work history.

### Opciones
- [ ] A) obscure <!-- feedback: Incorrect. Obscure means to hide. -->
- [x] B) highlight <!-- feedback: Correct. To highlight means to draw attention to or emphasize. -->
- [ ] C) dismiss <!-- feedback: Incorrect. To dismiss means to ignore or reject. -->
- [ ] D) fabricate <!-- feedback: Incorrect. To fabricate means to invent or lie. -->

### Explicacion Pedagogica
'Highlight' is the appropriate verb for emphasizing positive attributes in a professional document.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v4`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about lifelong learning.

### Enunciado
In a rapidly changing job market, the ability to adapt and learn new skills is essential for career longevity.

### Opciones
- [x] A) adapt <!-- feedback: Correct. To adapt means to become adjusted to new conditions. -->
- [ ] B) stagnate <!-- feedback: Incorrect. To stagnate means to stop developing. -->
- [ ] C) surrender <!-- feedback: Incorrect. To surrender means to give up. -->
- [ ] D) resist <!-- feedback: Incorrect. Resisting change is usually negative in this context. -->

### Explicacion Pedagogica
'Adapt' correctly identifies the proactive behavior needed to succeed in an evolving professional environment.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v5`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Debate on the value of a university degree versus vocational training.

### Enunciado
The author argues that while university provides theoretical knowledge, vocational training offers practical skills.

### Opciones
- [ ] A) redundant <!-- feedback: Incorrect. Redundant means unnecessary. -->
- [ ] B) abstract <!-- feedback: Incorrect. Abstract means existing in thought but not having a physical existence. -->
- [x] C) practical <!-- feedback: Correct. Practical skills are those related to real-world application. -->
- [ ] D) obsolete <!-- feedback: Incorrect. Obsolete means out of date. -->

### Explicacion Pedagogica
'Practical' is the term that best contrasts with 'theoretical' in the context of professional training.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v6`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Financial aid for education.

### Enunciado
A scholarship is a grant or payment made to support a student's education, awarded on the basis of academic achievement.

### Opciones
- [ ] A) loan <!-- feedback: Incorrect. A loan must be paid back. -->
- [x] B) scholarship <!-- feedback: Correct. A scholarship is usually merit-based and does not need to be repaid. -->
- [ ] C) mortgage <!-- feedback: Incorrect. A mortgage is a loan for a house. -->
- [ ] D) dividend <!-- feedback: Incorrect. A dividend is a sum of money paid to shareholders. -->

### Explicacion Pedagogica
'Scholarship' is the specific term for merit-based financial support for students.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v7`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Advice for job interviews.

### Enunciado
If I were you, I would research the company thoroughly before the interview.

### Opciones
- [ ] A) am <!-- feedback: Incorrect. Present tense. -->
- [x] B) were <!-- feedback: Correct. Subjunctive 'were' is used in second conditional advice. -->
- [ ] C) was <!-- feedback: Incorrect. 'Were' is preferred in formal conditional structures. -->
- [ ] D) had been <!-- feedback: Incorrect. Third conditional. -->

### Explicacion Pedagogica
The phrase 'If I were you' is a standard second conditional structure for giving advice.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v8`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Professional networking.

### Enunciado
Networking allows professionals to exchange information and develop contacts which may lead to career advancement.

### Opciones
- [ ] A) isolation <!-- feedback: Incorrect. Isolation is the opposite of networking. -->
- [x] B) networking <!-- feedback: Correct. Networking is the process of interacting with others to exchange information. -->
- [ ] C) competition <!-- feedback: Incorrect. While competition exists, the act of developing contacts is networking. -->
- [ ] D) withdrawal <!-- feedback: Incorrect. Withdrawal means pulling away. -->

### Explicacion Pedagogica
'Networking' is the term used for building professional relationships that can benefit one's career.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v9`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Discussing a past decision.

### Enunciado
She regrets not studying harder during her senior year.

### Opciones
- [ ] A) to not study <!-- feedback: Incorrect. Regret is usually followed by a gerund for past actions. -->
- [x] B) not studying <!-- feedback: Correct. 'Regret + (not) -ing' is used to talk about things we wish we hadn't (or had) done in the past. -->
- [ ] C) not study <!-- feedback: Incorrect. Base form doesn't fit. -->
- [ ] D) no study <!-- feedback: Incorrect. Incorrect grammar. -->

### Explicacion Pedagogica
The gerund 'studying' after 'regrets' describes the action being regretted.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v10`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Discussion on "soft skills".

### Enunciado
While "hard skills" are specific technical abilities, "soft skills" like communication and empathy are increasingly valued by employers.

### Opciones
- [ ] A) irrelevant <!-- feedback: Incorrect. The text says they are valued. -->
- [ ] B) innate <!-- feedback: Incorrect. While some are innate, they can also be developed. -->
- [x] C) increasingly <!-- feedback: Correct. Increasingly means more and more. -->
- [ ] D) barely <!-- feedback: Incorrect. Barely means hardly at all. -->

### Explicacion Pedagogica
'Increasingly' captures the trend of employers placing higher value on interpersonal skills.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Workplace hierarchy.

### Enunciado
The CEO is the highest-ranking person in a company and is responsible for making major corporate decisions.

### Opciones
- [ ] A) intern <!-- feedback: Incorrect. An intern is a low-level student worker. -->
- [ ] B) supervisor <!-- feedback: Incorrect. A supervisor manages a specific team. -->
- [x] C) CEO <!-- feedback: Correct. Chief Executive Officer. -->
- [ ] D) client <!-- feedback: Incorrect. A client is someone who buys services. -->

### Explicacion Pedagogica
'CEO' is the acronym for the top executive position in most organizations.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v12`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing a future possibility.

### Enunciado
I may consider taking a gap year after high school to travel and gain perspective.

### Opciones
- [x] A) taking <!-- feedback: Correct. 'Consider' is followed by a gerund. -->
- [ ] B) to take <!-- feedback: Incorrect. Infinitive is not used after 'consider'. -->
- [ ] C) take <!-- feedback: Incorrect. Base form doesn't fit. -->
- [ ] D) taken <!-- feedback: Incorrect. Past participle. -->

### Explicacion Pedagogica
The verb 'consider' requires the gerund form of the following verb.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v13`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Applying for a position.

### Enunciado
Candidates who meet all the requirements will be invited for an interview.

### Opciones
- [ ] A) ignore <!-- feedback: Incorrect. Ignoring requirements won't get an interview. -->
- [x] B) meet <!-- feedback: Correct. To meet a requirement means to fulfill it. -->
- [ ] C) neglect <!-- feedback: Incorrect. Neglect means to fail to care for. -->
- [ ] D) postpone <!-- feedback: Incorrect. Postpone means to delay. -->

### Explicacion Pedagogica
'Meet' is the standard verb used for fulfilling criteria or requirements.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v14`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Condition for a promotion.

### Enunciado
You won't get a promotion unless you show more initiative in your work.

### Opciones
- [ ] A) if <!-- feedback: Incorrect. 'If you show' would mean the promotion is certain if initiative is shown, but 'unless' expresses the condition better here. -->
- [x] B) unless <!-- feedback: Correct. 'Unless' means 'if not' and is used to express a negative condition. -->
- [ ] C) although <!-- feedback: Incorrect. Expresses contrast. -->
- [ ] D) whether <!-- feedback: Incorrect. Expresses choice between alternatives. -->

### Explicacion Pedagogica
'Unless' is used to indicate that the promotion depends on the condition of showing initiative.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v15`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion of an article on job automation.

### Enunciado
The article concludes that workers must continuously upskill to remain competitive in an automated world.

### Opciones
- [x] A) upskill <!-- feedback: Correct. To upskill means to learn new skills or teach workers new skills. -->
- [ ] B) retire <!-- feedback: Incorrect. Retiring doesn't help one remain competitive. -->
- [ ] C) complain <!-- feedback: Incorrect. Complaining is not a strategy for competitiveness. -->
- [ ] D) downsize <!-- feedback: Incorrect. Downsizing usually refers to companies cutting staff. -->

### Explicacion Pedagogica
'Upskill' is the precise term for acquiring new and relevant professional skills to stay current in the market.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v16`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Different types of jobs.

### Enunciado
A freelancer is a person who is self-employed and is not committed to a particular employer long-term.

### Opciones
- [ ] A) apprentice <!-- feedback: Incorrect. An apprentice is learning a trade. -->
- [x] B) freelancer <!-- feedback: Correct. A freelancer works for themselves on a project-by-project basis. -->
- [ ] C) volunteer <!-- feedback: Incorrect. A volunteer works for free. -->
- [ ] D) subordinate <!-- feedback: Incorrect. A subordinate is someone under someone else's authority. -->

### Explicacion Pedagogica
'Freelancer' is the standard term for independent professionals who sell their services to multiple clients.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v17`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Career advice from a mentor.

### Enunciado
He suggested that she apply for the management position.

### Opciones
- [ ] A) applies <!-- feedback: Incorrect. Present simple doesn't fit the 'that' clause here. -->
- [x] B) apply <!-- feedback: Correct. Subjunctive form after 'suggested'. -->
- [ ] C) to apply <!-- feedback: Incorrect. Infinitive doesn't fit the 'that' clause. -->
- [ ] D) applying <!-- feedback: Incorrect. Gerund doesn't fit the 'that' clause. -->

### Explicacion Pedagogica
Verbs like 'suggest' and 'recommend' are followed by a 'that' clause with the base form of the verb (subjunctive mood).

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v18`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Benefits of working in a team.

### Enunciado
Effective collaboration within a team can significantly improve the quality of a project.

### Opciones
- [ ] A) isolation <!-- feedback: Incorrect. Isolation prevents teamwork. -->
- [x] B) collaboration <!-- feedback: Correct. Collaboration means working together to achieve a goal. -->
- [ ] C) negligence <!-- feedback: Incorrect. Negligence means lack of care. -->
- [ ] D) friction <!-- feedback: Incorrect. Friction means conflict or disagreement. -->

### Explicacion Pedagogica
'Collaboration' is the positive process of multiple people working together effectively.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v19`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing a future goal.

### Enunciado
I hope to have finished my master's degree by the time I turn thirty.

### Opciones
- [ ] A) finish <!-- feedback: Incorrect. Simple infinitive. -->
- [ ] B) finishing <!-- feedback: Incorrect. Gerund. -->
- [x] C) to have finished <!-- feedback: Correct. Perfect infinitive indicates an action completed before a certain time. -->
- [ ] D) to finish <!-- feedback: Incorrect. While common, 'to have finished' better fits the 'by the time' structure. -->

### Explicacion Pedagogica
The perfect infinitive 'to have finished' expresses a completed action relative to a future point.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W05-education-careers-001-MASTERY-v20`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Conclusion of an essay on work-life balance.

### Enunciado
Ultimately, a career should provide not only financial stability but also personal fulfillment.

### Opciones
- [ ] A) frustration <!-- feedback: Incorrect. Frustration is negative. -->
- [ ] B) exhaustion <!-- feedback: Incorrect. Exhaustion is negative. -->
- [x] C) fulfillment <!-- feedback: Correct. Fulfillment means the achievement of something desired or promised. -->
- [ ] D) stagnancy <!-- feedback: Incorrect. Stagnancy is negative. -->

### Explicacion Pedagogica
'Fulfillment' refers to the satisfaction and sense of accomplishment derived from one's work.
