---
id: "CL-ING-11-2026-W19-literature-stories-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "literature-stories"
periodo: "weekly"
week: "W19"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Literature and Stories (W19)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v1`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Introduction to narrative structure.

### Enunciado
The plot is the sequence of events that make up a story.

### Opciones
- [ ] A) character <!-- feedback: Incorrect. Characters are the people in the story. -->
- [x] B) plot <!-- feedback: Correct. Plot is the sequence of events. -->
- [ ] C) setting <!-- feedback: Incorrect. Setting is the place and time. -->
- [ ] D) theme <!-- feedback: Incorrect. Theme is the main idea or message. -->

### Explicacion Pedagogica
'Plot' is the fundamental literary term for the arrangement of events in a narrative.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v2`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing a classic novel.

### Enunciado
The Great Gatsby, which was written by F. Scott Fitzgerald, explores themes of wealth and social class.

### Opciones
- [ ] A) that was written <!-- feedback: Incorrect. 'Which' is preferred for non-defining relative clauses. -->
- [x] B) which was written <!-- feedback: Correct. Non-defining relative clause providing extra information. -->
- [ ] C) who was written <!-- feedback: Incorrect. 'Who' refers to people. -->
- [ ] D) written <!-- feedback: Incorrect. Needs the verb 'to be' in this full relative clause structure. -->

### Explicacion Pedagogica
The relative pronoun 'which' introduces extra information about the subject (the novel) in a non-defining clause.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v3`
**Bloom:** Apply
**EJE:** Grammar
**Expected_Success:** 0.65
**Contexto:** Conditional about reading.

### Enunciado
If I hadn't read that book, I wouldn't have understood the historical context of the period.

### Opciones
- [ ] A) didn't read <!-- feedback: Incorrect. Second conditional. -->
- [x] B) hadn't read <!-- feedback: Correct. Third conditional for hypothetical past. -->
- [ ] C) haven't read <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) wouldn't read <!-- feedback: Incorrect. 'Would' not used in 'if' clause. -->

### Explicacion Pedagogica
The third conditional allows us to reflect on the impact of a past experience (reading a book).

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v4`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about magical realism.

### Enunciado
Magical realism is a genre that depicts the real world as having an undercurrent of magic or fantasy.

### Opciones
- [x] A) Magical realism <!-- feedback: Correct. A major genre in Latin American and global literature. -->
- [ ] B) Science fiction <!-- feedback: Incorrect. Focuses on future/technology. -->
- [ ] C) Biography <!-- feedback: Incorrect. Non-fiction life story. -->
- [ ] D) Mystery <!-- feedback: Incorrect. Focuses on solving a crime or puzzle. -->

### Explicacion Pedagogica
'Magical realism' is the specific literary term for the blending of mundane reality with fantastic elements.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v5`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on the role of the protagonist.

### Enunciado
The protagonist is the main character who drives the action and often undergoes a significant transformation.

### Opciones
- [ ] A) antagonist <!-- feedback: Incorrect. The antagonist opposes the main character. -->
- [x] B) protagonist <!-- feedback: Correct. The leading character. -->
- [ ] C) narrator <!-- feedback: Incorrect. The narrator tells the story but isn't always the protagonist. -->
- [ ] D) villain <!-- feedback: Incorrect. A type of antagonist. -->

### Explicacion Pedagogica
'Protagonist' is the technical term for the central figure in a literary work.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v6`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Describing a type of story.

### Enunciado
A fable is a short story, typically with animals as characters, conveying a moral.

### Opciones
- [ ] A) novel <!-- feedback: Incorrect. A novel is long. -->
- [x] B) fable <!-- feedback: Correct. Specifically uses animals to teach a lesson. -->
- [ ] C) essay <!-- feedback: Incorrect. An essay is non-fiction. -->
- [ ] D) journal <!-- feedback: Incorrect. A journal is a daily record. -->

### Explicacion Pedagogica
'Fable' is the specific term for an allegorical short story, often featuring anthropomorphized animals.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v7`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing an ongoing literary trend.

### Enunciado
Graphic novels are increasingly being recognized as a serious form of literature.

### Opciones
- [ ] A) are recognizing <!-- feedback: Incorrect. Active voice. -->
- [x] B) are being recognized <!-- feedback: Correct. Present continuous passive for an ongoing trend. -->
- [ ] C) recognized <!-- feedback: Incorrect. Past simple. -->
- [ ] D) have recognized <!-- feedback: Incorrect. Active voice. -->

### Explicacion Pedagogica
The present continuous passive describes the current process of shifting critical opinion.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v8`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Text on "literary devices".

### Enunciado
Foreshadowing is a literary device in which a writer gives an advance hint of what is to come later in the story.

### Opciones
- [ ] A) Flashback <!-- feedback: Incorrect. A flashback shows the past. -->
- [x] B) Foreshadowing <!-- feedback: Correct. Foreshadowing hints at the future. -->
- [ ] C) Metaphor <!-- feedback: Incorrect. A metaphor compares two things. -->
- [ ] D) Irony <!-- feedback: Incorrect. Irony involves contrast between expectation and reality. -->

### Explicacion Pedagogica
'Foreshadowing' is the technical term for the narrative technique of providing hints about future events.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v9`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing a reading habit.

### Enunciado
I used to read fairy tales every night when I was a child.

### Opciones
- [ ] A) am used to <!-- feedback: Incorrect. This means 'accustomed to' in the present. -->
- [x] B) used to <!-- feedback: Correct. 'Used to' describes a past habit. -->
- [ ] C) use to <!-- feedback: Incorrect grammar. -->
- [ ] D) would <!-- feedback: Incorrect. While 'would' works for habits, 'used to' is the standard for contrasting past and present. -->

### Explicacion Pedagogica
'Used to' is the standard way to describe actions that were habitual in the past but no longer occur.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v10`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of "stream of consciousness".

### Enunciado
The author uses "stream of consciousness" to give readers a direct view into the character's internal thoughts and feelings.

### Opciones
- [ ] A) dialogue <!-- feedback: Incorrect. Dialogue is external conversation. -->
- [x] B) stream of consciousness <!-- feedback: Correct. Literary technique for representing internal thought processes. -->
- [ ] C) summary <!-- feedback: Incorrect. A summary condenses events. -->
- [ ] D) prologue <!-- feedback: Incorrect. An introduction. -->

### Explicacion Pedagogica
'Stream of consciousness' is the term for the narrative mode that mimics the continuous flow of human thought.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Story endings.

### Enunciado
A cliffhanger is a dramatic and exciting ending to an episode of a serial, leaving the audience in suspense.

### Opciones
- [ ] A) resolution <!-- feedback: Incorrect. A resolution solves the plot. -->
- [x] B) cliffhanger <!-- feedback: Correct. Leaves the outcome uncertain. -->
- [ ] C) epilogue <!-- feedback: Incorrect. A section at the end that provides finality. -->
- [ ] D) climax <!-- feedback: Incorrect. The most intense point, but not necessarily a suspended ending. -->

### Explicacion Pedagogica
'Cliffhanger' is the informal but widely used term for a suspenseful, unresolved ending.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v12`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing an author's career.

### Enunciado
By the time the author retired, he had written over fifty novels.

### Opciones
- [ ] A) wrote <!-- feedback: Incorrect. Past simple. -->
- [x] B) had written <!-- feedback: Correct. Past perfect for an action completed before a past point. -->
- [ ] C) has written <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) was writing <!-- feedback: Incorrect. Past continuous. -->

### Explicacion Pedagogica
The past perfect 'had written' establishes the completion of the action before the author's retirement.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v13`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Discussing poetry.

### Enunciado
The poet used vivid imagery to evoke a sense of nostalgia in the reader.

### Opciones
- [ ] A) ignore <!-- feedback: Incorrect. Poetry aims to create a response. -->
- [x] B) evoke <!-- feedback: Correct. To evoke means to bring or recall to the conscious mind. -->
- [ ] C) erase <!-- feedback: Incorrect. Opposite meaning. -->
- [ ] D) simplify <!-- feedback: Incorrect. Imagery often adds complexity/richness. -->

### Explicacion Pedagogica
'Evoke' is the precise verb for describing how literature or art triggers an emotional response.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v14`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Conditional about writing.

### Enunciado
If I had more discipline, I would write a poem every day.

### Opciones
- [ ] A) have <!-- feedback: Incorrect. First conditional. -->
- [x] B) had <!-- feedback: Correct. Second conditional for a present hypothetical change. -->
- [ ] C) have had <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) would have <!-- feedback: Incorrect. 'Would' not used in 'if' clause. -->

### Explicacion Pedagogica
The second conditional 'if + past simple' describes the imaginary result of a hypothetical personality trait.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v15`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion of an essay on the power of stories.

### Enunciado
The author argues that stories are a fundamental human tool for making sense of the world.

### Opciones
- [ ] A) unimportant <!-- feedback: Incorrect. The author says they are fundamental. -->
- [x] B) fundamental <!-- feedback: Correct. Fundamental means forming a necessary base or core; of central importance. -->
- [ ] C) secondary <!-- feedback: Incorrect. Means less important. -->
- [ ] D) accidental <!-- feedback: Incorrect. Implies they are not intended or natural. -->

### Explicacion Pedagogica
'Fundamental' correctly identifies the author's perspective on the essential role of storytelling in human life.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v16`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Literary terminology.

### Enunciado
An anthology is a published collection of poems or other pieces of writing.

### Opciones
- [ ] A) dictionary <!-- feedback: Incorrect. A reference for words. -->
- [x] B) anthology <!-- feedback: Correct. A collection of selected writings. -->
- [ ] C) manual <!-- feedback: Incorrect. Instructions for a task. -->
- [ ] D) newspaper <!-- feedback: Incorrect. Periodic publication of news. -->

### Explicacion Pedagogica
'Anthology' is the specific term for a curated collection of literary works.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v17`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Expressing a hope about a future book.

### Enunciado
I hope to have read all of Shakespeare's plays by the end of the year.

### Opciones
- [ ] A) read <!-- feedback: Incorrect. Simple infinitive. -->
- [x] B) to have read <!-- feedback: Correct. Perfect infinitive for a completed action by a future point. -->
- [ ] C) reading <!-- feedback: Incorrect. Gerund. -->
- [ ] D) have read <!-- feedback: Incorrect. Needs 'to'. -->

### Explicacion Pedagogica
The perfect infinitive 'to have read' indicates a goal that will be a finished achievement by a specific deadline.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v18`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Text about "symbolism".

### Enunciado
Symbolism is the use of symbols to represent ideas or qualities.

### Opciones
- [ ] A) Realism <!-- feedback: Incorrect. Focuses on depicting things as they are. -->
- [x] B) Symbolism <!-- feedback: Correct. Using objects to stand for abstract ideas. -->
- [ ] C) Humor <!-- feedback: Incorrect. Focuses on being funny. -->
- [ ] D) Brevity <!-- feedback: Incorrect. Being short. -->

### Explicacion Pedagogica
'Symbolism' is the literary term for the representation of deeper meanings through objects or characters.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v19`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Reporting a writer's statement.

### Enunciado
The writer said that her latest story had been inspired by a dream.

### Opciones
- [ ] A) was inspired <!-- feedback: Incorrect. While possible in informal speech, 'had been' is more precise backshifting from 'was inspired' or 'has been inspired'. -->
- [x] B) had been inspired <!-- feedback: Correct. Backshifted from past simple or present perfect to past perfect. -->
- [ ] C) is inspired <!-- feedback: Incorrect. Present tense. -->
- [ ] D) will be inspired <!-- feedback: Incorrect. Future. -->

### Explicacion Pedagogica
In reported speech, we backshift tenses: the original 'My story was inspired...' becomes '...said that her story had been inspired...'.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W19-literature-stories-001-MASTERY-v20`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Final thought on universal themes.

### Enunciado
Universal themes like love, loss, and the search for identity transcend cultural and temporal boundaries.

### Opciones
- [x] A) transcend <!-- feedback: Correct. To transcend means to go beyond the range or limits of. -->
- [ ] B) obey <!-- feedback: Incorrect. They go *beyond* limits, not follow them. -->
- [ ] C) ignore <!-- feedback: Incorrect. Transcending is about rising above, not just ignoring. -->
- [ ] D) stop <!-- feedback: Incorrect. They continue to be relevant across boundaries. -->

### Explicacion Pedagogica
'Transcend' correctly describes how great literature remains relevant regardless of the reader's culture or time period.
