---
id: "CL-ING-11-2026-W10-social-trends-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "social-trends"
periodo: "weekly"
week: "W10"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Social Trends (W10)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v1`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Describing modern behaviors.

### Enunciado
A trend is a general direction in which something is developing or changing.

### Opciones
- [ ] A) ritual <!-- feedback: Incorrect. A ritual is a ceremony. -->
- [x] B) trend <!-- feedback: Correct. A trend is a current style or popular direction. -->
- [ ] C) tradition <!-- feedback: Incorrect. A tradition is long-standing. -->
- [ ] D) law <!-- feedback: Incorrect. A law is a formal rule. -->

### Explicacion Pedagogica
'Trend' is the specific term for a prevailing tendency or popular movement in society.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v2`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing demographic changes.

### Enunciado
The average age of the population has been increasing in many developed countries.

### Opciones
- [ ] A) increases <!-- feedback: Incorrect. Present simple. -->
- [x] B) has been increasing <!-- feedback: Correct. Present perfect continuous for an ongoing demographic trend. -->
- [ ] C) increased <!-- feedback: Incorrect. Past simple. -->
- [ ] D) will increase <!-- feedback: Incorrect. Simple future. -->

### Explicacion Pedagogica
The present perfect continuous highlights that the increase is a process that is currently continuing.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v3`
**Bloom:** Apply
**ICFES:** Grammar
**Expected_Success:** 0.65
**Contexto:** Speculating about a social change.

### Enunciado
If remote work had been possible twenty years ago, urbanization might have slowed down.

### Opciones
- [ ] A) was <!-- feedback: Incorrect. Second conditional. -->
- [x] B) had been <!-- feedback: Correct. Third conditional for hypothetical past situation. -->
- [ ] C) is <!-- feedback: Incorrect. Present tense. -->
- [ ] D) has been <!-- feedback: Incorrect. Present perfect. -->

### Explicacion Pedagogica
The third conditional (if + past perfect) is used to imagine how the past might have been different.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v4`
**Bloom:** Analyze
**ICFES:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about digital nomads.

### Enunciado
Digital nomads are individuals who use technology to work remotely and lead a lifestyle that allows them to travel frequently.

### Opciones
- [x] A) nomads <!-- feedback: Correct. A nomad is a person with no permanent home who moves from place to place. -->
- [ ] B) settlers <!-- feedback: Incorrect. Settlers stay in one place. -->
- [ ] C) natives <!-- feedback: Incorrect. Refers to where someone was born. -->
- [ ] D) recluses <!-- feedback: Incorrect. Recluses avoid society. -->

### Explicacion Pedagogica
'Digital nomad' is the standard term for professionals who work while traveling.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v5`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on the "sharing economy".

### Enunciado
The sharing economy emphasizes access over ownership, allowing people to rent or borrow goods rather than buying them.

### Opciones
- [ ] A) profit <!-- feedback: Incorrect. While companies profit, the concept is about access. -->
- [x] B) access <!-- feedback: Correct. Sharing economy platforms focus on providing temporary access to resources. -->
- [ ] C) scarcity <!-- feedback: Incorrect. It aims to reduce the impact of scarcity. -->
- [ ] D) status <!-- feedback: Incorrect. It often challenges traditional status symbols of ownership. -->

### Explicacion Pedagogica
The fundamental shift in the sharing economy is prioritizing the ability to use something ('access') rather than owning it.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v6`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Social media influence.

### Enunciado
An influencer is a person with the ability to influence potential buyers of a product or service by promoting or recommending it on social media.

### Opciones
- [ ] A) observer <!-- feedback: Incorrect. Observers are passive. -->
- [x] B) influencer <!-- feedback: Correct. This is the specific term for this social role. -->
- [ ] C) critic <!-- feedback: Incorrect. Critics analyze, but don't necessarily have the same commercial influence. -->
- [ ] D) apprentice <!-- feedback: Incorrect. An apprentice is learning. -->

### Explicacion Pedagogica
'Influencer' is the modern marketing term for individuals with significant social media followings.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v7`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a widespread social phenomenon.

### Enunciado
Social media platforms are being used by billions of people to stay connected with friends and family.

### Opciones
- [ ] A) are using <!-- feedback: Incorrect. Active voice. -->
- [x] B) are being used <!-- feedback: Correct. Present continuous passive for an ongoing state. -->
- [ ] C) were used <!-- feedback: Incorrect. Past simple. -->
- [ ] D) use <!-- feedback: Incorrect. Present simple. -->

### Explicacion Pedagogica
The present continuous passive 'are being used' describes the current and ongoing utility of these platforms.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v8`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Text on "minimalism".

### Enunciado
Minimalism is a lifestyle choice that focuses on living with fewer material possessions to prioritize experiences and relationships.

### Opciones
- [ ] A) materialism <!-- feedback: Incorrect. Materialism is the opposite. -->
- [x] B) minimalism <!-- feedback: Correct. Minimalism is the practice of living with only the essentials. -->
- [ ] C) consumerism <!-- feedback: Incorrect. Consumerism focuses on buying more. -->
- [ ] D) narcissism <!-- feedback: Incorrect. Narcissism is excessive self-interest. -->

### Explicacion Pedagogica
'Minimalism' correctly describes the social trend of reducing clutter and possessions.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v9`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing a change in behavior.

### Enunciado
Many people used to commute to an office every day, but now they work from home.

### Opciones
- [ ] A) are used to <!-- feedback: Incorrect. This means 'accustomed to'. -->
- [x] B) used to <!-- feedback: Correct. Describes a past habit that has changed. -->
- [ ] C) use to <!-- feedback: Incorrect grammar. -->
- [ ] D) would <!-- feedback: Incorrect. 'Used to' is better for contrasting the past and present. -->

### Explicacion Pedagogica
'Used to' is the standard structure for comparing past habits with present reality.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v10`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of "cancel culture".

### Enunciado
The article argues that cancel culture can sometimes lead to a "mob mentality" where individuals are condemned without due process.

### Opciones
- [ ] A) harmony <!-- feedback: Incorrect. Condemnation is not harmony. -->
- [x] B) mob mentality <!-- feedback: Correct. Mob mentality refers to people being influenced by their peers to adopt certain behaviors. -->
- [ ] C) logic <!-- feedback: Incorrect. Mob mentality is often described as lacking logic. -->
- [ ] D) empathy <!-- feedback: Incorrect. Mob mentality often lacks empathy. -->

### Explicacion Pedagogica
'Mob mentality' (or herd mentality) describes the collective behavior of a group that can be irrational or aggressive.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v11`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Demographic groups.

### Enunciado
The term "Millennials" refers to the generation of people born between the early 1980s and the mid-1990s.

### Opciones
- [ ] A) Boomers <!-- feedback: Incorrect. Born earlier (post-WWII). -->
- [ ] B) Gen Z <!-- feedback: Incorrect. Born later (late 90s to early 2010s). -->
- [x] C) Millennials <!-- feedback: Correct. This is the specific name for that generation. -->
- [ ] D) Builders <!-- feedback: Incorrect. Refers to an even older generation. -->

### Explicacion Pedagogica
'Millennials' (or Gen Y) is the standard sociological label for the group described.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v12`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing social pressure.

### Enunciado
Young people often feel pressured into conforming to the standards of their peer group.

### Opciones
- [x] A) conforming to <!-- feedback: Correct. 'Conform to' means to comply with rules or standards. -->
- [ ] B) resisting <!-- feedback: Incorrect. Resistance is the opposite of yielding to pressure. -->
- [ ] C) ignoring <!-- feedback: Incorrect. Ignoring means not paying attention. -->
- [ ] D) leading <!-- feedback: Incorrect. Leading is the opposite of conforming. -->

### Explicacion Pedagogica
'Conforming to' is the appropriate phrasal verb for yielding to group expectations.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v13`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Discussing work-life balance.

### Enunciado
Burnout is a state of emotional, physical, and mental exhaustion caused by excessive and prolonged stress.

### Opciones
- [x] A) Burnout <!-- feedback: Correct. This is the specific term for work-related exhaustion. -->
- [ ] B) Ambition <!-- feedback: Incorrect. Ambition is the desire to achieve. -->
- [ ] C) Resilience <!-- feedback: Incorrect. Resilience is the ability to recover. -->
- [ ] D) Enthusiasm <!-- feedback: Incorrect. Enthusiasm is positive energy. -->

### Explicacion Pedagogica
'Burnout' is the recognized psychological and professional term for the condition described.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v14`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Conditional about social media use.

### Enunciado
If people spent less time on their screens, they would probably feel more connected to the real world.

### Opciones
- [ ] A) spend <!-- feedback: Incorrect. First conditional. -->
- [x] B) spent <!-- feedback: Correct. Second conditional for a hypothetical change. -->
- [ ] C) have spent <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) would spend <!-- feedback: Incorrect. 'Would' not used in 'if' clause. -->

### Explicacion Pedagogica
The second conditional 'if + past simple' describes the likely result of a hypothetical change in behavior.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v15`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion on urban migration.

### Enunciado
The author concludes that the "urban exodus" is driven by a desire for more space and a lower cost of living.

### Opciones
- [ ] A) growth <!-- feedback: Incorrect. Exodus means leaving. -->
- [x] B) exodus <!-- feedback: Correct. An exodus is a mass departure of people. -->
- [ ] C) isolation <!-- feedback: Incorrect. Focuses on the state rather than the movement. -->
- [ ] D) congestion <!-- feedback: Incorrect. This is usually why people leave cities, not the name of the movement itself. -->

### Explicacion Pedagogica
'Urban exodus' is the specific term for the trend of people moving from cities to suburban or rural areas.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v16`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Describing viral content.

### Enunciado
A "viral" post is one that is shared rapidly and widely from one internet user to another.

### Opciones
- [ ] A) private <!-- feedback: Incorrect. Private posts are not shared widely. -->
- [x] B) viral <!-- feedback: Correct. Viral means spreading very quickly. -->
- [ ] C) boring <!-- feedback: Incorrect. Boring posts usually don't spread. -->
- [ ] D) expensive <!-- feedback: Incorrect. Sharing online is usually free. -->

### Explicacion Pedagogica
'Viral' is the standard adjective for content that spreads quickly across the internet.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v17`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a current social movement.

### Enunciado
A new movement for social justice is gaining momentum across the country.

### Opciones
- [ ] A) lose <!-- feedback: Incorrect. Simple present. -->
- [x] B) is gaining <!-- feedback: Correct. Present continuous for an ongoing process. -->
- [ ] C) has gained <!-- feedback: Incorrect. Present perfect; focus is on the current growth. -->
- [ ] D) will gain <!-- feedback: Incorrect. Simple future. -->

### Explicacion Pedagogica
'Is gaining momentum' is a common idiom meaning to become more popular or successful.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v18`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Text about sustainable fashion.

### Enunciado
"Fast fashion" refers to inexpensive clothing produced rapidly by mass-market retailers in response to the latest trends.

### Opciones
- [ ] A) Slow <!-- feedback: Incorrect. Slow fashion is the sustainable alternative. -->
- [x] B) Fast <!-- feedback: Correct. Fast fashion is the industry term for rapid, cheap production. -->
- [ ] C) Luxury <!-- feedback: Incorrect. Luxury fashion is expensive and produced slowly. -->
- [ ] D) Classic <!-- feedback: Incorrect. Traditional, unchanging styles. -->

### Explicacion Pedagogica
'Fast fashion' is the specific term for the business model described.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v19`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Reporting a social prediction.

### Enunciado
Experts said that the trend toward automation would continue to accelerate.

### Opciones
- [ ] A) will <!-- feedback: Incorrect. Backshifted from 'will' to 'would' in reported speech. -->
- [x] B) would <!-- feedback: Correct. 'Would' is the past form of 'will' used in reported speech. -->
- [ ] C) should <!-- feedback: Incorrect. Expresses advice or expectation. -->
- [ ] D) can <!-- feedback: Incorrect. Backshifted to 'could'. -->

### Explicacion Pedagogica
In reported speech, we change 'will' to 'would' when the reporting verb ('said') is in the past.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W10-social-trends-001-MASTERY-v20`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Final thought on modern connection.

### Enunciado
The paradox of our time is that while we are more "connected" than ever digitally, many report feeling more isolated.

### Opciones
- [x] A) paradox <!-- feedback: Correct. A paradox is a seemingly absurd or self-contradictory statement that may prove to be true. -->
- [ ] B) solution <!-- feedback: Incorrect. This is a problem, not a solution. -->
- [ ] C) agreement <!-- feedback: Incorrect. The two facts contradict each other. -->
- [ ] D) benefit <!-- feedback: Incorrect. Feeling isolated is not a benefit. -->

### Explicacion Pedagogica
'Paradox' correctly identifies the contradiction between digital connectivity and personal isolation.
