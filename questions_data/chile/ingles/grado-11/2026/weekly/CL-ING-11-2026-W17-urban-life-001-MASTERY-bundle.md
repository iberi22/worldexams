---
id: "CL-ING-11-2026-W17-urban-life-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "urban-life"
periodo: "weekly"
week: "W17"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Urban Life (W17)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v1`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Describing city infrastructure.

### Enunciado
Public transportation systems like the metro or buses are essential for reducing traffic congestion in big cities.

### Opciones
- [ ] A) isolation <!-- feedback: Incorrect. Public trans connects people. -->
- [x] B) congestion <!-- feedback: Correct. Congestion is the state of being overcrowded with traffic. -->
- [ ] C) speed <!-- feedback: Incorrect. It aims to increase speed by reducing congestion. -->
- [ ] D) ruralization <!-- feedback: Incorrect. This refers to the country, not the city. -->

### Explicacion Pedagogica
'Congestion' is the standard term for the overcrowding of traffic or people in urban areas.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v2`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing city growth.

### Enunciado
The population of this city has been growing at an unprecedented rate for the last ten years.

### Opciones
- [ ] A) grows <!-- feedback: Incorrect. Present simple. -->
- [x] B) has been growing <!-- feedback: Correct. Present perfect continuous for a trend starting in the past and continuing. -->
- [ ] C) grew <!-- feedback: Incorrect. Past simple. -->
- [ ] D) will grow <!-- feedback: Incorrect. Simple future. -->

### Explicacion Pedagogica
The present perfect continuous highlights the ongoing nature of the city's population growth.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v3`
**Bloom:** Apply
**EJE:** Grammar
**Expected_Success:** 0.65
**Contexto:** Speculating about urban planning.

### Enunciado
If the city had invested more in green spaces, urban heat islands wouldn't be such a significant problem today.

### Opciones
- [ ] A) invested <!-- feedback: Incorrect. Second conditional. -->
- [x] B) had invested <!-- feedback: Correct. Mixed conditional (past action, present result). -->
- [ ] C) has invested <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) would invest <!-- feedback: Incorrect. 'Would' not used in 'if' clause. -->

### Explicacion Pedagogica
The mixed conditional (if + past perfect, would + verb) connects a past failure to invest with a current negative situation.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v4`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about "gentrification".

### Enunciado
Gentrification occurs when the character of a poor urban area is changed by wealthier people moving in, improving housing and attracting new businesses.

### Opciones
- [x] A) Gentrification <!-- feedback: Correct. Specific term for the process described. -->
- [ ] B) Urbanization <!-- feedback: Incorrect. Broad term for city growth. -->
- [ ] C) Migration <!-- feedback: Incorrect. Broad term for moving. -->
- [ ] D) Renovation <!-- feedback: Incorrect. Part of the process, but not the whole sociological term. -->

### Explicacion Pedagogica
'Gentrification' is the sociological term for the transformation of working-class neighborhoods into wealthier ones.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v5`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on urban "sprawl".

### Enunciado
The primary disadvantage of urban sprawl is the increased dependence on cars and the resulting environmental impact.

### Opciones
- [ ] A) concentration <!-- feedback: Incorrect. Sprawl is the opposite of concentration. -->
- [x] B) sprawl <!-- feedback: Correct. Urban sprawl is the outward expansion of cities. -->
- [ ] C) density <!-- feedback: Incorrect. Sprawl leads to low density. -->
- [ ] D) unity <!-- feedback: Incorrect. Sprawl often separates communities. -->

### Explicacion Pedagogica
'Urban sprawl' refers to the rapid, often unplanned, geographic expansion of cities into nearby rural areas.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v6`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Modern city convenience.

### Enunciado
In a metropolis, you can find a wide variety of shops, restaurants, and entertainment options.

### Opciones
- [ ] A) hamlet <!-- feedback: Incorrect. A hamlet is a very small village. -->
- [x] B) metropolis <!-- feedback: Correct. A metropolis is a very large and busy city. -->
- [ ] C) suburb <!-- feedback: Incorrect. A suburb is on the edge of a city. -->
- [ ] D) province <!-- feedback: Incorrect. A province is a region. -->

### Explicacion Pedagogica
'Metropolis' is the formal term for a major city, especially the main city of a region.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v7`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Passive voice in a city report.

### Enunciado
New cycling paths are being constructed throughout the city to promote sustainable transport.

### Opciones
- [ ] A) are constructing <!-- feedback: Incorrect. Active voice. -->
- [x] B) are being constructed <!-- feedback: Correct. Present continuous passive for an ongoing project. -->
- [ ] C) constructed <!-- feedback: Incorrect. Past simple. -->
- [ ] D) have constructed <!-- feedback: Incorrect. Active voice. -->

### Explicacion Pedagogica
The present continuous passive describes a construction project that is currently in progress.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v8`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Text on "affordable housing".

### Enunciado
The lack of affordable housing is a major challenge for many young professionals living in urban centers.

### Opciones
- [ ] A) luxury <!-- feedback: Incorrect. Luxury housing is usually plentiful but too expensive. -->
- [x] B) affordable <!-- feedback: Correct. Affordable means inexpensive enough for people to pay. -->
- [ ] C) modern <!-- feedback: Incorrect. It may be modern but the main issue is the cost. -->
- [ ] D) industrial <!-- feedback: Incorrect. Not meant for living. -->

### Explicacion Pedagogica
'Affordable housing' is the specific term for residential units that are priced for low-to-moderate-income families.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v9`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing an experience in a new city.

### Enunciado
He is getting used to the noise and hustle of city life after moving from a small town.

### Opciones
- [ ] A) is used to <!-- feedback: Incorrect. This means he is already accustomed; 'getting used to' shows the process. -->
- [x] B) is getting used to <!-- feedback: Correct. 'Be getting used to' describes the process of becoming accustomed. -->
- [ ] C) used to <!-- feedback: Incorrect. Refers to a past habit. -->
- [ ] D) gets used to <!-- feedback: Incorrect. Simple present doesn't show the current process. -->

### Explicacion Pedagogica
'Is getting used to' describes the ongoing process of adapting to a new environment.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v10`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of "concrete jungles".

### Enunciado
The author uses the term "concrete jungle" to describe cities that lack sufficient vegetation and feel harsh or inhospitable.

### Opciones
- [ ] A) oasis <!-- feedback: Incorrect. An oasis is a fertile spot in a desert. -->
- [x] B) concrete jungle <!-- feedback: Correct. Metaphor for an unpleasant, crowded city environment. -->
- [ ] C) paradise <!-- feedback: Incorrect. Opposite meaning. -->
- [ ] D) playground <!-- feedback: Incorrect. Focuses on fun rather than the harshness. -->

### Explicacion Pedagogica
'Concrete jungle' is a common critical metaphor for a city that feels dominated by grey buildings and lack of nature.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Commuting terms.

### Enunciado
A commuter is a person who travels some distance to work on a regular basis.

### Opciones
- [ ] A) tourist <!-- feedback: Incorrect. Travels for pleasure. -->
- [x] B) commuter <!-- feedback: Correct. Travels for work. -->
- [ ] C) resident <!-- feedback: Incorrect. Just lives there, doesn't necessarily travel. -->
- [ ] D) native <!-- feedback: Incorrect. Born there. -->

### Explicacion Pedagogica
'Commuter' is the specific term for someone who travels regularly between home and work.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v12`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Passive voice in urban history.

### Enunciado
The old shipyard has been transformed into a vibrant cultural hub.

### Opciones
- [ ] A) has transformed <!-- feedback: Incorrect. Active voice. -->
- [x] B) has been transformed <!-- feedback: Correct. Present perfect passive for a completed change with present result. -->
- [ ] C) is transforming <!-- feedback: Incorrect. Active voice. -->
- [ ] D) transforms <!-- feedback: Incorrect. Simple present. -->

### Explicacion Pedagogica
The present perfect passive describes a completed process of change that defines the current state of a place.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v13`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Discussing city diversity.

### Enunciado
The cosmopolitan nature of the city is reflected in its diverse population and international cuisine.

### Opciones
- [x] A) cosmopolitan <!-- feedback: Correct. Cosmopolitan means containing people from many different countries. -->
- [ ] B) provincial <!-- feedback: Incorrect. Provincial means narrow-minded or typical of a province. -->
- [ ] C) isolated <!-- feedback: Incorrect. Isolation is the opposite of cosmopolitanism. -->
- [ ] D) homogeneous <!-- feedback: Incorrect. Homogeneous means all the same. -->

### Explicacion Pedagogica
'Cosmopolitan' is the ideal adjective for a city that is multicultural and globally connected.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v14`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Conditional about public transport.

### Enunciado
If the metro were more reliable, more people would leave their cars at home.

### Opciones
- [ ] A) is <!-- feedback: Incorrect. First conditional. -->
- [x] B) were <!-- feedback: Correct. Second conditional for a hypothetical situation. -->
- [ ] C) has been <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) had been <!-- feedback: Incorrect. Third conditional. -->

### Explicacion Pedagogica
The second conditional 'if + were' describes how a hypothetical improvement would change behavior.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v15`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion on urban sustainability.

### Enunciado
The author argues that cities must prioritize "walkability" to improve the health and well-being of their residents.

### Opciones
- [ ] A) parking <!-- feedback: Incorrect. Prioritizing parking usually hurts walkability. -->
- [x] B) walkability <!-- feedback: Correct. Walkability is the measure of how friendly an area is to walking. -->
- [ ] C) isolation <!-- feedback: Incorrect. Negative. -->
- [ ] D) construction <!-- feedback: Incorrect. Too broad. -->

### Explicacion Pedagogica
'Walkability' is a key concept in modern urban planning focused on human-centric design.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v16`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** City parts.

### Enunciado
The outskirts are the parts of a town or city that are furthest from the center.

### Opciones
- [ ] A) heart <!-- feedback: Incorrect. The heart is the center. -->
- [x] B) outskirts <!-- feedback: Correct. The outer edges of a city. -->
- [ ] C) plaza <!-- feedback: Incorrect. A plaza is a central square. -->
- [ ] D) landmark <!-- feedback: Incorrect. A landmark is a recognizable feature. -->

### Explicacion Pedagogica
'Outskirts' is the standard term for the peripheral areas of a city.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v17`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Expressing a wish about city life.

### Enunciado
I wish there were less noise in my neighborhood at night.

### Opciones
- [ ] A) is <!-- feedback: Incorrect. Present tense. -->
- [x] B) were <!-- feedback: Correct. 'Wish + were' for a present hypothetical desire. -->
- [ ] C) was <!-- feedback: Incorrect. 'Were' is preferred in formal wishes about the present. -->
- [ ] D) will be <!-- feedback: Incorrect. Future. -->

### Explicacion Pedagogica
The 'wish + past' structure is used to express a desire for the current situation to be different.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v18`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Text about "smart cities".

### Enunciado
Smart cities use technology and data to optimize urban services and improve the quality of life for citizens.

### Opciones
- [ ] A) ignore <!-- feedback: Incorrect. Smart cities actively use data. -->
- [x] B) optimize <!-- feedback: Correct. To optimize means to make the best or most effective use of a resource. -->
- [ ] C) complicated <!-- feedback: Incorrect. The goal is to improve, not just complicate. -->
- [ ] D) delete <!-- feedback: Incorrect. They use, don't delete, services. -->

### Explicacion Pedagogica
'Optimize' describes the goal of using data to make city systems more efficient.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v19`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Reporting a city planning decision.

### Enunciado
The council announced that they would be pedestrianizing the main street next month.

### Opciones
- [ ] A) will <!-- feedback: Incorrect. Backshifted in reported speech. -->
- [x] B) would be <!-- feedback: Correct. Backshifted from 'will be' to 'would be' in reported speech. -->
- [ ] C) are <!-- feedback: Incorrect. Present tense. -->
- [ ] D) have been <!-- feedback: Incorrect. Present perfect. -->

### Explicacion Pedagogica
In reported speech, we change 'will be' to 'would be' when reporting a future plan in the past tense.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W17-urban-life-001-MASTERY-v20`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Final thought on urban community.

### Enunciado
Despite the density of the city, many people find a strong sense of community in their local neighborhoods.

### Opciones
- [x] A) sense of community <!-- feedback: Correct. Feeling of belonging to a group. -->
- [ ] B) feeling of isolation <!-- feedback: Incorrect. The text says 'Despite...', implying a positive despite the density. -->
- [ ] C) lack of connection <!-- feedback: Incorrect. Also negative. -->
- [ ] D) desire for wealth <!-- feedback: Incorrect. Unrelated to the social point. -->

### Explicacion Pedagogica
'Sense of community' is the standard term for the feeling of belonging and mutual support within a neighborhood.
