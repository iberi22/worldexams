---
id: "CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "philosophy-logic"
periodo: "weekly"
week: "W23"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Philosophy and Logic (W23)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v1`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Introduction to reasoning.

### Enunciado
Logic is the study of correct reasoning and the principles of valid argument.

### Opciones
- [ ] A) Emotion <!-- feedback: Incorrect. Logic is based on reason, not emotion. -->
- [x] B) Logic <!-- feedback: Correct. The branch of philosophy concerned with reasoning. -->
- [ ] C) Fiction <!-- feedback: Incorrect. Fiction is imaginary. -->
- [ ] D) Rhetoric <!-- feedback: Incorrect. Rhetoric is the art of persuasion, which may or may not use logic. -->

### Explicacion Pedagogica
'Logic' is the formal term for the systematic study of valid inference and argument.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v2`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing existentialism.

### Enunciado
Existentialism is a philosophy that emphasizes the individual's freedom and responsibility in a seemingly meaningless world.

### Opciones
- [ ] A) emphasize <!-- feedback: Incorrect. Subject-verb agreement ('philosophy' is singular). -->
- [x] B) emphasizes <!-- feedback: Correct. Present simple for a definition. -->
- [ ] C) emphasizing <!-- feedback: Incorrect. Gerund. -->
- [ ] D) emphasized <!-- feedback: Incorrect. Past tense. -->

### Explicacion Pedagogica
The present simple 'emphasizes' is used to state a central characteristic of a philosophical movement.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v3`
**Bloom:** Apply
**EJE:** Grammar
**Expected_Success:** 0.65
**Contexto:** Using "if only" for philosophical regret.

### Enunciado
If only we had questioned our assumptions earlier, we would have avoided many logical fallacies.

### Opciones
- [ ] A) questioned <!-- feedback: Incorrect. 'If only' for past regret needs past perfect. -->
- [x] B) had questioned <!-- feedback: Correct. 'If only + past perfect' for past regret. -->
- [ ] C) have questioned <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) would question <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
'If only' followed by the past perfect expresses a strong desire for the past to have been different.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v4`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about Socrates.

### Enunciado
Socrates is famous for his method of inquiry, which involved asking a series of questions to stimulate critical thinking.

### Opciones
- [x] A) inquiry <!-- feedback: Correct. Inquiry is the act of asking for information. -->
- [ ] B) silence <!-- feedback: Incorrect. Socrates was famous for talking/asking. -->
- [ ] C) agreement <!-- feedback: Incorrect. He often challenged people's ideas. -->
- [ ] D) isolation <!-- feedback: Incorrect. He taught in public spaces. -->

### Explicacion Pedagogica
'Inquiry' (or 'Socratic inquiry') is the term for the investigative questioning style used by Socrates.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v5`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on "logical fallacies".

### Enunciado
A fallacy is a mistaken belief, especially one based on unsound argument or invalid reasoning.

### Opciones
- [ ] A) fact <!-- feedback: Incorrect. A fallacy is the opposite of a fact. -->
- [x] B) fallacy <!-- feedback: Correct. The term for an error in reasoning. -->
- [ ] C) proof <!-- feedback: Incorrect. A proof is a valid argument. -->
- [ ] D) certainty <!-- feedback: Incorrect. Fallacies lead to false certainty. -->

### Explicacion Pedagogica
'Fallacy' is the technical term in logic for a flaw in reasoning that makes an argument invalid.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v6`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Ethical frameworks.

### Enunciado
Deontology is an ethical theory that judges the morality of an action based on rules or duties.

### Opciones
- [ ] A) Hedonism <!-- feedback: Incorrect. Focuses on pleasure. -->
- [x] B) Deontology <!-- feedback: Correct. Focuses on duty and rules. -->
- [ ] C) Nihilism <!-- feedback: Incorrect. The belief that life is meaningless. -->
- [ ] D) Skepticism <!-- feedback: Incorrect. Doubting the truth. -->

### Explicacion Pedagogica
'Deontology' (from the Greek 'deon' meaning duty) is the rule-based branch of ethics.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v7`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a philosophical debate.

### Enunciado
The nature of consciousness is still being explored by philosophers and neuroscientists.

### Opciones
- [ ] A) is exploring <!-- feedback: Incorrect. Active voice. -->
- [x] B) is being explored <!-- feedback: Correct. Present continuous passive for an ongoing area of study. -->
- [ ] C) explored <!-- feedback: Incorrect. Past simple. -->
- [ ] D) has explored <!-- feedback: Incorrect. Active voice. -->

### Explicacion Pedagogica
The present continuous passive describes a topic that remains a focus of current, active investigation.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v8`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Text on "Empiricism".

### Enunciado
Empiricism is the theory that all knowledge is derived from sense-experience.

### Opciones
- [ ] A) Rationalism <!-- feedback: Incorrect. Focuses on reason as the source of knowledge. -->
- [x] B) Empiricism <!-- feedback: Correct. Focuses on experience/observation. -->
- [ ] C) Idealism <!-- feedback: Incorrect. Focuses on ideas or the mind as the primary reality. -->
- [ ] D) Relativism <!-- feedback: Incorrect. The idea that truth is not absolute. -->

### Explicacion Pedagogica
'Empiricism' is the philosophical tradition that prioritizes observation and experience in the formation of knowledge.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v9`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Reflecting on a philosopher's influence.

### Enunciado
Kant's work had been widely read long before it was translated into English.

### Opciones
- [ ] A) was reading <!-- feedback: Incorrect. Active voice and past continuous. -->
- [x] B) had been widely read <!-- feedback: Correct. Past perfect passive for duration/state before a past point. -->
- [ ] C) has been read <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) is read <!-- feedback: Incorrect. Present simple. -->

### Explicacion Pedagogica
The past perfect passive describes the state of the philosopher's work prior to its translation.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v10`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of "Postmodernism".

### Enunciado
Postmodernism often challenges the idea of objective truth, suggesting that all knowledge is filtered through social and cultural contexts.

### Opciones
- [ ] A) absolute <!-- feedback: Incorrect. Postmodernism usually denies absolute truth. -->
- [x] B) objective <!-- feedback: Correct. Objective means not influenced by personal feelings or opinions. -->
- [ ] C) simple <!-- feedback: Incorrect. Context makes knowledge more complex, not simple. -->
- [ ] D) irrelevant <!-- feedback: Incorrect. Truth is still a central topic of discussion. -->

### Explicacion Pedagogica
'Objective' refers to truth that is independent of individual or cultural perspective, a concept postmodernism famously questions.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Definition of an axiom.

### Enunciado
An axiom is a statement or proposition which is regarded as being self-evidently true.

### Opciones
- [ ] A) Theory <!-- feedback: Incorrect. A theory needs testing. -->
- [x] B) Axiom <!-- feedback: Correct. A starting point assumed to be true. -->
- [ ] C) Conclusion <!-- feedback: Incorrect. An end point. -->
- [ ] D) Question <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
'Axiom' is the term for a foundational principle that is accepted without proof as the basis for further reasoning.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v12`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a logical requirement.

### Enunciado
A valid argument is one in which the conclusion follows necessarily from the premises.

### Opciones
- [ ] A) follow <!-- feedback: Incorrect. Subject-verb agreement ('conclusion' is singular). -->
- [x] B) follows <!-- feedback: Correct. Present simple for a logical definition. -->
- [ ] C) followed <!-- feedback: Incorrect. Past tense. -->
- [ ] D) following <!-- feedback: Incorrect. Gerund. -->

### Explicacion Pedagogica
In logic, 'following from' refers to the necessary relationship between the starting points (premises) and the end result (conclusion).

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v13`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Discussing "Epistemology".

### Enunciado
Epistemology is the branch of philosophy that investigates the nature, origin, and limits of human knowledge.

### Opciones
- [ ] A) Aesthetics <!-- feedback: Incorrect. Study of beauty. -->
- [ ] B) Ethics <!-- feedback: Incorrect. Study of morality. -->
- [x] C) Epistemology <!-- feedback: Correct. Study of knowledge. -->
- [ ] D) Metaphysics <!-- feedback: Incorrect. Study of the nature of reality. -->

### Explicacion Pedagogica
'Epistemology' is the specific philosophical name for the theory of knowledge.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v14`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Conditional about a philosophical stance.

### Enunciado
If I were a nihilist, I would believe that nothing has any inherent meaning.

### Opciones
- [ ] A) am <!-- feedback: Incorrect. Present tense. -->
- [x] B) were <!-- feedback: Correct. Second conditional for a hypothetical identity. -->
- [ ] C) have been <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) will be <!-- feedback: Incorrect. Future. -->

### Explicacion Pedagogica
The second conditional 'if + were' is used to explore the hypothetical consequences of adopting a specific worldview.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v15`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion of an article on Stoicism.

### Enunciado
The author concludes that Stoicism provides a practical framework for achieving tranquility in a turbulent world.

### Opciones
- [x] A) tranquility <!-- feedback: Correct. Tranquility is the quality or state of being calm. -->
- [ ] B) chaos <!-- feedback: Incorrect. Stoicism aims to avoid chaos. -->
- [ ] C) wealth <!-- feedback: Incorrect. Stoicism focuses on internal state, not money. -->
- [ ] D) confusion <!-- feedback: Incorrect. Stoicism provides clarity. -->

### Explicacion Pedagogica
'Tranquility' (or ataraxia) is the primary goal of Stoic philosophy.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v16`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Definition of a paradox.

### Enunciado
A paradox is a seemingly absurd or self-contradictory statement that when investigated or explained may prove to be well-founded or true.

### Opciones
- [ ] A) Fact <!-- feedback: Incorrect. -->
- [x] B) Paradox <!-- feedback: Correct. Standard term for a self-contradictory idea. -->
- [ ] C) Slogan <!-- feedback: Incorrect. -->
- [ ] D) Myth <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
'Paradox' describes the specific type of contradictory statement that challenges normal logic.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v17`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a logical mistake.

### Enunciado
The argument is based on a circular reasoning, which means it assumes what it is trying to prove.

### Opciones
- [ ] A) logic <!-- feedback: Incorrect. Circular reasoning is a failure of logic. -->
- [x] B) reasoning <!-- feedback: Correct. Circular reasoning is a common logical fallacy. -->
- [ ] C) fact <!-- feedback: Incorrect. It's a mistake, not a fact. -->
- [ ] D) proof <!-- feedback: Incorrect. It's not a valid proof. -->

### Explicacion Pedagogica
'Reasoning' is the noun modified by 'circular' to describe this specific type of logical error.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v18`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Text about "Humanism".

### Enunciado
Humanism is an outlook or system of thought attaching prime importance to human rather than divine or supernatural matters.

### Opciones
- [ ] A) Religion <!-- feedback: Incorrect. Focuses on the divine. -->
- [x] B) Humanism <!-- feedback: Correct. Focuses on human value and agency. -->
- [ ] C) Mysticism <!-- feedback: Incorrect. Focuses on spiritual mystery. -->
- [ ] D) Fatalism <!-- feedback: Incorrect. The belief that events are predetermined. -->

### Explicacion Pedagogica
'Humanism' is the philosophical perspective that emphasizes the dignity and potential of human beings.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v19`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Reporting a philosophical argument.

### Enunciado
Descartes argued that the very act of thinking proved his own existence.

### Opciones
- [ ] A) proves <!-- feedback: Incorrect. Backshifted in reported speech. -->
- [x] B) proved <!-- feedback: Correct. Backshifted from 'proves' to 'proved'. -->
- [ ] C) will prove <!-- feedback: Incorrect. Future. -->
- [ ] D) have proved <!-- feedback: Incorrect. Present perfect. -->

### Explicacion Pedagogica
In reported speech, we backshift the tense of the philosopher's original conclusion.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W23-philosophy-logic-001-MASTERY-v20`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Final thought on critical thinking.

### Enunciado
Ultimately, philosophy encourages us to live an "examined life," constantly questioning the world around us.

### Opciones
- [x] A) examined <!-- feedback: Correct. Based on Socrates' famous quote: "The unexamined life is not worth living." -->
- [ ] B) simple <!-- feedback: Incorrect. It adds complexity. -->
- [ ] C) ignored <!-- feedback: Incorrect. Philosophy is about paying attention. -->
- [ ] D) wealthy <!-- feedback: Incorrect. Unrelated to the intellectual goal. -->

### Explicacion Pedagogica
'Examined life' is the classical term for a life lived with self-reflection and critical inquiry.
