---
id: "CL-ING-11-2026-W07-global-economy-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "global-economy"
periodo: "weekly"
week: "W07"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Global Economy (W07)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v1`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Basic economic terms.

### Enunciado
Inflation refers to the general increase in prices and the fall in the purchasing value of money.

### Opciones
- [ ] A) Recession <!-- feedback: Incorrect. A recession is a period of temporary economic decline. -->
- [x] B) Inflation <!-- feedback: Correct. Inflation is the rise in prices over time. -->
- [ ] C) Deflation <!-- feedback: Incorrect. Deflation is a decrease in prices. -->
- [ ] D) Interest <!-- feedback: Incorrect. Interest is the cost of borrowing money. -->

### Explicacion Pedagogica
'Inflation' is the standard economic term for the situation where the cost of living increases.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v2`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing market trends.

### Enunciado
The stock market has been fluctuating significantly due to recent political instability.

### Opciones
- [ ] A) fluctuates <!-- feedback: Incorrect. Present simple. -->
- [x] B) has been fluctuating <!-- feedback: Correct. Present perfect continuous for an action that started in the past and is still ongoing. -->
- [ ] C) fluctuated <!-- feedback: Incorrect. Past simple. -->
- [ ] D) will fluctuate <!-- feedback: Incorrect. Simple future. -->

### Explicacion Pedagogica
The present perfect continuous 'has been fluctuating' emphasizes the ongoing nature of the market's volatility.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v3`
**Bloom:** Apply
**EJE:** Grammar
**Expected_Success:** 0.65
**Contexto:** Conditional about trade policy.

### Enunciado
If the trade barriers were removed, the volume of international commerce would increase.

### Opciones
- [ ] A) will remove <!-- feedback: Incorrect. Inconsistent with 'would'. -->
- [x] B) were removed <!-- feedback: Correct. Second conditional passive for a hypothetical situation. -->
- [ ] C) are removed <!-- feedback: Incorrect. First conditional. -->
- [ ] D) had been removed <!-- feedback: Incorrect. Third conditional. -->

### Explicacion Pedagogica
The second conditional passive 'if + were + past participle' is used to discuss a hypothetical change in policy.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v4`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about globalization.

### Enunciado
Globalization has led to increased interdependence among nations, making local economies more susceptible to global shifts.

### Opciones
- [ ] A) independence <!-- feedback: Incorrect. Interdependence is the opposite of independence. -->
- [x] B) interdependence <!-- feedback: Correct. Interdependence means mutual reliance between countries. -->
- [ ] C) isolation <!-- feedback: Incorrect. Globalization reduces isolation. -->
- [ ] D) self-sufficiency <!-- feedback: Incorrect. Self-sufficiency means not needing others. -->

### Explicacion Pedagogica
'Interdependence' correctly identifies the mutual reliance that characterizes the modern global economy.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v5`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on income inequality.

### Enunciado
Critics of the current economic system argue that wealth is disproportionately concentrated in the hands of a small percentage of the population.

### Opciones
- [ ] A) equally <!-- feedback: Incorrect. The text says it's concentrated in a 'small percentage'. -->
- [x] B) disproportionately <!-- feedback: Correct. Disproportionately means in a way that is out of proportion. -->
- [ ] C) fairly <!-- feedback: Incorrect. 'Concentrated' implies it is not fair. -->
- [ ] D) minimally <!-- feedback: Incorrect. If it's concentrated, it's not minimal. -->

### Explicacion Pedagogica
'Disproportionately' describes an unequal distribution that doesn't match the size of the groups.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v6`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** International organizations.

### Enunciado
The World Bank provides financial and technical assistance to developing countries for development programs.

### Opciones
- [ ] A) tax <!-- feedback: Incorrect. Tax is money paid to the government. -->
- [x] B) assistance <!-- feedback: Correct. Assistance means help or support. -->
- [ ] C) competition <!-- feedback: Incorrect. Competition is not the primary role of the World Bank. -->
- [ ] D) debt <!-- feedback: Incorrect. While it involves loans, the general term for the support is assistance. -->

### Explicacion Pedagogica
'Assistance' is the appropriate term for the support provided by international organizations.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v7`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Prediction about interest rates.

### Enunciado
Economists predict that interest rates will have risen by the end of the fiscal year.

### Opciones
- [ ] A) rise <!-- feedback: Incorrect. Simple future needs 'will'. -->
- [ ] B) will rise <!-- feedback: Incorrect. Simple future. -->
- [x] C) will have risen <!-- feedback: Correct. Future perfect for an action completed by a future point. -->
- [ ] D) rose <!-- feedback: Incorrect. Past tense. -->

### Explicacion Pedagogica
The future perfect 'will have risen' describes a state that will be a completed fact at a specific future deadline.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v8`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Text on corporate social responsibility.

### Enunciado
Companies are under increasing pressure to operate sustainably and ethically.

### Opciones
- [ ] A) recklessly <!-- feedback: Incorrect. Recklessly means without care. -->
- [x] B) ethically <!-- feedback: Correct. Ethically means in a way that is morally right. -->
- [ ] C) secretly <!-- feedback: Incorrect. Transparency is usually required. -->
- [ ] D) aggressively <!-- feedback: Incorrect. While they might compete aggressively, the pressure is for ethical behavior. -->

### Explicacion Pedagogica
'Ethically' refers to the moral principles that companies are expected to follow.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v9`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing a past economic crisis.

### Enunciado
The government had implemented several austerity measures before the economy began to recover.

### Opciones
- [ ] A) implements <!-- feedback: Incorrect. Present tense. -->
- [ ] B) has implemented <!-- feedback: Incorrect. Present perfect. -->
- [x] C) had implemented <!-- feedback: Correct. Past perfect for an action before another past action. -->
- [ ] D) was implementing <!-- feedback: Incorrect. Past continuous. -->

### Explicacion Pedagogica
The past perfect 'had implemented' establishes the sequence of events during the crisis.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v10`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of consumerism.

### Enunciado
The author suggests that the relentless pursuit of economic growth may be incompatible with environmental preservation.

### Opciones
- [ ] A) conducive <!-- feedback: Incorrect. Conducive means helping to bring about. -->
- [x] B) incompatible <!-- feedback: Correct. Incompatible means not able to exist or work together. -->
- [ ] C) essential <!-- feedback: Incorrect. Essential means absolutely necessary. -->
- [ ] D) irrelevant <!-- feedback: Incorrect. Irrelevant means not connected. -->

### Explicacion Pedagogica
'Incompatible' correctly identifies the conflict described between growth and the environment.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Definition of a budget.

### Enunciado
A budget is an estimate of income and expenditure for a set period of time.

### Opciones
- [ ] A) profit <!-- feedback: Incorrect. Profit is income minus expenses. -->
- [x] B) budget <!-- feedback: Correct. A budget is a plan for money. -->
- [ ] C) asset <!-- feedback: Incorrect. An asset is something valuable. -->
- [ ] D) liability <!-- feedback: Incorrect. A liability is a debt. -->

### Explicacion Pedagogica
'Budget' is the standard term for a financial plan.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v12`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Passive voice in an economic report.

### Enunciado
Thousands of jobs have been lost in the manufacturing sector over the last decade.

### Opciones
- [ ] A) have lost <!-- feedback: Incorrect. Active voice. -->
- [x] B) have been lost <!-- feedback: Correct. Present perfect passive for a process starting in the past. -->
- [ ] C) are losing <!-- feedback: Incorrect. Active voice. -->
- [ ] D) lost <!-- feedback: Incorrect. Past simple. -->

### Explicacion Pedagogica
The present perfect passive 'have been lost' describes the impact on employment over time.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v13`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Discussing emerging markets.

### Enunciado
Emerging markets often offer higher potential returns but also involve greater risk for investors.

### Opciones
- [x] A) Emerging <!-- feedback: Correct. Emerging markets are those of developing countries. -->
- [ ] B) Saturated <!-- feedback: Incorrect. Saturated markets are full. -->
- [ ] C) Stagnant <!-- feedback: Incorrect. Stagnant markets are not growing. -->
- [ ] D) Monopolized <!-- feedback: Incorrect. Monopolized means controlled by one. -->

### Explicacion Pedagogica
'Emerging' is the standard term for economies that are in the process of rapid growth and industrialization.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v14`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Conditional about currency devaluation.

### Enunciado
If the currency were devalued, exports would become cheaper for foreign buyers.

### Opciones
- [ ] A) is <!-- feedback: Incorrect. First conditional. -->
- [x] B) were <!-- feedback: Correct. Second conditional for a hypothetical situation. -->
- [ ] C) has been <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) had been <!-- feedback: Incorrect. Third conditional. -->

### Explicacion Pedagogica
The second conditional 'if + were' is used to discuss a hypothetical change in currency value.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v15`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion of an essay on fair trade.

### Enunciado
The author argues that fair trade initiatives help to alleviate poverty by ensuring producers receive a fair price for their goods.

### Opciones
- [ ] A) aggravate <!-- feedback: Incorrect. Aggravate means to make worse. -->
- [x] B) alleviate <!-- feedback: Correct. To alleviate means to make a problem less severe. -->
- [ ] C) ignore <!-- feedback: Incorrect. Fair trade focuses on the problem. -->
- [ ] D) maintain <!-- feedback: Incorrect. The goal is to reduce poverty. -->

### Explicacion Pedagogica
'Alleviate' is the appropriate verb for reducing the severity of poverty.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v16`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Business entities.

### Enunciado
A multinational corporation is a company that operates in several countries.

### Opciones
- [ ] A) local <!-- feedback: Incorrect. Local companies work in one area. -->
- [x] B) multinational <!-- feedback: Correct. Multinational means in many nations. -->
- [ ] C) private <!-- feedback: Incorrect. Private refers to ownership. -->
- [ ] D) nonprofit <!-- feedback: Incorrect. Nonprofit refers to the goal. -->

### Explicacion Pedagogica
'Multinational' correctly describes the international scale of the corporation's operations.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v17`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Expressing a hope for economic stability.

### Enunciado
The citizens wish the government would control inflation more effectively.

### Opciones
- [ ] A) controls <!-- feedback: Incorrect. Present simple. -->
- [ ] B) to control <!-- feedback: Incorrect. Infinitive. -->
- [x] C) would control <!-- feedback: Correct. 'Wish + would' for a desired change in behavior or situation. -->
- [ ] D) had controlled <!-- feedback: Incorrect. Wish about the past. -->

### Explicacion Pedagogica
'Wish + would' is used to express a desire for someone or something to change or act differently in the future.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v18`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Text about the "gig economy".

### Enunciado
The gig economy is characterized by the prevalence of short-term contracts or freelance work as opposed to permanent jobs.

### Opciones
- [ ] A) stable <!-- feedback: Incorrect. Gig work is often unstable. -->
- [x] B) gig <!-- feedback: Correct. Gig economy is the term for this type of labor market. -->
- [ ] C) traditional <!-- feedback: Incorrect. Traditional jobs are usually permanent. -->
- [ ] D) industrial <!-- feedback: Incorrect. Industrial refers to factories. -->

### Explicacion Pedagogica
'Gig economy' is the modern term for a labor market dominated by short-term or freelance positions.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v19`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Reporting a financial forecast.

### Enunciado
The analyst reported that the company's revenue had exceeded expectations last quarter.

### Opciones
- [ ] A) exceeds <!-- feedback: Incorrect. Present simple. -->
- [ ] B) has exceeded <!-- feedback: Incorrect. Present perfect. -->
- [x] C) had exceeded <!-- feedback: Correct. Past perfect for backshifting in reported speech. -->
- [ ] D) will exceed <!-- feedback: Incorrect. Future tense. -->

### Explicacion Pedagogica
In reported speech, we backshift tenses: 'exceeded' (past simple) or 'has exceeded' (present perfect) becomes 'had exceeded'.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W07-global-economy-001-MASTERY-v20`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Final thought on economic ethics.

### Enunciado
A truly resilient economy must be built on the foundation of social justice and environmental stewardship.

### Opciones
- [x] A) resilient <!-- feedback: Correct. Resilient means able to withstand or recover quickly from difficult conditions. -->
- [ ] B) fragile <!-- feedback: Incorrect. Fragile is the opposite of resilient. -->
- [ ] C) stagnant <!-- feedback: Incorrect. Stagnant means not growing. -->
- [ ] D) volatile <!-- feedback: Incorrect. Volatile means liable to change rapidly and unpredictably. -->

### Explicacion Pedagogica
'Resilient' describes an economy that can endure and recover from shocks.
