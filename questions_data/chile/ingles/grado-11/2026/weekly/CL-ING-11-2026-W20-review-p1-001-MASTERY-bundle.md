---
id: "CL-ING-11-2026-W20-review-p1-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "review-p1"
periodo: "weekly"
week: "W20"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Review Period 1 (W20)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3-D4]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v1`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Review of global issues (W01).

### Enunciado
Climate change refers to long-term shifts in temperatures and weather patterns.

### Opciones
- [ ] A) Weather <!-- feedback: Incorrect. Weather is short-term. -->
- [x] B) Climate change <!-- feedback: Correct. Refers to long-term global shifts. -->
- [ ] C) Season <!-- feedback: Incorrect. A season is a part of the year. -->
- [ ] D) Forecast <!-- feedback: Incorrect. A forecast is a prediction. -->

### Explicacion Pedagogica
'Climate change' is the term for the broad, long-term modifications of global climate.

---
## Question 2 [D3-D4]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v2`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Review of technology (W02).

### Enunciado
By 2040, many experts believe that most vehicles will have been automated.

### Opciones
- [ ] A) will automate <!-- feedback: Incorrect. Active voice. -->
- [x] B) will have been automated <!-- feedback: Correct. Future perfect passive for a completed state. -->
- [ ] C) automated <!-- feedback: Incorrect. Past tense. -->
- [ ] D) have automated <!-- feedback: Incorrect. Present perfect. -->

### Explicacion Pedagogica
The future perfect passive indicates that the automation process will be a finished reality by 2040.

---
## Question 3 [D3-D4]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v3`
**Bloom:** Apply
**EJE:** Grammar
**Expected_Success:** 0.65
**Contexto:** Review of health (W03).

### Enunciado
If she hadn't started exercising regularly, she wouldn't feel so energetic today.

### Opciones
- [ ] A) didn't start <!-- feedback: Incorrect. Second conditional. -->
- [x] B) hadn't started <!-- feedback: Correct. Mixed conditional (past action, present result). -->
- [ ] C) wouldn't start <!-- feedback: Incorrect grammar in 'if' clause. -->
- [ ] D) hasn't started <!-- feedback: Incorrect. Present perfect. -->

### Explicacion Pedagogica
The mixed conditional connects a past hypothetical action with its present consequence.

---
## Question 4 [D3-D4]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v4`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Review of media (W04).

### Enunciado
Misinformation is false or inaccurate information that is spread, regardless of intent to deceive.

### Opciones
- [x] A) Misinformation <!-- feedback: Correct. The general term for false info. -->
- [ ] B) Editorial <!-- feedback: Incorrect. An opinion piece. -->
- [ ] C) Fact-check <!-- feedback: Incorrect. The process of verifying. -->
- [ ] D) Slogan <!-- feedback: Incorrect. An advertising phrase. -->

### Explicacion Pedagogica
'Misinformation' is the technical term for false information regardless of whether the intent is malicious.

---
## Question 5 [D5-D6]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v5`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Review of education (W05).

### Enunciado
The text suggests that lifelong learning is essential for professional survival in the modern era.

### Opciones
- [ ] A) optional <!-- feedback: Incorrect. Modern markets make it necessary. -->
- [x] B) essential <!-- feedback: Correct. Essential means absolutely necessary. -->
- [ ] C) redundant <!-- feedback: Incorrect. Unnecessary. -->
- [ ] D) academic <!-- feedback: Incorrect. It refers to learning in all areas of life. -->

### Explicacion Pedagogica
'Essential' captures the necessity described in the text for continuous learning.

---
## Question 6 [D5-D6]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v6`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Review of sustainability (W06).

### Enunciado
Renewable energy comes from natural sources that are constantly replenished.

### Opciones
- [ ] A) Finite <!-- feedback: Incorrect. Finite means limited. -->
- [x] B) Renewable <!-- feedback: Correct. Sources like solar and wind are renewable. -->
- [ ] C) Depleted <!-- feedback: Incorrect. Used up. -->
- [ ] D) Fossil <!-- feedback: Incorrect. Fossil fuels are non-renewable. -->

### Explicacion Pedagogica
'Renewable' is the term for energy sources that do not run out.

---
## Question 7 [D5-D6]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v7`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Review of economy (W07).

### Enunciado
The global economy is being impacted by shifting trade policies and geopolitical tensions.

### Opciones
- [ ] A) is impacting <!-- feedback: Incorrect. Active voice. -->
- [x] B) is being impacted <!-- feedback: Correct. Present continuous passive for an ongoing situation. -->
- [ ] C) impacted <!-- feedback: Incorrect. Past simple. -->
- [ ] D) impacts <!-- feedback: Incorrect. Simple present. -->

### Explicacion Pedagogica
The present continuous passive describes the current and ongoing effect of these factors on the economy.

---
## Question 8 [D5-D6]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v8`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Review of travel (W08).

### Enunciado
Sustainable tourism seeks to minimize the environmental footprint of travelers.

### Opciones
- [ ] A) track <!-- feedback: Incorrect. -->
- [x] B) footprint <!-- feedback: Correct. Footprint refers to the impact left behind. -->
- [ ] C) shadow <!-- feedback: Incorrect. -->
- [ ] D) path <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
'Environmental footprint' is the standard term for the measure of human impact on nature.

---
## Question 9 [D5-D6]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v9`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Review of art (W09).

### Enunciado
Never had I seen such an impressive collection of modern art before visiting this museum.

### Opciones
- [ ] A) I had seen <!-- feedback: Incorrect. After 'Never', we need inversion. -->
- [x] B) had I seen <!-- feedback: Correct. Inversion after the negative adverbial 'Never'. -->
- [ ] C) I saw <!-- feedback: Incorrect. -->
- [ ] D) saw I <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
Inversion (verb before subject) is required when a sentence starts with a negative adverbial like 'Never'.

---
## Question 10 [D5-D6]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v10`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Review of social trends (W10).

### Enunciado
The paradox of modern technology is that it can foster connection while also increasing feelings of isolation.

### Opciones
- [x] A) connection <!-- feedback: Correct. Positive side of technology. -->
- [ ] B) wealth <!-- feedback: Incorrect. Unrelated to the social point. -->
- [ ] C) speed <!-- feedback: Incorrect. While true, connection is the direct contrast to isolation. -->
- [ ] D) safety <!-- feedback: Incorrect. Unrelated to the paradox described. -->

### Explicacion Pedagogica
'Connection' is the social benefit that contrasts with the 'isolation' mentioned in the paradox.

---
## Question 11 [D7-D8]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Review of dream jobs (W11).

### Enunciado
An entrepreneur is someone who starts a new business and takes on financial risk.

### Opciones
- [ ] A) employee <!-- feedback: Incorrect. -->
- [x] B) entrepreneur <!-- feedback: Correct. Starts and manages their own business. -->
- [ ] C) consumer <!-- feedback: Incorrect. -->
- [ ] D) intern <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
'Entrepreneur' is the specific term for someone who launches a business venture.

---
## Question 12 [D7-D8]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v12`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Review of ethics (W12).

### Enunciado
He ought to have confessed the truth before the situation got out of hand.

### Opciones
- [ ] A) ought confess <!-- feedback: Incorrect grammar. -->
- [x] B) ought to have confessed <!-- feedback: Correct. Past moral obligation that wasn't met. -->
- [ ] C) should confess <!-- feedback: Incorrect. Present/future obligation. -->
- [ ] D) must confess <!-- feedback: Incorrect. Present/future. -->

### Explicacion Pedagogica
'Ought to have + past participle' expresses a moral obligation in the past.

---
## Question 13 [D7-D8]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v13`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Review of science (W13).

### Enunciado
Scientific research must be subject to peer review to ensure its validity and accuracy.

### Opciones
- [ ] A) gossip <!-- feedback: Incorrect. -->
- [x] B) peer review <!-- feedback: Correct. Professional evaluation system. -->
- [ ] C) censorship <!-- feedback: Incorrect. -->
- [ ] D) promotion <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
'Peer review' is the critical quality control process in the scientific community.

---
## Question 14 [D7-D8]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v14`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Review of history (W14).

### Enunciado
The movement had been growing for years before it finally achieved its goals.

### Opciones
- [ ] A) was growing <!-- feedback: Incorrect. Past continuous. -->
- [x] B) had been growing <!-- feedback: Correct. Past perfect continuous for duration before a past point. -->
- [ ] C) has been growing <!-- feedback: Incorrect. Present perfect continuous. -->
- [ ] D) grows <!-- feedback: Incorrect. Present simple. -->

### Explicacion Pedagogica
The past perfect continuous emphasizes the length of the struggle leading up to the turning point.

---
## Question 15 [D7-D8]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v15`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Review of psychology (W15).

### Enunciado
Resilience is the capacity to recover quickly from difficulties; toughness.

### Opciones
- [ ] A) Fragility <!-- feedback: Incorrect. Opposite of resilience. -->
- [x] B) Resilience <!-- feedback: Correct. Definition provided. -->
- [ ] C) Apathy <!-- feedback: Incorrect. Lack of interest. -->
- [ ] D) Greed <!-- feedback: Incorrect. Selfish desire. -->

### Explicacion Pedagogica
'Resilience' is the recognized term for psychological and emotional strength in the face of adversity.

---
## Question 16 [D7-D8]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v16`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Review of communication (W16).

### Enunciado
Empathy is the ability to understand and share the feelings of another.

### Opciones
- [ ] A) Sympathy <!-- feedback: Incorrect. Sympathy is feeling for; empathy is feeling with. -->
- [x] B) Empathy <!-- feedback: Correct. Core communication skill. -->
- [ ] C) Apathy <!-- feedback: Incorrect. -->
- [ ] D) Hostility <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
'Empathy' is fundamental for effective and compassionate human communication.

---
## Question 17 [D9-D10]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v17`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Review of urban life (W17).

### Enunciado
The city is getting used to the new sustainable energy regulations.

### Opciones
- [ ] A) used to <!-- feedback: Incorrect. Past habit. -->
- [x] B) is getting used to <!-- feedback: Correct. Process of becoming accustomed. -->
- [ ] C) uses to <!-- feedback: Incorrect grammar. -->
- [ ] D) get used to <!-- feedback: Incorrect grammar. -->

### Explicacion Pedagogica
'Be getting used to' describes the current process of adaptation to a new situation.

---
## Question 18 [D9-D10]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v18`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Review of digital citizenship (W18).

### Enunciado
A digital footprint is the record of your activities on the internet.

### Opciones
- [ ] A) path <!-- feedback: Incorrect. -->
- [x] B) footprint <!-- feedback: Correct. Standard term. -->
- [ ] C) shadow <!-- feedback: Incorrect. -->
- [ ] D) ID <!-- feedback: Incorrect. -->

### Explicacion Pedagogica
'Digital footprint' is the metaphorical term for the data trail left by internet users.

---
## Question 19 [D9-D10]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v19`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Review of literature (W19).

### Enunciado
The author said that his book had been translated into twelve languages.

### Opciones
- [ ] A) has been <!-- feedback: Incorrect. Backshifted in reported speech. -->
- [x] B) had been <!-- feedback: Correct. Backshifted from present perfect to past perfect. -->
- [ ] C) is <!-- feedback: Incorrect. Present tense. -->
- [ ] D) will be <!-- feedback: Incorrect. Future. -->

### Explicacion Pedagogica
In reported speech, we backshift tenses: 'My book has been...' becomes '...said that his book had been...'.

---
## Question 20 [D9-D10]
**ID:** `CL-ING-11-2026-W20-review-p1-001-MASTERY-v20`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Summary of Period 1 themes.

### Enunciado
The most important lesson from Period 1 is the interconnectedness of global issues and personal responsibility.

### Opciones
- [x] A) interconnectedness <!-- feedback: Correct. The state of being connected with each other. -->
- [ ] B) isolation <!-- feedback: Incorrect. Themes showed how everything is linked. -->
- [ ] C) simplicity <!-- feedback: Incorrect. Most themes were complex. -->
- [ ] D) irrelevance <!-- feedback: Incorrect. These issues are highly relevant. -->

### Explicacion Pedagogica
'Interconnectedness' captures the recurring theme of how different global and personal factors affect one another.
