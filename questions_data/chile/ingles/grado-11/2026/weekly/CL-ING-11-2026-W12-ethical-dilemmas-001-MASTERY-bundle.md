---
id: "CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "ethical-dilemmas"
periodo: "weekly"
week: "W12"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Ethical Dilemmas (W12)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v1`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Introduction to ethics.

### Enunciado
An ethical dilemma is a situation in which a difficult choice has to be made between two courses of action, either of which entails transgressing a moral principle.

### Opciones
- [ ] A) solution <!-- feedback: Incorrect. A dilemma is a problem, not a solution. -->
- [x] B) dilemma <!-- feedback: Correct. An ethical dilemma is a choice between two moral imperatives. -->
- [ ] C) certainty <!-- feedback: Incorrect. Dilemmas involve uncertainty and conflict. -->
- [ ] D) agreement <!-- feedback: Incorrect. Dilemmas usually involve internal or external disagreement. -->

### Explicacion Pedagogica
'Ethical dilemma' is the standard term for a complex moral choice.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v2`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing moral philosophy.

### Enunciado
Utilitarianism is a theory that suggests actions are right if they are useful or for the benefit of a majority.

### Opciones
- [ ] A) suggest <!-- feedback: Incorrect. Subject-verb agreement ('theory' is singular). -->
- [x] B) suggests <!-- feedback: Correct. Present simple for a general definition of a theory. -->
- [ ] C) suggesting <!-- feedback: Incorrect. Gerund. -->
- [ ] D) suggested <!-- feedback: Incorrect. Past tense. -->

### Explicacion Pedagogica
The present simple 'suggests' is used to define the core tenet of a philosophical theory.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v3`
**Bloom:** Apply
**EJE:** Grammar
**Expected_Success:** 0.65
**Contexto:** Hypothetical moral choice.

### Enunciado
If you found a wallet full of money on the street, what would you do?

### Opciones
- [ ] A) do you do <!-- feedback: Incorrect. First conditional. -->
- [x] B) would you do <!-- feedback: Correct. Second conditional for a hypothetical scenario. -->
- [ ] C) will you do <!-- feedback: Incorrect. Inconsistent with 'found'. -->
- [ ] D) have you done <!-- feedback: Incorrect. Present perfect. -->

### Explicacion Pedagogica
The second conditional 'if + past simple, would + verb' is used to ask about an imaginary situation.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v4`
**Bloom:** Analyze
**EJE:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Article about whistleblowing.

### Enunciado
A whistleblower is a person who informs on a person or organization regarded as engaging in an unlawful or unethical activity.

### Opciones
- [x] A) whistleblower <!-- feedback: Correct. This is the specific term for someone who exposes wrongdoing. -->
- [ ] B) accomplice <!-- feedback: Incorrect. An accomplice helps in the wrongdoing. -->
- [ ] C) bystander <!-- feedback: Incorrect. A bystander watches but doesn't act. -->
- [ ] D) benefactor <!-- feedback: Incorrect. A benefactor provides financial help. -->

### Explicacion Pedagogica
'Whistleblower' is the correct term for an individual who reveals unethical behavior within an organization.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v5`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on corporate ethics.

### Enunciado
The text implies that prioritizing profit over safety is inherently unethical and can lead to long-term reputational damage.

### Opciones
- [ ] A) beneficial <!-- feedback: Incorrect. It might be profitable but is not 'beneficial' in an ethical context. -->
- [x] B) unethical <!-- feedback: Correct. Not conforming to high moral standards. -->
- [ ] C) legal <!-- feedback: Incorrect. While it might be legal in some cases, the focus is on the moral 'ethics'. -->
- [ ] D) mandatory <!-- feedback: Incorrect. It is never mandatory to be unethical. -->

### Explicacion Pedagogica
'Unethical' is the appropriate adjective for actions that violate moral principles.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v6`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Legal vs. Moral.

### Enunciado
Just because an action is legal does not necessarily mean it is moral.

### Opciones
- [ ] A) illegal <!-- feedback: Incorrect. If it's illegal, it's usually considered wrong anyway. -->
- [x] B) legal <!-- feedback: Correct. The text contrasts law with morality. -->
- [ ] C) public <!-- feedback: Incorrect. Public doesn't contrast with moral here. -->
- [ ] D) expensive <!-- feedback: Incorrect. Price is unrelated to morality. -->

### Explicacion Pedagogica
The distinction between 'legal' (following the law) and 'moral' (doing what is right) is a key concept in ethics.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v7`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Discussing a past ethical failure.

### Enunciado
The company should have disclosed the risks to the public much sooner.

### Opciones
- [ ] A) should disclose <!-- feedback: Incorrect. This refers to the present or future. -->
- [x] B) should have disclosed <!-- feedback: Correct. 'Should have + past participle' for past criticism or regret. -->
- [ ] C) must disclose <!-- feedback: Incorrect. Obligation in the present. -->
- [ ] D) could disclose <!-- feedback: Incorrect. Possibility in the present. -->

### Explicacion Pedagogica
'Should have disclosed' expresses a past obligation that was not met.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v8`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Text on "conflict of interest".

### Enunciado
A conflict of interest occurs when an individual's personal interests interfere with their professional duties.

### Opciones
- [ ] A) resolution <!-- feedback: Incorrect. A resolution solves a conflict. -->
- [x] B) conflict <!-- feedback: Correct. A conflict of interest is the formal term. -->
- [ ] C) alignment <!-- feedback: Incorrect. Alignment is the opposite of conflict. -->
- [ ] D) benefit <!-- feedback: Incorrect. While there may be a personal benefit, the situation is a conflict. -->

### Explicacion Pedagogica
'Conflict of interest' is the specific term for the ethical situation described.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v9`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing an ongoing ethical debate.

### Enunciado
The ethics of genetic engineering are being debated by scientists and philosophers alike.

### Opciones
- [ ] A) are debating <!-- feedback: Incorrect. Active voice. -->
- [x] B) are being debated <!-- feedback: Correct. Present continuous passive for an ongoing discussion. -->
- [ ] C) have debated <!-- feedback: Incorrect. Active voice. -->
- [ ] D) debate <!-- feedback: Incorrect. Present simple. -->

### Explicacion Pedagogica
The present continuous passive 'are being debated' describes the current, ongoing nature of the ethical discussion.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v10`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of "consequentialism".

### Enunciado
Consequentialism is the class of normative ethical theories holding that the consequences of one's conduct are the ultimate basis for any judgment about the rightness or wrongness of that conduct.

### Opciones
- [ ] A) intention <!-- feedback: Incorrect. Consequentialism focuses on results, not intentions. -->
- [x] B) consequences <!-- feedback: Correct. The name of the theory itself gives the answer. -->
- [ ] C) tradition <!-- feedback: Incorrect. Focuses on results. -->
- [ ] D) legality <!-- feedback: Incorrect. Focuses on moral results. -->

### Explicacion Pedagogica
'Consequences' are the primary focus of consequentialist ethical frameworks.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v11`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Integrity in the workplace.

### Enunciado
Integrity is the quality of being honest and having strong moral principles; moral uprightness.

### Opciones
- [ ] A) Greed <!-- feedback: Incorrect. Greed is a vice. -->
- [x] B) Integrity <!-- feedback: Correct. Integrity is a fundamental ethical value. -->
- [ ] C) Ambiguity <!-- feedback: Incorrect. Uncertainty. -->
- [ ] D) Deception <!-- feedback: Incorrect. Being dishonest. -->

### Explicacion Pedagogica
'Integrity' is a core virtue in both personal and professional ethics.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v12`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing a difficult decision.

### Enunciado
Choosing between two equally important values can be agonizing.

### Opciones
- [ ] A) agonize <!-- feedback: Incorrect. Verb form. -->
- [x] B) agonizing <!-- feedback: Correct. Adjective describing the effect of the choice. -->
- [ ] C) agony <!-- feedback: Incorrect. Noun. -->
- [ ] D) agonized <!-- feedback: Incorrect. Adjective describing a person's state. -->

### Explicacion Pedagogica
The adjective 'agonizing' is used to describe a situation that causes great mental pain or difficulty.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v13`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Discussing social justice.

### Enunciado
Equality of opportunity is the idea that everyone should have a fair chance to succeed, regardless of their background.

### Opciones
- [ ] A) wealth <!-- feedback: Incorrect. Focuses on money. -->
- [x] B) opportunity <!-- feedback: Correct. This is the specific ethical/social concept. -->
- [ ] C) outcomes <!-- feedback: Incorrect. Equality of outcomes is a different, more controversial concept. -->
- [ ] D) intelligence <!-- feedback: Incorrect. Not the standard phrase. -->

### Explicacion Pedagogica
'Equality of opportunity' is a central concept in modern ethical and political debate.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v14`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Conditional about personal values.

### Enunciado
If I didn't value honesty so much, I would have lied to protect myself.

### Opciones
- [ ] A) hadn't valued <!-- feedback: Incorrect. This would be a third conditional (past state). -->
- [x] B) didn't value <!-- feedback: Correct. Mixed conditional (present state 'value', past result 'would have lied'). -->
- [ ] C) don't value <!-- feedback: Incorrect. Present simple. -->
- [ ] D) wouldn't value <!-- feedback: Incorrect. 'Would' not in 'if' clause. -->

### Explicacion Pedagogica
The mixed conditional (if + past simple, would + have + past participle) connects a permanent present state or value with a past hypothetical action.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v15`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion of an essay on ethics.

### Enunciado
The author concludes that we must cultivate our moral compass through reflection and experience.

### Opciones
- [ ] A) budget <!-- feedback: Incorrect. Financial plan. -->
- [x] B) moral compass <!-- feedback: Correct. Metaphor for one's inner sense of right and wrong. -->
- [ ] C) reputation <!-- feedback: Incorrect. How others see you. -->
- [ ] D) career <!-- feedback: Incorrect. Professional life. -->

### Explicacion Pedagogica
'Moral compass' is a common metaphorical term for a person's internal sense of ethical guidance.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v16`
**Bloom:** Remember
**EJE:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Definition of empathy.

### Enunciado
Empathy is the ability to understand and share the feelings of another.

### Opciones
- [ ] A) Apathy <!-- feedback: Incorrect. Lack of interest. -->
- [x] B) Empathy <!-- feedback: Correct. Fundamental for ethical behavior. -->
- [ ] C) Antipathy <!-- feedback: Incorrect. Deep-seated feeling of dislike. -->
- [ ] D) Simplicity <!-- feedback: Incorrect. Being simple. -->

### Explicacion Pedagogica
'Empathy' is a key emotional skill required for ethical reasoning and social connection.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v17`
**Bloom:** Understand
**EJE:** Grammar
**Expected_Success:** 0.75
**Contexto:** Expressing a moral obligation.

### Enunciado
We ought to treat others with the same respect that we expect for ourselves.

### Opciones
- [ ] A) must to <!-- feedback: Incorrect. 'Must' doesn't take 'to'. -->
- [x] B) ought to <!-- feedback: Correct. 'Ought to' expresses moral obligation. -->
- [ ] C) would to <!-- feedback: Incorrect. 'Would' doesn't take 'to'. -->
- [ ] D) can to <!-- feedback: Incorrect. 'Can' doesn't take 'to'. -->

### Explicacion Pedagogica
'Ought to' is a modal verb used to express a moral duty or recommendation.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v18`
**Bloom:** Apply
**EJE:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Text about human rights.

### Enunciado
Fundamental human rights are inalienable, meaning they cannot be taken away from any person.

### Opciones
- [ ] A) flexible <!-- feedback: Incorrect. Rights should not be flexible in this way. -->
- [x] B) inalienable <!-- feedback: Correct. Inalienable means unable to be taken away or given away by the possessor. -->
- [ ] C) optional <!-- feedback: Incorrect. Rights are not optional. -->
- [ ] D) temporary <!-- feedback: Incorrect. Rights should be permanent. -->

### Explicacion Pedagogica
'Inalienable' is the formal term used in human rights documents to describe the permanent nature of fundamental rights.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v19`
**Bloom:** Analyze
**EJE:** Grammar
**Expected_Success:** 0.70
**Contexto:** Reporting a philosophical statement.

### Enunciado
The philosopher argued that the ends did not always justify the means.

### Opciones
- [ ] A) don't <!-- feedback: Incorrect. Backshifted in reported speech. -->
- [x] B) did not <!-- feedback: Correct. Backshifted from 'do not' to 'did not'. -->
- [ ] C) have not <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) will not <!-- feedback: Incorrect. Future. -->

### Explicacion Pedagogica
In reported speech, we backshift tenses: 'The ends do not justify...' becomes '...argued that the ends did not justify...'.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W12-ethical-dilemmas-001-MASTERY-v20`
**Bloom:** Evaluate
**EJE:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Final thought on responsibility.

### Enunciado
Accountability is an assurance that an individual or organization will be evaluated on their performance or behavior.

### Opciones
- [x] A) Accountability <!-- feedback: Correct. Accountability is the obligation of an individual or organization to account for its activities. -->
- [ ] B) Anonymity <!-- feedback: Incorrect. Being unknown. -->
- [ ] C) Impunity <!-- feedback: Incorrect. Exemption from punishment or loss. -->
- [ ] D) Indifference <!-- feedback: Incorrect. Lack of interest. -->

### Explicacion Pedagogica
'Accountability' is a central ethical concept involving responsibility and the expectation of evaluation.
