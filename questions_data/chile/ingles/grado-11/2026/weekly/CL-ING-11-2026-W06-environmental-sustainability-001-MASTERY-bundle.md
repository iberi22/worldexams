---
id: "CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-bundle"
country: "chile"
grado: 11
asignatura: "Ingles"
tema: "environmental-sustainability"
periodo: "weekly"
week: "W06"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Bases Curriculares Chile + CEFR B2"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ingles: Environmental Sustainability (W06)
**20 questions | Ingles | Grade 11**

---
## Question 1 [D3]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v1`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Discussion on conservation.

### Enunciado
The preservation of natural resources for future generations is the core principle of sustainability.

### Opciones
- [ ] A) exploitation <!-- feedback: Incorrect. Exploitation means using something for benefit. -->
- [x] B) preservation <!-- feedback: Correct. Preservation is the act of keeping something as it is. -->
- [ ] C) destruction <!-- feedback: Incorrect. Destruction is the opposite. -->
- [ ] D) consumption <!-- feedback: Incorrect. Consumption means using something up. -->

### Explicacion Pedagogica
'Preservation' refers to the careful management and protection of resources to ensure they last.

---
## Question 2 [D4]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v2`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Describing an ongoing environmental process.

### Enunciado
Species are disappearing at an alarming rate due to habitat loss and climate change.

### Opciones
- [ ] A) disappear <!-- feedback: Incorrect. Present simple. -->
- [x] B) are disappearing <!-- feedback: Correct. Present continuous for an ongoing trend. -->
- [ ] C) have disappeared <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) will disappear <!-- feedback: Incorrect. Simple future. -->

### Explicacion Pedagogica
The present continuous 'are disappearing' emphasizes that this is a current and ongoing phenomenon.

---
## Question 3 [D5]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v3`
**Bloom:** Apply
**ICFES:** Grammar
**Expected_Success:** 0.65
**Contexto:** Talking about past environmental damage.

### Enunciado
If we had acted sooner, we could have prevented much of the damage to the ozone layer.

### Opciones
- [ ] A) acted <!-- feedback: Incorrect. Second conditional. -->
- [x] B) had acted <!-- feedback: Correct. Third conditional for hypothetical past situations. -->
- [ ] C) act <!-- feedback: Incorrect. Present tense. -->
- [ ] D) would act <!-- feedback: Incorrect. 'Would' is not used in the 'if' clause. -->

### Explicacion Pedagogica
The third conditional (if + past perfect) describes an alternative past that did not happen.

---
## Question 4 [D4]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v4`
**Bloom:** Analyze
**ICFES:** Reading Comprehension
**Expected_Success:** 0.70
**Contexto:** Text about "greenwashing".

### Enunciado
Greenwashing occurs when a company spends more time on marketing itself as environmentally friendly than on actually minimizing its impact.

### Opciones
- [x] A) greenwashing <!-- feedback: Correct. Greenwashing is deceptive environmental marketing. -->
- [ ] B) recycling <!-- feedback: Incorrect. Recycling is a real environmental action. -->
- [ ] C) reforestation <!-- feedback: Incorrect. Reforestation is planting trees. -->
- [ ] D) composting <!-- feedback: Incorrect. Composting is recycling organic waste. -->

### Explicacion Pedagogica
'Greenwashing' is the specific term for misleading consumers regarding the environmental practices of a company.

---
## Question 5 [D6]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v5`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.60
**Contexto:** Discussion on the "plastic crisis".

### Enunciado
The primary challenge in managing plastic waste is its persistence in the environment.

### Opciones
- [ ] A) solubility <!-- feedback: Incorrect. Solubility refers to dissolving. -->
- [x] B) persistence <!-- feedback: Correct. Persistence refers to the ability to remain for a long time. -->
- [ ] C) variety <!-- feedback: Incorrect. Variety refers to diversity. -->
- [ ] D) weight <!-- feedback: Incorrect. Weight is not the main problem. -->

### Explicacion Pedagogica
'Persistence' correctly describes the quality of plastic remaining in the environment without breaking down.

---
## Question 6 [D3]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v6`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.80
**Contexto:** Describing renewable energy.

### Enunciado
Solar panels convert sunlight into electricity, providing a clean source of power.

### Opciones
- [ ] A) fuel <!-- feedback: Incorrect. Fuel usually refers to combustibles. -->
- [x] B) electricity <!-- feedback: Correct. Solar panels produce electrical energy. -->
- [ ] C) heat <!-- feedback: Incorrect. While related, electricity is the main discussion. -->
- [ ] D) gas <!-- feedback: Incorrect. Gas is a different source. -->

### Explicacion Pedagogica
The primary function of photovoltaic solar panels is to generate electricity.

---
## Question 7 [D4]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v7`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Recommendation for water conservation.

### Enunciado
It is essential that we reduce our water consumption to prepare for future droughts.

### Opciones
- [x] A) reduce <!-- feedback: Correct. Subjunctive form after 'It is essential that...'. -->
- [ ] B) reduces <!-- feedback: Incorrect. Third person singular. -->
- [ ] C) to reduce <!-- feedback: Incorrect. Infinitive doesn't fit here. -->
- [ ] D) reducing <!-- feedback: Incorrect. Gerund doesn't fit here. -->

### Explicacion Pedagogica
'Reduce' is the base form used in the subjunctive mood to express necessity.

---
## Question 8 [D5]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v8`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Text on urban planning.

### Enunciado
Urban sprawl often leads to the destruction of natural habitats and increased reliance on automobiles.

### Opciones
- [ ] A) rejuvenation <!-- feedback: Incorrect. Rejuvenation means making something young again. -->
- [x] B) sprawl <!-- feedback: Correct. Urban sprawl is the expansion of cities. -->
- [ ] C) density <!-- feedback: Incorrect. Density refers to compact building. -->
- [ ] D) forestry <!-- feedback: Incorrect. Forestry is about managing forests. -->

### Explicacion Pedagogica
'Urban sprawl' describes the outward growth of cities into rural areas.

---
## Question 9 [D4]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v9`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Describing a past habit.

### Enunciado
Years ago, people used to throw away much more waste than we do now.

### Opciones
- [ ] A) use to <!-- feedback: Incorrect. Needs 'used' for past habits. -->
- [x] B) used to <!-- feedback: Correct. 'Used to' describes past habits. -->
- [ ] C) would <!-- feedback: Incorrect. 'Used to' is better for states. -->
- [ ] D) are used to <!-- feedback: Incorrect. This means 'accustomed to'. -->

### Explicacion Pedagogica
'Used to' is the appropriate structure for describing a past habit or state that has changed.

---
## Question 10 [D6]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v10`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Critique of the "throwaway culture".

### Enunciado
The "throwaway culture" refers to a society that prioritizes convenience over the environmental cost.

### Opciones
- [ ] A) economy <!-- feedback: Incorrect. While related, convenience is the focus. -->
- [x] B) convenience <!-- feedback: Correct. Desired for easy, one-time use. -->
- [ ] C) tradition <!-- feedback: Incorrect. Modern phenomenon. -->
- [ ] D) safety <!-- feedback: Incorrect. General culture is about ease. -->

### Explicacion Pedagogica
'Convenience' is the primary driver of the consumption of single-use products.

---
## Question 11 [D3]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v11`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Environmental legislation.

### Enunciado
An endangered species is a type of organism which is at risk of extinction.

### Opciones
- [ ] A) invasive <!-- feedback: Incorrect. Invasive species cause harm. -->
- [ ] B) extinct <!-- feedback: Incorrect. Extinct species are gone. -->
- [x] C) endangered <!-- feedback: Correct. Endangered means at risk. -->
- [ ] D) resilient <!-- feedback: Incorrect. Resilient means able to recover. -->

### Explicacion Pedagogica
'Endangered' is the formal status for species close to being lost.

---
## Question 12 [D4]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v12`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Passive voice in an ecology report.

### Enunciado
Vast areas of the rainforest are being cleared to make way for agriculture.

### Opciones
- [ ] A) are clearing <!-- feedback: Incorrect. Active voice. -->
- [x] B) are being cleared <!-- feedback: Correct. Present continuous passive for ongoing process. -->
- [ ] C) cleared <!-- feedback: Incorrect. Past simple. -->
- [ ] D) have cleared <!-- feedback: Incorrect. Active voice. -->

### Explicacion Pedagogica
The present continuous passive 'are being cleared' describes the current destruction.

---
## Question 13 [D5]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v13`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.65
**Contexto:** Discussing climate mitigation.

### Enunciado
Reforestation projects help to sequester carbon from the atmosphere.

### Opciones
- [x] A) sequester <!-- feedback: Correct. To sequester means to store carbon. -->
- [ ] B) release <!-- feedback: Incorrect. Releasing carbon is bad. -->
- [ ] C) ignore <!-- feedback: Incorrect. Trees don't ignore carbon. -->
- [ ] D) manufacture <!-- feedback: Incorrect. Trees absorb it. -->

### Explicacion Pedagogica
'Sequester' is the technical term for the absorption and storage of carbon dioxide.

---
## Question 14 [D4]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v14`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Conditional about policy change.

### Enunciado
If the government imposed higher taxes on carbon, companies would likely invest more in clean energy.

### Opciones
- [ ] A) imposes <!-- feedback: Incorrect. First conditional. -->
- [x] B) imposed <!-- feedback: Correct. Second conditional for hypothetical policy. -->
- [ ] C) had imposed <!-- feedback: Incorrect. Third conditional. -->
- [ ] D) would impose <!-- feedback: Incorrect. 'Would' not used in 'if' clause. -->

### Explicacion Pedagogica
The second conditional is used to describe a hypothetical scenario and its likely outcome.

---
## Question 15 [D6]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v15`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.50
**Contexto:** Conclusion of a study on marine biology.

### Enunciado
The study concludes that unless we address acidification, coral reefs will face collapse.

### Opciones
- [ ] A) if <!-- feedback: Incorrect. Collapse happens if we don't fix it. -->
- [x] B) unless <!-- feedback: Correct. 'Unless' means 'if not'. -->
- [ ] C) although <!-- feedback: Incorrect. Expresses contrast. -->
- [ ] D) provided <!-- feedback: Incorrect. 'Provided' means 'if'. -->

### Explicacion Pedagogica
'Unless' sets the condition that collapse is inevitable if acidification is not addressed.

---
## Question 16 [D3]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v16`
**Bloom:** Remember
**ICFES:** Vocabulary
**Expected_Success:** 0.85
**Contexto:** Waste management.

### Enunciado
Biodegradable materials can be broken down by bacteria.

### Opciones
- [ ] A) Synthetic <!-- feedback: Incorrect. Often not biodegradable. -->
- [x] B) Biodegradable <!-- feedback: Correct. Capable of being decomposed. -->
- [ ] C) Permanent <!-- feedback: Incorrect. Lasts forever. -->
- [ ] D) Hazardous <!-- feedback: Incorrect. Means dangerous. -->

### Explicacion Pedagogica
'Biodegradable' is the property of substances that decompose naturally.

---
## Question 17 [D4]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v17`
**Bloom:** Understand
**ICFES:** Grammar
**Expected_Success:** 0.75
**Contexto:** Speculating about the arctic.

### Enunciado
By 2040, the Arctic Ocean might be ice-free during the summer.

### Opciones
- [x] A) might be <!-- feedback: Correct. Expresses possibility. -->
- [ ] B) is <!-- feedback: Incorrect. Present simple. -->
- [ ] C) was <!-- feedback: Incorrect. Past tense. -->
- [ ] D) will have been <!-- feedback: Incorrect. Future perfect. -->

### Explicacion Pedagogica
'Might' is appropriate for expressing a scientific projection with uncertainty.

---
## Question 18 [D5]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v18`
**Bloom:** Apply
**ICFES:** Vocabulary
**Expected_Success:** 0.60
**Contexto:** Text about the "carbon cycle".

### Enunciado
The burning of fossil fuels releases large amounts of carbon dioxide.

### Opciones
- [ ] A) absorbs <!-- feedback: Incorrect. Burning releases. -->
- [x] B) releases <!-- feedback: Correct. Set free or allow to escape. -->
- [ ] C) creates <!-- feedback: Incorrect. Carbon was already there. -->
- [ ] D) destroys <!-- feedback: Incorrect. Carbon cannot be destroyed. -->

### Explicacion Pedagogica
'Releases' correctly describes how carbon stored in fossil fuels is emitted.

---
## Question 19 [D4]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v19`
**Bloom:** Analyze
**ICFES:** Grammar
**Expected_Success:** 0.70
**Contexto:** Reporting a scientist's warning.

### Enunciado
The scientist warned that the effects of climate change were becoming irreversible.

### Opciones
- [ ] A) are becoming <!-- feedback: Incorrect. Doesn't match 'warned'. -->
- [x] B) were becoming <!-- feedback: Correct. Backshifting from 'are' to 'were'. -->
- [ ] C) have become <!-- feedback: Incorrect. Present perfect. -->
- [ ] D) will become <!-- feedback: Incorrect. Future tense. -->

### Explicacion Pedagogica
In reported speech, we backshift tenses: 'are becoming' becomes 'were becoming'.

---
## Question 20 [D6]
**ID:** `CL-ING-11-2026-W06-environmental-sustainability-001-MASTERY-v20`
**Bloom:** Evaluate
**ICFES:** Reading Comprehension
**Expected_Success:** 0.55
**Contexto:** Final thought on environmental ethics.

### Enunciado
Sustainability requires a fundamental shift in how we perceive our relationship with nature.

### Opciones
- [ ] A) ignore <!-- feedback: Incorrect. Won't lead to sustainability. -->
- [ ] B) exploit <!-- feedback: Incorrect. Opposite of sustainability. -->
- [x] C) perceive <!-- feedback: Correct. Conscious of something. -->
- [ ] D) destroy <!-- feedback: Incorrect. Not part of sustainability. -->

### Explicacion Pedagogica
'Perceive' refers to the mental perspective that must change.
