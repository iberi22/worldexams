---
id: "BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle"
country: "bolivia"
grado: 11
asignatura: "ciencias-naturales"
tema: "tema-w04"
periodo: "weekly"
week: "W04"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Ministerio de Educacion del Estado Plurinacional de Bolivia (MINEDU)"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

## Question 1 [D3]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v1
**Bloom:** Remember
**EJE:** Genética Molecular
**Expected_Success:** 0.85
**Contexto:** En La Paz, el estudiante Grover del Colegio Nacional Ayacucho analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar proceso mediante el cual la célula sintetiza una copia fiel de su genoma completo antes de dividirse, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) La replicación del adn
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a proceso mediante el cual la célula sintetiza una copia fiel de su genoma completo antes de dividirse. -->
- [ ] B) La transcripción de arn
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) La traducción de proteínas
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) La splicing de intrones
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es La replicación del adn. Científicamente, esto se explica por la duplicación semiconservativa catalizada por ADN polimerasas durante la fase S celular. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 2 [D3]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v2
**Bloom:** Remember
**EJE:** Genética Molecular
**Expected_Success:** 0.85
**Contexto:** En Cochabamba, el estudiante Roxana del Unidad Educativa Don Bosco analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar cada doble hélice hija conserva una cadena molde original y una cadena recién sintetizada, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Replicación semiconservativa
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a cada doble hélice hija conserva una cadena molde original y una cadena recién sintetizada. -->
- [ ] B) Replicación conservativa
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Replicación dispersiva
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Replicación fragmentaria
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Replicación semiconservativa. Científicamente, esto se explica por el mecanismo de replicación demostrado por los experimentos de Meselson y Stahl. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 3 [D4]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v3
**Bloom:** Understand
**EJE:** Genética Molecular
**Expected_Success:** 0.80
**Contexto:** En Santa Cruz, el estudiante Jhonny del Colegio Bolívar analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar enzima encargada de añadir nucleótidos de forma complementaria a la cadena molde de ADN en dirección 5' a 3', ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) La adn polimerasa
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a enzima encargada de añadir nucleótidos de forma complementaria a la cadena molde de ADN en dirección 5' a 3'. -->
- [ ] B) La arn polimerasa
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) La helicasa
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) La ligasa
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es La adn polimerasa. Científicamente, esto se explica por la actividad catalítica con función de corrección de pruebas de exonucleasa. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 4 [D4]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v4
**Bloom:** Understand
**EJE:** Genética Molecular
**Expected_Success:** 0.80
**Contexto:** En El Alto, el estudiante Marisol del Colegio San Simón analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar enzima que rompe los puentes de hidrógeno entre las bases nitrogenadas para abrir la doble hélice de ADN, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) La helicasa
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a enzima que rompe los puentes de hidrógeno entre las bases nitrogenadas para abrir la doble hélice de ADN. -->
- [ ] B) La ligasa
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) La topoisomerasa
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) La primasa
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es La helicasa. Científicamente, esto se explica por la generación de la horquilla de replicación activa mediante consumo de ATP. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 5 [D5]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v5
**Bloom:** Understand
**EJE:** Genética Molecular
**Expected_Success:** 0.75
**Contexto:** En Sucre, el estudiante Walter del Colegio Nacional Junín analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar proceso en el cual la ARN polimerasa sintetiza una molécula de ARN mensajero a partir de un molde de ADN, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) La transcripción
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a proceso en el cual la ARN polimerasa sintetiza una molécula de ARN mensajero a partir de un molde de ADN. -->
- [ ] B) La traducción
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) La replicación
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) La maduración de arn
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es La transcripción. Científicamente, esto se explica por la transferencia de la información genética del núcleo hacia el citoplasma eucariota. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 6 [D5]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v6
**Bloom:** Understand
**EJE:** Genética Molecular
**Expected_Success:** 0.75
**Contexto:** En Potosí, el estudiante Ronald del Unidad Educativa Juan Pablo II analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar proceso en el que se decodifica el ARNm en los ribosomas para ensamblar una secuencia específica de aminoácidos, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) La traducción
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a proceso en el que se decodifica el ARNm en los ribosomas para ensamblar una secuencia específica de aminoácidos. -->
- [ ] B) La transcripción
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) El splicing
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) La replicación
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es La traducción. Científicamente, esto se explica por el acoplamiento de codones y anticodones mediado por ARNt aminoacilados. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 7 [D5]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v7
**Bloom:** Understand
**EJE:** Genética Molecular
**Expected_Success:** 0.75
**Contexto:** En Oruro, el estudiante Limbert del Colegio San Calixto analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar propiedad del código genético donde múltiples codones diferentes pueden codificar para un mismo aminoácido, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) El código genético es redundante
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a propiedad del código genético donde múltiples codones diferentes pueden codificar para un mismo aminoácido. -->
- [ ] B) El código genético es ambiguo
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) El código genético es solapado
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) El código genético es variable
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es El código genético es redundante. Científicamente, esto se explica por la existencia de 64 codones posibles para sintetizar solo 20 aminoácidos biológicos standard. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 8 [D6]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v8
**Bloom:** Apply
**EJE:** Genética Molecular
**Expected_Success:** 0.70
**Contexto:** En Tarija, el estudiante Shirley del Colegio Nacional Pichincha analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar triplete de nucleótidos consecutivos en el ARNm que especifica un aminoácido concreto o señal de parada, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Un codón
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a triplete de nucleótidos consecutivos en el ARNm que especifica un aminoácido concreto o señal de parada. -->
- [ ] B) Un anticodón
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Un cistrón
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Un nucleótido
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Un codón. Científicamente, esto se explica por la unidad básica de traducción leída de forma sucesiva y no solapada por el ribosoma. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 9 [D6]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v9
**Bloom:** Apply
**EJE:** Genética Molecular
**Expected_Success:** 0.70
**Contexto:** En Trinidad, el estudiante Boris del Colegio Antonio Vaca Díez analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar triplete de nucleótidos complementario presente en el asa del ARNt que reconoce al codón del ARNm, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Un anticodón
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a triplete de nucleótidos complementario presente en el asa del ARNt que reconoce al codón del ARNm. -->
- [ ] B) Un codón
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Un iniciador
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Un terminador
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Un anticodón. Científicamente, esto se explica por el emparejamiento específico mediante puentes de hidrógeno antiparalelos en el ribosoma. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 10 [D6]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v10
**Bloom:** Apply
**EJE:** Genética Molecular
**Expected_Success:** 0.70
**Contexto:** En Cobija, el estudiante Franz del Colegio Nacional Florida analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar cambio de nucleótido que altera el codón pero sigue codificando para el mismo aminoácido sin variar la proteína, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Mutación silenciosa
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a cambio de nucleótido que altera el codón pero sigue codificando para el mismo aminoácido sin variar la proteína. -->
- [ ] B) Mutación de cambio de sentido
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Mutación sin sentido
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Mutación de pauta de lectura
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Mutación silenciosa. Científicamente, esto se explica por la protección termodinámica y evolutiva debida a la redundancia del código genético. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 11 [D7]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v11
**Bloom:** Analyze
**EJE:** Genética Molecular
**Expected_Success:** 0.65
**Contexto:** En Tupiza, el estudiante Nancy del Colegio Nacional Ayacucho analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar cambio de base que convierte un codón de aminoácido en un codón de parada prematuro, truncando la proteína, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Mutación sin sentido (nonsense)
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a cambio de base que convierte un codón de aminoácido en un codón de parada prematuro, truncando la proteína. -->
- [ ] B) Mutación silenciosa
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Mutación de cambio de sentido
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Mutación de desfase
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Mutación sin sentido (nonsense). Científicamente, esto se explica por la producción de polipéptidos incompletos y generalmente disfuncionales en la célula. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 12 [D7]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v12
**Bloom:** Analyze
**EJE:** Genética Molecular
**Expected_Success:** 0.65
**Contexto:** En Uyuni, el estudiante Edwin del Unidad Educativa Don Bosco analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar proceso post-transcripcional donde se eliminan los intrones no codificantes y se empalman los exones codificantes, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Splicing de intrones
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a proceso post-transcripcional donde se eliminan los intrones no codificantes y se empalman los exones codificantes. -->
- [ ] B) Caperuza 5' (5-cap)
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Cola de poli-a 3'
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Metilación de histonas
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Splicing de intrones. Científicamente, esto se explica por la maduración del pre-ARNm catalizada por el complejo espliceosoma en el núcleo eucariota. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 13 [D7]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v13
**Bloom:** Analyze
**EJE:** Genética Molecular
**Expected_Success:** 0.65
**Contexto:** En Viacha, el estudiante Marcelo del Colegio Bolívar analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar mecanismo por el cual un solo gen puede producir múltiples isoformas de proteínas diferentes empalmando exones, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Splicing alternativo
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a mecanismo por el cual un solo gen puede producir múltiples isoformas de proteínas diferentes empalmando exones. -->
- [ ] B) Transcripción regulada
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Duplicación génica
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Recombinación homóloga
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Splicing alternativo. Científicamente, esto se explica por la notable diversidad proteica eucariota sin requerir un número equivalente de genes. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 14 [D8]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v14
**Bloom:** Apply
**EJE:** Genética Molecular
**Expected_Success:** 0.60
**Contexto:** En Quillacollo, el estudiante Freddy del Colegio San Simón analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar unidad genética funcional procariota que agrupa genes estructurales regulados bajo un solo promotor, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Un operón
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a unidad genética funcional procariota que agrupa genes estructurales regulados bajo un solo promotor. -->
- [ ] B) Un promotor
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Un potenciador
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Un intrón
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Un operón. Científicamente, esto se explica por la organización y regulación coordinada de genes metabólicos en bacterias. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 15 [D8]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v15
**Bloom:** Apply
**EJE:** Genética Molecular
**Expected_Success:** 0.60
**Contexto:** En Sacaba, el estudiante René del Colegio Nacional Junín analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar sustancia física o química que altera la estructura del ADN aumentando la tasa de mutación celular, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Agente mutagénico
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a sustancia física o química que altera la estructura del ADN aumentando la tasa de mutación celular. -->
- [ ] B) Agente oncogénico
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Agente catalítico
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Agente patógeno
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Agente mutagénico. Científicamente, esto se explica por la inducción de daños como dímeros de timina por radiación UV o agentes alquilantes químicos. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 16 [D8]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v16
**Bloom:** Apply
**EJE:** Genética Molecular
**Expected_Success:** 0.60
**Contexto:** En Montero, el estudiante Lidia del Unidad Educativa Juan Pablo II analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar mecanismo celular que corta y reemplaza fragmentos de ADN dañados por radiación UV o químicos, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Reparación por escisión de nucleótidos
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a mecanismo celular que corta y reemplaza fragmentos de ADN dañados por radiación UV o químicos. -->
- [ ] B) Reparación homóloga
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Fotorreactivación enzimática
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Reparación sos
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Reparación por escisión de nucleótidos. Científicamente, esto se explica por el mantenimiento de la integridad del genoma mediante endonucleasas, polimerasas y ligasas. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 17 [D9]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v17
**Bloom:** Analyze
**EJE:** Genética Molecular
**Expected_Success:** 0.50
**Contexto:** En Riberalta, el estudiante Edwin del Colegio San Calixto analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar técnica biotecnológica para amplificar in vitro millones de copias de un segmento específico de ADN, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) La reacción en cadena de la polimerasa (pcr)
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a técnica biotecnológica para amplificar in vitro millones de copias de un segmento específico de ADN. -->
- [ ] B) La electroforesis en gel
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) La clonación molecular
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) La hibridación southern
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es La reacción en cadena de la polimerasa (pcr). Científicamente, esto se explica por la desnaturalización, hibridación y extensión de cadenas usando polimerasas termoestables (Taq). Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 18 [D9]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v18
**Bloom:** Analyze
**EJE:** Genética Molecular
**Expected_Success:** 0.50
**Contexto:** En Warnes, el estudiante Marcelo del Colegio Nacional Pichincha analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar técnica que separa fragmentos de ADN según su peso molecular y tamaño aplicando un campo eléctrico, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Electroforesis en gel
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a técnica que separa fragmentos de ADN según su peso molecular y tamaño aplicando un campo eléctrico. -->
- [ ] B) Secuenciación sanger
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Centrifugación diferencial
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Espectrometría de masas
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Electroforesis en gel. Científicamente, esto se explica por la migración de las moléculas de ADN cargadas negativamente hacia el polo positivo (ánodo). Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 19 [D10]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v19
**Bloom:** Evaluate
**EJE:** Genética Molecular
**Expected_Success:** 0.45
**Contexto:** En Colcapirhua, el estudiante Lidia del Colegio Antonio Vaca Díez analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar representación visual organizada de los cromosomas de una célula ordenados por forma, tamaño y número, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) El cariotipo
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a representación visual organizada de los cromosomas de una célula ordenados por forma, tamaño y número. -->
- [ ] B) El genograma
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) El fenograma
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) El pedigrí molecular
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es El cariotipo. Científicamente, esto se explica por el diagnóstico clínico de anomalías cromosómicas numéricas como trisomías o monosomías. Lo cual es fundamental para comprender genética molecular y expresión génica.

---

## Question 20 [D10]
**ID:** BO-CIE-11-2026-W04-tema-w04-001-MASTERY-bundle-v20
**Bloom:** Evaluate
**EJE:** Genética Molecular
**Expected_Success:** 0.45
**Contexto:** En Yacuiba, el estudiante Nancy del Colegio Nacional Florida analiza un problema sobre genética molecular y expresión génica.

### Enunciado
Al realizar experimentos sobre genética molecular y expresión génica y estudiar mutación genómica caracterizada por la ganancia o pérdida de cromosomas individuales en el genoma celular, ¿cuál es la opción que identifica correctamente el concepto o proceso estudiado?

### Opciones
- [x] A) Aneuploidía
  <!-- feedback: Excelente. Esta es la respuesta correcta ya que corresponde precisamente a mutación genómica caracterizada por la ganancia o pérdida de cromosomas individuales en el genoma celular. -->
- [ ] B) Poliploidía
  <!-- feedback: Incorrecto. Este concepto hace referencia a un proceso o estructura biológica diferente. -->
- [ ] C) Euploidía
  <!-- feedback: Incorrecto. No corresponde con la descripción dada. Revisa la teoría de genética molecular y expresión génica. -->
- [ ] D) Translocación recíproca
  <!-- feedback: Incorrecto. Es un distractor conceptual incorrecto para este caso de estudio. -->

### Explicacion Pedagogica
La respuesta correcta es Aneuploidía. Científicamente, esto se explica por el fallo de no disyunción cromosómica durante la anafase de la división meiótica. Lo cual es fundamental para comprender genética molecular y expresión génica.

---
