---
id: "ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY"
country: "southafrica"
exam: "NSC Matric — Life Sciences (CAPS)"
grado: 12
asignatura: "Life Sciences"
tema: "photosynthesis-respiration"
semana: 9
periodo: 4
bundle_index: 1
protocol_version: "5.2"
year: 2026
bundle_size: 20
alignment: "CAPS Grade 12 Life Sciences (DBE 2026)"
---

# MASTERY Bundle — Photosynthesis & Cellular Respiration
**Difficulty: D3-D10 | 20 Questions | Life Sciences — NSC Matric**

---

## Question 1 — D3
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v1`
**Bloom:** Evaluate

**Context:** Climate change in a Durban class.

**Stem:** The greenhouse effect is primarily caused by:

- [x] **A)** Increased CO2 and methane levels
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** The hole in the ozone layer
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Deforestation only
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Volcanic eruptions
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Greenhouse gases (CO2, CH4) trap heat in the atmosphere.

---

## Question 2 — D3
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v2`
**Bloom:** Apply

**Context:** Endocrine system in a Pretoria class.

**Stem:** Which gland produces insulin?

- [x] **A)** Pancreas
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Thyroid
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Pituitary
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Adrenal
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Islets of Langerhans in pancreas produce insulin.

---

## Question 3 — D4
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v3`
**Bloom:** Understand

**Context:** Cell division in a Pretoria classroom.

**Stem:** Meiosis results in:

- [x] **A)** Four haploid daughter cells
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Two diploid daughter cells
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Four diploid daughter cells
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Two haploid daughter cells
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Meiosis produces four genetically different haploid cells.

---

## Question 4 — D4
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v4`
**Bloom:** Understand

**Context:** Protein synthesis - translation.

**Stem:** The role of tRNA is to:

- [x] **A)** Carry amino acids to the ribosome
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Carry the genetic code from DNA
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Form the ribosome structure
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Catalyse peptide bond formation
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** tRNA molecules bring specific amino acids during translation.

---

## Question 5 — D4
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v5`
**Bloom:** Analyze

**Context:** Genetics and blood groups.

**Stem:** Blood group O has:

- [x] **A)** No antigens on red blood cells
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** A and B antigens
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Only A antigens
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Only B antigens
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Type O has neither A nor B antigens.

---

## Question 6 — D5
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v6`
**Bloom:** Apply

**Context:** Respiratory system in Grade 12.

**Stem:** Gas exchange occurs in the:

- [x] **A)** Alveoli
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Bronchi
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Trachea
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Bronchioles
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Alveoli are the air sacs for gas exchange.

---

## Question 7 — D5
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v7`
**Bloom:** Remember

**Context:** Cellular respiration in a Durban class.

**Stem:** Most ATP per glucose produced by:

- [x] **A)** Aerobic respiration
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Anaerobic respiration
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Fermentation
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Glycolysis alone
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Aerobic respiration yields ~36-38 ATP per glucose.

---

## Question 8 — D5
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v8`
**Bloom:** Analyze

**Context:** Human impact on the environment.

**Stem:** Eutrophication is caused by:

- [x] **A)** Excess nutrients (nitrates/phosphates) in water
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Too much oxygen in water
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Global warming
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Acid rain
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Fertilisers wash into water, causing algal blooms and oxygen depletion.

---

## Question 9 — D6
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v9`
**Bloom:** Understand

**Context:** Genetic inheritance patterns.

**Stem:** In a pedigree chart, a shaded circle represents:

- [x] **A)** An affected female
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** An unaffected female
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** An affected male
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** An unaffected male
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Circles represent females; shaded means affected.

---

## Question 10 — D6
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v10`
**Bloom:** Evaluate

**Context:** Mendelian genetics in a Durban class.

**Stem:** In Tt x Tt, probability of tt offspring:

- [x] **A)** 25%
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** 50%
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** 75%
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** 100%
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Punnett: TT, Tt, Tt, tt -> 1/4 = 25%.

---

## Question 11 — D6
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v11`
**Bloom:** Understand

**Context:** The cell cycle in Grade 12 Life Sciences.

**Stem:** DNA replication occurs in which phase?

- [x] **A)** S phase
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** G1 phase
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** G2 phase
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** M phase
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** DNA synthesis happens in the S phase of interphase.

---

## Question 12 — D7
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v12`
**Bloom:** Apply

**Context:** Meiosis in a Johannesburg classroom.

**Stem:** Crossing over occurs during:

- [x] **A)** Prophase I of meiosis
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Prophase II of meiosis
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Anaphase I of meiosis
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Mitosis
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Crossing over between homologous chromatids occurs in Prophase I.

---

## Question 13 — D7
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v13`
**Bloom:** Remember

**Context:** Human reproduction in a Soweto class.

**Stem:** Function of the placenta:

- [x] **A)** Exchange nutrients and waste between mother and foetus
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Produce eggs
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Store urine from foetus
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Pump blood for foetus
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Placenta allows diffusion of substances between blood supplies.

---

## Question 14 — D7
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v14`
**Bloom:** Apply

**Context:** Excretory system in Grade 12.

**Stem:** Nephron is the functional unit of the:

- [x] **A)** Kidney
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Liver
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Lung
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Heart
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Nephrons filter blood and produce urine.

---

## Question 15 — D8
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v15`
**Bloom:** Analyze

**Context:** Cell biology in a Cape Town classroom.

**Stem:** Organelle responsible for cellular respiration:

- [x] **A)** Mitochondrion
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Nucleus
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Ribosome
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Golgi apparatus
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Mitochondria produce ATP via cellular respiration.

---

## Question 16 — D8
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v16`
**Bloom:** Analyze

**Context:** Photosynthesis in a Cape Town class.

**Stem:** Light-dependent reactions produce:

- [x] **A)** ATP and NADPH
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Glucose and oxygen
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Water and CO2
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** ATP and glucose
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Light-dependent reactions produce ATP and NADPH.

---

## Question 17 — D8
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v17`
**Bloom:** Apply

**Context:** Biodiversity and classification.

**Stem:** Binomial name for humans:

- [x] **A)** Homo sapiens
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Homo erectus
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Hominidae sapiens
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Homo habilis
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Linnaean: genus Homo, species sapiens.

---

## Question 18 — D9
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v18`
**Bloom:** Apply

**Context:** DNA replication in a Cape Town classroom.

**Stem:** The semi-conservative model of DNA replication means:

- [x] **A)** Each new DNA has one old and one new strand
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Both strands are completely new
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** The original DNA remains intact
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Replication is random
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Each daughter molecule contains one original and one newly synthesised strand.

---

## Question 19 — D9
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v19`
**Bloom:** Apply

**Context:** DNA mutations and their effects.

**Stem:** A mutation in a gene changes:

- [x] **A)** The DNA sequence
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** The phenotype directly
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** The chromosome number
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** The cell membrane
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** A mutation alters the nucleotide sequence of DNA.

---

## Question 20 — D10
**ID:** `ZA-LIF-12-2026-W09-photosynthesis-respiration-001-MASTERY-v20`
**Bloom:** Analyze

**Context:** Biomes and biodiversity.

**Stem:** Greatest biodiversity found in:

- [x] **A)** Tropical rainforest
  <!-- feedback: Correct! ✓ -->
- [ ] **B)** Desert
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **C)** Tundra
  <!-- feedback: Incorrect. Review the concept. -->
- [ ] **D)** Grassland
  <!-- feedback: Incorrect. Review the concept. -->

**Explanation:** Tropical rainforests have the highest species diversity.

---
