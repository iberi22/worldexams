---
id: "CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle"
country: "colombia"
grado: 6
asignatura: "lengua"
tema: "cohesion-textual"
periodo: "weekly"
week: "W37"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 10
bundle_size: 10
alignment: "DBA MEN Colombia / Saber 2026"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Weekly Pack W37 — Cohesión Textual

**Grado:** 6° | **Periodo:** 4 | **Semana:** 37 | **Año:** 2026

---

## Question 1 [D3]
**ID:** CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Competencia Lectora (Literal)
**Expected_Success:** 0.88
**Contexto:** En el Colegio La Enseñanza de Barranquilla, los estudiantes aprenden los mecanismos que "amarran" las palabras dentro de un texto.

### Enunciado
¿A qué se refiere la **cohesión** en un texto escrito?

### Opciones
- [ ] A) A la idea principal que el autor quiere transmitir. <!-- feedback: Incorrecto. Eso se refiere a la coherencia. -->
- [x] B) A la unión y conexión gramatical entre las palabras, oraciones y párrafos mediante el uso de nexos, pronombres y repeticiones. <!-- feedback: Correcto. La cohesión es la propiedad "visible" que articula las partes del texto. -->
- [ ] C) A que el texto sea muy divertido para el lector. <!-- feedback: Incorrecto. Un texto puede ser serio y estar perfectamente cohesionado. -->
- [ ] D) Al número de páginas que tiene un libro. <!-- feedback: Incorrecto. La extensión no garantiza la conexión interna del texto. -->

### Explicacion Pedagogica
La cohesión es una propiedad formal del texto que permite que las partes se relacionen entre sí de manera correcta según las reglas del idioma.

---

## Question 2 [D4]
**ID:** CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle-v2
**Bloom:** Understand
**ICFES:** Competencia Lectora (Literal)
**Expected_Success:** 0.82
**Contexto:** Un estudiante de 6° grado en Medellín analiza cómo evitar la repetición innecesaria de palabras.

### Enunciado
¿Cuál es el mecanismo de cohesión que consiste en reemplazar una palabra por otra con significado similar, como usar "canino" en lugar de "perro"?

### Opciones
- [ ] A) La repetición literal. <!-- feedback: Incorrecto. La repetición usaría la misma palabra "perro". -->
- [x] B) La sinonimia. <!-- feedback: Correcto. El uso de sinónimos evita la monotonía y enriquece el texto manteniendo el referente. -->
- [ ] C) El uso de mayúsculas. <!-- feedback: Incorrecto. No es un mecanismo de cohesión léxica. -->
- [ ] D) La puntuación. <!-- feedback: Incorrecto. La puntuación organiza pausas, no reemplaza términos. -->

### Explicacion Pedagogica
La sinonimia es un recurso de cohesión léxica que permite volver sobre un tema sin cansar al lector con la misma palabra repetida constantemente.

---

## Question 3 [D5]
**ID:** CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle-v3
**Bloom:** Apply
**ICFES:** Competencia Lectora (Literal)
**Expected_Success:** 0.80
**Contexto:** Un grupo de estudiantes en Cali redacta un texto sobre el deporte y debe usar **conectores lógicos**.

### Enunciado
Completa la oración con el conector de **oposición** adecuado: "Los jugadores entrenaron muy fuerte; ________, no lograron ganar el partido".

### Opciones
- [ ] A) Además. <!-- feedback: Incorrecto. "Además" indica suma, no oposición. -->
- [ ] B) Porque. <!-- feedback: Incorrecto. "Porque" indica causa. -->
- [x] C) Sin embargo. <!-- feedback: Correcto. "Sin embargo" es un conector adversativo que introduce una idea contraria a la anterior. -->
- [ ] D) Por lo tanto. <!-- feedback: Incorrecto. Indica consecuencia, no oposición. -->

### Explicacion Pedagogica
Los conectores lógicos son palabras que establecen relaciones de sentido (causa, consecuencia, oposición, tiempo) entre las oraciones, facilitando el hilo conductor del texto.

---

## Question 4 [D6]
**ID:** CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle-v4
**Bloom:** Understand
**ICFES:** Competencia Lectora (Literal)
**Expected_Success:** 0.75
**Contexto:** En un taller de lengua en Bogotá, se estudia el uso de los **pronombres** como herramienta de cohesión.

### Enunciado
En la oración: "María y Juan fueron al parque. **Ellos** compraron helados", la palabra resaltada cumple una función de cohesión porque:

### Opciones
- [ ] A) Indica que la acción ocurrió en el pasado. <!-- feedback: Incorrecto. Esa es función del verbo "compraron". -->
- [x] B) Sustituye a los sustantivos "María y Juan" para evitar repetirlos. <!-- feedback: Correcto. Este mecanismo se llama referencia o anáfora. -->
- [ ] C) Describe cómo son físicamente María y Juan. <!-- feedback: Incorrecto. El pronombre no da características descriptivas. -->
- [ ] D) Une dos oraciones mediante una coma. <!-- feedback: Incorrecto. El pronombre no es un signo de puntuación. -->

### Explicacion Pedagogica
La referencia pronominal permite mantener el hilo temático refiriéndose a personas o cosas ya mencionadas sin necesidad de nombrarlas nuevamente.

---

## Question 5 [D6]
**ID:** CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Competencia Lectora (Literal)
**Expected_Success:** 0.74
**Contexto:** Un estudiante de Bucaramanga quiere organizar las ideas de su exposición siguiendo un orden temporal.

### Enunciado
¿Cuál de los siguientes grupos de conectores sirve para organizar la **secuencia temporal** de una narración?

### Opciones
- [ ] A) Pero, mas, sino, aunque. <!-- feedback: Incorrecto. Son conectores de oposición. -->
- [x] B) Primero, luego, después, finalmente. <!-- feedback: Correcto. Son conectores temporales que ordenan los hechos cronológicamente. -->
- [ ] C) Ya que, puesto que, debido a. <!-- feedback: Incorrecto. Son conectores de causa. -->
- [ ] D) Es decir, o sea, en otras palabras. <!-- feedback: Incorrecto. Son conectores explicativos. -->

### Explicacion Pedagogica
Los conectores de orden o secuencia son vitales para la cohesión en textos narrativos e instructivos, guiando al lector a través de los pasos o momentos del relato.

---

## Question 6 [D7]
**ID:** CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle-v6
**Bloom:** Analyze
**ICFES:** Competencia Lectora (Inferencial)
**Expected_Success:** 0.68
**Contexto:** Un grupo de alumnos en Cartagena analiza un texto que utiliza la **elipsis** para ser más fluido.

### Enunciado
¿En qué consiste el mecanismo de cohesión llamado **elipsis**?

### Opciones
- [ ] A) En repetir la misma idea con las mismas palabras. <!-- feedback: Incorrecto. Eso es redundancia. -->
- [x] B) En omitir (quitar) una palabra que ya se sobreentiende para no ser repetitivo. <!-- feedback: Correcto. Ejemplo: "Yo estudio lengua; mi hermano, matemáticas" (se omite el verbo estudia). -->
- [ ] C) En escribir círculos alrededor de las palabras importantes. <!-- feedback: Incorrecto. No es un recurso gráfico, sino lingüístico. -->
- [ ] D) En usar palabras que rimen entre sí. <!-- feedback: Incorrecto. Eso es propio del género lírico (rima). -->

### Explicacion Pedagogica
La elipsis aprovecha el contexto para evitar la pesadez de repetir elementos obvios, haciendo que el texto sea más ágil y elegante.

---

## Question 7 [D8]
**ID:** CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle-v7
**Bloom:** Analyze
**ICFES:** Competencia Lectora (Inferencial)
**Expected_Success:** 0.62
**Contexto:** En una clase en Pereira, se estudia cómo los conectores de **consecuencia** cierran razonamientos lógicos.

### Enunciado
¿Cuál de las siguientes oraciones utiliza correctamente un conector de **consecuencia**?

### Opciones
- [ ] A) Llovió mucho **pero** no me mojé. <!-- feedback: Incorrecto. Indica oposición. -->
- [ ] B) Llovió mucho **y** saqué el paraguas. <!-- feedback: Incorrecto. Indica suma de acciones. -->
- [x] C) Llovió mucho; **por consiguiente**, se inundó el patio. <!-- feedback: Correcto. El segundo hecho es un resultado lógico del primero. -->
- [ ] D) Llovió mucho **mientras** yo dormía. <!-- feedback: Incorrecto. Indica simultaneidad temporal. -->

### Explicacion Pedagogica
Los conectores de consecuencia (así que, por lo tanto, en consecuencia) establecen una relación de efecto entre la primera idea y la segunda.

---

## Question 8 [D8]
**ID:** CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle-v8
**Bloom:** Evaluate
**ICFES:** Competencia Lectora (Crítica)
**Expected_Success:** 0.58
**Contexto:** Un docente de Popayán evalúa un párrafo donde el estudiante usa mal el conector "pero".

### Enunciado
Analiza la oración: "Me gusta el chocolate, **pero** es muy rico". ¿Por qué el uso del conector es incorrecto desde el punto de vista de la cohesión?

### Opciones
- [ ] A) Porque el chocolate no es rico. <!-- feedback: Incorrecto. Eso es una opinión personal, no un análisis gramatical. -->
- [ ] B) Porque "pero" es una palabra muy corta. <!-- feedback: Incorrecto. La longitud de la palabra no afecta su corrección. -->
- [x] C) Porque el conector "pero" indica oposición, y ser "rico" no se opone a que "me guste". <!-- feedback: Correcto. Los conectores deben respetar la lógica de las ideas que unen. -->
- [ ] D) Porque falta un punto final después de la palabra chocolate. <!-- feedback: Incorrecto. El error es de significado lógico del nexo. -->

### Explicacion Pedagogica
La cohesión requiere que el nexo elegido coincida con la relación real entre las ideas. Si las ideas se apoyan entre sí, se requiere un conector de suma (y) o de causa (porque).

---

## Question 9 [D9]
**ID:** CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle-v9
**Bloom:** Analyze
**ICFES:** Competencia Lectora (Inferencial)
**Expected_Success:** 0.54
**Contexto:** En un ejercicio de análisis en Villavicencio, se estudia la **cohesión léxica** por campos semánticos.

### Enunciado
En un texto sobre la fauna de los llanos orientales, aparecen las palabras: "chigüiro", "venado", "babilla" y "garza". ¿Cómo ayudan estas palabras a la cohesión del texto?

### Opciones
- [ ] A) No ayudan, solo sirven para aprender nombres de animales. <!-- feedback: Incorrecto. El vocabulario seleccionado tiene una función estructural. -->
- [x] B) Crean un campo semántico relacionado con la fauna llanera, dándole unidad léxica al texto. <!-- feedback: Correcto. La selección de palabras de una misma familia o tema refuerza la cohesión. -->
- [ ] C) Ayudan a que el texto rime y suene como una canción. <!-- feedback: Incorrecto. No necesariamente hay rima en un listado temático. -->
- [ ] D) Confunden al lector porque son muchos animales diferentes. <!-- feedback: Incorrecto. Al contrario, ayudan a situar al lector en el contexto ambiental. -->

### Explicacion Pedagogica
La cohesión léxica se logra cuando el autor utiliza palabras que pertenecen a un mismo ámbito de la realidad, creando una red de significados coherente.

---

## Question 10 [D10]
**ID:** CO-LEN-6-2026-W37-cohesion-textual-001-MASTERY-bundle-v10
**Bloom:** Evaluate
**ICFES:** Competencia Lectora (Crítica)
**Expected_Success:** 0.48
**Contexto:** Un estudiante de 6° grado en Pasto debe corregir un párrafo que parece una lista de mercado porque no tiene nexos.

### Enunciado
Lee el texto: "Fui al mercado. Compré papas. Compré cebollas. Volví a casa. Estaba cansado". ¿Cuál es la mejor forma de mejorar la **cohesión** de este párrafo?

### Opciones
- [ ] A) Escribir las oraciones en diferentes páginas. <!-- feedback: Incorrecto. Eso empeora la conexión. -->
- [x] B) Usar conectores y pronombres: "Fui al mercado y compré papas y cebollas; luego volví a casa porque estaba cansado". <!-- feedback: Correcto. El uso de nexos (y, luego, porque) y la elipsis del sujeto dan fluidez y conexión al texto. -->
- [ ] C) Ponerle un título más largo al texto. <!-- feedback: Incorrecto. El título no soluciona los problemas internos de conexión. -->
- [ ] D) Usar solo puntos seguidos entre cada palabra. <!-- feedback: Incorrecto. Eso haría el texto aún más entrecortado e incoherente. -->

### Explicacion Pedagogica
Un texto sin cohesión es una suma de oraciones aisladas. La verdadera escritura consiste en tejer esas oraciones mediante mecanismos lingüísticos para crear un mensaje fluido y profesional.

[//]: # (QUALITY_REVIEW)

| Dimensión | Score | Notas |
|-----------|-------|-------|
| Técnico | 30/30 | Protocolo v5.2 respetado. |
| Curricular | 40/40 | Alineado con DBA de G6 sobre cohesión. |
| Contexto | 20/20 | Referencias a Barranquilla, Medellín, Cali, etc. |
| Redacción | 10/10 | Alta calidad en las preguntas y respuestas. |
| **TOTAL** | **100/100** | |
