---
id: "CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-bundle"
country: "colombia"
grado: 7
asignatura: "lengua"
tema: "evaluacion-integral"
periodo: "weekly"
week: "W33"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 10
bundle_size: 10
alignment: "DBA MEN Colombia / Saber 2026"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

## Question 1 [D3-D4]
**ID:** CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-v1
**Bloom:** Remember
**ICFES:** Competencia Lectora (Literal)
**Expected_Success:** 0.85
**Contexto:** Un estudiante revisa los temas vistos en las últimas semanas sobre textos multimodales: infografías, caricaturas, tablas y publicidad.

### Enunciado
¿Cuál es el denominador común de todos estos tipos de textos que los diferencia de una novela tradicional de solo texto?

### Opciones
- [ ] A) Que todos son textos informativos de carácter científico. <!-- feedback: La caricatura y la publicidad no son necesariamente científicas. -->
- [x] B) Que combinan el lenguaje verbal con otros sistemas de signos (imágenes, gráficos, colores). <!-- feedback: La multimodalidad es la característica compartida que requiere habilidades de lectura visual y textual. -->
- [ ] C) Que todos fueron inventados después de la creación de internet. <!-- feedback: La caricatura y la publicidad existen desde mucho antes que internet. -->
- [ ] D) Que no utilizan palabras para transmitir sus mensajes. <!-- feedback: Todos estos textos utilizan palabras, pero las combinan con otros modos. -->

### Explicacion Pedagogica
Los textos multimodales integran diversos modos semióticos. Para comprenderlos integralmente, el lector debe ser capaz de relacionar lo que dicen las palabras con lo que muestran las imágenes o gráficos.

## Question 2 [D3-D4]
**ID:** CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-v2
**Bloom:** Remember
**ICFES:** Competencia Lectora (Literal)
**Expected_Success:** 0.80
**Contexto:** En una evaluación en Manizales, los estudiantes deben identificar la estructura de un texto. El texto tiene un titular llamativo, una imagen de un producto y un eslogan al final.

### Enunciado
¿A qué tipo de texto multimodal corresponde la estructura descrita?

### Opciones
- [ ] A) Noticia periodística. <!-- feedback: Una noticia suele tener lead y cuerpo, no un eslogan de marca. -->
- [ ] B) Infografía técnica. <!-- feedback: Una infografía suele tener un cuerpo con datos y procesos, no solo una imagen de producto. -->
- [x] C) Texto publicitario. <!-- feedback: El eslogan y la imagen de producto son elementos distintivos de la publicidad. -->
- [ ] D) Gráfico de barras. <!-- feedback: Un gráfico representa datos numéricos, no usa eslóganes. -->

### Explicacion Pedagogica
Cada tipo de texto multimodal tiene una silueta o estructura característica que ayuda al lector a identificar su propósito comunicativo (informar, convencer, denunciar).

## Question 3 [D5-D6]
**ID:** CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-v3
**Bloom:** Understand
**ICFES:** Competencia Lectora (Inferencial)
**Expected_Success:** 0.75
**Contexto:** Lee el siguiente fragmento de una noticia: "La inflación en Colombia llegó al 9% este año". Al lado, hay una caricatura de un ciudadano tratando de inflar un globo gigante que dice "Precios" hasta que está a punto de explotar.

### Enunciado
¿Cuál es la relación de sentido entre el texto de la noticia y la imagen de la caricatura?

### Opciones
- [ ] A) La caricatura desmiente los datos de la noticia. <!-- feedback: La caricatura apoya visualmente el concepto de la noticia mediante una metáfora. -->
- [x] B) La caricatura ilustra y critica el dato estadístico mediante una metáfora visual del globo. <!-- feedback: La imagen ayuda a comprender la "presión" de la inflación que menciona la cifra. -->
- [ ] C) Son dos temas completamente diferentes que no deberían estar juntos. <!-- feedback: Ambos tratan sobre el aumento de precios y la situación económica. -->
- [ ] D) El texto es serio y la imagen es solo para que los niños entiendan. <!-- feedback: La caricatura es un género de opinión para adultos y jóvenes, no solo un recurso infantil. -->

### Explicacion Pedagogica
En el periodismo, los diferentes géneros (noticia y caricatura de opinión) suelen complementarse para ofrecer una visión de los hechos y una interpretación crítica de los mismos.

## Question 4 [D5-D6]
**ID:** CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-v4
**Bloom:** Understand
**ICFES:** Competencia Lectora (Inferencial)
**Expected_Success:** 0.70
**Contexto:** Una infografía sobre el reciclaje usa el color verde para lo orgánico, azul para el papel y rojo para los residuos peligrosos. El texto explicativo es breve y directo.

### Enunciado
¿Por qué se dice que el lenguaje de la infografía es "sintético"?

### Opciones
- [ ] A) Porque está hecho de materiales artificiales. <!-- feedback: Es una confusión con el sentido químico de la palabra. -->
- [x] B) Porque resume y condensa la información para que sea rápida de leer. <!-- feedback: La síntesis es la capacidad de expresar lo esencial con pocos elementos, en este caso, visuales y textuales. -->
- [ ] C) Porque utiliza palabras muy difíciles que solo los expertos entienden. <!-- feedback: Al contrario, busca la claridad y la accesibilidad para un público amplio. -->
- [ ] D) Porque solo utiliza colores primarios. <!-- feedback: La síntesis se refiere a la brevedad y precisión de la información, no a la paleta de colores. -->

### Explicacion Pedagogica
La lectura de textos sintéticos requiere que el lector sea capaz de extraer el significado completo a partir de fragmentos de información (píldoras informativas) y su relación con el diseño gráfico.

## Question 5 [D5-D6]
**ID:** CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-v5
**Bloom:** Apply
**ICFES:** Competencia Lectora (Inferencial)
**Expected_Success:** 0.70
**Contexto:** Tienes la tarea de informar a la comunidad de tu barrio en Bogotá sobre los días de recolección de basura, pero también quieres denunciar que hay mucha gente tirando escombros.

### Enunciado
¿Qué combinación de textos multimodales sería la más adecuada para cumplir ambos objetivos?

### Opciones
- [ ] A) Un solo gráfico de torta con los porcentajes de basura. <!-- feedback: El gráfico no informa los días ni hace la denuncia de manera clara. -->
- [ ] B) Una caricatura de un vecino tirando escombros. <!-- feedback: Cumple con la denuncia, pero no informa sobre los horarios de recolección. -->
- [x] C) Una infografía con los horarios y una caricatura crítica sobre el manejo de escombros. <!-- feedback: Esta combinación utiliza el género informativo (infografía) y el de opinión (caricatura) para cubrir ambos propósitos. -->
- [ ] D) Un anuncio publicitario que venda bolsas de basura. <!-- feedback: El objetivo no es vender un producto, sino informar y concienciar a la comunidad. -->

### Explicacion Pedagogica
La selección de géneros textuales depende de la intención comunicativa. Para propósitos múltiples, a menudo es necesario combinar diferentes formatos que se adapten a la naturaleza de cada mensaje.

## Question 6 [D7-D8]
**ID:** CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-v6
**Bloom:** Apply
**ICFES:** Competencia Lectora (Inferencial)
**Expected_Success:** 0.65
**Contexto:** Un anuncio publicitario de una bebida hidratante muestra a un atleta cruzando la meta y, al fondo, un gráfico de líneas que sube constantemente representando su rendimiento.

### Enunciado
¿Cómo se está utilizando el gráfico de líneas en este anuncio publicitario?

### Opciones
- [ ] A) Como una prueba científica rigurosa del laboratorio de la empresa. <!-- feedback: En un anuncio, el gráfico suele ser un recurso estético, no necesariamente un dato de laboratorio. -->
- [x] B) Como un recurso retórico visual para simbolizar éxito y mejora continua. <!-- feedback: El gráfico de líneas ascendente es un símbolo universal de progreso que refuerza el mensaje de la marca. -->
- [ ] C) Para que el deportista sepa cuánto tiempo le falta para terminar la carrera. <!-- feedback: Es una interpretación literal que ignora la función persuasiva del anuncio. -->
- [ ] D) Para decorar el fondo porque el atleta se ve muy solo. <!-- feedback: Todos los elementos en publicidad tienen una carga semántica intencionada. -->

### Explicacion Pedagogica
La publicidad "toma prestados" recursos de otros géneros (como los gráficos estadísticos) para dar una apariencia de rigor o para usar su fuerza simbólica en favor de la persuasión.

## Question 7 [D7-D8]
**ID:** CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-v7
**Bloom:** Analyze
**ICFES:** Competencia Lectora (Inferencial)
**Expected_Success:** 0.65
**Contexto:** Analiza una tabla de resultados de las pruebas Saber de un colegio. Muestra que el promedio en Matemáticas subió, pero una nota al pie indica que el 40% de los estudiantes no presentó el examen.

### Enunciado
¿Cuál es la conclusión más acertada al integrar toda la información de la tabla (incluida la nota al pie)?

### Opciones
- [ ] A) El colegio es el mejor en Matemáticas porque el promedio subió. <!-- feedback: La conclusión es incompleta porque ignora el dato del 40% de ausentes. -->
- [ ] B) Las Matemáticas son muy fáciles según los datos presentados. <!-- feedback: Los datos de una tabla no califican la dificultad de una asignatura en sí. -->
- [x] C) El aumento del promedio podría no ser representativo de todo el colegio debido a la alta inasistencia. <!-- feedback: La nota al pie ofrece un contexto crítico que matiza o pone en duda la tendencia positiva de la tabla principal. -->
- [ ] D) La nota al pie de página es un error de impresión y no debe leerse. <!-- feedback: Las notas al pie son información fundamental para la interpretación correcta de los datos. -->

### Explicacion Pedagogica
La lectura crítica de tablas exige integrar la información principal con las aclaraciones o notas marginales, las cuales suelen contener los límites o condiciones de los datos presentados.

## Question 8 [D7-D8]
**ID:** CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-v8
**Bloom:** Analyze
**ICFES:** Competencia Lectora (Crítica)
**Expected_Success:** 0.60
**Contexto:** Observa una caricatura sobre el lenguaje periodístico: un periodista escribe una noticia sobre un árbol que se cayó, pero en el dibujo se ve al periodista poniendo una lupa gigante sobre una sola hormiga que caminaba por ahí, ignorando el árbol.

### Enunciado
¿Qué crítica está planteando el autor sobre el ejercicio periodístico actual?

### Opciones
- [ ] A) Que los periodistas deberían estudiar más sobre hormigas e insectos. <!-- feedback: Es una interpretación literal que ignora el sentido metafórico de la caricatura. -->
- [x] B) Que el periodismo a veces se enfoca en detalles insignificantes o sensacionalistas y pierde de vista lo verdaderamente importante. <!-- feedback: La lupa sobre la hormiga simboliza la pérdida de perspectiva y la distorsión del hecho principal. -->
- [ ] C) Que las lupas son herramientas necesarias para que los periodistas vean mejor. <!-- feedback: La lupa es aquí un símbolo de la atención selectiva y sesgada del comunicador. -->
- [ ] D) Que no hay noticias interesantes, por eso hay que inventarlas con hormigas. <!-- feedback: El tema es la jerarquización y el enfoque de la información, no la falta de noticias. -->

### Explicacion Pedagogica
La meta-comunicación (comunicación sobre la comunicación) es común en la caricatura periodística. El análisis crítico permite identificar los vicios o errores de los medios denunciados por los humoristas gráficos.

## Question 9 [D9-D10]
**ID:** CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-v9
**Bloom:** Analyze
**ICFES:** Competencia Lectora (Crítica)
**Expected_Success:** 0.55
**Contexto:** Un medio de comunicación publica una infografía sobre el éxito de una nueva ley. La infografía fue diseñada por el equipo de publicidad del gobierno que propuso la ley.

### Enunciado
Desde la perspectiva de la lectura crítica de textos multimodales, ¿por qué es importante conocer la fuente o el creador de la infografía?

### Opciones
- [ ] A) Para saber si los colores utilizados están de moda. <!-- feedback: La estética no es el punto central del análisis crítico de fuentes. -->
- [ ] B) Para poder llamar al diseñador y felicitarlo por su trabajo. <!-- feedback: El análisis busca evaluar el mensaje, no establecer contacto personal. -->
- [x] C) Para identificar posibles conflictos de interés y sesgos que busquen favorecer una postura política. <!-- feedback: Si quien emite el mensaje es el mismo que se beneficia de él, la objetividad de la infografía es cuestionable. -->
- [ ] D) Porque el nombre de la fuente siempre garantiza que los datos son reales. <!-- feedback: La fuente indica el origen y la intención, pero no es una garantía automática de veracidad absoluta. -->

### Explicacion Pedagogica
La procedencia de un texto (quién lo escribe y por qué) es un dato clave para evaluar su confiabilidad. En los textos multimodales, los sesgos pueden estar ocultos en la elección de los iconos o el diseño de los gráficos.

## Question 10 [D9-D10]
**ID:** CO-LEN-7-2026-W33-evaluacion-integral-001-MASTERY-v10
**Bloom:** Evaluate
**ICFES:** Competencia Lectora (Crítica)
**Expected_Success:** 0.50
**Contexto:** Evalúa el impacto de la publicidad multimodal en redes sociales: los anuncios aparecen mezclados con las fotos de tus amigos, imitando su estilo visual y usando el mismo tipo de lenguaje cercano.

### Enunciado
¿Cuál es la conclusión más profunda sobre esta nueva forma de comunicación publicitaria?

### Opciones
- [ ] A) Es muy buena porque así la publicidad no es aburrida y parece una charla. <!-- feedback: No evalúa el impacto en la percepción del usuario ni el posible engaño. -->
- [x] B) Borra las fronteras entre la información personal y el interés comercial, dificultando que el usuario identifique cuándo está siendo persuadido. <!-- feedback: La "publicidad nativa" busca camuflarse para reducir la resistencia crítica del receptor. -->
- [ ] C) Ayuda a que las redes sociales sean gratis para todos los usuarios. <!-- feedback: Aunque sea una razón económica, no es una evaluación del fenómeno comunicativo en sí. -->
- [ ] D) Obliga a los amigos a ser más profesionales al tomar sus fotos. <!-- feedback: Es una consecuencia improbable y ajena al análisis de la estrategia publicitaria. -->

### Explicacion Pedagogica
La evaluación crítica de los entornos digitales contemporáneos revela cómo la multimodalidad se usa para mimetizar mensajes comerciales con mensajes afectivos, lo que requiere un nivel de alerta superior por parte del ciudadano digital.

[//]: # (QUALITY_REVIEW)
| Criterio | Puntuación | Notas |
| :--- | :---: | :--- |
| Técnico | 30/30 | Estructura v5.2 cumplida. |
| Curricular | 40/40 | Evaluación integral que conecta todos los temas del periodo. |
| Contexto | 20/20 | Ejemplos locales (Manizales, Bogotá) y temas de actualidad nacional. |
| Redacción | 10/10 | Lenguaje claro y retroalimentaciones pedagógicas profundas. |
| **Total** | **100/100** | |
