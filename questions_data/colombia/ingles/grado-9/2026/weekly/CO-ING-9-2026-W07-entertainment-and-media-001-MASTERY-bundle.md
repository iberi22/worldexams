---
id: "CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle"
country: "colombia"
grado: 9
asignatura: "ingles"
tema: "entertainment-and-media"
periodo: "weekly"
week: "W07"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 12
bundle_size: 12
alignment: "DBA MEN Colombia"
cefr_level: "B2"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---
# Weekly Pack W07: Entertainment and Media (B2)

This bundle explores the world of movies, music, and social media, focusing on Passive Voice in various tenses to emphasize the action rather than the doer, aligned with CEFR B2 level for Grade 9.

---

## Question 1 [D3]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Lingüística
**Expected_Success:** 0.85
**Contexto:** Describing a movie production.

### Enunciado
The new superhero movie ________ in several locations across Colombia last year.

### Opciones
- [ ] A) filmed <!-- feedback: Incorrect. This would mean the movie did the filming. We need the passive form. -->
- [x] B) was filmed <!-- feedback: Correct! Past Simple Passive (was/were + past participle) for a finished action. -->
- [ ] C) is filmed <!-- feedback: Incorrect. This refers to a current or regular action. -->
- [ ] D) has been filmed <!-- feedback: Incorrect. This would be used if the specific time wasn't mentioned. -->

### Explicacion Pedagogica
Usamos la voz pasiva en el pasado (*was filmed*) cuando queremos resaltar qué se filmó en lugar de quién lo filmó, especialmente si mencionamos un tiempo específico como *last year*.

---

## Question 2 [D3]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v2
**Bloom:** Remember
**ICFES:** Lingüística
**Expected_Success:** 0.80
**Contexto:** Talking about a famous artist's impact.

### Enunciado
Millions of songs ________ every day on streaming platforms around the world.

### Opciones
- [ ] A) played <!-- feedback: Incorrect. Active voice in the past. -->
- [ ] B) are playing <!-- feedback: Incorrect. Active voice in the continuous. -->
- [x] C) are played <!-- feedback: Correct! Present Simple Passive (am/is/are + past participle) for regular actions. -->
- [ ] D) were played <!-- feedback: Incorrect. This refers to the past. -->

### Explicacion Pedagogica
Para hablar de acciones habituales o hechos generales en voz pasiva, utilizamos el Presente Simple del verbo *to be* más el participio pasado del verbo principal.

---

## Question 3 [D4]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v3
**Bloom:** Understand
**ICFES:** Lingüística
**Expected_Success:** 0.75
**Contexto:** Discussing recent changes in social media algorithms.

### Enunciado
The way we see content on social media ________ by complex algorithms recently.

### Opciones
- [ ] A) has changed <!-- feedback: Incorrect. This is active voice; we need the passive. -->
- [x] B) has been changed <!-- feedback: Correct! Present Perfect Passive for an action that has happened recently. -->
- [ ] C) was changed <!-- feedback: Incorrect. Less suitable for "recently" than the Present Perfect. -->
- [ ] D) is being changed <!-- feedback: Incorrect. This refers to an action in progress right now. -->

### Explicacion Pedagogica
La voz pasiva en el Presente Perfecto (*has been changed*) se utiliza para acciones que han ocurrido en un tiempo no determinado pero reciente, afectando el estado presente.

---

## Question 4 [D5]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v4
**Bloom:** Understand
**ICFES:** Pragmática
**Expected_Success:** 0.70
**Contexto:** Explaining why a concert was delayed.

### Enunciado
The concert ________ because of the heavy rain in Bogotá last night.

### Opciones
- [ ] A) cancelled <!-- feedback: Incorrect. Active voice. -->
- [x] B) was cancelled <!-- feedback: Correct! Past Simple Passive. -->
- [ ] C) is cancelled <!-- feedback: Incorrect. Present tense. -->
- [ ] D) has been cancelled <!-- feedback: Incorrect. "Last night" specifies a finished past time. -->

### Explicacion Pedagogica
Cuando el agente de la acción (quien canceló el concierto) no es lo más importante, usamos la voz pasiva para enfocarnos en el evento afectado.

---

## Question 5 [D5]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Lingüística
**Expected_Success:** 0.65
**Contexto:** Describing a future media event.

### Enunciado
The winners of the music awards ________ next Sunday during a live broadcast.

### Opciones
- [ ] A) will announce <!-- feedback: Incorrect. Active voice. -->
- [x] B) will be announced <!-- feedback: Correct! Future Simple Passive (will be + past participle). -->
- [ ] C) are going to announce <!-- feedback: Incorrect. Active voice. -->
- [ ] D) will being announced <!-- feedback: Incorrect. Grammatically incorrect form. -->

### Explicacion Pedagogica
Para eventos futuros donde el foco es lo que sucederá, usamos la pasiva con *will be* seguida del participio pasado.

---

## Question 6 [D6]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v6
**Bloom:** Apply
**ICFES:** Sociolingüística
**Expected_Success:** 0.60
**Contexto:** Discussing the history of newspapers.

### Enunciado
Before the rise of the internet, news ________ primarily through printed newspapers.

### Opciones
- [x] A) was delivered <!-- feedback: Correct! Past Simple Passive for a past regular situation. -->
- [ ] B) has been delivered <!-- feedback: Incorrect. Present Perfect doesn't fit the "before the internet" context. -->
- [ ] C) were delivered <!-- feedback: Incorrect. "News" is an uncountable noun in English and takes a singular verb. -->
- [ ] D) is delivered <!-- feedback: Incorrect. Present tense. -->

### Explicacion Pedagogica
Es fundamental recordar que la palabra *news* es incontable y siempre va con el verbo en singular (*was*), incluso en voz pasiva.

---

## Question 7 [D7]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v7
**Bloom:** Analyze
**ICFES:** Lingüística
**Expected_Success:** 0.55
**Contexto:** Identifying an ongoing passive action.

### Enunciado
Shhh! A new episode of your favorite series ________ in the next room right now.

### Opciones
- [ ] A) is recorded <!-- feedback: Incorrect. This refers to a habit. -->
- [x] B) is being recorded <!-- feedback: Correct! Present Continuous Passive (is/are being + past participle) for actions in progress. -->
- [ ] C) was being recorded <!-- feedback: Incorrect. Past continuous. -->
- [ ] D) has been recorded <!-- feedback: Incorrect. Present perfect. -->

### Explicacion Pedagogica
Para acciones que están ocurriendo en este preciso momento en voz pasiva, añadimos la palabra *being* entre el verbo *to be* y el participio pasado.

---

## Question 8 [D7]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v8
**Bloom:** Analyze
**ICFES:** Pragmática
**Expected_Success:** 0.50
**Contexto:** Understanding the use of "by" to indicate the agent.

### Enunciado
Which sentence is the most natural to use in a formal movie review?

### Opciones
- [ ] A) People directed the movie in 1994. <!-- feedback: Incorrect. "People" is too vague as a subject. -->
- [x] B) The movie was directed by Steven Spielberg in 1994. <!-- feedback: Correct! Formal style often uses passive with "by" for the creator. -->
- [ ] C) Steven Spielberg directed the movie in 1994. <!-- feedback: Incorrect. While correct, B is often preferred in reviews to highlight the movie first. -->
- [ ] D) The movie directed Steven Spielberg in 1994. <!-- feedback: Incorrect. Logically impossible. -->

### Explicacion Pedagogica
En críticas formales de arte o cine, es común usar la voz pasiva para poner la obra como sujeto, mencionando al autor con la preposición *by*.

---

## Question 9 [D8]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v9
**Bloom:** Analyze
**ICFES:** Lingüística
**Expected_Success:** 0.45
**Contexto:** Passive with modal verbs.

### Enunciado
Sensitive personal data ________ on social media to avoid identity theft.

### Opciones
- [ ] A) shouldn't share <!-- feedback: Incorrect. Active voice. -->
- [x] B) shouldn't be shared <!-- feedback: Correct! Passive with modal verbs (modal + be + past participle). -->
- [ ] C) shouldn't being shared <!-- feedback: Incorrect. Grammatically incorrect. -->
- [ ] D) shouldn't shared <!-- feedback: Incorrect. Missing "be". -->

### Explicacion Pedagogica
Al usar verbos modales en pasiva, la estructura es: *modal + be + participio pasado*. Esto es muy útil para dar consejos o reglas generales.

---

## Question 10 [D9]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v10
**Bloom:** Evaluate
**ICFES:** Lingüística
**Expected_Success:** 0.40
**Contexto:** Finding an error in a technical text about media.

### Enunciado
Identify the error in this paragraph:
"The script **was written** (A) by a young author. Later, it **was being sent** (B) to the producers, who loved it. Now, the film **is being produced** (C) in Hollywood and it **will be released** (D) next year."

### Opciones
- [ ] A) was written <!-- feedback: Incorrect. Correct use of Past Simple Passive. -->
- [x] B) was being sent <!-- feedback: Incorrect. This implies a continuous action in the past; it should be Past Simple Passive (*was sent*) for a single completed action. -->
- [ ] C) is being produced <!-- feedback: Incorrect. Correct use of Present Continuous Passive. -->
- [ ] D) will be released <!-- feedback: Incorrect. Correct use of Future Simple Passive. -->

### Explicacion Pedagogica
A nivel B2, el estudiante debe distinguir entre un proceso continuo en el pasado (*was being sent*) y un evento puntual (*was sent*). Enviar un guion es habitualmente una acción puntual.

---

## Question 11 [D10]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v11
**Bloom:** Evaluate
**ICFES:** Pragmática
**Expected_Success:** 0.35
**Contexto:** Choosing between active and passive for impact.

### Enunciado
Which headline is more appropriate for a serious news report about a cybersecurity breach?

### Opciones
- [ ] A) Someone stole the passwords of 5,000 users! <!-- feedback: Incorrect. "Someone" is too informal and vague for news. -->
- [x] B) Passwords of 5,000 users have been stolen in a cyberattack. <!-- feedback: Correct! Passive voice focuses on the victims and the serious event. -->
- [ ] C) A hacker was stealing passwords yesterday. <!-- feedback: Incorrect. Focuses on the hacker, which might be unknown. -->
- [ ] D) 5,000 users lost their passwords. <!-- feedback: Incorrect. This makes the users the actors, which is not the case. -->

### Explicacion Pedagogica
En el periodismo, la voz pasiva es fundamental para reportar hechos donde el perpetrador es desconocido o donde el impacto en las víctimas es lo más relevante.

---

## Question 12 [D10]
**ID:** CO-ING-9-2026-W07-entertainment-and-media-001-MASTERY-bundle-v12
**Bloom:** Evaluate
**ICFES:** Sociolingüística
**Expected_Success:** 0.30
**Contexto:** Summarizing the evolution of media.

### Enunciado
Complete the sentence:
"In the past, information ________ by a few, but today, content ________ by everyone."

### Opciones
- [ ] A) was controlled / created <!-- feedback: Incorrect. Active voice in the second part. -->
- [x] B) was controlled / is being created <!-- feedback: Correct! Passive voice in both parts, reflecting the shift in power. -->
- [ ] C) is controlled / was created <!-- feedback: Incorrect. Tenses are inverted. -->
- [ ] D) has been controlled / is created <!-- feedback: Incorrect. First part refers to a finished past era. -->

### Explicacion Pedagogica
Este ejercicio requiere evaluar el cambio histórico en los medios: de un control centralizado en el pasado (*was controlled*) a una creación masiva y actual (*is being created*).
