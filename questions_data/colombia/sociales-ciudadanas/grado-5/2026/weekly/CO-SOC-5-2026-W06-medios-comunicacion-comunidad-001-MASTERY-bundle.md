---
id: "CO-SOC-5-2026-W06-medios-comunicacion-comunidad-001-MASTERY-bundle"
country: "colombia"
grado: 5
asignatura: "sociales-ciudadanas"
tema: "medios-comunicacion-comunidad"
periodo: "weekly"
week: "W06"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 8
bundle_size: 8
alignment: "DBA MEN Colombia / Saber 5"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: Medios de Comunicación y Comunidad - Grado 5 (W06)

Tipos de medios de comunicación (tradicionales y digitales), su función informativa y su influencia en la vida comunitaria.

## Question 1 [D3-D4]
**ID:** CO-SOC-5-2026-W06-medios-comunicacion-comunidad-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Ciudadano
**Expected_Success:** 0.90
**Contexto:** En una clase en Cali, la profesora pregunta qué medios de comunicación usan los estudiantes en su casa.

### Enunciado
¿Cuál de los siguientes es un medio de comunicación masivo tradicional?

### Opciones
- [x] A) La televisión.
  <!-- feedback: La televisión es un medio masivo tradicional que llega a millones de personas. -->
- [ ] B) Un mensaje hablado entre dos amigos en el patio.
  <!-- feedback: La conversación entre dos personas es comunicación interpersonal, no masiva. -->
- [ ] C) Una carta personal escrita a mano.
  <!-- feedback: Una carta personal es comunicación privada, no un medio masivo. -->
- [ ] D) Un susurro entre compañeros de clase.
  <!-- feedback: Un susurro no es un medio masivo ni tradicional de comunicación. -->

### Explicacion Pedagogica
Reconocer ejemplos de medios de comunicación masivos tradicionales y diferenciarlos de la comunicación interpersonal.

## Question 2 [D3-D4]
**ID:** CO-SOC-5-2026-W06-medios-comunicacion-comunidad-001-MASTERY-bundle-v2
**Bloom:** Understand
**ICFES:** Ciudadano
**Expected_Success:** 0.85
**Contexto:** Lee: "La radio comunitaria de un pueblo permite a los vecinos conocer noticias locales, compartir avisos y difundir la cultura del territorio."

### Enunciado
¿Por qué las radios comunitarias son importantes para los pueblos?

### Opciones
- [x] A) Porque informan a la comunidad sobre asuntos locales y fortalecen la identidad del territorio.
  <!-- feedback: Las radios comunitarias acercan la información a la gente y reflejan la cultura local. -->
- [ ] B) Porque reemplazan a las autoridades locales en sus decisiones.
  <!-- feedback: Las radios comunitarias no gobiernan ni reemplazan a las autoridades; solo informan. -->
- [ ] C) Porque su función es vender productos extranjeros sin relación con la comunidad.
  <!-- feedback: Las radios comunitarias suelen difundir contenidos propios de la comunidad, no solo extranjeros. -->
- [ ] D) Porque solo transmiten música sin ningún contenido informativo.
  <!-- feedback: Además de música, transmiten noticias, avisos y programas educativos. -->

### Explicacion Pedagogica
Comprender la función social de los medios comunitarios en los territorios colombianos.

## Question 3 [D5-D6]
**ID:** CO-SOC-5-2026-W06-medios-comunicacion-comunidad-001-MASTERY-bundle-v3
**Bloom:** Apply
**ICFES:** Ciudadano
**Expected_Success:** 0.76
**Contexto:** Un grupo de padres de familia en un pueblo de Santander quiere difundir información sobre una jornada de vacunación.

### Enunciado
¿Cuál combinación de medios sería más adecuada para llegar a personas de todas las edades?

### Opciones
- [x] A) Radio local, carteleras en la plaza y mensajes por redes sociales del colegio.
  <!-- feedback: Combinar radio, carteleras y redes sociales amplía el alcance a distintas edades. -->
- [ ] B) Solo mensajes en una lengua extranjera desconocida para la comunidad.
  <!-- feedback: Usar una lengua desconocida limita la comprensión del mensaje. -->
- [ ] C) Solo susurros entre los padres en el parque.
  <!-- feedback: Los susurros no alcanzan a toda la comunidad. -->
- [ ] D) Solo correos físicos enviados al extranjero.
  <!-- feedback: Los correos al extranjero no llegan a la comunidad local. -->

### Explicacion Pedagogica
Seleccionar medios de comunicación adecuados para informar a una comunidad diversa.

## Question 4 [D5-D6]
**ID:** CO-SOC-5-2026-W06-medios-comunicacion-comunidad-001-MASTERY-bundle-v4
**Bloom:** Apply
**ICFES:** Ciudadano
**Expected_Success:** 0.72
**Contexto:** Lee: "Internet permite acceder a noticias, pero también a información falsa o sin verificar, conocida como fake news."

### Enunciado
¿Cuál es una actitud responsable frente a la información encontrada en internet?

### Opciones
- [x] A) Verificar la fuente, comparar con otros medios y comprobar la fecha de la información.
  <!-- feedback: Comprobar fuentes y fechas ayuda a distinguir información confiable de noticias falsas. -->
- [ ] B) Compartir la primera noticia que aparezca sin revisarla.
  <!-- feedback: Compartir sin verificar contribuye a difundir información falsa. -->
- [ ] C) Creer todo lo que dice una sola página web sin importar su origen.
  <!-- feedback: Una sola página no garantiza veracidad; hay que comparar fuentes. -->
- [ ] D) Ignorar toda la información sin leerla.
  <!-- feedback: Ignorar todo no permite tomar decisiones informadas. -->

### Explicacion Pedagogica
Aplicar criterios de verificación de información en medios digitales para evitar la desinformación.

## Question 5 [D5-D6]
**ID:** CO-SOC-5-2026-W06-medios-comunicacion-comunidad-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Ciudadano
**Expected_Success:** 0.66
**Contexto:** En una escuela de Armenia, los estudiantes realizan un periódico mural para informar a sus compañeros.

### Enunciado
¿Qué características debe tener una noticia para informar correctamente a la comunidad escolar?

### Opciones
- [x] A) Información veraz, clara, con datos comprobables y sin lenguaje ofensivo.
  <!-- feedback: Una buena noticia es veraz, clara y respetuosa con las personas. -->
- [ ] B) Rumores inventados para llamar la atención.
  <!-- feedback: Los rumores sin base desinforman a la comunidad escolar. -->
- [ ] C) Datos falsos exagerados para hacer reír.
  <!-- feedback: La exageración y la mentira no son características de una buena noticia. -->
- [ ] D) Solo chismes sobre compañeros del salón.
  <!-- feedback: Los chismes no son información confiable y pueden dañar a las personas. -->

### Explicacion Pedagogica
Aplicar criterios periodísticos básicos para producir noticias escolares responsables.

## Question 6 [D7-D8]
**ID:** CO-SOC-5-2026-W06-medios-comunicacion-comunidad-001-MASTERY-bundle-v6
**Bloom:** Analyze
**ICFES:** Ciudadano
**Expected_Success:** 0.58
**Contexto:** Compara: "Un periódico impreso llega una vez al día con noticias nacionales. Una página de noticias en internet publica información cada minuto y se actualiza constantemente."

### Enunciado
¿Qué diferencia existe entre el periódico impreso y la página de noticias en internet?

### Opciones
- [x] A) El impreso se publica con periodicidad fija; la página de internet se actualiza de forma continua.
  <!-- feedback: La diferencia principal es la frecuencia de actualización y el soporte usado. -->
- [ ] B) El periódico impreso nunca tiene fotografías.
  <!-- feedback: Los periódicos impresos suelen incluir fotografías e ilustraciones. -->
- [ ] C) La página de internet solo publica información falsa.
  <!-- feedback: Las páginas de internet pueden tener información confiable, aunque también requieren verificación. -->
- [ ] D) El periódico impreso y la página de internet publican exactamente a la misma hora.
  <!-- feedback: Los periódicos tienen horarios fijos, mientras que las páginas web se actualizan constantemente. -->

### Explicacion Pedagogica
Comparar medios tradicionales y digitales según su periodicidad y soporte.

## Question 7 [D7-D8]
**ID:** CO-SOC-5-2026-W06-medios-comunicacion-comunidad-001-MASTERY-bundle-v7
**Bloom:** Analyze
**ICFES:** Ciudadano
**Expected_Success:** 0.52
**Contexto:** Lee: "Algunos medios de comunicación pueden difundir información con sesgos políticos o comerciales, lo que influye en la opinión de las personas."

### Enunciado
¿Qué consecuencia puede tener consumir información de un solo medio con sesgo político?

### Opciones
- [x] A) Formarse una visión incompleta o distorsionada de la realidad.
  <!-- feedback: Recibir información sesgada limita la mirada crítica sobre los hechos. -->
- [ ] B) Conocer siempre la verdad absoluta de los hechos.
  <!-- feedback: Ningún medio, y menos uno sesgado, garantiza la verdad absoluta. -->
- [ ] C) Mejorar automáticamente el pensamiento crítico.
  <!-- feedback: Un solo medio sesgado no favorece el pensamiento crítico, sino que lo reduce. -->
- [ ] D) Eliminar la posibilidad de informarse por otros medios.
  <!-- feedback: Las personas siempre pueden consultar otras fuentes para ampliar su visión. -->

### Explicacion Pedagogica
Analizar la influencia de los medios sesgados en la opinión pública y la importancia del contraste informativo.

## Question 8 [D9-D10]
**ID:** CO-SOC-5-2026-W06-medios-comunicacion-comunidad-001-MASTERY-bundle-v8
**Bloom:** Evaluate
**ICFES:** Ciudadano
**Expected_Success:** 0.31
**Contexto:** Lee: 'Los medios de comunicación cumplen un papel clave en la sociedad: informar, entretener, educar y permitir la participación ciudadana. Sin embargo, su influencia debe ir acompañada de pensamiento crítico y de respeto por la dignidad humana.'

### Enunciado
¿Cuál es la mejor forma de aprovechar los medios de comunicación como ciudadano responsable?

### Opciones
- [x] A) Informarse en distintas fuentes, verificar datos y compartir información respetuosa y veraz.
  <!-- feedback: Diversificar fuentes y verificar datos fortalece la participación ciudadana informada. -->
- [ ] B) Consumir solo lo que diga un único medio favorito sin cuestionar.
  <!-- feedback: Depender de un solo medio limita la visión de la realidad. -->
- [ ] C) Compartir únicamente contenido ofensivo para llamar la atención.
  <!-- feedback: Difundir contenido ofensivo no es responsable ni respetuoso. -->
- [ ] D) Evitar todo tipo de información para no ser influenciados.
  <!-- feedback: Aislarse de la información impide participar de forma activa en la sociedad. -->

### Explicacion Pedagogica
Evaluar el uso ético y democrático de los medios de comunicación en la vida comunitaria.