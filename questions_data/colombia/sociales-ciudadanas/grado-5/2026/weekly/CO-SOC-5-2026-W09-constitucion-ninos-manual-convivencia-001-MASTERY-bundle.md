---
id: "CO-SOC-5-2026-W09-constitucion-ninos-manual-convivencia-001-MASTERY-bundle"
country: "colombia"
grado: 5
asignatura: "sociales-ciudadanas"
tema: "constitucion-ninos-manual-convivencia"
periodo: "weekly"
week: "W09"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 8
bundle_size: 8
alignment: "DBA MEN Colombia / Saber 5"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: Constitución de los Niños y Manual de Convivencia - Grado 5 (W09)

Derechos fundamentales de los niños en la Constitución colombiana de 1991 y normas básicas del manual de convivencia escolar.

## Question 1 [D3-D4]
**ID:** CO-SOC-5-2026-W09-constitucion-ninos-manual-convivencia-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Entorno vivo
**Expected_Success:** 0.90
**Contexto:** En una escuela de Bucaramanga, la profesora explica los derechos de los niños según la Constitución colombiana.

### Enunciado
¿Cuál de los siguientes es un derecho fundamental de los niños en Colombia?

### Opciones
- [x] A) El derecho a la educación, a la salud y a la protección especial.
  <!-- feedback: La Constitución y la infancia reconocen la educación, la salud y la protección como derechos clave. -->
- [ ] B) El derecho a portar armas en la escuela.
  <!-- feedback: Portar armas en la escuela es contrario a los derechos de los niños y a la ley. -->
- [ ] C) El derecho a no asistir nunca al colegio.
  <!-- feedback: La educación es obligatoria; no asistir siempre vulnera este derecho. -->
- [ ] D) El derecho a votar en elecciones nacionales a los 5 años.
  <!-- feedback: El voto está reservado a los ciudadanos mayores de 18 años en Colombia. -->

### Explicacion Pedagogica
Reconocer los derechos fundamentales de los niños en la Constitución colombiana.

## Question 2 [D3-D4]
**ID:** CO-SOC-5-2026-W09-constitucion-ninos-manual-convivencia-001-MASTERY-bundle-v2
**Bloom:** Understand
**ICFES:** Entorno vivo
**Expected_Success:** 0.85
**Contexto:** Lee: "La Constitución de 1991 establece que la familia, la sociedad y el Estado deben proteger a los niños contra toda forma de violencia, abandono y explotación."

### Enunciado
¿Por qué la Constitución da una protección especial a los niños?

### Opciones
- [x] A) Porque los niños son más vulnerables y necesitan garantías para su desarrollo integral.
  <!-- feedback: La infancia requiere protección especial por su condición de vulnerabilidad. -->
- [ ] B) Porque los niños ya no necesitan derechos al crecer.
  <!-- feedback: Los derechos de la infancia son la base para el ejercicio de los demás derechos en la adultez. -->
- [ ] C) Porque la Constitución prohíbe hablar de los niños.
  <!-- feedback: La Constitución, por el contrario, visibiliza y protege a la infancia. -->
- [ ] D) Porque solo el Estado debe protegerlos, sin участие de la familia.
  <!-- feedback: La Constitución incluye a la familia, la sociedad y el Estado en la protección. -->

### Explicacion Pedagogica
Comprender las razones de la protección especial que la Constitución da a los niños colombianos.

## Question 3 [D5-D6]
**ID:** CO-SOC-5-2026-W09-constitucion-ninos-manual-convivencia-001-MASTERY-bundle-v3
**Bloom:** Apply
**ICFES:** Entorno vivo
**Expected_Success:** 0.76
**Contexto:** En un colegio de Popayán, los estudiantes deben firmar el manual de convivencia al inicio del año escolar.

### Enunciado
¿Cuál es el propósito principal de un manual de convivencia escolar?

### Opciones
- [x] A) Establecer las reglas, los derechos y los deberes de los miembros de la comunidad educativa.
  <!-- feedback: El manual regula la convivencia y orienta la vida escolar. -->
- [ ] B) Reemplazar la Constitución Nacional dentro de la escuela.
  <!-- feedback: El manual no reemplaza la Constitución; la complementa en el ámbito escolar. -->
- [ ] C) Permitir que un solo estudiante imponga las reglas del colegio.
  <!-- feedback: Las reglas se construyen con la comunidad educativa, no por un solo integrante. -->
- [ ] D) Eliminar el derecho a la educación de los estudiantes.
  <!-- feedback: El manual protege el derecho a la educación, no lo elimina. -->

### Explicacion Pedagogica
Reconocer el propósito del manual de convivencia en la organización de la vida escolar.

## Question 4 [D5-D6]
**ID:** CO-SOC-5-2026-W09-constitucion-ninos-manual-convivencia-001-MASTERY-bundle-v4
**Bloom:** Apply
**ICFES:** Entorno vivo
**Expected_Success:** 0.72
**Contexto:** Lee: "Los manuales de convivencia suelen incluir procedimientos para resolver conflictos, mecanismos de representación estudiantil y protocolos de protección."

### Enunciado
Si dos estudiantes tienen un conflicto en el aula, ¿qué camino sugiere el manual de convivencia?

### Opciones
- [x] A) Dialogar, pedir mediación del docente o del comité de convivencia y buscar una solución pacífica.
  <!-- feedback: Los manuales promueven el diálogo y la mediación antes que el castigo. -->
- [ ] B) Resolver el conflicto con golpes para imponer la razón.
  <!-- feedback: La violencia no es un mecanismo válido en un manual de convivencia. -->
- [ ] C) Guardar silencio absoluto y nunca buscar soluciones.
  <!-- feedback: Callar sin buscar solución no resuelve el conflicto y puede agravarlo. -->
- [ ] D) Excluir al estudiante más débil sin escuchar su versión.
  <!-- feedback: Excluir sin proceso es contrario a los derechos y al debido proceso. -->

### Explicacion Pedagogica
Aplicar los procedimientos del manual de convivencia para resolver conflictos escolares de manera pacífica.

## Question 5 [D5-D6]
**ID:** CO-SOC-5-2026-W09-constitucion-ninos-manual-convivencia-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Entorno vivo
**Expected_Success:** 0.66
**Contexto:** En la escuela, los estudiantes eligen un personero estudiantil que defiende sus derechos ante las directivas.

### Enunciado
¿Cuál es la función principal del personero estudiantil?

### Opciones
- [x] A) Promover y defender los derechos de los estudiantes ante la comunidad educativa.
  <!-- feedback: El personero actúa como defensor de los derechos del estudiantado. -->
- [ ] B) Imponer castigos sin escuchar a los compañeros.
  <!-- feedback: El personero defiende derechos, no impone castigos. -->
- [ ] C) Dirigir la clase en lugar del profesor titular.
  <!-- feedback: Quien dirige la clase es el docente, no el personero. -->
- [ ] D) Cobrar dinero a los estudiantes por los servicios del colegio.
  <!-- feedback: El personero no realiza cobros; su labor es pedagógica y de representación. -->

### Explicacion Pedagogica
Reconocer el papel del personero estudiantil en la defensa de los derechos de los niños en el colegio.

## Question 6 [D7-D8]
**ID:** CO-SOC-5-2026-W09-constitucion-ninos-manual-convivencia-001-MASTERY-bundle-v6
**Bloom:** Analyze
**ICFES:** Entorno vivo
**Expected_Success:** 0.58
**Contexto:** Compara: "En el colegio A, los estudiantes pueden proponer reglas al manual de convivencia a través del gobierno escolar. En el colegio B, las reglas solo las imponen los directivos sin participación estudiantil."

### Enunciado
¿Qué diferencia existe entre la convivencia en el colegio A y en el colegio B?

### Opciones
- [x] A) El colegio A promueve la participación estudiantil; el B excluye a los estudiantes del proceso.
  <!-- feedback: La participación de los estudiantes fortalece la convivencia democrática en la escuela. -->
- [ ] B) Los dos colegios funcionan exactamente del mismo modo.
  <!-- feedback: La participación marca una diferencia clara entre ambos modelos. -->
- [ ] C) El colegio B es más democrático porque impone reglas sin consultar.
  <!-- feedback: Imponer sin consultar es contrario a la democracia escolar. -->
- [ ] D) Solo el colegio A tiene manual de convivencia.
  <!-- feedback: Ambos colegios tienen manual, pero con enfoques diferentes. -->

### Explicacion Pedagogica
Comparar modelos de convivencia escolar según la participación de los estudiantes en su construcción.

## Question 7 [D7-D8]
**ID:** CO-SOC-5-2026-W09-constitucion-ninos-manual-convivencia-001-MASTERY-bundle-v7
**Bloom:** Analyze
**ICFES:** Entorno vivo
**Expected_Success:** 0.52
**Contexto:** Lee: "Cuando un estudiante sufre acoso escolar, el manual de convivencia debe activar rutas de protección que incluyen la escucha, el seguimiento y el apoyo psicosocial."

### Enunciado
¿Qué elementos debe tener una ruta de protección contra el acoso escolar?

### Opciones
- [x] A) Atención inmediata, escucha a la víctima, seguimiento del caso y apoyo a las familias.
  <!-- feedback: Las rutas efectivas son integrales: escuchan, acompañan y previenen nuevos casos. -->
- [ ] B) Ignorar el caso para no generar problemas.
  <!-- feedback: Ignorar el acoso escolar lo agrava y vulnera derechos. -->
- [ ] C) Castigar al estudiante acosado sin investigar.
  <!-- feedback: Castigar a la víctima sin investigar es contrario a la protección de derechos. -->
- [ ] D) Culpar únicamente al acosador y expulsarlo sin proceso.
  <!-- feedback: Se debe seguir el debido proceso y considerar la rehabilitación. -->

### Explicacion Pedagogica
Analizar los componentes esenciales de las rutas de protección contra el acoso escolar.

## Question 8 [D9-D10]
**ID:** CO-SOC-5-2026-W09-constitucion-ninos-manual-convivencia-001-MASTERY-bundle-v8
**Bloom:** Evaluate
**ICFES:** Entorno vivo
**Expected_Success:** 0.31
**Contexto:** Lee: 'Conocer los derechos de los niños y las reglas del manual de convivencia permite a los estudiantes exigir respeto, resolver conflictos de forma pacífica y participar activamente en la vida escolar.'

### Enunciado
¿Por qué es importante que los estudiantes conozcan sus derechos y el manual de convivencia?

### Opciones
- [x] A) Porque les permite defender sus derechos, resolver conflictos y participar de forma democrática en la escuela.
  <!-- feedback: Conocer derechos y normas fortalece la ciudadanía desde la infancia. -->
- [ ] B) Porque los derechos no tienen relación con la vida escolar.
  <!-- feedback: Los derechos de los niños se aplican directamente en la escuela. -->
- [ ] C) Porque conocer el manual elimina cualquier conflicto en la escuela.
  <!-- feedback: El manual ayuda a manejar los conflictos, aunque no los elimina por completo. -->
- [ ] D) Porque solo sirve para sancionar a los estudiantes.
  <!-- feedback: El manual no es solo sancionatorio, sino también formativo y protector. -->

### Explicacion Pedagogica
Evaluar la importancia de conocer los derechos infantiles y el manual de convivencia para la vida escolar.