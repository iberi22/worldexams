---
id: "CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle"
country: "colombia"
grado: 6
asignatura: "ciencias-naturales"
tema: "ecosistemas-colombianos"
periodo: "weekly"
week: "W02"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 10
bundle_size: 10
alignment: "DBA MEN Colombia / Saber 11"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: Ecosistemas Colombianos - Grado 6

Este bundle contiene 10 preguntas sobre **ecosistemas-colombianos** para grado 6, alineadas con los DBA del MEN Colombia.

## Question 1 [D3-D4]
**ID:** CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Entorno vivo
**Expected_Success:** 0.85
**Contexto:** En clase en Bogotá, los estudiantes exploran los ecosistemas colombianos.
### Enunciado
Un ecosistema está formado por:
### Opciones
- [x] A) Organismos vivos (factores bióticos) y componentes no vivos (factores abióticos) que interactúan.
  <!-- feedback: Correcto. Bióticos + abióticos. -->
- [ ] B) Solo animales.
  <!-- feedback: Incorrecto. Incluye plantas, hongos y otros. -->
- [ ] C) Solo el suelo y el agua.
  <!-- feedback: Incorrecto. Faltan los organismos. -->
- [ ] D) Solo plantas y rocas.
  <!-- feedback: Incorrecto. Incluye también animales y microorganismos. -->
### Explicacion Pedagogica
Colombia es uno de los países más megadiversos del mundo gracias a su variedad de ecosistemas.

## Question 2 [D3-D4]
**ID:** CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle-v2
**Bloom:** Remember
**ICFES:** Entorno vivo
**Expected_Success:** 0.85
**Contexto:** En clase en Medellín, se diferencia bioma y ecosistema.
### Enunciado
Un bioma se diferencia de un ecosistema porque:
### Opciones
- [x] A) El bioma agrupa ecosistemas con condiciones climáticas similares a gran escala.
  <!-- feedback: Correcto. Ej.: bioma selva tropical. -->
- [ ] B) El bioma es más pequeño que un ecosistema.
  <!-- feedback: Incorrecto. Es más extenso. -->
- [ ] C) El ecosistema se limita a zonas urbanas.
  <!-- feedback: Incorrecto. Puede estar en cualquier lugar. -->
- [ ] D) Son sinónimos exactos.
  <!-- feedback: Incorrecto. Tienen escalas distintas. -->
### Explicacion Pedagogica
Los biomas se definen principalmente por el clima y la vegetación dominante.

## Question 3 [D3-D4]
**ID:** CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle-v3
**Bloom:** Understand
**ICFES:** Entorno vivo
**Expected_Success:** 0.84
**Contexto:** En clase en Cali, se identifica la selva húmeda tropical.
### Enunciado
La Amazonía colombiana se caracteriza por:
### Opciones
- [x] A) Alta biodiversidad, clima cálido y húmedo, y ríos caudalosos.
  <!-- feedback: Correcto. Es selva húmeda tropical. -->
- [ ] B) Climas fríos y nevados permanentes.
  <!-- feedback: Incorrecto. Eso corresponde a los Andes altos. -->
- [ ] C) Suelos desérticos.
  <!-- feedback: Incorrecto. Tiene alta humedad. -->
- [ ] D) Escasa vegetación.
  <!-- feedback: Incorrecto. Es muy boscosa. -->
### Explicacion Pedagogica
La Amazonía alberga cerca del 10 % de la biodiversidad mundial.

## Question 4 [D3-D4]
**ID:** CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle-v4
**Bloom:** Understand
**ICFES:** Entorno vivo
**Expected_Success:** 0.84
**Contexto:** En clase en Pereira, se estudia el páramo.
### Enunciado
El ecosistema de páramo se caracteriza por:
### Opciones
- [x] A) Vegetación baja como frailejones, baja temperatura y agua en frailejones y lagunas.
  <!-- feedback: Correcto. Es un ecosistema altoandino. -->
- [ ] B) Selvas densas y clima cálido.
  <!-- feedback: Incorrecto. Eso es selva tropical. -->
- [ ] C) Dunas y clima desértico.
  <!-- feedback: Incorrecto. Eso es desierto. -->
- [ ] D) Sabanas y clima cálido.
  <!-- feedback: Incorrecto. Eso es la Orinoquía. -->
### Explicacion Pedagogica
Los páramos almacenan y regulan el agua, por eso son fundamentales para el recurso hídrico.

## Question 5 [D5-D6]
**ID:** CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Entorno vivo
**Expected_Success:** 0.75
**Contexto:** En clase en Tunja, se estudian cadenas alimentarias.
### Enunciado
En un ecosistema, los productores son aquellos organismos que:
### Opciones
- [x] A) Fabrican su propio alimento mediante fotosíntesis o quimiosíntesis.
  <!-- feedback: Correcto. Son las plantas, algas y algunas bacterias. -->
- [ ] B) Se alimentan de otros seres vivos.
  <!-- feedback: Incorrecto. Son consumidores. -->
- [ ] C) Descomponen materia orgánica muerta.
  <!-- feedback: Incorrecto. Son descomponedores. -->
- [ ] D) Parasitan a otros organismos.
  <!-- feedback: Incorrecto. Son parásitos. -->
### Explicacion Pedagogica
Las plantas son la base de las cadenas tróficas al transformar la energía solar.

## Question 6 [D5-D6]
**ID:** CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle-v6
**Bloom:** Apply
**ICFES:** Entorno vivo
**Expected_Success:** 0.75
**Contexto:** En clase en Manizales, se clasifican consumidores.
### Enunciado
Un jaguar que caza un venado en la selva es considerado:
### Opciones
- [x] A) Consumidor secundario (carnívoro).
  <!-- feedback: Correcto. Se alimenta de herbívoros. -->
- [ ] B) Productor primario.
  <!-- feedback: Incorrecto. No fotosintetiza. -->
- [ ] C) Descomponedor.
  <!-- feedback: Incorrecto. No descompone materia. -->
- [ ] D) Consumidor primario (herbívoro).
  <!-- feedback: Incorrecto. No come plantas. -->
### Explicacion Pedagogica
Los consumidores secundarios se alimentan de consumidores primarios como el venado.

## Question 7 [D5-D6]
**ID:** CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle-v7
**Bloom:** Apply
**ICFES:** Entorno vivo
**Expected_Success:** 0.74
**Contexto:** En Armenia, se estudian los manglares del Pacífico.
### Enunciado
Los manglares del Pacífico colombiano cumplen una función importante porque:
### Opciones
- [x] A) Protegen la costa, albergan fauna y fijan carbono.
  <!-- feedback: Correcto. Son ecosistemas estratégicos. -->
- [ ] B) Solo producen madera.
  <!-- feedback: Incorrecto. Tienen más funciones. -->
- [ ] C) Impiden el desarrollo pesquero.
  <!-- feedback: Incorrecto. Lo protegen. -->
- [ ] D) No aportan a la biodiversidad.
  <!-- feedback: Incorrecto. Son muy ricos. -->
### Explicacion Pedagogica
Los manglares son cruciales para la reproducción de especies marinas.

## Question 8 [D7-D8]
**ID:** CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle-v8
**Bloom:** Analyze
**ICFES:** Entorno vivo
**Expected_Success:** 0.65
**Contexto:** En clase en Popayán, se analiza la deforestación.
### Enunciado
La deforestación afecta los ecosistemas porque:
### Opciones
- [x] A) Reduce la biodiversidad, altera el ciclo del agua y aumenta el calentamiento global.
  <!-- feedback: Correcto. Tiene múltiples consecuencias. -->
- [ ] B) Aumenta la producción agrícola.
  <!-- feedback: Incorrecto. Puede empobrecer el suelo. -->
- [ ] C) Disminuye el efecto invernadero.
  <!-- feedback: Incorrecto. Lo aumenta. -->
- [ ] D) Protege a las especies endémicas.
  <!-- feedback: Incorrecto. Las pone en riesgo. -->
### Explicacion Pedagogica
La deforestación es una de las principales amenazas a la biodiversidad colombiana.

## Question 9 [D9-D10]
**ID:** CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle-v9
**Bloom:** Evaluate
**ICFES:** Entorno vivo
**Expected_Success:** 0.50
**Contexto:** En un proyecto en Bucaramanga, se discute la conservación.
### Enunciado
¿Qué estrategia es más efectiva para conservar los ecosistemas colombianos?
### Opciones
- [ ] A) Solo declarar parques sin gestionarlos.
  <!-- feedback: Incorrecto. La gestión es clave. -->
- [x] B) Combinar áreas protegidas, educación ambiental y uso sostenible.
  <!-- feedback: Correcto. Requiere estrategias integrales. -->
- [ ] C) Prohibir todo uso humano.
  <!-- feedback: Incorrecto. Las comunidades son parte del ecosistema. -->
- [ ] D) Reemplazar bosques por monocultivos.
  <!-- feedback: Incorrecto. Reduce biodiversidad. -->
### Explicacion Pedagogica
La conservación efectiva combina ciencia, política y participación comunitaria.

## Question 10 [D9-D10]
**ID:** CO-CN-6-2026-W02-ecosistemas-colombianos-001-MASTERY-bundle-v10
**Bloom:** Evaluate
**ICFES:** Entorno vivo
**Expected_Success:** 0.50
**Contexto:** En un cierre en Medellín, se reflexiona sobre la riqueza nacional.
### Enunciado
¿Por qué Colombia es considerada un país megadiverso?
### Opciones
- [x] A) Por su variedad de ecosistemas, alta biodiversidad y posición geográfica.
  <!-- feedback: Correcto. Factores físicos y biológicos. -->
- [ ] B) Por tener muchas ciudades.
  <!-- feedback: Incorrecto. La diversidad no depende de ciudades. -->
- [ ] C) Por su idioma oficial únicamente.
  <!-- feedback: Incorrecto. La diversidad es biológica y cultural. -->
- [ ] D) Por su equipo de fútbol.
  <!-- feedback: Incorrecto. No tiene relación. -->
### Explicacion Pedagogica
Colombia ocupa los primeros lugares en biodiversidad de aves, anfibios y plantas.
