---
id: "CO-CIE-6-2026-W20-sistema-respiratorio-001-MASTERY-bundle"
country: "colombia"
grado: 6
asignatura: "ciencias-naturales"
tema: "sistema-respiratorio"
periodo: 5
week: 20
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 10
bundle_size: 10
alignment: "DBA MEN + Estándares Básicos Grado 6"
license: "FREE"
tier: "legacy"

creador: "Jules-Agent"
---


# Weekly Pack W20 — Sistema Respiratorio

**Grado:** 6° | **Periodo:** 5 | **Semana:** 20 | **Año:** 2026

---

## Question 1 [D1]

**ID:** `CO-CIE-6-2026-W20-sistema-respiratorio-001-v1`
**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Comunicación y Representación
**Contexto:** En Bogotá, inician el estudio del sistema respiratorio.

### Enunciado
¿Cuál es la función principal del sistema respiratorio?

### Opciones
- [ ] A) Bombear sangre <!-- feedback: Es función del circulatorio. -->
- [ ] B) Digerir alimentos <!-- feedback: Es función del digestivo. -->
- [x] C) Intercambiar gases (O2 y CO2) entre el cuerpo y el ambiente <!-- feedback: Correcto. -->
- [ ] D) Filtrar desechos <!-- feedback: Es función del excretor. -->

### Explicación Pedagógica
El sistema respiratorio captura oxígeno del aire y elimina dióxido de carbono. Cada célula del cuerpo necesita oxígeno para la respiración celular (producción de energía) y produce CO2 como desecho. Órganos: fosas nasales, faringe, laringe, tráquea, bronquios, bronquiolos y alvéolos.

---

## Question 2 [D1]

**ID:** `CO-CIE-6-2026-W20-sistema-respiratorio-002-v1`
**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Comunicación y Representación
**Contexto:** En Medellín, identifican los órganos respiratorios.

### Enunciado
¿Dónde ocurre el intercambio de gases en los pulmones?

### Opciones
- [ ] A) En la tráquea <!-- feedback: La tráquea conduce el aire. -->
- [ ] B) En los bronquios <!-- feedback: Conducen el aire a los pulmones. -->
- [x] C) En los alvéolos pulmonares <!-- feedback: Correcto. -->
- [ ] D) En la laringe <!-- feedback: La laringe contiene las cuerdas vocales. -->

### Explicación Pedagógica
Los alvéolos son pequeños sacos (300-500 millones) rodeados de capilares sanguíneos. El oxígeno pasa del alvéolo a la sangre, y el CO2 de la sangre al alvéolo para ser exhalado. La superficie total de intercambio es ~70 m² (cancha de tenis).

---

## Question 3 [D2]

**ID:** `CO-CIE-6-2026-W20-sistema-respiratorio-003-v1`
**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comunicación y Representación
**Contexto:** En Cali, estudian la mecánica respiratoria.

### Enunciado
¿Qué músculo es el principal responsable de la inhalación?

### Opciones
- [ ] A) El bíceps <!-- feedback: Es del brazo. -->
- [ ] B) El corazón <!-- feedback: Es del circulatorio, no músculo esquelético. -->
- [x] C) El diafragma <!-- feedback: Correcto. -->
- [ ] D) El recto abdominal <!-- feedback: Ayuda en la exhalación forzada. -->

### Explicación Pedagógica
El diafragma es un músculo en forma de domo debajo de los pulmones. Al inhalar, se contrae y aplana, expandiendo la cavidad torácica y creando presión negativa que succiona aire. Al exhalar, se relaja y el aire sale pasivamente.

---

## Question 4 [D2]

**ID:** `CO-CIE-6-2026-W20-sistema-respiratorio-004-v1`
**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comunicación y Representación
**Contexto:** En Barranquilla, explican la diferencia entre respiración pulmonar y celular.

### Enunciado
¿Cuál es la diferencia entre respiración pulmonar y respiración celular?

### Opciones
- [ ] A) Son lo mismo <!-- feedback: Son procesos diferentes. -->
- [ ] B) La pulmonar ocurre en las células <!-- feedback: Al revés. -->
- [x] C) La pulmonar intercambia gases en los pulmones; la celular usa oxígeno en las mitocondrias para producir energía <!-- feedback: Correcto. -->
- [ ] D) La celular ocurre en la nariz <!-- feedback: Ocurre en las mitocondrias. -->

### Explicación Pedagógica
La respiración pulmonar (ventilación) mueve aire dentro y fuera de los pulmones. La respiración celular ocurre en las mitocondrias: glucosa + O2 → CO2 + H2O + energía (ATP). El CO2 producido sale por la respiración pulmonar. Están conectadas: sin oxígeno no hay energía celular.

---

## Question 5 [D3]

**ID:** `CO-CIE-6-2026-W20-sistema-respiratorio-005-v1`
**Bloom:** Apply
**Expected_Success:** 0.75
**ICFES:** Formulación y Ejecución
**Contexto:** En Bucaramanga, aplican sobre contaminación del aire.

### Enunciado
¿Por qué la contaminación del aire en Bogotá afecta más a niños y ancianos?

### Opciones
- [ ] A) Porque respiran más lento <!-- feedback: Los niños respiran más rápido. -->
- [ ] B) Porque sus pulmones no se enferman <!-- feedback: Son más vulnerables. -->
- [x] C) Porque sus sistemas respiratorio e inmunológico son más vulnerables, y tienen mayor frecuencia respiratoria <!-- feedback: Correcto. -->
- [ ] D) Porque no sienten dolor <!-- feedback: Sí sienten molestias. -->

### Explicación Pedagógica
Los niños respiran más veces por minuto y sus pulmones están en desarrollo. Los ancianos tienen menor capacidad pulmonar y defensas más débiles. Bogotá frecuentemente supera los límites de material particulado (PM2.5) recomendados por la OMS, afectando la salud respiratoria.

---

## Question 6 [D3]

**ID:** `CO-CIE-6-2026-W20-sistema-respiratorio-006-v1`
**Bloom:** Apply
**Expected_Success:** 0.75
**ICFES:** Formulación y Ejecución
**Contexto:** En Cartagena, aplican sobre RCP.

### Enunciado
En RCP, ¿por qué se debe inclinar la cabeza hacia atrás antes de dar respiración boca a boca?

### Opciones
- [ ] A) Para ver mejor la cara <!-- feedback: No es la razón. -->
- [ ] B) Para que la persona no vea <!-- feedback: No. -->
- [x] C) Para abrir la vía aérea, evitando que la lengua bloquee el paso del aire <!-- feedback: Correcto. -->
- [ ] D) Para que el aire entre más frío <!-- feedback: No. -->

### Explicación Pedagógica
La maniobra de inclinación de cabeza y elevación del mentón abre la vía aérea al desplazar la lengua hacia adelante. En una persona inconsciente, la lengua puede caer hacia atrás y obstruir la tráquea. Esto es crítico para que el aire llegue a los pulmones.

---

## Question 7 [D4]

**ID:** `CO-CIE-6-2026-W20-sistema-respiratorio-007-v1`
**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Razonamiento y Argumentación
**Contexto:** En Pereira, analizan el asma.

### Enunciado
¿Por qué una persona con asma tiene dificultad para respirar durante una crisis?

### Opciones
- [ ] A) Los pulmones se llenan de líquido <!-- feedback: Eso es el edema pulmonar. -->
- [ ] B) Los alvéolos se rompen <!-- feedback: Eso es el enfisema. -->
- [x] C) Los bronquios se inflaman y estrechan por contracción del músculo liso, reduciendo el flujo de aire <!-- feedback: Correcto. -->
- [ ] D) El diafragma se paraliza <!-- feedback: No es el mecanismo. -->

### Explicación Pedagógica
El asma es una enfermedad inflamatoria crónica de las vías respiratorias. En una crisis, los bronquios se estrechan por: (1) inflamación de la mucosa, (2) contracción del músculo liso bronquial, (3) producción excesiva de moco. Factores desencadenantes: alérgenos, ejercicio, aire frío, estrés. Tratamiento: broncodilatadores (salbutamol) que relajan el músculo bronquial.

---

## Question 8 [D4]

**ID:** `CO-CIE-6-2026-W20-sistema-respiratorio-008-v1`
**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Razonamiento y Argumentación
**Contexto:** En Manizales, analizan los efectos del tabaco.

### Enunciado
¿Por qué fumar causa enfisema pulmonar?

### Opciones
- [ ] A) Porque el humo calienta los pulmones <!-- feedback: El calor no es el mecanismo principal. -->
- [ ] B) Porque el humo engrosa la sangre <!-- feedback: No es relevante. -->
- [x] C) Porque los químicos del tabaco destruyen las paredes de los alvéolos, reduciendo la superficie de intercambio de gases <!-- feedback: Correcto. -->
- [ ] D) Porque el humo tapa la tráquea <!-- feedback: No. -->

### Explicación Pedagógica
El enfisema es una enfermedad pulmonar obstructiva crónica (EPOC) donde los alvéolos se destruyen, reduciendo la superficie para el intercambio de gases. El humo del tabaco contiene >7,000 químicos (70 cancerígenos). En Colombia, ~13% de los adultos fuma, causando miles de muertes prevenibles al año.

---

## Question 9 [D5]

**ID:** `CO-CIE-6-2026-W20-sistema-respiratorio-009-v1`
**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Razonamiento y Argumentación
**Contexto:** En Cúcuta, evalúan estrategias contra la contaminación.

### Enunciado
¿Cuál es la mejor estrategia para reducir la contaminación del aire en las ciudades colombianas?

### Opciones
- [ ] A) Que todos usen tapabocas <!-- feedback: Protege pero no soluciona. -->
- [ ] B) Plantar árboles en todas partes <!-- feedback: Ayuda pero no es suficiente. -->
- [x] C) Reducir emisiones vehiculares (mejorar transporte público, promover bicicletas, revisión técnico-mecánica estricta, tranvías eléctricos) <!-- feedback: Correcto. -->
- [ ] D) Cerrar las fábricas <!-- feedback: Es inviable económicamente. -->

### Explicación Pedagógica
En ciudades como Bogotá, Medellín y Cali, los vehículos son la principal fuente de contaminación (70% de emisiones). Soluciones: (1) transporte público eléctrico (Transmilenio, Metro, tranvías), (2) ciclorrutas y cultura de bicicleta, (3) revisión técnico-mecánica rigurosa, (4) restricción vehicular (pico y placa), (5) zonas de bajas emisiones.

---

## Question 10 [D5]

**ID:** `CO-CIE-6-2026-W20-sistema-respiratorio-010-v1`
**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Razonamiento y Argumentación
**Contexto:** En Ibagué, evalúan hábitos respiratorios.

### Enunciado
¿Qué hábito protege MÁS la salud respiratoria?

### Opciones
- [ ] A) Hacer gárgaras con agua salada <!-- feedback: Ayuda para garganta pero no previene. -->
- [ ] B) Respirar por la boca al hacer ejercicio <!-- feedback: Respirar por nariz filtra mejor el aire. -->
- [x] C) No fumar, hacer ejercicio aeróbico y evitar ambientes contaminados <!-- feedback: Correcto. -->
- [ ] D) Usar ambientadores en casa <!-- feedback: Algunos ambientadores contienen químicos irritantes. -->

### Explicación Pedagógica
Para proteger los pulmones: (1) no fumar ni vapear (el vaping también daña los pulmones), (2) ejercicio aeróbico fortalece la capacidad pulmonar, (3) evitar lugares contaminados, (4) vacunarse contra influenza y COVID-19, (5) mantener buena postura para que los pulmones se expandan bien, (6) lavado de manos para prevenir infecciones respiratorias.

---

[//]: # (QUALITY_REVIEW)
[//]: # (STATUS: LEGACY - FREE USE)
[//]: # (SCORE: N/A - Pre-QR era)
