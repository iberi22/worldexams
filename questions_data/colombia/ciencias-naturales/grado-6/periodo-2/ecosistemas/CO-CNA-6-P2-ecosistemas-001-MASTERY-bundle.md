---
id: "CO-CNA-6-P2-ecosistemas-001-MASTERY-bundle"
country: "colombia"
grado: 6
asignatura: "ciencias-naturales"
tema: "ecosistemas"
periodo: 2
protocol_version: "5.2"
bundle_index: 1
bundle_size: 10
alignment: "ICFES Saber + DBA"
modern_context: true
distractor_profile: "plausible_peer_set"
calibration:
  expected_success_rate: 0.60
  discrimination_index_target: ">= 0.22"
  simulated_responses: 100
rubric_baseline: "ecologia, ecosistemas, factores_bioticos_abioticos, cadenas_troficas"
bundle_type: "weekly"
total_questions: 10
year: 2026
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---


# Bundle Mastery: Ecosistemas y Relaciones Bióticas

Este bundle evalúa la comprensión de los componentes de un ecosistema, la distinción entre factores bióticos y abióticos, y el flujo de energía a través de las cadenas y redes tróficas.

---

## Question 1 (Difficulty D1)

**ID:** `CO-CNA-6-P2-ecosistemas-001-MASTERY-v1`
**Bloom:** Remember
**ICFES:** Uso comprensivo del conocimiento científico

### Enunciado
¿Qué es un **ecosistema**?

### Opciones
- [ ] A) Un grupo de animales de la misma especie que viven juntos. <!-- feedback: Incorrecto. Esto es una población. -->
- [ ] B) El clima de un lugar específico. <!-- feedback: Incorrecto. El clima es solo un factor del ecosistema. -->
- [x] C) El conjunto formado por los seres vivos, el medio físico y las relaciones entre ellos. <!-- feedback: Correcto. Incluye tanto lo vivo como lo no vivo en interacción constante. -->
- [ ] D) Una colección de plantas en un invernadero. <!-- feedback: Incorrecto. Es un ejemplo limitado, el ecosistema es un concepto más amplio. -->

### Explicación Pedagógica
La ecología estudia el ecosistema como la unidad funcional donde la vida interactúa con su entorno físico.

---

## Question 2 (Difficulty D1)

**ID:** `CO-CNA-6-P2-ecosistemas-001-MASTERY-v2`
**Bloom:** Remember
**ICFES:** Uso comprensivo del conocimiento científico

### Enunciado
¿Cuál de los siguientes es un **factor abiótico** de un ecosistema?

### Opciones
- [ ] A) Las bacterias. <!-- feedback: Incorrecto. Las bacterias son seres vivos (bióticos). -->
- [ ] B) Los árboles de guayacán. <!-- feedback: Incorrecto. Las plantas son seres vivos (bióticos). -->
- [x] C) La luz solar. <!-- feedback: Correcto. Los factores abióticos son los elementos no vivos (luz, agua, suelo, aire). -->
- [ ] D) Los pajaritos. <!-- feedback: Incorrecto. Los animales son seres vivos (bióticos). -->

### Explicación Pedagógica
Los factores abióticos determinan las condiciones físicas y químicas que permiten o limitan la vida en un lugar.

---

## Question 3 (Difficulty D2)

**ID:** `CO-CNA-6-P2-ecosistemas-001-MASTERY-v3`
**Bloom:** Understand
**ICFES:** Uso comprensivo del conocimiento científico

### Enunciado
¿Cómo se les llama a los organismos que fabrican su propio alimento mediante la fotosíntesis?

### Opciones
- [ ] A) Consumidores. <!-- feedback: Incorrecto. Estos deben comer a otros seres vivos. -->
- [x] B) Productores. <!-- feedback: Correcto. Son la base de la cadena alimentaria (plantas, algas). -->
- [ ] C) Descomponedores. <!-- feedback: Incorrecto. Estos se alimentan de materia orgánica muerta. -->
- [ ] D) Depredadores. <!-- feedback: Incorrecto. Los depredadores son consumidores que cazan. -->

### Explicación Pedagógica
Los productores o autótrofos transforman la energía inorgánica en energía orgánica aprovechable por el resto del ecosistema.

---

## Question 4 (Difficulty D2)

**ID:** `CO-CNA-6-P2-ecosistemas-001-MASTERY-v4`
**Bloom:** Understand
**ICFES:** Explicación de fenómenos

### Contexto
Un hongo crece sobre un tronco caído, descomponiendo la madera.

### Enunciado
¿Qué función cumple el hongo en este ecosistema?

### Opciones
- [ ] A) Productor. <!-- feedback: Incorrecto. Los hongos no hacen fotosíntesis. -->
- [ ] B) Consumidor primario. <!-- feedback: Incorrecto. No come plantas vivas. -->
- [x] C) Descomponedor. <!-- feedback: Correcto. Recicla los nutrientes de la materia muerta para devolverlos al suelo. -->
- [ ] D) Competidor. <!-- feedback: Incorrecto. Su función principal descrita es la descomposición. -->

### Explicación Pedagógica
Los descomponedores cierran el ciclo de la materia en el ecosistema, permitiendo que los nutrientes vuelvan a estar disponibles para los productores.

---

## Question 5 (Difficulty D3)

**ID:** `CO-CNA-6-P2-ecosistemas-001-MASTERY-v5`
**Bloom:** Apply
**ICFES:** Uso comprensivo del conocimiento científico

### Contexto
En una cadena alimentaria tenemos: **Pasto -> Grillo -> Rana -> Serpiente**.

### Enunciado
¿Quién es el **consumidor secundario** en esta cadena?

### Opciones
- [ ] A) El grillo. <!-- feedback: Incorrecto. El grillo es el consumidor primario (come pasto). -->
- [x] B) La rana. <!-- feedback: Correcto. La rana se come al grillo (consumidor primario), por lo tanto es secundario. -->
- [ ] C) La serpiente. <!-- feedback: Incorrecto. La serpiente es el consumidor terciario. -->
- [ ] D) El pasto. <!-- feedback: Incorrecto. El pasto es el productor. -->

### Explicación Pedagógica
El nivel trófico se define por la posición que ocupa un organismo en la transferencia de energía.

---

## Question 6 (Difficulty D3)

**ID:** `CO-CNA-6-P2-ecosistemas-001-MASTERY-v6`
**Bloom:** Analyze
**ICFES:** Explicación de fenómenos

### Contexto
Un páramo en Colombia es un ecosistema de alta montaña con bajas temperaturas y mucha humedad.

### Enunciado
¿Qué pasaría con las plantas del páramo (como el frailejón) si la temperatura global aumenta demasiado?

### Opciones
- [ ] A) Crecerían más rápido y serían más grandes. <!-- feedback: Incorrecto. Están adaptadas al frío, el calor extremo las dañaría. -->
- [x] B) Podrían morir o tener dificultades para sobrevivir al alterarse su clima ideal. <!-- feedback: Correcto. Las especies de páramo son muy sensibles a los cambios de temperatura. -->
- [ ] C) Se convertirían en cactus automáticamente. <!-- feedback: Incorrecto. La adaptación biológica no ocurre de forma instantánea. -->
- [ ] D) No pasaría nada, el clima no afecta a las plantas. <!-- feedback: Incorrecto. El clima es un factor abiótico fundamental para la vida. -->

### Explicación Pedagógica
El cambio climático afecta el equilibrio de los ecosistemas, especialmente de aquellos con especies altamente especializadas.

---

## Question 7 (Difficulty D4)

**ID:** `CO-CNA-6-P2-ecosistemas-001-MASTERY-v7`
**Bloom:** Analyze
**ICFES:** Indagación

### Contexto
Dos aves de la misma especie pelean por el mejor lugar para construir su nido en un árbol.

### Enunciado
¿Qué tipo de relación biótica se está presentando?

### Opciones
- [ ] A) Mutualismo. <!-- feedback: Incorrecto. En el mutualismo ambas se beneficiarían sin pelear. -->
- [x] B) Competencia intraespecífica. <!-- feedback: Correcto. "Intra" significa dentro de la misma especie; compiten por un recurso limitado (el espacio). -->
- [ ] C) Depredación. <!-- feedback: Incorrecto. No se están comiendo una a la otra. -->
- [ ] D) Comensalismo. <!-- feedback: Incorrecto. Una se beneficia y la otra no se ve afectada, pero aquí hay conflicto. -->

### Explicación Pedagógica
La competencia ocurre cuando los recursos (alimento, territorio, pareja) no son suficientes para todos los individuos.

---

## Question 8 (Difficulty D4)

**ID:** `CO-CNA-6-P2-ecosistemas-001-MASTERY-v8`
**Bloom:** Analyze
**ICFES:** Explicación de fenómenos

### Contexto
Las abejas obtienen néctar de las flores y, a cambio, ayudan a la planta transportando el polen para su reproducción.

### Enunciado
¿Cómo se clasifica esta relación biótica donde **ambas especies se benefician**?

### Opciones
- [ ] A) Parasitismo. <!-- feedback: Incorrecto. En el parasitismo una especie se beneficia y la otra se perjudica. -->
- [x] B) Mutualismo. <!-- feedback: Correcto. Es una relación de ayuda mutua indispensable para muchas especies. -->
- [ ] C) Competencia. <!-- feedback: Incorrecto. No están peleando por recursos, están colaborando. -->
- [ ] D) Amensalismo. <!-- feedback: Incorrecto. Esto es cuando una especie perjudica a otra sin querer. -->

### Explicación Pedagógica
El mutualismo es una interacción biológica vital para la biodiversidad y la producción de alimentos (polinización).

---

## Question 9 (Difficulty D5)

**ID:** `CO-CNA-6-P2-ecosistemas-001-MASTERY-v9`
**Bloom:** Evaluate
**ICFES:** Indagación

### Contexto
En un lago se introducen peces de una especie extraña que se comen a todos los peces pequeños nativos.

### Enunciado
¿Cuál es el efecto más probable en la cadena alimentaria del lago?

### Opciones
- [ ] A) Los niveles superiores (aves que comen peces) tendrán más comida. <!-- feedback: Incorrecto. Al acabarse los nativos, las aves eventualmente se quedarán sin alimento. -->
- [x] B) Se produce un desequilibrio que puede llevar a la extinción de especies locales. <!-- feedback: Correcto. Las especies invasoras alteran las redes tróficas naturales. -->
- [ ] C) El agua se volverá más limpia. <!-- feedback: Incorrecto. No hay relación directa con la limpieza del agua. -->
- [ ] D) Todas las plantas del lago morirán de hambre. <!-- feedback: Incorrecto. Las plantas son productoras, no dependen de comer peces. -->

### Explicación Pedagógica
La introducción de especies exóticas es una de las principales causas de pérdida de biodiversidad en el mundo.

---

## Question 10 (Difficulty D5)

**ID:** `CO-CNA-6-P2-ecosistemas-001-MASTERY-v10`
**Bloom:** Evaluate
**ICFES:** Explicación de fenómenos

### Enunciado
¿Por qué se dice que el flujo de energía en un ecosistema es **unidireccional** (en un solo sentido)?

### Opciones
- [ ] A) Porque la energía vuelve siempre al Sol. <!-- feedback: Incorrecto. La energía del sol se usa, pero no regresa a él. -->
- [x] B) Porque la energía se pierde en forma de calor en cada nivel trófico y no puede ser reutilizada por los niveles anteriores. <!-- feedback: Correcto. Los productores captan la energía y esta fluye hacia los consumidores, disipándose. -->
- [ ] C) Porque los animales corren en una sola dirección. <!-- feedback: Incorrecto. Es una interpretación literal y absurda del término "unidireccional". -->
- [ ] D) Porque solo las plantas usan energía. <!-- feedback: Incorrecto. Todos los seres vivos requieren energía para sus funciones vitales. -->

### Explicación Pedagógica
A diferencia de la materia (que es cíclica), la energía entra al ecosistema desde el sol y se gasta conforme los organismos viven y se alimentan.
