import os

CN_P4 = r"""---
id: "CO-CN-10-2026-P4-comprehensive-001-MASTERY"
country: "colombia"
grado: 10
asignatura: "ciencias-naturales"
tema: "genetica-poblaciones, electroquimica, optica-fisica"
periodo: 4
protocol_version: "5.2"
bundle_index: 1
bundle_size: 20
alignment: "DBA MEN + Pre-ICFES"
modern_context: true
distractor_profile: "plausible_peer_set"
---
# Bundle MASTERY: Genetica de Poblaciones, Electroquimica, Optica Fisica (P4)

## Question 1 (D3)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v1`
**Bloom:** Remember | **ICFES:** Uso comprensivo del conocimiento cientifico
**Context:** En clase de biologia en la Universidad del Cauca, se estudia genetica de poblaciones.
**Enunciado:** Que establece el principio de Hardy-Weinberg?
**Options:**
- [ ] A) Que los alelos dominantes siempre aumentan su frecuencia. <!-- feedback: Incorrecto. Hardy-Weinberg establece condiciones bajo las cuales las frecuencias alelicas permanecen constantes. -->
- [x] B) Que en ausencia de fuerzas evolutivas, las frecuencias alelicas permanecen constantes. <!-- feedback: Correcto. El principio HW establece que las frecuencias alelicas y genotipicas se mantienen constantes si no hay mutacion, seleccion, deriva, migracion ni apareamiento no aleatorio. -->
- [ ] C) Que la seleccion natural siempre aumenta la fitness. <!-- feedback: Incorrecto. Eso es una consecuencia de la seleccion natural, no el principio HW. -->
- [ ] D) Que las poblaciones pequenas tienen mayor diversidad genetica. <!-- feedback: Incorrecto. Las poblaciones pequenas tienden a perder diversidad por deriva genetica. -->

## Question 2 (D3)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v2`
**Bloom:** Remember | **ICFES:** Uso comprensivo del conocimiento cientifico
**Context:** En clase de optica en el Colegio Montessori de Bucaramanga.
**Enunciado:** Una lente que hace converger los rayos de luz se llama:
**Options:**
- [x] A) Lente convergente (convexa). <!-- feedback: Correcto. Las lentes convexas (mas gruesas en el centro) hacen converger los rayos paralelos. -->
- [ ] B) Lente divergente (concava). <!-- feedback: Incorrecto. Las lentes divergentes separan los rayos de luz. -->
- [ ] C) Lente cilindrica. <!-- feedback: Incorrecto. Las lentes cilindricas enfocan en un solo eje. -->
- [ ] D) Espejo plano. <!-- feedback: Incorrecto. Un espejo plano refleja la luz, no la refracta a traves de el. -->

## Question 3 (D4)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v3`
**Bloom:** Apply | **ICFES:** Explicacion de fenomenos
**Context:** En el laboratorio de quimica del Colegio INEM de Pereira, se construye una celda electroquimica con Zn/Zn2+ y Cu/Cu2+. E(Zn2+/Zn) = -0.76 V, E(Cu2+/Cu) = +0.34 V.
**Enunciado:** Cual es la fem estandar de la celda?
**Options:**
- [ ] A) -0.42 V <!-- feedback: Incorrecto. Ecelda = Ecathodo - Eanodo. El cathodo es Cu (reduccion) y el anodo Zn (oxidacion). -->
- [ ] B) +0.42 V <!-- feedback: Incorrecto. Ecelda = 0.34 - (-0.76) = 1.10 V. -->
- [x] C) +1.10 V <!-- feedback: Correcto. Ecelda = E(cathodo) - E(anodo) = 0.34 - (-0.76) = 1.10 V. -->
- [ ] D) -1.10 V <!-- feedback: Incorrecto. La celda voltaica tiene fem positiva porque la reaccion es espontanea. -->

## Question 4 (D4)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v4`
**Bloom:** Understand | **ICFES:** Explicacion de fenomenos
**Context:** En un tanque de agua en la azotea de un colegio en Sincelejo, un lapiz parece quebrado al sumergirse en agua.
**Enunciado:** Este fenomeno se debe a:
**Options:**
- [ ] A) Reflexion difusa. <!-- feedback: Incorrecto. La reflexion difusa dispersa la luz en todas direcciones. -->
- [ ] B) Difraccion de la luz. <!-- feedback: Incorrecto. La difraccion es el rodeo de obstaculos. -->
- [x] C) Refraccion de la luz. <!-- feedback: Correcto. La luz cambia de velocidad al pasar del agua al aire (diferentes indices de refraccion), desviando la imagen. -->
- [ ] D) Polarizacion. <!-- feedback: Incorrecto. La polarizacion orienta las vibraciones de la luz en un plano especifico. -->

## Question 5 (D5)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v5`
**Bloom:** Apply | **ICFES:** Uso comprensivo del conocimiento cientifico
**Context:** En una poblacion de 400 estudiantes de un colegio de Cali, 36 presentan una condicion genetica recesiva (rr).
**Enunciado:** Cual es la frecuencia del alelo recesivo (q) segun Hardy-Weinberg?
**Options:**
- [ ] A) 0.09 <!-- feedback: Incorrecto. q2 = 36/400 = 0.09, pero q = sqrt(0.09) = 0.3. -->
- [x] B) 0.30 <!-- feedback: Correcto. rr = 36/400 = 0.09 = q2. q = sqrt(0.09) = 0.3. -->
- [ ] C) 0.70 <!-- feedback: Incorrecto. 0.7 es p (alelo dominante), no q. -->
- [ ] D) 0.50 <!-- feedback: Incorrecto. q2 = 0.09, q = 0.3. -->

## Question 6 (D5)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v6`
**Bloom:** Analyze | **ICFES:** Indagacion
**Context:** En una celda voltaica, el voltaje medido disminuye con el tiempo de operacion.
**Enunciado:** A que se debe la disminucion del voltaje?
**Options:**
- [ ] A) Aumenta la concentracion de iones en ambas soluciones. <!-- feedback: Parcialmente correcto pero incompleto. -->
- [ ] B) La celda se calienta y pierde eficiencia. <!-- feedback: Incorrecto. El calor no es la causa principal. -->
- [x] C) La concentracion de reactivos disminuye y los potenciales de semicelda se acercan al equilibrio. <!-- feedback: Correcto. Al descargarse la celda, cambian las concentraciones y la fem disminuye hasta llegar a equilibrio (E=0). -->
- [ ] D) Los electrones se agotan en el circuito. <!-- feedback: Incorrecto. Los electrones fluyen mientras haya reaccion. -->

## Question 7 (D5)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v7`
**Bloom:** Apply | **ICFES:** Uso comprensivo del conocimiento cientifico
**Context:** Se tiene un espejo concavo con foco a 10 cm. Un objeto se coloca a 30 cm del espejo. 1/f = 1/do + 1/di.
**Enunciado:** Donde se forma la imagen?
**Options:**
- [x] A) di = 15 cm, imagen real <!-- feedback: Correcto. 1/di = 1/10 - 1/30 = 2/30, di = 15 cm. Positivo -> imagen real. -->
- [ ] B) di = 7.5 cm, imagen virtual <!-- feedback: Incorrecto. 1/30+1/7.5 = 1/6, no 1/10. -->
- [ ] C) di = -15 cm, imagen virtual <!-- feedback: Incorrecto. El calculo correcto da di positiva (real). -->
- [ ] D) di = 20 cm, imagen real <!-- feedback: Incorrecto. 1/30+1/20 = 5/60 = 1/12, no 1/10. -->

## Question 8 (D6)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v8`
**Bloom:** Analyze | **ICFES:** Explicacion de fenomenos
**Context:** Un estudiante en el Liceo de la Universidad Pedagogica en Bogota realiza un experimento con un prisma triangular. La luz blanca se separa en colores.
**Enunciado:** Cual es el orden de los colores de menor a mayor desviacion?
**Options:**
- [x] A) Rojo, amarillo, verde, azul, violeta. <!-- feedback: Correcto. El rojo tiene mayor longitud de onda (menor n), se desvia menos. El violeta tiene menor lambda, mayor n y se desvia mas. -->
- [ ] B) Violeta, azul, verde, amarillo, rojo. <!-- feedback: Incorrecto. Invertiste el orden. -->
- [ ] C) Rojo, naranja, amarillo, verde, azul, indigo, violeta. <!-- feedback: Incorrecto por incluir colores adicionales no solicitados en la pregunta. -->
- [ ] D) Todos se desvian igual. <!-- feedback: Incorrecto. n depende de lambda. -->

## Question 9 (D6)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v9`
**Bloom:** Apply | **ICFES:** Uso comprensivo del conocimiento cientifico
**Context:** Una celda electrolitica usa corriente de 2 A durante 30 minutos. F = 96,500 C/mol.
**Enunciado:** Cuantos moles de electrones pasan? (Q = I*t)
**Options:**
- [ ] A) 3600 mol <!-- feedback: Incorrecto. Q = 2*1800 = 3600 C. n(e-) = 3600/96500 = 0.0373 mol. -->
- [ ] B) 0.0187 mol <!-- feedback: Incorrecto. n(e-) = Q/F = 3600/96500 = 0.0373 mol. -->
- [x] C) 0.0373 mol <!-- feedback: Correcto. Q = I*t = 2*1800 = 3600 C. n(e-) = 3600/96500 = 0.0373 mol. -->
- [ ] D) 2.0 mol <!-- feedback: Incorrecto. Confundiste la corriente con moles de electrones. -->

## Question 10 (D6)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v10`
**Bloom:** Understand | **ICFES:** Explicacion de fenomenos
**Context:** En clase de genetica se estudian las fuerzas evolutivas.
**Enunciado:** Cual de las siguientes NO es una fuerza evolutiva que altera el equilibrio HW?
**Options:**
- [ ] A) Seleccion natural. <!-- feedback: Incorrecto. La seleccion natural cambia frecuencias alelicas. -->
- [ ] B) Deriva genetica. <!-- feedback: Incorrecto. La deriva (especialmente en poblaciones pequnas) cambia frecuencias al azar. -->
- [ ] C) Flujo genico (migracion). <!-- feedback: Incorrecto. La migracion introduce o elimina alelos. -->
- [x] D) Apareamiento aleatorio. <!-- feedback: Correcto. El apareamiento aleatorio es uno de los supuestos del equilibrio HW, no una fuerza evolutiva. Las fuerzas son: mutacion, seleccion natural, deriva genetica y flujo genico. -->

## Question 11 (D7)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v11`
**Bloom:** Apply | **ICFES:** Uso comprensivo del conocimiento cientifico
**Context:** Se tienen semiceldas: Ag+(1M)/Ag(s) E=+0.80 V y Ni2+(1M)/Ni(s) E=-0.25 V.
**Enunciado:** Cual es el agente oxidante mas fuerte?
**Options:**
- [ ] A) Ni(s) <!-- feedback: Incorrecto. El Ni metalico es un agente reductor, no oxidante. -->
- [x] B) Ag+(aq) <!-- feedback: Correcto. Mayor potencial de reduccion (+0.80V) indica que Ag+ tiende mas a reducirse, por tanto es mejor agente oxidante. -->
- [ ] C) Ni2+(aq) <!-- feedback: Incorrecto. E=-0.25V es menor, Ni2+ es peor agente oxidante que Ag+. -->
- [ ] D) Ag(s) <!-- feedback: Incorrecto. Ag(s) ya esta reducida, es un agente reductor debil. -->

## Question 12 (D7)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v12`
**Bloom:** Analyze | **ICFES:** Indagacion
**Context:** n1*sen(theta1) = n2*sen(theta2). Un rayo pasa del vidrio (n=1.5) al aire (n=1.0) con incidencia de 50 grados. sen^-1(1/1.5) = 41.8 grados.
**Enunciado:** Que sucede con el rayo?
**Options:**
- [ ] A) Se refracta normalmente hacia el aire. <!-- feedback: Incorrecto. 50 grados > 41.8 grados (angulo critico). -->
- [ ] B) Se acerca a la normal. <!-- feedback: Incorrecto. Cuando va de medio denso a menos denso, se aleja de la normal. -->
- [x] C) Ocurre reflexion total interna. <!-- feedback: Correcto. theta_c = sen^-1(n2/n1) = sen^-1(1/1.5) = 41.8 grados. Como theta=50 grados > 41.8, no puede refractarse y se refleja totalmente. -->
- [ ] D) El rayo se divide en dos. <!-- feedback: Incorrecto. Eso seria birrefringencia. -->

## Question 13 (D7)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v13`
**Bloom:** Apply | **ICFES:** Explicacion de fenomenos
**Context:** Una poblacion de 1000 aves en la Sierra Nevada de Santa Marta: color gris (dominante G) y blanco (recesivo g). Hay 160 aves blancas.
**Enunciado:** En equilibrio HW, cuantas aves heterocigotas (Gg) se esperan?
**Options:**
- [ ] A) 160 <!-- feedback: Incorrecto. 160 es la frecuencia de gg. -->
- [ ] B) 360 <!-- feedback: Incorrecto. q^2 = 160/1000 = 0.16, q = 0.4, p = 0.6. 2pq = 2*0.6*0.4 = 0.48 -> 480. -->
- [x] C) 480 <!-- feedback: Correcto. q^2 = 0.16, q = 0.4, p = 0.6. 2pq = 2*0.6*0.4 = 0.48. 0.48*1000 = 480. -->
- [ ] D) 600 <!-- feedback: Incorrecto. p^2 = 0.36 -> 360 aves GG. 480 es el numero de heterocigotas. -->

## Question 14 (D8)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v14`
**Bloom:** Evaluate | **ICFES:** Uso comprensivo del conocimiento cientifico
**Context:** Datos: E(Li+/Li) = -3.04V, E(Cu2+/Cu) = +0.34V, E(Fe2+/Fe) = -0.44V, E(Ag+/Ag) = +0.80V.
**Enunciado:** Cual combinacion produce la mayor diferencia de potencial?
**Options:**
- [ ] A) Cu/Ag <!-- feedback: Incorrecto. Ecelda = 0.80 - 0.34 = 0.46 V. -->
- [ ] B) Fe/Cu <!-- feedback: Incorrecto. Ecelda = 0.34 - (-0.44) = 0.78 V. -->
- [x] C) Li/Ag <!-- feedback: Correcto. Ecelda = 0.80 - (-3.04) = 3.84 V. La mayor diferencia entre el mas reductor (Li) y el mejor oxidante (Ag+). -->
- [ ] D) Fe/Ag <!-- feedback: Incorrecto. Ecelda = 0.80 - (-0.44) = 1.24 V. -->

## Question 15 (D8)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v15`
**Bloom:** Evaluate | **ICFES:** Indagacion
**Context:** Un genetista estudia plantas en el Jardin Botanico de Medellin. Tras una sequia severa, la frecuencia de un alelo raro disminuye drasticamente.
**Enunciado:** Cual es la explicacion mas probable?
**Options:**
- [ ] A) Seleccion natural direccional. <!-- feedback: Posible pero no la mas probable. -->
- [ ] B) Mutacion. <!-- feedback: Incorrecto. Las mutaciones son raras y no explicarian un cambio drastico. -->
- [x] C) Deriva genetica por cuello de botella. <!-- feedback: Correcto. Un cuello de botella (reduccion poblacional drastica) causa perdida aleatoria de alelos por deriva genetica. -->
- [ ] D) Flujo genico desde otra poblacion. <!-- feedback: Incorrecto. No hay indicios de migracion. -->

## Question 16 (D8)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v16`
**Bloom:** Analyze | **ICFES:** Explicacion de fenomenos
**Context:** Objeto a 5 cm de lente convergente de foco 10 cm. 1/f = 1/do + 1/di.
**Enunciado:** Que tipo de imagen se forma y donde?
**Options:**
- [ ] A) Imagen real a 10 cm del otro lado. <!-- feedback: Incorrecto. 1/di = 1/10 - 1/5 = -1/10, di = -10 cm. -->
- [ ] B) Imagen real a 3.33 cm. <!-- feedback: Incorrecto. El calculo da di negativa (virtual). -->
- [x] C) Imagen virtual a 10 cm del mismo lado del objeto. <!-- feedback: Correcto. 1/di = 1/10 - 1/5 = -1/10, di = -10 cm (virtual, mismo lado). -->
- [ ] D) No se forma imagen. <!-- feedback: Incorrecto. Si se forma, es virtual y derecha. -->

## Question 17 (D9)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v17`
**Bloom:** Evaluate | **ICFES:** Uso comprensivo del conocimiento cientifico
**Context:** Electrolisis del agua: 2H2O(l) -> 2H2(g) + O2(g). Corriente 0.5 A durante 1930 s. F=96500 C/mol.
**Enunciado:** Quien calcula correctamente los moles de H2 producidos? (Reaccion catodica: 2H+ + 2e- -> H2)
**Options:**
- [ ] A) Estudiante A: n(H2) = 0.5 mol <!-- feedback: Incorrecto. Q = 0.5*1930 = 965 C. n(e-) = 965/96500 = 0.01 mol. Por cada H2 se requieren 2e-: n(H2) = 0.005 mol. -->
- [ ] B) Estudiante B: n(H2) = 0.01 mol <!-- feedback: Incorrecto. 0.01 son los moles de electrones, no de H2. -->
- [x] C) Estudiante C: n(H2) = 0.005 mol <!-- feedback: Correcto. Q = 965 C, n(e-) = 0.01 mol. 2H+ + 2e- -> H2, n(H2) = n(e-)/2 = 0.005 mol. -->
- [ ] D) Estudiante D: n(H2) = 0.0025 mol <!-- feedback: Incorrecto. Dividiste entre 4 en lugar de 2. -->

## Question 18 (D9)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v18`
**Bloom:** Evaluate | **ICFES:** Indagacion
**Context:** Un estudiante dice: "La reflexion total interna no puede ocurrir en fibras opticas si el angulo de incidencia es menor al critico."
**Enunciado:** Evalua esta afirmacion.
**Options:**
- [x] A) Es correcta; la reflexion total interna solo ocurre si theta > theta_c. <!-- feedback: Correcto. Si theta < theta_c, la luz se escapa del nucleo al revestimiento. Las fibras opticas mantienen theta > theta_c. -->
- [ ] B) Es incorrecta; la reflexion total ocurre solo si theta > theta_c, lo que contradice al estudiante. <!-- feedback: Incorrecto. El estudiante dice "no puede ocurrir si theta < theta_c", lo cual es correcto. -->
- [ ] C) Es correcta; los angulos criticos en fibra optica son de 90 grados. <!-- feedback: Incorrecto. theta_c = sen^-1(n2/n1), no es 90 grados. -->
- [ ] D) No tiene relacion con el angulo critico. <!-- feedback: Incorrecto. La reflexion total interna depende del angulo critico. -->

## Question 19 (D9)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v19`
**Bloom:** Create | **ICFES:** Indagacion
**Context:** Disenar un experimento para determinar si una poblacion de mariposas en el Amazonas colombiano esta en equilibrio HW para el color de alas.
**Enunciado:** Cual es el diseno mas adecuado?
**Options:**
- [ ] A) Cruzar mariposas en laboratorio y contar la descendencia. <!-- feedback: Incorrecto. Eso estudia genetica mendeliana, no equilibrio poblacional. -->
- [x] B) Tomar una muestra aleatoria grande, contar los fenotipos, calcular frecuencias alelicas y genotipicas, y aplicar prueba chi^2 contra las esperadas HW. <!-- feedback: Correcto. El metodo estandar para probar equilibrio HW es muestrear genotipos, estimar p y q, calcular frecuencias esperadas y hacer chi^2. -->
- [ ] C) Secuenciar el ADN de dos mariposas. <!-- feedback: Incorrecto. Secuenciar dos individuos no da informacion poblacional. -->
- [ ] D) Observar las mariposas por un dia y contar las de cada color. <!-- feedback: Incorrecto. Sin analisis estadistico no se puede determinar equilibrio HW. -->

## Question 20 (D10)
**ID:** `CO-CN-10-2026-P4-comprehensive-001-MASTERY-v20`
**Bloom:** Evaluate | **ICFES:** Explicacion de fenomenos
**Context:** Tres celdas electroliticas en serie con la misma corriente. Celda 1: Ag+ -> Ag, Celda 2: Cu2+ -> Cu, Celda 3: Al3+ -> Al. Masas atomicas: Ag=107.9, Cu=63.5, Al=27.0.
**Enunciado:** Si la misma cantidad de carga pasa por las tres, cual deposita la mayor masa de metal?
**Options:**
- [x] A) Celda 1 (Ag) porque tiene mayor masa atomica. <!-- feedback: Correcto. Todos reciben la misma carga. Ag+ + e- -> Ag: por mol de e- se deposita 1 mol Ag = 107.9 g. Cu2+ + 2e- -> Cu: por mol de e- se deposita 0.5 mol Cu = 31.75 g. Al3+ + 3e- -> Al: por mol de e- se deposita 1/3 mol Al = 9.0 g. -->
- [ ] B) Celda 2 (Cu) porque el cobre tiene valencia 2. <!-- feedback: Incorrecto. Por mol de e-: Ag = 107.9 g, Cu = 31.75 g. -->
- [ ] C) Celda 3 (Al) porque necesita 3 electrones por atomo. <!-- feedback: Incorrecto. Mayor numero de electrones necesarios por atomo significa MENOS atomos depositados. -->
- [ ] D) Todas depositan la misma masa. <!-- feedback: Incorrecto. La masa depende de MA/n donde n es electrones por ion. -->
"""

# Write P4
path = r"E:\scripts-python\worldexams\questions_data\colombia\ciencias-naturales\grado-10\2026\periodos\CO-CN-10-2026-P4-comprehensive-001-MASTERY-bundle.md"
os.makedirs(os.path.dirname(path), exist_ok=True)
with open(path, "w", encoding="utf-8") as f:
    f.write(CN_P4)
print("Written: CN P4")
