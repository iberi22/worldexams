---
id: "CO-CN-5-2026-W06-sistema-solar-planetas-001-MASTERY-bundle"
country: "colombia"
grado: 5
asignatura: "ciencias-naturales"
tema: "sistema-solar-planetas"
periodo: "weekly"
week: "W06"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 8
bundle_size: 8
alignment: "DBA MEN Colombia / Saber 5"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: Sistema Solar y Planetas - Grado 5 (W06)

Características generales del Sistema Solar, orden de los planetas, movimientos de rotación y traslación, y comparación entre planetas rocosos y gaseosos.

## Question 1 [D3-D4]
**ID:** CO-CN-5-2026-W06-sistema-solar-planetas-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Explicación de fenómenos
**Expected_Success:** 0.90
**Contexto:** En clase de ciencias en Pasto, la profesora muestra un cartel con los planetas del Sistema Solar ordenados desde el Sol.

### Enunciado
¿Cuál es el planeta más cercano al Sol en el Sistema Solar?

### Opciones
- [x] A) Mercurio.
  <!-- feedback: Mercurio es el planeta más cercano al Sol en el Sistema Solar. -->
- [ ] B) Tierra.
  <!-- feedback: La Tierra es el tercer planeta desde el Sol, no el primero. -->
- [ ] C) Júpiter.
  <!-- feedback: Júpiter es el quinto planeta desde el Sol, mucho más lejos que Mercurio. -->
- [ ] D) Neptuno.
  <!-- feedback: Neptuno es el planeta más lejano del Sol, no el más cercano. -->

### Explicacion Pedagogica
Reconocer el orden de los planetas del Sistema Solar comenzando desde el más cercano al Sol.

## Question 2 [D3-D4]
**ID:** CO-CN-5-2026-W06-sistema-solar-planetas-001-MASTERY-bundle-v2
**Bloom:** Understand
**ICFES:** Explicación de fenómenos
**Expected_Success:** 0.85
**Contexto:** Lee: "La Tierra gira sobre sí misma alrededor de un eje imaginario, lo que produce el día y la noche. Este movimiento se llama rotación."

### Enunciado
¿Por qué el movimiento de rotación de la Tierra produce el día y la noche?

### Opciones
- [x] A) Porque al girar, distintas partes del planeta quedan iluminadas por el Sol o quedan en sombra.
  <!-- feedback: La rotación hace que una mitad de la Tierra mire al Sol (día) y la otra quede oscura (noche). -->
- [ ] B) Porque la Tierra se aleja completamente del Sol durante la noche.
  <!-- feedback: La Tierra no se aleja del Sol; sigue en su órbita. -->
- [ ] C) Porque el Sol se apaga y se enciende cada doce horas.
  <!-- feedback: El Sol no se apaga; la percepción de día y noche se debe a la rotación. -->
- [ ] D) Porque la Luna cambia de color durante la rotación.
  <!-- feedback: El día y la noche no dependen de la rotación de la Luna, sino de la Tierra. -->

### Explicacion Pedagogica
Comprender la relación entre el movimiento de rotación terrestre y la sucesión de día y noche.

## Question 3 [D5-D6]
**ID:** CO-CN-5-2026-W06-sistema-solar-planetas-001-MASTERY-bundle-v3
**Bloom:** Apply
**ICFES:** Explicación de fenómenos
**Expected_Success:** 0.76
**Contexto:** Un estudiante debe escribir los planetas en orden desde el Sol: Mercurio, Venus, Tierra, Marte, Júpiter, Saturno, Urano y Neptuno.

### Enunciado
¿Cuál es la secuencia correcta de los cuatro primeros planetas desde el Sol?

### Opciones
- [x] A) Mercurio, Venus, Tierra, Marte.
  <!-- feedback: Los cuatro planetas más cercanos al Sol son Mercurio, Venus, Tierra y Marte. -->
- [ ] B) Tierra, Marte, Júpiter, Saturno.
  <!-- feedback: Esa secuencia omite a Mercurio y Venus, los dos más cercanos al Sol. -->
- [ ] C) Neptuno, Urano, Saturno, Júpiter.
  <!-- feedback: Esa secuencia va de mayor a menor distancia, no desde el Sol. -->
- [ ] D) Venus, Mercurio, Marte, Júpiter.
  <!-- feedback: Mercurio está antes que Venus, así que este orden es incorrecto. -->

### Explicacion Pedagogica
Aplicar el orden de los planetas del Sistema Solar a partir del más cercano al Sol.

## Question 4 [D5-D6]
**ID:** CO-CN-5-2026-W06-sistema-solar-planetas-001-MASTERY-bundle-v4
**Bloom:** Apply
**ICFES:** Explicación de fenómenos
**Expected_Success:** 0.72
**Contexto:** Lee: "Los planetas rocosos como Mercurio, Venus, Tierra y Marte tienen superficie sólida. Los planetas gaseosos como Júpiter y Saturno están formados principalmente por gases."

### Enunciado
Si un astronauta quiere pisar una superficie sólida, ¿a cuál de estos planetas debería viajar?

### Opciones
- [x] A) A un planeta rocoso como Marte o la Tierra.
  <!-- feedback: Los planetas rocosos tienen superficie sólida donde se puede caminar. -->
- [ ] B) A un planeta gaseoso como Júpiter.
  <!-- feedback: Los planetas gaseosos no tienen superficie sólida donde pisar. -->
- [ ] C) Al Sol, porque es una gran bola de fuego.
  <!-- feedback: El Sol es una estrella, no un planeta, y no se puede pisar. -->
- [ ] D) A la Luna, considerada un planeta gaseoso.
  <!-- feedback: La Luna es un satélite rocoso, no un planeta gaseoso. -->

### Explicacion Pedagogica
Aplicar la clasificación de planetas rocosos y gaseosos para predecir sus condiciones físicas.

## Question 5 [D5-D6]
**ID:** CO-CN-5-2026-W06-sistema-solar-planetas-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Explicación de fenómenos
**Expected_Success:** 0.66
**Contexto:** En clase se explica: "La Tierra tarda aproximadamente 365 días en dar una vuelta completa alrededor del Sol. Este movimiento se llama traslación."

### Enunciado
¿Qué consecuencia tiene principalmente el movimiento de traslación de la Tierra?

### Opciones
- [x] A) La sucesión de las cuatro estaciones del año (primavera, verano, otoño e invierno).
  <!-- feedback: La traslación, junto con la inclinación del eje, produce las estaciones. -->
- [ ] B) El cambio entre el día y la noche en pocas horas.
  <!-- feedback: El día y la noche se explican por la rotación, no por la traslación. -->
- [ ] C) El cambio de fase de la Luna cada mes.
  <!-- feedback: Las fases lunares dependen de la posición Luna-Tierra-Sol, no solo de la traslación terrestre. -->
- [ ] D) La salida y la puesta del Sol.
  <!-- feedback: La salida y puesta del Sol se relacionan con la rotación, no con la traslación. -->

### Explicacion Pedagogica
Relacionar el movimiento de traslación de la Tierra con la duración del año y las estaciones.

## Question 6 [D7-D8]
**ID:** CO-CN-5-2026-W06-sistema-solar-planetas-001-MASTERY-bundle-v6
**Bloom:** Analyze
**ICFES:** Explicación de fenómenos
**Expected_Success:** 0.58
**Contexto:** Compara: "Mercurio es pequeño, muy cercano al Sol y con temperaturas extremas. Júpiter es enorme, está lejos del Sol y formado principalmente por gases."

### Enunciado
¿Qué diferencia estructural y de ubicación existe entre Mercurio y Júpiter?

### Opciones
- [x] A) Mercurio es un planeta rocoso pequeño cercano al Sol; Júpiter es un planeta gaseoso gigante y lejano.
  <!-- feedback: Mercurio es rocoso y cercano al Sol; Júpiter es gaseoso, gigante y mucho más lejano. -->
- [ ] B) Ambos son planetas gaseosos del mismo tamaño.
  <!-- feedback: Mercurio es rocoso y pequeño, mientras Júpiter es gaseoso y enorme. -->
- [ ] C) Mercurio es el planeta más grande y Júpiter el más pequeño.
  <!-- feedback: Es al revés: Júpiter es el planeta más grande del Sistema Solar. -->
- [ ] D) Ninguno de los dos pertenece al Sistema Solar.
  <!-- feedback: Mercurio y Júpiter son planetas del Sistema Solar. -->

### Explicacion Pedagogica
Comparar las características de un planeta rocoso cercano al Sol con un planeta gaseoso gigante.

## Question 7 [D7-D8]
**ID:** CO-CN-5-2026-W06-sistema-solar-planetas-001-MASTERY-bundle-v7
**Bloom:** Analyze
**ICFES:** Explicación de fenómenos
**Expected_Success:** 0.52
**Contexto:** Lee: "Plutón fue considerado el noveno planeta, pero en 2006 la Unión Astronómica Internacional lo reclasificó como planeta enano porque no cumple todas las condiciones para ser planeta."

### Enunciado
¿Qué criterio diferenció a Plutón de los planetas clásicos como la Tierra o Júpiter?

### Opciones
- [x] A) No cumple con todas las condiciones para ser considerado un planeta, por ejemplo limpiar su órbita.
  <!-- feedback: Plutón comparte su órbita con otros cuerpos, por eso se reclasificó como planeta enano. -->
- [ ] B) Está más cerca del Sol que Mercurio.
  <!-- feedback: Plutón está más lejos del Sol que Neptuno, no más cerca que Mercurio. -->
- [ ] C) Es el único planeta con anillos visibles.
  <!-- feedback: Plutón no posee anillos visibles como Saturno. -->
- [ ] D) Tiene agua líquida en su superficie.
  <!-- feedback: Plutón no tiene agua líquida en su superficie; es un cuerpo helado. -->

### Explicacion Pedagogica
Analizar los criterios científicos que permiten distinguir planetas, planetas enanos y otros cuerpos.

## Question 8 [D9-D10]
**ID:** CO-CN-5-2026-W06-sistema-solar-planetas-001-MASTERY-bundle-v8
**Bloom:** Evaluate
**ICFES:** Explicación de fenómenos
**Expected_Success:** 0.30
**Contexto:** Lee: 'La Tierra es el único planeta del Sistema Solar con condiciones conocidas para albergar vida: agua líquida, atmósfera con oxígeno y temperatura adecuada. Cuidar la atmósfera y el agua es esencial para preservar la vida.'

### Enunciado
¿Por qué la Tierra es especial en el Sistema Solar y qué debemos hacer para conservar esas condiciones?

### Opciones
- [x] A) Porque reúne condiciones para la vida, y debemos cuidar la atmósfera, el agua y la temperatura.
  <!-- feedback: Las condiciones especiales de la Tierra permiten la vida y deben ser protegidas. -->
- [ ] B) Porque es el planeta más grande del Sistema Solar.
  <!-- feedback: La Tierra no es el planeta más grande; Júpiter lo es. -->
- [ ] C) Porque no tiene atmósfera ni agua como los demás planetas.
  <!-- feedback: La Tierra sí tiene atmósfera y agua, a diferencia de muchos otros planetas. -->
- [ ] D) Porque no se mueve ni rota sobre sí misma.
  <!-- feedback: La Tierra sí tiene movimientos de rotación y traslación. -->

### Explicacion Pedagogica
Evaluar las condiciones que hacen de la Tierra un planeta habitable y la responsabilidad de cuidarlas.