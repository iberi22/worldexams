---
id: "CO-CN-5-2026-W07-el-sonido-001-MASTERY-bundle"
country: "colombia"
grado: 5
asignatura: "ciencias-naturales"
tema: "el-sonido"
periodo: "weekly"
week: "W07"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 8
bundle_size: 8
alignment: "DBA MEN Colombia / Saber 5"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: El Sonido - Grado 5 (W07)

Naturaleza del sonido como onda mecánica, cómo se propaga en distintos medios y características como intensidad y tono.

## Question 1 [D3-D4]
**ID:** CO-CN-5-2026-W07-el-sonido-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Indagación
**Expected_Success:** 0.90
**Contexto:** En una clase de ciencias en Villavicencio, la profesora hace vibrar una cuerda de guitarra y pregunta qué se produce.

### Enunciado
¿Qué es el sonido?

### Opciones
- [x] A) Una vibración que se propaga por un medio material como el aire, el agua o los sólidos.
  <!-- feedback: El sonido es una onda mecánica que necesita un medio para viajar. -->
- [ ] B) Un tipo de luz que sale de los objetos.
  <!-- feedback: El sonido no es luz, sino una vibración que se transmite por medios materiales. -->
- [ ] C) Un gas que producen las personas al hablar.
  <!-- feedback: El sonido no es un gas, sino la vibración que generan las cuerdas vocales. -->
- [ ] D) Un olor que viaja por el aire.
  <!-- feedback: El sonido no es olor; los olores son percibidos por otro sentido. -->

### Explicacion Pedagogica
Comprender el concepto básico del sonido como vibración que se propaga por un medio.

## Question 2 [D3-D4]
**ID:** CO-CN-5-2026-W07-el-sonido-001-MASTERY-bundle-v2
**Bloom:** Understand
**ICFES:** Indagación
**Expected_Success:** 0.85
**Contexto:** Lee: "En el espacio exterior no hay aire ni moléculas suficientes para que el sonido se transmita, por eso allí no se escuchan las explosiones de las estrellas."

### Enunciado
¿Por qué el sonido no se propaga en el espacio exterior como en la Tierra?

### Opciones
- [x] A) Porque el sonido necesita un medio material y en el espacio casi no hay moléculas que vibren.
  <!-- feedback: El sonido requiere un medio con partículas; en el espacio exterior este es muy escaso. -->
- [ ] B) Porque los astronautas tienen los oídos tapados en el espacio.
  <!-- feedback: El motivo es la falta de medio, no un problema en los oídos de los astronautas. -->
- [ ] C) Porque el sonido se transforma en luz en el espacio exterior.
  <!-- feedback: El sonido no se convierte en luz; simplemente no encuentra un medio para propagarse. -->
- [ ] D) Porque en el espacio exterior hace mucho frío y eso apaga el sonido.
  <!-- feedback: La temperatura no apaga el sonido; lo esencial es la ausencia de un medio material. -->

### Explicacion Pedagogica
Explicar por qué el sonido requiere un medio material para propagarse y no viaja en el vacío.

## Question 3 [D5-D6]
**ID:** CO-CN-5-2026-W07-el-sonido-001-MASTERY-bundle-v3
**Bloom:** Apply
**ICFES:** Indagación
**Expected_Success:** 0.76
**Contexto:** Un estudiante se tapa los oídos y escucha menos los sonidos del salón de clase.

### Enunciado
¿Qué ocurre cuando el sonido llega a un obstáculo como las manos o las paredes?

### Opciones
- [x] A) Parte del sonido se refleja o se absorbe, por lo que se escucha más débil al otro lado.
  <!-- feedback: Los obstáculos reflejan y absorben parte del sonido, debilitándolo. -->
- [ ] B) El sonido se vuelve más fuerte detrás del obstáculo.
  <!-- feedback: El sonido no aumenta al pasar por un obstáculo, sino que disminuye. -->
- [ ] C) El sonido se transforma completamente en luz al chocar.
  <!-- feedback: El sonido no se convierte en luz al chocar con un objeto. -->
- [ ] D) El sonido desaparece por completo sin importar el material del obstáculo.
  <!-- feedback: Dependiendo del material, el sonido puede absorberse mucho o solo parcialmente. -->

### Explicacion Pedagogica
Aplicar el concepto de reflexión y absorción del sonido al chocar con diferentes materiales.

## Question 4 [D5-D6]
**ID:** CO-CN-5-2026-W07-el-sonido-001-MASTERY-bundle-v4
**Bloom:** Apply
**ICFES:** Indagación
**Expected_Success:** 0.72
**Contexto:** Un médico usa un estetoscopio para escuchar los latidos del corazón de un paciente.

### Enunciado
¿Por qué el estetoscopio permite escuchar sonidos muy suaves del cuerpo?

### Opciones
- [x] A) Porque concentra y conduce las vibraciones sonoras desde el cuerpo hasta los oídos.
  <!-- feedback: El estetoscopio canaliza el sonido y amplifica las vibraciones internas. -->
- [ ] B) Porque convierte el sonido del cuerpo en electricidad.
  <!-- feedback: El estetoscopio no transforma el sonido en electricidad, solo lo conduce mejor. -->
- [ ] C) Porque produce un sonido nuevo que tapa los latidos.
  <!-- feedback: El estetoscopio no emite sonidos adicionales, solo recoge los del cuerpo. -->
- [ ] D) Porque elimina todos los sonidos del ambiente.
  <!-- feedback: El estetoscopio no elimina sonidos del ambiente, pero ayuda a focalizar los internos. -->

### Explicacion Pedagogica
Aplicar la conducción del sonido en materiales sólidos para explicar el funcionamiento del estetoscopio.

## Question 5 [D5-D6]
**ID:** CO-CN-5-2026-W07-el-sonido-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Indagación
**Expected_Success:** 0.66
**Contexto:** En una piscina, dos niños hablan debajo del agua y se escuchan claramente, aunque sus voces suenan diferentes.

### Enunciado
¿Por qué se puede escuchar el sonido debajo del agua?

### Opciones
- [x] A) Porque el agua es un medio material que permite la propagación de las vibraciones sonoras.
  <!-- feedback: El agua transmite las vibraciones, aunque el sonido viaje más rápido que en el aire. -->
- [ ] B) Porque bajo el agua la luz se convierte en sonido.
  <!-- feedback: La luz no se convierte en sonido al sumergirse. -->
- [ ] C) Porque los oídos cambian de forma al estar mojados.
  <!-- feedback: Los oídos no cambian de forma; lo importante es que el agua transmite el sonido. -->
- [ ] D) Porque el agua atrapa los rayos del Sol y los transforma en ruido.
  <!-- feedback: Los rayos solares no se transforman en ruido dentro del agua. -->

### Explicacion Pedagogica
Reconocer que el sonido puede propagarse por medios líquidos como el agua.

## Question 6 [D7-D8]
**ID:** CO-CN-5-2026-W07-el-sonido-001-MASTERY-bundle-v6
**Bloom:** Analyze
**ICFES:** Indagación
**Expected_Success:** 0.58
**Contexto:** Compara: "Un silbato produce un sonido agudo, y un tambor grande produce un sonido grave. El silbato tiene vibraciones rápidas; el tambor, vibraciones más lentas."

### Enunciado
¿Qué relación hay entre la frecuencia de vibración y el tono del sonido?

### Opciones
- [x] A) Mayor frecuencia produce sonidos más agudos; menor frecuencia produce sonidos más graves.
  <!-- feedback: La frecuencia alta genera tonos agudos y la baja genera tonos graves. -->
- [ ] B) La frecuencia no influye en el tono del sonido.
  <!-- feedback: La frecuencia es precisamente la propiedad que determina el tono. -->
- [ ] C) Siempre que la frecuencia baja, el sonido es más agudo.
  <!-- feedback: Cuando la frecuencia baja, el sonido se vuelve más grave, no más agudo. -->
- [ ] D) Solo los instrumentos musicales cambian de tono, los demás sonidos no.
  <!-- feedback: Cualquier sonido puede variar de tono según su frecuencia de vibración. -->

### Explicacion Pedagogica
Analizar la relación entre la frecuencia de vibración y las cualidades del sonido (agudo o grave).

## Question 7 [D7-D8]
**ID:** CO-CN-5-2026-W07-el-sonido-001-MASTERY-bundle-v7
**Bloom:** Analyze
**ICFES:** Indagación
**Expected_Success:** 0.52
**Contexto:** Lee: "La contaminación sonora es el exceso de sonido en el ambiente. Vivir cerca de aeropuertos o carreteras muy transitadas puede afectar la salud auditiva y el sueño."

### Enunciado
¿Qué consecuencia puede traer la contaminación sonora en las personas?

### Opciones
- [x] A) Pérdida de la capacidad auditiva, estrés y problemas para dormir o concentrarse.
  <!-- feedback: El ruido constante puede dañar la audición y afectar la salud mental. -->
- [ ] B) Mejora de la memoria y el aprendizaje.
  <!-- feedback: La contaminación sonora no mejora la memoria ni el aprendizaje. -->
- [ ] C) Aumento de la temperatura corporal.
  <!-- feedback: La contaminación sonora no cambia la temperatura del cuerpo. -->
- [ ] D) Crecimiento más rápido del cabello.
  <!-- feedback: El ruido excesivo no tiene relación con el crecimiento del cabello. -->

### Explicacion Pedagogica
Analizar los efectos de la contaminación sonora en la salud humana y en la calidad de vida.

## Question 8 [D9-D10]
**ID:** CO-CN-5-2026-W07-el-sonido-001-MASTERY-bundle-v8
**Bloom:** Evaluate
**ICFES:** Indagación
**Expected_Success:** 0.30
**Contexto:** Lee: 'Proteger los oídos del ruido excesivo es fundamental para mantener una buena salud auditiva a lo largo de la vida. Usar protectores en ambientes ruidosos y mantener un volumen bajo en audífonos ayuda a prevenir daños.'

### Enunciado
¿Cuál es la mejor forma de cuidar la salud auditiva frente al ruido?

### Opciones
- [x] A) Usar protectores en lugares ruidosos y mantener bajo el volumen en audífonos y equipos de sonido.
  <!-- feedback: Reducir la exposición y bajar el volumen protege los oídos del daño. -->
- [ ] B) Escuchar música al máximo volumen siempre que se pueda.
  <!-- feedback: El volumen alto es justamente la causa principal del daño auditivo. -->
- [ ] C) Taparse los oídos con los dedos todo el día, incluso en silencio.
  <!-- feedback: Taparse los oídos no es una estrategia realista ni saludable. -->
- [ ] D) Ninguna acción es necesaria porque el oído se adapta solo.
  <!-- feedback: El oído no se adapta solo al ruido excesivo; puede dañarse irreversiblemente. -->

### Explicacion Pedagogica
Evaluar estrategias prácticas para proteger la salud auditiva frente a la contaminación sonora.