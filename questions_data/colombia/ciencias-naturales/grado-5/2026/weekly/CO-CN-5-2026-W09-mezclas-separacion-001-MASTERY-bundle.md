---
id: "CO-CN-5-2026-W09-mezclas-separacion-001-MASTERY-bundle"
country: "colombia"
grado: 5
asignatura: "ciencias-naturales"
tema: "mezclas-separacion"
periodo: "weekly"
week: "W09"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 8
bundle_size: 8
alignment: "DBA MEN Colombia / Saber 5"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: Mezclas y Métodos de Separación - Grado 5 (W09)

Tipos de mezclas (homogéneas y heterogéneas) y métodos físicos para separar sus componentes: tamizado, filtración, decantación y evaporación.

## Question 1 [D3-D4]
**ID:** CO-CN-5-2026-W09-mezclas-separacion-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Indagación
**Expected_Success:** 0.90
**Contexto:** En una clase de ciencias en Girardot, la profesora muestra un vaso con agua y sal disuelta, y otro con agua y arena.

### Enunciado
¿Cuál es la diferencia entre una mezcla homogénea y una mezcla heterogénea?

### Opciones
- [x] A) En la homogénea no se distinguen sus componentes a simple vista; en la heterogénea sí se distinguen.
  <!-- feedback: Las mezclas homogéneas se ven uniformes, mientras que las heterogéneas muestran sus componentes a simple vista. -->
- [ ] B) En la homogénea se ven los componentes separados, en la heterogénea no se ven.
  <!-- feedback: Es al revés: en las heterogéneas se ven los componentes, no en las homogéneas. -->
- [ ] C) Las mezclas homogéneas siempre son sólidas y las heterogéneas líquidas.
  <!-- feedback: Tanto las homogéneas como las heterogéneas pueden presentarse en distintos estados. -->
- [ ] D) No existe ninguna diferencia visible entre ellas.
  <!-- feedback: La diferencia principal es precisamente la uniformidad visible de la mezcla. -->

### Explicacion Pedagogica
Diferenciar mezclas homogéneas de heterogéneas a partir de la observación de sus componentes.

## Question 2 [D3-D4]
**ID:** CO-CN-5-2026-W09-mezclas-separacion-001-MASTERY-bundle-v2
**Bloom:** Understand
**ICFES:** Indagación
**Expected_Success:** 0.85
**Contexto:** Lee: "El agua con sal es una mezcla homogénea porque la sal se disuelve y no se puede distinguir a simple vista."

### Enunciado
¿Por qué el agua con sal se considera una mezcla homogénea?

### Opciones
- [x] A) Porque la sal disuelta no se ve y la mezcla luce uniforme en toda la solución.
  <!-- feedback: Al disolverse, la sal se reparte de manera uniforme y no se distingue. -->
- [ ] B) Porque la sal desaparece completamente y deja de existir.
  <!-- feedback: La sal no desaparece: se disuelve en el agua y se puede recuperar evaporando. -->
- [ ] C) Porque el agua deja de ser líquida al mezclarse con la sal.
  <!-- feedback: El agua sigue siendo líquida; lo que cambia es la composición de la mezcla. -->
- [ ] D) Porque la sal se evapora automáticamente al contacto con el agua.
  <!-- feedback: La sal no se evapora; permanece en la mezcla y puede separarse luego. -->

### Explicacion Pedagogica
Explicar por qué una disolución se clasifica como mezcla homogénea.

## Question 3 [D5-D6]
**ID:** CO-CN-5-2026-W09-mezclas-separacion-001-MASTERY-bundle-v3
**Bloom:** Apply
**ICFES:** Indagación
**Expected_Success:** 0.76
**Contexto:** Un estudiante quiere separar una mezcla de agua con tierra dejada en reposo en un vaso durante varios minutos.

### Enunciado
¿Qué método de separación es el más adecuado en este caso?

### Opciones
- [x] A) Decantación, porque la tierra más densa se deposita en el fondo y el agua queda arriba.
  <!-- feedback: La decantación permite separar líquidos de sólidos sedimentados por gravedad. -->
- [ ] B) Filtración con un colador muy fino, aunque la tierra ya se haya asentado.
  <!-- feedback: La filtración también funciona, pero la decantación es más sencilla cuando el sólido ya reposó. -->
- [ ] C) Evaporación del agua, porque así se obtiene la tierra en forma de gas.
  <!-- feedback: La evaporación recupera el sólido disuelto, no el sólido sedimentado, y no produce gas. -->
- [ ] D) Tamizado, porque es ideal para mezclas de sólidos grandes con líquidos.
  <!-- feedback: El tamizado se usa para separar sólidos de distinto tamaño, no un sólido fino en líquido. -->

### Explicacion Pedagogica
Aplicar el método de decantación para separar mezclas donde un sólido se asienta en un líquido.

## Question 4 [D5-D6]
**ID:** CO-CN-5-2026-W09-mezclas-separacion-001-MASTERY-bundle-v4
**Bloom:** Apply
**ICFES:** Indagación
**Expected_Success:** 0.72
**Contexto:** Una cocinera en Bogotá necesita separar el agua de la cocción de los granos de arroz.

### Enunciado
¿Cuál es el método más adecuado para separar el agua de los granos sólidos de arroz?

### Opciones
- [x] A) Filtración usando un colador o un paño limpio.
  <!-- feedback: El colador retiene los granos sólidos y deja pasar el líquido, separándolos. -->
- [ ] B) Decantación, dejando los granos flotando libremente.
  <!-- feedback: Los granos de arroz se asientan, pero el método más práctico es el colador. -->
- [ ] C) Evaporación del agua con los granos en la olla.
  <!-- feedback: Evaporar el agua no separa los granos; solo elimina el líquido. -->
- [ ] D) Tamizado con una malla muy gruesa.
  <!-- feedback: El tamizado se usa entre sólidos de distinto tamaño, no entre sólidos y líquidos. -->

### Explicacion Pedagogica
Aplicar la filtración para separar un sólido particulado del líquido en una mezcla cotidiana.

## Question 5 [D5-D6]
**ID:** CO-CN-5-2026-W09-mezclas-separacion-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Indagación
**Expected_Success:** 0.66
**Contexto:** Una investigadora quiere recuperar la sal que está disuelta en agua salada.

### Enunciado
¿Qué método es el más apropiado para obtener la sal sólida a partir de agua con sal?

### Opciones
- [x] A) Evaporación del agua, dejando la sal en el recipiente.
  <!-- feedback: Al evaporarse el agua, la sal queda como sólido en el fondo del recipiente. -->
- [ ] B) Filtración con un papel filtro común.
  <!-- feedback: El filtro no retiene la sal disuelta, solo partículas sólidas. -->
- [ ] C) Decantación, dejando la sal en el fondo.
  <!-- feedback: La sal disuelta no sedimenta; permanece en la disolución. -->
- [ ] D) Tamizado con una malla fina.
  <!-- feedback: El tamizado no es útil con un sólido disuelto en líquido. -->

### Explicacion Pedagogica
Aplicar la evaporación para separar el soluto disuelto en una mezcla homogénea.

## Question 6 [D7-D8]
**ID:** CO-CN-5-2026-W09-mezclas-separacion-001-MASTERY-bundle-v6
**Bloom:** Analyze
**ICFES:** Indagación
**Expected_Success:** 0.58
**Contexto:** Compara: "Una mezcla de arena y agua se ve turbia y al reposar la arena baja al fondo. Una mezcla de agua con sal se ve transparente y no forma capas al reposar."

### Enunciado
¿Qué diferencia fundamental existe entre la mezcla de arena con agua y la de sal con agua?

### Opciones
- [x] A) La de arena con agua es heterogénea y la arena sedimenta; la de sal con agua es homogénea y no sedimenta.
  <!-- feedback: La arena no se disuelve y forma una mezcla heterogénea; la sal se disuelve formando una mezcla homogénea. -->
- [ ] B) Las dos son heterogéneas porque no se ven los componentes.
  <!-- feedback: En la mezcla de arena con agua sí se ven los granos de arena; es heterogénea. -->
- [ ] C) Las dos son homogéneas porque ambas son transparentes.
  <!-- feedback: Solo la mezcla de agua con sal es transparente; la de arena con agua es turbia. -->
- [ ] D) Ninguna se puede separar por métodos físicos.
  <!-- feedback: Ambas se pueden separar usando métodos físicos como filtración y evaporación. -->

### Explicacion Pedagogica
Analizar el comportamiento de mezclas heterogéneas y homogéneas al dejarlas en reposo.

## Question 7 [D7-D8]
**ID:** CO-CN-5-2026-W09-mezclas-separacion-001-MASTERY-bundle-v7
**Bloom:** Analyze
**ICFES:** Indagación
**Expected_Success:** 0.52
**Contexto:** Lee: "Para purificar el agua de un río y volverla potable, las plantas de tratamiento usan varios métodos combinados: decantación, filtración y cloración."

### Enunciado
¿Por qué se combinan varios métodos en lugar de usar solo uno para potabilizar el agua?

### Opciones
- [x] A) Porque cada método elimina un tipo distinto de impureza, y juntos producen agua más segura.
  <!-- feedback: La combinación de métodos retira partículas, microorganismos y sustancias químicas. -->
- [ ] B) Porque un solo método elimina todo tipo de contaminante por completo.
  <!-- feedback: Ningún método único elimina todo tipo de contaminante; se necesitan varios pasos. -->
- [ ] C) Porque combinar métodos hace el agua más sucia y peligrosa.
  <!-- feedback: Combinar métodos busca hacer el agua más limpia, no más sucia. -->
- [ ] D) Porque el agua del río no necesita ningún tratamiento.
  <!-- feedback: El agua del río sí requiere tratamiento para eliminar contaminantes. -->

### Explicacion Pedagogica
Comprender la combinación de métodos físicos y químicos en el tratamiento de aguas.

## Question 8 [D9-D10]
**ID:** CO-CN-5-2026-W09-mezclas-separacion-001-MASTERY-bundle-v8
**Bloom:** Evaluate
**ICFES:** Indagación
**Expected_Success:** 0.30
**Contexto:** Lee: 'El conocimiento de las mezclas y sus métodos de separación es útil en la cocina, en la industria de alimentos, en el tratamiento de aguas y en el reciclaje de residuos. Elegir el método correcto permite aprovechar mejor los recursos.'

### Enunciado
¿Por qué es importante aprender a separar mezclas en la vida cotidiana?

### Opciones
- [x] A) Porque permite obtener sustancias puras, reciclar materiales y cuidar el ambiente.
  <!-- feedback: Separar mezclas es clave para obtener productos útiles y reducir residuos. -->
- [ ] B) Porque solo se usa en laboratorios avanzados sin aplicación en la vida diaria.
  <!-- feedback: Los métodos de separación se aplican en el hogar, la industria y el ambiente. -->
- [ ] C) Porque ningún producto cotidiano proviene de la separación de mezclas.
  <!-- feedback: Muchos productos como el agua potable, la sal y los aceites dependen de separar mezclas. -->
- [ ] D) Porque separar mezclas contamina más el ambiente que mezclarlas.
  <!-- feedback: Separar mezclas de forma adecuada ayuda a reducir la contaminación, no a aumentarla. -->

### Explicacion Pedagogica
Evaluar la importancia de los métodos de separación de mezclas en contextos cotidianos y ambientales.