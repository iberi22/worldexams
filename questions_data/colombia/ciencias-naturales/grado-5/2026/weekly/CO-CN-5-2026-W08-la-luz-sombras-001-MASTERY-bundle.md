---
id: "CO-CN-5-2026-W08-la-luz-sombras-001-MASTERY-bundle"
country: "colombia"
grado: 5
asignatura: "ciencias-naturales"
tema: "la-luz-sombras"
periodo: "weekly"
week: "W08"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 8
bundle_size: 8
alignment: "DBA MEN Colombia / Saber 5"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: La Luz y las Sombras - Grado 5 (W08)

Propagación rectilínea de la luz, formación de sombras, fuentes luminosas y reflexión de la luz en espejos y superficies.

## Question 1 [D3-D4]
**ID:** CO-CN-5-2026-W08-la-luz-sombras-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Indagación
**Expected_Success:** 0.90
**Contexto:** En clase de ciencias en Sogamoso, la profesora enciende una linterna en un salón oscuro.

### Enunciado
¿Cuál de los siguientes objetos es una fuente natural de luz?

### Opciones
- [x] A) El Sol.
  <!-- feedback: El Sol es una estrella que emite luz propia de manera natural. -->
- [ ] B) Una bombilla encendida.
  <!-- feedback: La bombilla es una fuente artificial de luz producida por el ser humano. -->
- [ ] C) Una linterna con pilas.
  <!-- feedback: La linterna es una fuente artificial de luz. -->
- [ ] D) Una vela encendida.
  <!-- feedback: La vela es una fuente artificial de luz creada por el ser humano. -->

### Explicacion Pedagogica
Diferenciar fuentes naturales y artificiales de luz a partir de ejemplos cotidianos.

## Question 2 [D3-D4]
**ID:** CO-CN-5-2026-W08-la-luz-sombras-001-MASTERY-bundle-v2
**Bloom:** Understand
**ICFES:** Indagación
**Expected_Success:** 0.85
**Contexto:** Lee: "Cuando la luz del Sol no puede atravesar un objeto opaco, detrás de él se forma una zona oscura llamada sombra."

### Enunciado
¿Por qué detrás de un objeto opaco iluminado por el Sol se forma una sombra?

### Opciones
- [x] A) Porque la luz viaja en línea recta y no puede atravesar el objeto opaco.
  <!-- feedback: La luz sigue una trayectoria rectilínea y al chocar con un cuerpo opaco se detiene, formando sombra. -->
- [ ] B) Porque el objeto absorbe el aire que está detrás.
  <!-- feedback: Los objetos no absorben aire; simplemente bloquean el paso de la luz. -->
- [ ] C) Porque el Sol se apaga detrás de los objetos.
  <!-- feedback: El Sol sigue iluminando; la sombra se debe al bloqueo de la luz, no a que se apague. -->
- [ ] D) Porque la sombra aparece sola sin importar la luz.
  <!-- feedback: Sin luz no se forman sombras; la sombra depende de la presencia de una fuente luminosa. -->

### Explicacion Pedagogica
Comprender la formación de sombras a partir de la propagación rectilínea de la luz.

## Question 3 [D5-D6]
**ID:** CO-CN-5-2026-W08-la-luz-sombras-001-MASTERY-bundle-v3
**Bloom:** Apply
**ICFES:** Indagación
**Expected_Success:** 0.76
**Contexto:** Un estudiante observa su sombra en el patio durante la tarde y nota que es más larga que en el mediodía.

### Enunciado
¿Por qué la sombra cambia de tamaño a lo largo del día?

### Opciones
- [x] A) Porque la posición del Sol en el cielo cambia y modifica el ángulo con que la luz llega al objeto.
  <!-- feedback: El ángulo de incidencia de la luz solar varía durante el día, cambiando el tamaño de la sombra. -->
- [ ] B) Porque el Sol se aleja físicamente de la Tierra en pocas horas.
  <!-- feedback: La distancia Sol-Tierra no cambia de manera perceptible en horas. -->
- [ ] C) Porque la sombra se infla con la humedad del aire.
  <!-- feedback: La humedad no infla las sombras; el cambio se debe al ángulo de la luz solar. -->
- [ ] D) Porque el objeto crece durante el día.
  <!-- feedback: Los objetos no crecen por estar expuestos al Sol; el cambio está en la sombra. -->

### Explicacion Pedagogica
Aplicar la variación del ángulo de la luz solar para explicar los cambios de tamaño de las sombras.

## Question 4 [D5-D6]
**ID:** CO-CN-5-2026-W08-la-luz-sombras-001-MASTERY-bundle-v4
**Bloom:** Apply
**ICFES:** Indagación
**Expected_Success:** 0.72
**Contexto:** En el salón, una niña coloca un objeto cerca de una lámpara encendida y observa una sombra en la pared.

### Enunciado
Si el objeto se aleja más de la lámpara, ¿qué pasa con el tamaño de la sombra en la pared?

### Opciones
- [x] A) La sombra se hace más grande porque los rayos se separan al alejarse.
  <!-- feedback: Al alejarse, los rayos divergen y producen una sombra más grande. -->
- [ ] B) La sombra se hace más pequeña porque se concentra en un punto.
  <!-- feedback: Al alejar el objeto, la sombra aumenta, no disminuye. -->
- [ ] C) La sombra desaparece sin importar la distancia.
  <!-- feedback: La sombra se mantiene mientras haya luz y un objeto opaco en su camino. -->
- [ ] D) La sombra mantiene siempre el mismo tamaño exacto.
  <!-- feedback: El tamaño de la sombra varía con la distancia a la fuente de luz. -->

### Explicacion Pedagogica
Aplicar la relación entre distancia a la fuente de luz y tamaño de la sombra proyectada.

## Question 5 [D5-D6]
**ID:** CO-CN-5-2026-W08-la-luz-sombras-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Indagación
**Expected_Success:** 0.66
**Contexto:** En un experimento, un estudiante mira un espejo plano y ve su propio reflejo.

### Enunciado
¿Cómo se llama el fenómeno que ocurre cuando la luz "rebota" en una superficie lisa como el espejo?

### Opciones
- [x] A) Reflexión de la luz.
  <!-- feedback: El rebote ordenado de la luz en una superficie lisa se llama reflexión. -->
- [ ] B) Refracción de la luz.
  <!-- feedback: La refracción ocurre cuando la luz pasa de un medio a otro, no cuando rebota. -->
- [ ] C) Absorción total de la luz.
  <!-- feedback: La absorción atrapa la luz; el espejo la devuelve, por eso no es absorción. -->
- [ ] D) Sombra proyectada.
  <!-- feedback: La sombra aparece cuando la luz es bloqueada, no cuando rebota. -->

### Explicacion Pedagogica
Identificar el fenómeno de reflexión de la luz en superficies lisas como los espejos.

## Question 6 [D7-D8]
**ID:** CO-CN-5-2026-W08-la-luz-sombras-001-MASTERY-bundle-v6
**Bloom:** Analyze
**ICFES:** Indagación
**Expected_Success:** 0.58
**Contexto:** Compara: "Un objeto translúcido, como un papel vegetal, deja pasar parte de la luz y produce una sombra tenue. Un objeto opaco, como un libro, no deja pasar la luz y produce una sombra oscura."

### Enunciado
¿Qué diferencia hay entre un objeto translúcido y uno opaco frente a la luz?

### Opciones
- [x] A) El translúcido deja pasar algo de luz; el opaco no la deja pasar.
  <!-- feedback: Los translúcidos permiten el paso parcial de luz, mientras los opacos lo bloquean totalmente. -->
- [ ] B) El translúcido produce sombras oscuras y el opaco no produce sombras.
  <!-- feedback: Es al revés: el opaco produce sombras oscuras, el translúcido produce sombras tenues. -->
- [ ] C) Ambos producen exactamente la misma sombra.
  <!-- feedback: La sombra varía según el material: tenue en translúcidos y oscura en opacos. -->
- [ ] D) Ninguno de los dos interactúa con la luz.
  <!-- feedback: Ambos interactúan con la luz, solo que de manera diferente. -->

### Explicacion Pedagogica
Comparar el comportamiento de la luz al atravesar materiales translúcidos y opacos.

## Question 7 [D7-D8]
**ID:** CO-CN-5-2026-W08-la-luz-sombras-001-MASTERY-bundle-v7
**Bloom:** Analyze
**ICFES:** Indagación
**Expected_Success:** 0.52
**Contexto:** Lee: "Durante un eclipse solar, la Luna se interpone entre el Sol y la Tierra y proyecta su sombra sobre una zona del planeta."

### Enunciado
¿Qué demuestra el eclipse solar sobre el comportamiento de la luz?

### Opciones
- [x] A) Que la luz viaja en línea recta y es bloqueada por cuerpos opacos como la Luna.
  <!-- feedback: El eclipse confirma la propagación rectilínea y el bloqueo por cuerpos opacos. -->
- [ ] B) Que la luz se apaga cuando hay Luna llena.
  <!-- feedback: La luz del Sol no se apaga por la Luna llena; el eclipse ocurre en Luna nueva. -->
- [ ] C) Que la Luna produce su propia luz durante el eclipse.
  <!-- feedback: La Luna no emite luz propia; refleja la luz del Sol. -->
- [ ] D) Que el Sol cambia de color y se vuelve transparente.
  <!-- feedback: El Sol no se vuelve transparente; sigue siendo opaco y luminoso. -->

### Explicacion Pedagogica
Relacionar los eclipses con la propagación rectilínea de la luz y la formación de sombras.

## Question 8 [D9-D10]
**ID:** CO-CN-5-2026-W08-la-luz-sombras-001-MASTERY-bundle-v8
**Bloom:** Evaluate
**ICFES:** Indagación
**Expected_Success:** 0.30
**Contexto:** Lee: 'La luz del Sol permite la vida en la Tierra, pero la exposición excesiva a sus rayos ultravioleta puede dañar la piel y los ojos. Usar protector solar, gafas con filtro UV y ropa adecuada ayuda a protegerse.'

### Enunciado
¿Por qué es importante protegerse de la luz solar excesiva y cómo hacerlo correctamente?

### Opciones
- [x] A) Porque los rayos ultravioleta pueden dañar piel y ojos; se protege con bloqueador, gafas con filtro UV y ropa adecuada.
  <!-- feedback: La protección adecuada reduce el daño por radiación UV sin dejar de aprovechar los beneficios de la luz solar. -->
- [ ] B) Porque la luz del Sol es tóxica y debe evitarse por completo a cualquier hora.
  <!-- feedback: La luz solar es esencial para la vida; el problema es el exceso, no la luz en sí. -->
- [ ] C) Porque solo las gafas oscuras ofrecen protección frente a los rayos UV.
  <!-- feedback: Las gafas oscuras sin filtro UV no protegen; debe haber filtro UV certificado. -->
- [ ] D) Porque la ropa de color negro deja pasar más rayos UV que la ropa blanca.
  <!-- feedback: La ropa oscura puede bloquear mejor la luz visible, pero lo clave es el material y el factor UV, no solo el color. -->

### Explicacion Pedagogica
Evaluar la importancia de la protección solar y reconocer las medidas adecuadas para cuidar la salud.