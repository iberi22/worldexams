---
id: "CO-LEC-8-2026-P2-intencion-autoral-INDEX-MASTERY"
country: "colombia"
grado: 8
asignatura: "lectura-critica"
tema: "intencion-autoral"
periodo: 2
protocol_version: "5.2"
bundle_index: 1
bundle_size: 15
alignment: "DBA MEN Ciclo 3 + Estándares Básicos"
modern_context: true
distractor_profile: "plausible_peer_set"
calibration:
  expected_success_rate: 0.50
  discrimination_index_target: ">= 0.20"
  simulated_responses: 100
rubric_baseline: "intención autoral, propósito comunicativo, tono, recursos retóricos, postura ideológica, mensaje implícito"
---

# Lectura Crítica G8 — Intención Autoral

## Tres Textos, Tres Intenciones

**Texto A: Editorial de Periódico — "El Río Bogotá: Una Vergüenza Nacional"**
*El río Bogotá es el más contaminado del país. A pesar de las promesas de todos los gobiernos, sigue siendo un vertedero de aguas negras y desechos industriales. Han pasado 30 años desde el primer plan de descontaminación y el río sigue muerto. Mientras tanto, los habitantes de la cuenca sufren enfermedades por la contaminación. El nuevo Plan de Ordenamiento Territorial de Bogotá incluye plantas de tratamiento, pero los plazos se han incumplido sistemáticamente. Como ciudadanos, debemos exigir acciones concretas, no más discursos. La salud de millones de personas y el futuro del ecosistema dependen de que dejemos de hablar y empecemos a actuar.*

**Texto B: Cuento — "El Río que Soñaba"**
*Había una vez un río que soñaba con llegar al mar. Pero cada mañana despertaba más sucio, más triste. Las fábricas lo pintaban de colores extraños. Los tubos negros le inyectaban veneno. Un día, una niña llamada Laura lanzó una semilla al agua. La semilla creció y se convirtió en un jardín flotante que filtraba el veneno. Poco a poco, el río recordó cómo era estar limpio. Cuando por fin llegó al mar, el mar lo abrazó y le dijo: "Siempre supe que podías".*

**Texto C: Anuncio Gubernamental — "¡El Río Bogotá, Más Vivo que Nunca!"**
*La Alcaldía Mayor de Bogotá se enorgullece en anunciar que el Plan de Descontaminación del Río Bogotá avanza según lo programado. Ya se han instalado 3 de las 5 plantas de tratamiento previstas. La meta es tener el río completamente recuperado para 2030. ¡Juntos estamos haciendo realidad la Bogotá que soñamos! Con tu ayuda, separando residuos y cuidando el agua, el río Bogotá volverá a ser el orgullo de la capital. ¡Por un río vivo, comprometámonos hoy!*

---

## Question 1 [D3]

**Bloom:** Remember
**ICFES:** Identificación de contenidos locales

### Enunciado
Según el Texto A, ¿cuánto tiempo ha pasado desde el primer plan de descontaminación?

### Options
- [ ] A) 10 años. <!-- feedback: Incorrecto. -->
- [x] B) 30 años. <!-- feedback: Correcto. -->
- [ ] C) 50 años. <!-- feedback: Incorrecto. -->
- [ ] D) 5 años. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de un dato temporal explícito.

## Question 2 [D3]

**Bloom:** Remember
**ICFES:** Identificación de contenidos locales

### Enunciado
En el Texto B, ¿qué lanzó Laura al río?

### Options
- [ ] A) Una moneda. <!-- feedback: Incorrecto. -->
- [x] B) Una semilla. <!-- feedback: Correcto. -->
- [ ] C) Un pez. <!-- feedback: Incorrecto. -->
- [ ] D) Una carta. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de un elemento narrativo.

## Question 3 [D4 - Intención]

**Bloom:** Understand
**ICFES:** Comprensión del sentido global

### Enunciado
¿Cuál es la intención principal del Texto A (editorial)?

### Options
- [ ] A) Informar neutralmente sobre el río. <!-- feedback: Incorrecto. -->
- [x] B) Denunciar y criticar la inacción gubernamental para generar indignación y exigir acción. <!-- feedback: Correcto. -->
- [ ] C) Entretener con una historia. <!-- feedback: Incorrecto. -->
- [ ] D) Promocionar un producto. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de la intención crítica y denunciante de una editorial.

## Question 4 [D4 - Intención]

**Bloom:** Understand
**ICFES:** Comprensión del sentido global

### Enunciado
¿Cuál es la intención principal del Texto B (cuento)?

### Options
- [ ] A) Denunciar al gobierno. <!-- feedback: Incorrecto. -->
- [x] B) Transmitir un mensaje de esperanza y conciencia ecológica a través de una historia simbólica. <!-- feedback: Correcto. -->
- [ ] C) Informar sobre plantas de tratamiento. <!-- feedback: Incorrecto. -->
- [ ] D) Promocionar una marca. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de la intención narrativa y simbólica.

## Question 5 [D4 - Intención]

**Bloom:** Understand
**ICFES:** Comprensión del sentido global

### Enunciado
¿Cuál es la intención principal del Texto C (anuncio gubernamental)?

### Options
- [ ] A) Criticar a la administración pasada. <!-- feedback: Incorrecto. -->
- [x] B) Presentar una imagen positiva de la gestión actual y motivar la participación ciudadana. <!-- feedback: Correcto. -->
- [ ] C) Contar una historia sobre el río. <!-- feedback: Incorrecto. -->
- [ ] D) Denunciar la contaminación. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de la intención propagandística o comunicacional.

## Question 6 [D5 - Tono]

**Bloom:** Analyze
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué tono predomina en el Texto A?

### Options
- [ ] A) Alegre y optimista. <!-- feedback: Incorrecto. -->
- [x] B) Crítico, indignado y urgente. <!-- feedback: Correcto. -->
- [ ] C) Neutral y objetivo. <!-- feedback: Incorrecto. -->
- [ ] D) Poético y suave. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Análisis del tono como indicador de la intención autoral.

## Question 7 [D5 - Tono]

**Bloom:** Analyze
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué tono predomina en el Texto C?

### Options
- [ ] A) Crítico y negativo. <!-- feedback: Incorrecto. -->
- [x] B) Optimista, triunfalista y motivador. <!-- feedback: Correcto. -->
- [ ] C) Triste y melancólico. <!-- feedback: Incorrecto. -->
- [ ] D) Científico y frío. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Análisis del tono optimista como herramienta de comunicación gubernamental.

## Question 8 [D5 - Recursos retóricos]

**Bloom:** Analyze
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué recurso usa el Texto A para generar indignación?

### Options
- [ ] A) Datos positivos. <!-- feedback: Incorrecto. -->
- [x] B) La palabra "vergüenza" y la repetición del incumplimiento ("30 años", "sistemáticamente"). <!-- feedback: Correcto. -->
- [ ] C) Personificación del río. <!-- feedback: Incorrecto. -->
- [ ] D) Rima y poesía. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de recursos retóricos usado para crear una reacción emocional.

## Question 9 [D5 - Mensaje implícito]

**Bloom:** Analyze
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué mensaje implícito transmite el Texto B con el jardín flotante?

### Options
- [ ] A) Que la contaminación no tiene solución. <!-- feedback: Incorrecto. -->
- [x] B) Que incluso un acto pequeño (una semilla) puede iniciar un cambio positivo. <!-- feedback: Correcto. -->
- [ ] C) Que los ríos pueden soñar. <!-- feedback: Incorrecto. -->
- [ ] D) Que las fábricas son buenas. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Interpretación del mensaje simbólico detrás de la metáfora.

## Question 10 [D5 - Contraste de intenciones]

**Bloom:** Analyze
**ICFES:** Dimensión inferencial

### Enunciado
¿Por qué el Texto A y el Texto C tienen posturas opuestas sobre el mismo plan?

### Options
- [ ] A) Porque uno dice verdad y otro miente. <!-- feedback: Incorrecto. -->
- [x] B) Porque tienen intenciones distintas: uno busca denunciar, el otro busca proyectar una imagen exitosa. <!-- feedback: Correcto. -->
- [ ] C) Porque se escribieron en años distintos. <!-- feedback: Incorrecto. -->
- [ ] D) Porque hablan de ríos diferentes. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Análisis de cómo la intención determina la selección y presentación de la información.

## Question 11 [D5 - Público objetivo]

**Bloom:** Analyze
**ICFES:** Dimensión inferencial

### Enunciado
¿A qué público se dirige principalmente el Texto C?

### Options
- [ ] A) A científicos ambientales. <!-- feedback: Incorrecto. -->
- [x] B) A la ciudadanía bogotana en general. <!-- feedback: Correcto. -->
- [ ] C) A los niños. <!-- feedback: Incorrecto. -->
- [ ] D) A los empresarios. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación del público objetivo según el lenguaje y el canal.

## Question 12 [D5 - Lenguaje]

**Bloom:** Analyze
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué tipo de lenguaje usa el Texto B que lo diferencia del A y del C?

### Options
- [ ] A) Lenguaje técnico y legal. <!-- feedback: Incorrecto. -->
- [ ] B) Lenguaje estadístico. <!-- feedback: Incorrecto. -->
- [x] C) Lenguaje literario y figurado (personificación, metáfora). <!-- feedback: Correcto. -->
- [ ] D) Lenguaje publicitario. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Diferenciación del lenguaje según el género textual y la intención.

## Question 13 [D6 - Evaluación de intención]

**Bloom:** Evaluate
**ICFES:** Reflexión sobre el contenido

### Enunciado
¿Cuál texto es más objetivo y cuál es más subjetivo?

### Options
- [ ] A) Todos son objetivos. <!-- feedback: Incorrecto. -->
- [ ] B) A es objetivo, B y C subjetivos. <!-- feedback: Incorrecto. -->
- [ ] C) B es objetivo, A y C subjetivos. <!-- feedback: Incorrecto. -->
- [x] D) Ninguno es completamente objetivo. A y C son abiertamente subjetivos (editorial y propaganda), B es ficción simbólica con intención subjetiva. <!-- feedback: Correcto. -->

### Explicación Pedagógica
Evaluación de la objetividad/subjetividad como reflejo de la intención autoral.

## Question 14 [D6]

**Bloom:** Evaluate
**ICFES:** Reflexión sobre el contenido

### Enunciado
¿Por qué es importante identificar la intención del autor al leer un texto?

### Options
- [ ] A) Para saber si el autor es bueno o malo. <!-- feedback: Incorrecto. -->
- [x] B) Para comprender por qué el autor seleccionó cierta información y la presentó de cierta manera. <!-- feedback: Correcto. -->
- [ ] C) Para memorizar datos. <!-- feedback: Incorrecto. -->
- [ ] D) Para ganar argumentos. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Reflexión metacognitiva sobre la importancia del análisis de intención en la lectura crítica.

## Question 15 [D5]

**Bloom:** Analyze
**ICFES:** Dimensión inferencial

### Enunciado
Si el Texto A y el Texto C hablan del mismo plan de descontaminación, ¿cómo explicarías sus diferencias?

### Options
- [ ] A) Uno tiene razón y el otro no. <!-- feedback: Incorrecto. -->
- [x] B) Responden a intereses distintos: el editorial busca crítica y vigilancia ciudadana; el anuncio busca legitimar la gestión pública. Ambas pueden contener verdades parciales. <!-- feedback: Correcto. -->
- [ ] C) Uno es viejo y otro es nuevo. <!-- feedback: Incorrecto. -->
- [ ] D) Uno es de izquierda y otro de derecha. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Síntesis: comprensión de cómo la misma realidad puede ser presentada de formas radicalmente diferentes según la intención.

### Explicación Pedagógica Final
Bundle sobre intención autoral para grado 8, con tres textos contrastantes sobre el río Bogotá (editorial crítica, cuento simbólico y anuncio gubernamental). Desarrolla habilidades de identificación de propósito comunicativo, análisis de tono, recursos retóricos, público objetivo, y evaluación de subjetividad/objetividad en función de la intención del autor.
