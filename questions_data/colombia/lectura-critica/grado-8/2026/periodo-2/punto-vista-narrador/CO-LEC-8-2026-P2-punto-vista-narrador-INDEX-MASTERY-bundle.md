---
id: "CO-LEC-8-2026-P2-punto-vista-narrador-INDEX-MASTERY-bundle"
country: "colombia"
grado: 8
asignatura: "lectura-critica"
tema: "punto-vista-narrador"
periodo: 2
protocol_version: "5.2"
bundle_index: 1
bundle_size: 15
alignment: "DBA MEN Ciclo 3 + Estándares Básicos"
modern_context: true
distractor_profile: "plausible_peer_set"
calibration:
  expected_success_rate: 0.50
  discrimination_index_target: ">= 0.20"
  simulated_responses: 100
rubric_baseline: "punto de vista, narrador, voz narrativa, perspectiva, primera persona, tercera persona, narrador omnisciente, narrador testigo, confiabilidad"
bundle_type: "weekly"
total_questions: 15
year: 2026
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---


# Lectura Crítica G8 — Punto de Vista del Narrador

## La Protesta: Tres Miradas, Una Historia

*El 25 de abril de 2025, una protesta de estudiantes universitarios en Bogotá fue noticia nacional. Aquí están tres versiones del mismo evento:*

**Versión 1: Narrador Testigo (Estudiante en primera persona)**
*"Estaba ahí. A las 9 de la mañana llegamos a la Plaza de Bolívar. Éramos miles. Cantábamos, llevábamos carteles. De repente, escuchamos las sirenas. El ESMAD avanzó. Sentí miedo, mucho miedo. Corrí. Una señora me jaló del brazo y me metió a su tienda. 'Métase, mijo, que eso está feo', me dijo. Desde la puerta vi todo: los gases lacrimógenos, la gente tosiendo, un muchacho en el suelo. No sé si era consciente de lo que hacía o solo tenía miedo. Yo solo quería que parara."*

**Versión 2: Narrador Omnisciente (Tercera persona, lo sabe todo)**
*"A las 9:03 de aquel viernes, la Plaza de Bolívar bullía de estudiantes. María, de 19 años, pensaba en su papá que había protestado en el 2021. El sargento Rodríguez, al mando del ESMAD, recordó las órdenes: despejar la plaza a las 10. Ninguno de los dos sabía que a las 9:47 un vidrio se rompería y cambiaría todo. La señora Jiménez, dueña de la tienda de la esquina, observaba desde su ventana, presa de una ansiedad que no sentía desde el Paro Nacional. Todos ignoraban que el muchacho del megáfono había perdido a su hermano en una protesta anterior. Cada uno llevaba una historia que los demás no conocían."*

**Versión 3: Narrador en Segunda Persona (Policía, informe oficial)**
*"Usted se presentó en la Plaza de Bolívar a las 08:45 horas. Usted formaba parte del dispositivo de seguridad. Usted recibió la orden de dispersar a la multitud a las 09:50 horas. Usted hizo uso de gas lacrimógeno siguiendo el protocolo establecido. Usted no identifica a ningún manifestante herido de gravedad. Usted declara que la protesta se desarrolló dentro de los parámetros legales."*

---

## Question 1 [D3]

**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de contenidos locales

### Enunciado
¿En qué lugar ocurrieron los hechos?

### Opciones
- [ ] A) En el Parque Simón Bolívar. <!-- feedback: Incorrecto. -->
- [x] B) En la Plaza de Bolívar, Bogotá. <!-- feedback: Correcto. -->
- [ ] C) En la Universidad Nacional. <!-- feedback: Incorrecto. -->
- [ ] D) En el Capitolio Nacional. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación del lugar explícito.

## Question 2 [D3]

**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de contenidos locales

### Enunciado
Según la Versión 1, ¿quién ayudó al estudiante?

### Opciones
- [ ] A) Un policía. <!-- feedback: Incorrecto. -->
- [x] B) Una señora dueña de una tienda. <!-- feedback: Correcto. -->
- [ ] C) Un profesor. <!-- feedback: Incorrecto. -->
- [ ] D) Un compañero. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de un personaje secundario.

## Question 3 [D4 - Punto de vista]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
¿En qué persona gramatical está narrada la Versión 1?

### Opciones
- [x] A) Primera persona ("Estaba ahí", "Sentí", "Corrí"). <!-- feedback: Correcto. -->
- [ ] B) Segunda persona ("Usted"). <!-- feedback: Incorrecto. -->
- [ ] C) Tercera persona ("Él", "Ella"). <!-- feedback: Incorrecto. -->
- [ ] D) Tercera persona omnisciente. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación del tipo de narrador según la persona gramatical.

## Question 4 [D4 - Punto de vista]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
¿Qué tipo de narrador tiene la Versión 2?

### Opciones
- [ ] A) Narrador testigo (solo ve lo que pasa). <!-- feedback: Incorrecto. -->
- [x] B) Narrador omnisciente (conoce los pensamientos y emociones de todos los personajes). <!-- feedback: Correcto. -->
- [ ] C) Narrador en primera persona (participante). <!-- feedback: Incorrecto. -->
- [ ] D) Narrador en segunda persona. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación del narrador omnisciente.

## Question 5 [D4 - Punto de vista]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
La Versión 3 usa la segunda persona ("Usted"). ¿Quién es el narrador?

### Opciones
- [ ] A) Un estudiante. <!-- feedback: Incorrecto. -->
- [ ] B) Un periodista. <!-- feedback: Incorrecto. -->
- [x] C) Un policía redactando un informe oficial. <!-- feedback: Correcto. -->
- [ ] D) Un juez. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación del emisor según el uso de la segunda persona y el lenguaje formal.

## Question 6 [D5 - Perspectiva]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué información tenemos en la Versión 2 que no tenemos en las otras?

### Opciones
- [ ] A) Lo que pasó en la protesta. <!-- feedback: Incorrecto (todas lo tienen). -->
- [x] B) Los pensamientos y emociones internas de varios personajes (María, sargento, señora Jiménez). <!-- feedback: Correcto. -->
- [ ] C) La hora exacta de los hechos. <!-- feedback: Incorrecto. -->
- [ ] D) El nombre de los policías. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de la ventaja informativa del narrador omnisciente.

## Question 7 [D5 - Confiabilidad]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué posible sesgo tiene la Versión 1 (estudiante)?

### Opciones
- [ ] A) Es completamente objetiva. <!-- feedback: Incorrecto. -->
- [x] B) Está contada desde el miedo y la experiencia personal, lo que puede exagerar la violencia. <!-- feedback: Correcto. -->
- [ ] C) Defiende la actuación policial. <!-- feedback: Incorrecto. -->
- [ ] D) Es neutral. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Evaluación de la confiabilidad del narrador en primera persona.

## Question 8 [D5 - Confiabilidad]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué posible sesgo tiene la Versión 3 (informe policial)?

### Opciones
- [ ] A) Es poética y subjetiva. <!-- feedback: Incorrecto. -->
- [x] B) Usa lenguaje técnico y formal que minimiza la violencia ("dispersar", "parámetros legales") y omite las consecuencias humanas. <!-- feedback: Correcto. -->
- [ ] C) Exagera el conflicto. <!-- feedback: Incorrecto. -->
- [ ] D) Apoya a los estudiantes. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Evaluación del sesgo institucional en narraciones oficiales.

## Question 9 [D5 - Efecto narrativo]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué efecto produce que la Versión 1 incluya emociones personales ("Sentí miedo, mucho miedo")?

### Opciones
- [ ] A) Hace el texto más científico. <!-- feedback: Incorrecto. -->
- [x] B) Genera empatía en el lector y transmite la experiencia humana del evento. <!-- feedback: Correcto. -->
- [ ] C) Aleja al lector. <!-- feedback: Incorrecto. -->
- [ ] D) Confunde la información. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Análisis del efecto emocional del narrador en primera persona.

## Question 10 [D5]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué efecto produce la Versión 3 al tratar al lector como "Usted"?

### Opciones
- [ ] A) Hace el texto más cercano. <!-- feedback: Incorrecto. -->
- [x] B) Hace el texto impersonal y formal, como si el lector fuera el interrogado. <!-- feedback: Correcto. -->
- [ ] C) Hace el texto poético. <!-- feedback: Incorrecto. -->
- [ ] D) Hace el texto divertido. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Análisis del efecto de la segunda persona en la relación lector-texto.

## Question 11 [D5 - Comparación]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Cuál versión parece más "objetiva"? ¿Y por qué?

### Opciones
- [ ] A) Versión 1, porque tiene emociones. <!-- feedback: Incorrecto. -->
- [ ] B) Versión 2, porque incluye múltiples perspectivas (omnisciente). <!-- feedback: Correcto. -->
- [ ] C) Versión 3, porque usa lenguaje técnico. <!-- feedback: Incorrecto (la tecnicidad puede ser una máscara de objetividad). -->
- [ ] D) Ninguna es objetiva. <!-- feedback: Parcialmente correcto, pero la 2 ofrece más perspectivas. -->

### Explicación Pedagógica
Evaluación de la objetividad según el tipo de narrador.

## Question 12 [D5 - Propósito según narrador]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Por qué un periodista que busca cubrir la protesta imparcialmente usaría un narrador como el de la Versión 2?

### Opciones
- [ ] A) Para mostrar su opinión. <!-- feedback: Incorrecto. -->
- [x] B) Para ofrecer múltiples perspectivas y dar una visión más completa del evento. <!-- feedback: Correcto. -->
- [ ] C) Para acusar a los policías. <!-- feedback: Incorrecto. -->
- [ ] D) Para defender a los estudiantes. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Relación entre la elección del narrador y el propósito informativo/periodístico.

## Question 13 [D6 - Evaluación]

**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido

### Enunciado
Si quisieras entender realmente lo que pasó en la protesta, ¿qué harías?

### Opciones
- [ ] A) Leer solo la versión del estudiante. <!-- feedback: Incorrecto. -->
- [ ] B) Leer solo el informe policial. <!-- feedback: Incorrecto. -->
- [x] C) Leer las tres versiones y contrastarlas para construir una visión más completa. <!-- feedback: Correcto. -->
- [ ] D) No leer ninguna porque todas mienten. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Reflexión sobre la importancia del contraste de fuentes para la comprensión de la realidad.

## Question 14 [D5]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué recurso estilístico comparten las Versiones 1 y 3, aunque con intenciones opuestas?

### Opciones
- [ ] A) Ambas usan metáforas. <!-- feedback: Incorrecto. -->
- [x] B) Ambas usan la primera persona (Versión 1: "yo"; Versión 3: "usted" como sujeto). <!-- feedback: Correcto (la Versión 3 trata al lector como agente directo). -->
- [ ] C) Ambas usan rima. <!-- feedback: Incorrecto. -->
- [ ] D) Ambas son poesía. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de elementos comunes en narradores diferentes.

## Question 15 [D6]

**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido

### Enunciado
¿Por qué es importante identificar el punto de vista del narrador al leer?

### Opciones
- [ ] A) Para saber si la historia es verdadera. <!-- feedback: Incorrecto. -->
- [x] B) Para entender desde qué posición se cuenta la historia y cómo eso afecta la información que recibimos. <!-- feedback: Correcto. -->
- [ ] C) Para calificar al autor. <!-- feedback: Incorrecto. -->
- [ ] D) Para saber cuántos personajes hay. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Reflexión metacognitiva sobre la importancia del punto de vista narrativo.

### Explicación Pedagógica Final
Bundle sobre punto de vista del narrador para grado 8, con tres versiones contrastantes de una protesta estudiantil. Desarrolla habilidades de identificación de tipos de narrador (primera persona, omnisciente, segunda persona), análisis de sesgo y confiabilidad según la perspectiva narrativa, y comprensión del efecto que el punto de vista tiene en la construcción de la realidad textual.
