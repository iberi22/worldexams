---
id: "CO-LEC-8-2026-P1-analisis-critico-INDEX-MASTERY-bundle"
country: "colombia"
grado: 8
asignatura: "lectura-critica"
tema: "analisis-critico"
periodo: 1
protocol_version: "5.2"
bundle_index: 1
bundle_size: 15
alignment: "DBA MEN Ciclo 3 + Estándares Básicos"
modern_context: true
distractor_profile: "plausible_peer_set"
calibration:
  expected_success_rate: 0.48
  discrimination_index_target: ">= 0.20"
  simulated_responses: 100
rubric_baseline: "análisis crítico, sesgo, credibilidad, fuente, perspectiva, veracidad, argumentación múltiple, evaluación de evidencia"
bundle_type: "weekly"
total_questions: 15
year: 2026
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---


# Lectura Crítica G8 — Análisis Crítico

## ¿Debe Colombia Legalizar la Marihuana Recreativa?

*El debate sobre la legalización del cannabis recreativo en Colombia ha tomado fuerza. A continuación, tres fuentes con perspectivas distintas.*

**Fuente 1: Ministerio de Justicia de Colombia (Documento Oficial, 2024)**
*"El consumo de marihuana en Colombia ha aumentado un 35% entre jóvenes de 12 a 17 años en la última década. Actualmente, el porte de dosis personal está despenalizado. Sin embargo, el narcotráfico sigue controlando la producción ilegal. Estudios indican que la legalización permitiría regular el mercado, reducir la violencia asociada al narcotráfico y generar impuestos estimados en 2 billones de pesos anuales. Se recomienda un debate amplio que considere salud pública y seguridad."*

**Fuente 2: Asociación Colombiana de Psiquiatría (Comunicado, 2025)**
*"La evidencia científica muestra que el consumo frecuente de cannabis en adolescentes afecta el desarrollo del cerebro, especialmente la memoria y la concentración. El 17% de los jóvenes que consumen marihuana desarrollan dependencia. La legalización sin restricciones podría aumentar el consumo y los problemas de salud mental. Recomendamos un modelo restrictivo que proteja a menores de edad y priorice la prevención."*

**Fuente 3: Plataforma de Campesinos Cultivadores de Cannabis (Carta Abierta, 2025)**
*"Somos más de 200.000 familias campesinas que dependen del cultivo de cannabis. Con la prohibición, somos perseguidos, encarcelados y desplazados. Mientras las grandes farmacéuticas se enriquecen con medicamentos derivados del cannabis, a nosotros nos queman nuestros cultivos. La legalización no es solo una cuestión de consumo: es una cuestión de justicia social para el campesinado colombiano. Exigimos una ley que nos incluya."*

---

## Question 1 [D3]

**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de contenidos locales

### Enunciado
¿Cuántas familias campesinas dependen del cultivo de cannabis según la Fuente 3?

### Opciones
- [ ] A) 50.000. <!-- feedback: Incorrecto. -->
- [x] B) Más de 200.000. <!-- feedback: Correcto. -->
- [ ] C) 500.000. <!-- feedback: Incorrecto. -->
- [ ] D) 1 millón. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de un dato explícito.

## Question 2 [D3]

**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de contenidos locales

### Enunciado
Según la Fuente 1, ¿cuánto aumentó el consumo de marihuana en jóvenes de 12 a 17 años?

### Opciones
- [ ] A) 15%. <!-- feedback: Incorrecto. -->
- [x] B) 35%. <!-- feedback: Correcto. -->
- [ ] C) 50%. <!-- feedback: Incorrecto. -->
- [ ] D) 10%. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de datos estadísticos.

## Question 3 [D4 - Análisis de fuente]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
¿Qué tipo de fuente es la Fuente 1?

### Opciones
- [ ] A) Una ONG internacional. <!-- feedback: Incorrecto. -->
- [x] B) Una entidad gubernamental colombiana (Ministerio de Justicia). <!-- feedback: Correcto. -->
- [ ] C) Un periódico. <!-- feedback: Incorrecto. -->
- [ ] D) Una universidad. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de la naturaleza institucional de una fuente.

## Question 4 [D4 - Análisis de fuente]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
¿Qué tipo de organización es la Fuente 2?

### Opciones
- [ ] A) Gubernamental. <!-- feedback: Incorrecto. -->
- [x] B) Asociación científica de profesionales de la psiquiatría. <!-- feedback: Correcto. -->
- [ ] C) Empresa privada. <!-- feedback: Incorrecto. -->
- [ ] D) Partido político. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de la naturaleza profesional de una fuente.

## Question 5 [D5 - Sesgo]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué posible sesgo tiene la Fuente 3?

### Opciones
- [ ] A) Defiende la prohibición. <!-- feedback: Incorrecto. -->
- [x] B) Representa los intereses económicos y de justicia social del campesinado cultivador. <!-- feedback: Correcto. -->
- [ ] C) Es neutral. <!-- feedback: Incorrecto. -->
- [ ] D) Es científica. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación del sesgo desde la perspectiva del emisor.

## Question 6 [D5 - Sesgo]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué posible sesgo tiene la Fuente 2?

### Opciones
- [ ] A) Está a favor de la legalización sin restricciones. <!-- feedback: Incorrecto. -->
- [x] B) Su enfoque clínico puede priorizar la prevención de riesgos de salud sobre otros aspectos. <!-- feedback: Correcto. -->
- [ ] C) Defiende intereses económicos. <!-- feedback: Incorrecto. -->
- [ ] D) Apoya a los campesinos. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación del sesgo desde la perspectiva profesional de la salud.

## Question 7 [D5 - Credibilidad]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Cuál fuente tiene mayor credibilidad para datos epidemiológicos sobre consumo juvenil?

### Opciones
- [ ] A) Fuente 3 (campesinos). <!-- feedback: Incorrecto. -->
- [ ] B) Fuente 2 (psiquiatría). <!-- feedback: Parcialmente correcto, pero la 1 también. -->
- [x] C) Ambas, Fuente 1 (Ministerio, datos oficiales) y Fuente 2 (psiquiatras, evidencia científica). <!-- feedback: Correcto. -->
- [ ] D) Ninguna tiene credibilidad. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Evaluación de la credibilidad según el tipo de dato y la especialidad de la fuente.

## Question 8 [D5 - Perspectiva]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué perspectiva agrega la Fuente 3 que no está en las otras dos?

### Opciones
- [ ] A) La perspectiva de salud pública. <!-- feedback: Incorrecto. -->
- [ ] B) La perspectiva económica del Estado. <!-- feedback: Incorrecto. -->
- [x] C) La perspectiva del campesinado y la justicia social. <!-- feedback: Correcto. -->
- [ ] D) La perspectiva internacional. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de la perspectiva única que cada fuente aporta al debate.

## Question 9 [D5 - Confiabilidad]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué tipo de evidencia usa la Fuente 2 para respaldar su postura?

### Opciones
- [ ] A) Testimonios personales. <!-- feedback: Incorrecto. -->
- [x] B) Evidencia científica sobre desarrollo cerebral y porcentaje de dependencia (17%). <!-- feedback: Correcto. -->
- [ ] C) Opiniones políticas. <!-- feedback: Incorrecto. -->
- [ ] D) Datos de mercado. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación del tipo de evidencia utilizada (científica).

## Question 10 [D5 - Confiabilidad]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué tipo de evidencia usa la Fuente 3?

### Opciones
- [ ] A) Datos de laboratorio. <!-- feedback: Incorrecto. -->
- [x] B) Experiencia vivencial y denuncia de injusticia social. <!-- feedback: Correcto. -->
- [ ] C) Estudios estadísticos. <!-- feedback: Incorrecto. -->
- [ ] D) Leyes internacionales. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Diferenciación entre evidencia científica y testimonial.

## Question 11 [D5 - Puntos en común]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿En qué coinciden las tres fuentes?

### Opciones
- [ ] A) En apoyar la prohibición total. <!-- feedback: Incorrecto. -->
- [ ] B) En apoyar la legalización sin restricciones. <!-- feedback: Incorrecto. -->
- [x] C) En que el tema es complejo y requiere regulación (cada una desde su perspectiva). <!-- feedback: Correcto. -->
- [ ] D) En que los campesinos deben ser ignorados. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de puntos de consenso entre perspectivas diferentes.

## Question 12 [D6 - Análisis crítico]

**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido

### Enunciado
Para tomar una decisión informada sobre este tema, ¿qué más necesitarías?

### Opciones
- [ ] A) Solo la opinión del Ministerio. <!-- feedback: Incorrecto. -->
- [ ] B) Solo la opinión de los psiquiatras. <!-- feedback: Incorrecto. -->
- [x] C) Datos de impuestos estimados, modelos de otros países, estudios de impacto social y opinión de comunidades. <!-- feedback: Correcto. -->
- [ ] D) Solo la opinión de los campesinos. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Reflexión sobre la necesidad de múltiples fuentes y datos para un análisis crítico completo.

## Question 13 [D6]

**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido

### Enunciado
¿Cuál fuente presenta un argumento más equilibrado?

### Opciones
- [ ] A) Fuente 3, porque habla de justicia social. <!-- feedback: Parcial. -->
- [x] B) Fuente 1, porque reconoce tanto los riesgos como los beneficios potenciales. <!-- feedback: Correcto. -->
- [ ] C) Fuente 2, porque solo habla de riesgos. <!-- feedback: Incorrecto. -->
- [ ] D) Todas son equilibradas. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Evaluación del balance en la presentación de argumentos.

## Question 14 [D5]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué interés particular podría tener la Fuente 1 en presentar la cifra de 2 billones en impuestos?

### Opciones
- [ ] A) No tiene interés. <!-- feedback: Incorrecto. -->
- [x] B) Mostrar un beneficio económico que justifique la legalización como política fiscal. <!-- feedback: Correcto. -->
- [ ] C) Asustar a los lectores. <!-- feedback: Incorrecto. -->
- [ ] D) Promover el consumo. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Análisis crítico de los interdetrás de los datos presentados.

## Question 15 [D6]

**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido

### Enunciado
¿Cuál es la principal debilidad de la Fuente 3 como fuente de información objetiva?

### Opciones
- [ ] A) Que no tiene datos. <!-- feedback: Incorrecto (sí menciona 200.000 familias). -->
- [x] B) Que es una fuente con un interés directo en el resultado, lo que afecta su objetividad. <!-- feedback: Correcto. -->
- [ ] C) Que es muy larga. <!-- feedback: Incorrecto. -->
- [ ] D) Que apoya la prohibición. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Evaluación de la objetividad de las fuentes según su interés en el resultado.

### Explicación Pedagógica Final
Bundle de análisis crítico para grado 8, con tres fuentes diversas sobre la legalización del cannabis recreativo. Desarrolla habilidades de identificación de sesgos, evaluación de credibilidad, análisis de perspectivas múltiples, y reflexión sobre la necesidad de información diversa para formar opiniones informadas.
