---
id: "CO-LC-5-2026-W08-historieta-secuencia-001-MASTERY-bundle"
country: "colombia"
grado: 5
asignatura: "lectura-critica"
tema: "historieta-secuencia"
periodo: "weekly"
week: "W08"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 8
bundle_size: 8
alignment: "DBA MEN Colombia / Saber 5"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: Historieta y Secuencia Narrativa - Grado 5 (W08)

Elementos de la historieta (viñetas, globos de texto, onomatopeyas), orden lógico de una secuencia narrativa y lectura inferencial de imágenes.

## Question 1 [D3-D4]
**ID:** CO-LC-5-2026-W08-historieta-secuencia-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Semántico
**Expected_Success:** 0.89
**Contexto:** En clase de lectura en una escuela de Ibague, la profesora muestra varias historietas colombianas como Mafalda y Condorito.

### Enunciado
¿Cuál es un componente típico de una historieta?

### Opciones
- [x] A) Las viñetas con imágenes, los globos de texto y las onomatopeyas.
  <!-- feedback: Las viñetas, los globos de texto y las onomatopeyas son elementos clásicos de la historieta. -->
- [ ] B) Solo párrafos extensos sin imágenes.
  <!-- feedback: La historieta combina imagen y texto, no solo párrafos largos. -->
- [ ] C) Únicamente ecuaciones matemáticas.
  <!-- feedback: Las historietas no se centran en ecuaciones matemáticas. -->
- [ ] D) Solo instrucciones para cocinar.
  <!-- feedback: Las historietas narran historias, no son recetas de cocina. -->

### Explicacion Pedagogica
Reconocer los elementos básicos que componen una historieta.

## Question 2 [D3-D4]
**ID:** CO-LC-5-2026-W08-historieta-secuencia-001-MASTERY-bundle-v2
**Bloom:** Understand
**ICFES:** Semántico
**Expected_Success:** 0.84
**Contexto:** Lee: "Una historieta cuenta una historia mediante una secuencia de viñetas que el lector debe seguir de izquierda a derecha y de arriba hacia abajo."

### Enunciado
¿Por qué es importante seguir el orden de las viñetas al leer una historieta?

### Opciones
- [x] A) Porque el orden de las viñetas determina la secuencia lógica de la historia narrada.
  <!-- feedback: Cambiar el orden altera el significado y la coherencia de la historieta. -->
- [ ] B) Porque el orden solo importa al final del libro.
  <!-- feedback: El orden importa en toda la lectura, no solo al final. -->
- [ ] C) Porque el orden no influye en la comprensión del relato.
  <!-- feedback: El orden de las viñetas es clave para entender la historia. -->
- [ ] D) Porque las viñetas siempre se leen en sentido contrario.
  <!-- feedback: En la tradición occidental, las viñetas se leen de izquierda a derecha y arriba hacia abajo. -->

### Explicacion Pedagogica
Comprender la importancia del orden secuencial de las viñetas en una historieta.

## Question 3 [D5-D6]
**ID:** CO-LC-5-2026-W08-historieta-secuencia-001-MASTERY-bundle-v3
**Bloom:** Apply
**ICFES:** Semántico
**Expected_Success:** 0.75
**Contexto:** Un estudiante debe ordenar las viñetas de una historieta en la que un niño pierde su perro y luego lo encuentra gracias a un aviso en el barrio.

### Enunciado
¿Cuál es el orden lógico de las viñetas en esta historieta?

### Opciones
- [x] A) El niño juega con su perro, luego lo busca llorando y al final lo encuentra por un aviso.
  <!-- feedback: El orden sigue una secuencia lógica: inicio, conflicto y desenlace. -->
- [ ] B) El niño encuentra al perro, luego lo busca y después juega con él.
  <!-- feedback: Este orden altera la lógica temporal del relato. -->
- [ ] C) El niño busca al perro, lo encuentra sin buscarlo y finalmente lo pierde otra vez.
  <!-- feedback: El orden debe respetar la progresión natural de los hechos. -->
- [ ] D) El niño coloca el aviso, luego pierde al perro y después lo busca.
  <!-- feedback: El aviso aparece como solución, no antes de la pérdida. -->

### Explicacion Pedagogica
Aplicar el orden lógico de una secuencia narrativa en una historieta.

## Question 4 [D5-D6]
**ID:** CO-LC-5-2026-W08-historieta-secuencia-001-MASTERY-bundle-v4
**Bloom:** Apply
**ICFES:** Semántico
**Expected_Success:** 0.71
**Contexto:** Lee: "En una historieta, los globos de texto pueden mostrar diálogos (con cola), pensamientos (con forma de nube) y gritos (con borde dentado)."

### Enunciado
¿Qué indica un globo con borde dentado en una historieta?

### Opciones
- [x] A) Que el personaje está gritando o expresando una emoción intensa.
  <!-- feedback: Los bordes dentados suelen indicar gritos o emociones fuertes. -->
- [ ] B) Que el personaje está durmiendo profundamente.
  <!-- feedback: El sueño se representa de otra manera, no con borde dentado. -->
- [ ] C) Que el personaje está leyendo un libro en silencio.
  <!-- feedback: La lectura silenciosa no se asocia con bordes dentados. -->
- [ ] D) Que el personaje está dibujando la historieta.
  <!-- feedback: El dibujo de la historieta no se representa con bordes dentados. -->

### Explicacion Pedagogica
Identificar el significado de los distintos tipos de globos de texto en una historieta.

## Question 5 [D5-D6]
**ID:** CO-LC-5-2026-W08-historieta-secuencia-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Semántico
**Expected_Success:** 0.66
**Contexto:** En una historieta, aparece una nube con la palabra "¡ZAS!" y un personaje cayendo.

### Enunciado
¿Qué recurso narrativo se está usando en esa viñeta?

### Opciones
- [x] A) Una onomatopeya para representar un sonido fuerte del golpe.
  <!-- feedback: "¡ZAS!" es una onomatopeya que reproduce un golpe o impacto. -->
- [ ] B) Un pensamiento profundo del personaje.
  <!-- feedback: Los pensamientos suelen ir en nubes suaves, no con gritos fuertes. -->
- [ ] C) Un diálogo en voz baja.
  <!-- feedback: Los diálogos no se escriben con onomatopeyas como "¡ZAS!". -->
- [ ] D) Una indicación para doblar la página.
  <!-- feedback: Las indicaciones de doblar página no aparecen dentro de las viñetas. -->

### Explicacion Pedagogica
Aplicar el concepto de onomatopeya al interpretar los recursos sonoros de una historieta.

## Question 6 [D7-D8]
**ID:** CO-LC-5-2026-W08-historieta-secuencia-001-MASTERY-bundle-v6
**Bloom:** Analyze
**ICFES:** Semántico
**Expected_Success:** 0.58
**Contexto:** Compara: "La historieta A presenta una sola viñeta con un texto largo. La historieta B tiene diez viñetas pequeñas con poco texto y muchas imágenes."

### Enunciado
¿Qué diferencia existe entre la historieta A y la historieta B?

### Opciones
- [x] A) La A usa pocas imágenes y mucho texto; la B combina varias viñetas con poco texto e imágenes.
  <!-- feedback: La cantidad de viñetas y la relación entre texto e imagen marcan la diferencia. -->
- [ ] B) Las dos historietas son idénticas en estructura.
  <!-- feedback: Las estructuras descritas son claramente diferentes. -->
- [ ] C) La B no tiene imágenes, solo texto.
  <!-- feedback: La B tiene imágenes en cada viñeta. -->
- [ ] D) La A tiene más imágenes y menos texto que la B.
  <!-- feedback: Es al revés: la A tiene menos imágenes y más texto. -->

### Explicacion Pedagogica
Comparar la estructura de dos historietas según la cantidad de viñetas y la relación texto-imagen.

## Question 7 [D7-D8]
**ID:** CO-LC-5-2026-W08-historieta-secuencia-001-MASTERY-bundle-v7
**Bloom:** Analyze
**ICFES:** Semántico
**Expected_Success:** 0.52
**Contexto:** Lee: "En muchas historietas, el autor usa el encuadre para mostrar el rostro del personaje cuando quiere transmitir una emoción fuerte."

### Enunciado
¿Qué efecto busca el autor al encuadrar un rostro en primer plano?

### Opciones
- [x] A) Intensificar la emoción del personaje y captar la atención del lector.
  <!-- feedback: El primer plano acerca al lector a las emociones del personaje. -->
- [ ] B) Ocultar completamente la emoción del personaje.
  <!-- feedback: El primer plano muestra la emoción, no la oculta. -->
- [ ] C) Cambiar el orden de las viñetas en la página.
  <!-- feedback: El encuadre no altera el orden, sino el enfoque visual. -->
- [ ] D) Eliminar el uso de globos de texto.
  <!-- feedback: El primer plano no reemplaza los textos; los complementa. -->

### Explicacion Pedagogica
Analizar el efecto del encuadre en primer plano dentro de una historieta.

## Question 8 [D9-D10]
**ID:** CO-LC-5-2026-W08-historieta-secuencia-001-MASTERY-bundle-v8
**Bloom:** Evaluate
**ICFES:** Semántico
**Expected_Success:** 0.31
**Contexto:** Lee: 'Leer historietas desarrolla la comprensión de secuencias narrativas, la interpretación de imágenes y el gusto por la lectura. Es un puente entre la literatura, el cine y el arte visual.'

### Enunciado
¿Por qué es valioso leer historietas en la escuela primaria?

### Opciones
- [x] A) Porque desarrollan la comprensión narrativa, la lectura de imágenes y el gusto por la lectura en distintos formatos.
  <!-- feedback: Las historietas son un recurso valioso para formar lectores competentes y críticos. -->
- [ ] B) Porque las historietas no aportan nada a la formación lectora.
  <!-- feedback: Las historietas aportan mucho al desarrollo de la lectura y la interpretación visual. -->
- [ ] C) Porque son solo para entretenimiento sin valor educativo.
  <!-- feedback: Las historietas tienen valor educativo y cultural, no solo entretenimiento. -->
- [ ] D) Porque deberían sustituir completamente a los libros de texto.
  <!-- feedback: Las historietas complementan, no sustituyen, a otros materiales de lectura. -->

### Explicacion Pedagogica
Evaluar la importancia de la historieta como recurso para el desarrollo de la lectura crítica.