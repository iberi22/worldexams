---
id: "CO-LC-5-2026-W09-carta-amistosa-001-MASTERY-bundle"
country: "colombia"
grado: 5
asignatura: "lectura-critica"
tema: "carta-amistosa"
periodo: "weekly"
week: "W09"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 8
bundle_size: 8
alignment: "DBA MEN Colombia / Saber 5"
bundle_index: 1
calibration: {difficulty_band: "D3-D4", expected_success: 0.8}
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: Carta Amistosa - Grado 5 (W09)

Estructura y características de la carta amistosa, su propósito comunicativo, elementos formales y tono adecuado.

## Question 1 [D3-D4]
**ID:** CO-LC-5-2026-W09-carta-amistosa-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Pragmático
**Expected_Success:** 0.89
**Contexto:** En clase de lectura y escritura en una escuela de Yopal, la profesora explica las partes de una carta.

### Enunciado
¿Cuál es la estructura básica de una carta amistosa?

### Opciones
- [x] A) Encabezado (lugar y fecha), saludo, cuerpo, despedida y firma.
  <!-- feedback: La carta amistosa incluye lugar, fecha, saludo, mensaje, despedida y firma. -->
- [ ] B) Solo una lista de ingredientes numerados.
  <!-- feedback: Una carta amistosa no es una lista de ingredientes. -->
- [ ] C) Únicamente una ecuación matemática.
  <!-- feedback: Las cartas amistosas no contienen ecuaciones matemáticas. -->
- [ ] D) Solo un título de una película.
  <!-- feedback: El título no es una carta amistosa. -->

### Explicacion Pedagogica
Reconocer las partes que componen la estructura de una carta amistosa.

## Question 2 [D3-D4]
**ID:** CO-LC-5-2026-W09-carta-amistosa-001-MASTERY-bundle-v2
**Bloom:** Understand
**ICFES:** Pragmático
**Expected_Success:** 0.84
**Contexto:** Lee: "Una carta amistosa es un mensaje escrito que una persona envía a un amigo o familiar para compartir noticias, expresar sentimientos o fortalecer la relación."

### Enunciado
¿Cuál es el propósito principal de una carta amistosa?

### Opciones
- [x] A) Mantener la comunicación cercana y expresar sentimientos entre personas que se conocen.
  <!-- feedback: La carta amistosa busca fortalecer vínculos afectivos entre personas cercanas. -->
- [ ] B) Informar únicamente sobre noticias internacionales en lenguaje formal.
  <!-- feedback: Las cartas amistosas no se centran en noticias internacionales ni usan tono formal. -->
- [ ] C) Publicar un anuncio comercial para vender productos.
  <!-- feedback: La carta amistosa no es publicidad; es comunicación personal. -->
- [ ] D) Dictar normas legales a los ciudadanos.
  <!-- feedback: Las normas legales se comunican por otros medios, no en cartas amistosas. -->

### Explicacion Pedagogica
Comprender el propósito comunicativo y afectivo de una carta amistosa.

## Question 3 [D5-D6]
**ID:** CO-LC-5-2026-W09-carta-amistosa-001-MASTERY-bundle-v3
**Bloom:** Apply
**ICFES:** Pragmático
**Expected_Success:** 0.75
**Contexto:** Un estudiante escribe a su primo que vive en otra ciudad para contarle sobre sus vacaciones.

### Enunciado
¿Qué saludo es más apropiado para comenzar una carta amistosa?

### Opciones
- [x] A) "Querido primo Camilo:".
  <!-- feedback: "Querido primo Camilo" es un saludo cercano y apropiado para una carta amistosa. -->
- [ ] B) "Estimado señor proveedor:".
  <!-- feedback: Este saludo es formal y comercial, no amistoso. -->
- [ ] C) "A quien pueda interesar:".
  <!-- feedback: Es un saludo neutro de avisos, no de cartas personales. -->
- [ ] D) "Respetados miembros del jurado:".
  <!-- feedback: Este saludo es propio de discursos académicos, no de cartas amistosas. -->

### Explicacion Pedagogica
Aplicar el uso de saludos adecuados según el tono y el destinatario.

## Question 4 [D5-D6]
**ID:** CO-LC-5-2026-W09-carta-amistosa-001-MASTERY-bundle-v4
**Bloom:** Apply
**ICFES:** Pragmático
**Expected_Success:** 0.71
**Contexto:** Lee: "Al final de la carta, el emisor se despide usando frases afectuosas como 'Con cariño', 'Un abrazo' o 'Hasta pronto', y firma con su nombre."

### Enunciado
¿Qué despedida es más apropiada para una carta dirigida a una amiga cercana?

### Opciones
- [x] A) "Con cariño, Laura.".
  <!-- feedback: "Con cariño, Laura" es una despedida cálida y cercana, propia de cartas amistosas. -->
- [ ] B) "Atentamente, Laura.".
  <!-- feedback: "Atentamente" se usa en cartas formales, no amistosas. -->
- [ ] C) "Suyo seguro servidor, Laura.".
  <!-- feedback: Esta fórmula es anticuada y se usa en contextos muy formales. -->
- [ ] D) "Sin más, Laura.".
  <!-- feedback: "Sin más" es una despedida neutra que no refleja cercanía. -->

### Explicacion Pedagogica
Aplicar despedidas afectuosas adecuadas en cartas amistosas.

## Question 5 [D5-D6]
**ID:** CO-LC-5-2026-W09-carta-amistosa-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Pragmático
**Expected_Success:** 0.66
**Contexto:** Un niño debe contar en una carta cómo le fue en una excursión del colegio.

### Enunciado
¿Qué información debería incluir en el cuerpo de la carta para que su amigo entienda la experiencia?

### Opciones
- [x] A) Lugar visitado, actividades realizadas, momentos divertidos y emociones sentidas.
  <!-- feedback: Estos datos permiten al lector reconstruir mentalmente la excursión. -->
- [ ] B) Una larga lista de números y operaciones matemáticas.
  <!-- feedback: Una carta personal no es el espacio para cálculos matemáticos extensos. -->
- [ ] C) Solo saludos formales sin contar nada de la excursión.
  <!-- feedback: El cuerpo de la carta debe desarrollar el mensaje, no quedar vacío. -->
- [ ] D) Un código secreto que el amigo no pueda descifrar.
  <!-- feedback: Una carta amistosa busca comunicarse claramente, no confundir al lector. -->

### Explicacion Pedagogica
Aplicar el contenido adecuado del cuerpo de una carta para describir una experiencia personal.

## Question 6 [D7-D8]
**ID:** CO-LC-5-2026-W09-carta-amistosa-001-MASTERY-bundle-v6
**Bloom:** Analyze
**ICFES:** Pragmático
**Expected_Success:** 0.58
**Contexto:** Compara: "La carta 1 dice: 'Espero que estés bien. Te extraño mucho. Aquí todo sigue igual.' La carta 2 dice: 'Por medio de la presente, me permito informar que mi situación es estable.'"

### Enunciado
¿Qué diferencia de tono existe entre la carta 1 y la carta 2?

### Opciones
- [x] A) La carta 1 usa un tono cercano y afectuoso; la carta 2 usa un tono formal y distante.
  <!-- feedback: La carta 1 muestra cercanía emocional; la carta 2 mantiene distancia formal. -->
- [ ] B) Las dos cartas usan exactamente el mismo tono.
  <!-- feedback: El tono es claramente diferente entre las dos cartas. -->
- [ ] C) La carta 1 es formal y la carta 2 es amistosa.
  <!-- feedback: Es al revés: la carta 1 es amistosa y la carta 2 es formal. -->
- [ ] D) Ninguna de las cartas tiene tono definido.
  <!-- feedback: Toda carta tiene un tono, y aquí se distinguen claramente. -->

### Explicacion Pedagogica
Comparar el tono entre cartas formales y amistosas para reconocer sus diferencias.

## Question 7 [D7-D8]
**ID:** CO-LC-5-2026-W09-carta-amistosa-001-MASTERY-bundle-v7
**Bloom:** Analyze
**ICFES:** Pragmático
**Expected_Success:** 0.52
**Contexto:** Lee: "Una buena carta amistosa incluye una fecha y lugar, un saludo afectuoso, un cuerpo organizado en ideas claras, una despedida cariñosa y la firma del autor."

### Enunciado
¿Qué sucede si una carta amistosa omite la fecha y el lugar?

### Opciones
- [x] A) Pierde información contextual importante para ubicar el mensaje en el tiempo y el espacio.
  <!-- feedback: La fecha y el lugar permiten saber cuándo y dónde se escribió la carta. -->
- [ ] B) La carta mejora porque queda más corta.
  <!-- feedback: La brevedad no sustituye la información contextual. -->
- [ ] C) Se vuelve automáticamente una carta formal.
  <!-- feedback: El tono no cambia por omitir la fecha y el lugar. -->
- [ ] D) No tiene ninguna consecuencia para el lector.
  <!-- feedback: Sin contexto, el lector pierde referencias importantes. -->

### Explicacion Pedagogica
Analizar la importancia de la fecha y el lugar en la estructura de una carta.

## Question 8 [D9-D10]
**ID:** CO-LC-5-2026-W09-carta-amistosa-001-MASTERY-bundle-v8
**Bloom:** Evaluate
**ICFES:** Pragmático
**Expected_Success:** 0.31
**Contexto:** Lee: 'Escribir cartas amistosas fortalece la expresión escrita, la capacidad de organizar ideas y los vínculos afectivos. Aunque hoy se usan mucho los mensajes digitales, la carta sigue siendo una forma valiosa de comunicación.'

### Enunciado
¿Por qué escribir cartas amistosas sigue siendo una actividad valiosa en la escuela?

### Opciones
- [x] A) Porque fortalece la escritura, la organización de ideas y los lazos afectivos con otras personas.
  <!-- feedback: La carta amistosa combina aprendizaje lingüístico con desarrollo emocional. -->
- [ ] B) Porque no aporta ningún beneficio frente a los mensajes digitales.
  <!-- feedback: La carta aporta beneficios distintos y complementarios a la mensajería digital. -->
- [ ] C) Porque reemplaza completamente al uso de teléfonos y chats.
  <!-- feedback: La carta no reemplaza las nuevas tecnologías, sino que las complementa. -->
- [ ] D) Porque solo se utiliza en contextos formales de oficina.
  <!-- feedback: La carta amistosa se usa en contextos personales, no solo formales. -->

### Explicacion Pedagogica
Evaluar la importancia de la carta amistosa como práctica de lectura y escritura significativa.