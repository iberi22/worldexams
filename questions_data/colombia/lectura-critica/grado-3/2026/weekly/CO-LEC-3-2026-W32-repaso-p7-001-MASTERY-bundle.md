---
id: "CO-LEC-3-2026-W32-repaso-p7-001-MASTERY"
country: "colombia"
grado: 3
asignatura: "lectura-critica"
tema: "repaso-p7"
periodo: "2026-W32"
protocol_version: "5.2"
bundle_size: 8
alignment: "DBA MEN"
bundle_index: 32
license: "FREE"
tier: "legacy"

---

# Bundle Mastery: Repaso de Periodo 7

Este bundle integra los conocimientos de las semanas 29 a 31: poesía (verso/estrofa), lenguaje figurado y textos teatrales. Se enfoca en la expresión artística y literaria.

---

## Question 1 [D1-D2]
**ID:** `CO-LEC-3-2026-W32-repaso-p7-001-MASTERY-v1`
**Bloom:** Remember
**ICFES:** Semántico
**Expected_Success:** 0.90

### Contexto
"Sana que sana,
colita de rana.
Si no sana hoy,
sanará mañana".

### Enunciado
¿Cuál es la característica principal de este texto literario?

### Options
- [ ] A) Que cuenta una noticia sobre una rana. <!-- feedback: Incorrecto. Es un texto tradicional con ritmo y rima, no una noticia. -->
- [x] B) Que es un fragmento poético con rima y versos cortos. <!-- feedback: ¡Correcto! La estructura y el sonido indican que es parte del género lírico. -->
- [ ] C) Que da instrucciones para curar animales. <!-- feedback: Incorrecto. Es una expresión cariñosa, no un manual médico. -->
- [ ] D) Que es un diálogo entre un doctor y una rana. <!-- feedback: Incorrecto. Solo hay una voz recitando el verso. -->

### Explicación Pedagógica
Repaso de poesía (Semana 29). Identificar las marcas del género lírico (rima, ritmo, brevedad) ayuda a diferenciarlo de otros tipos de textos infantiles.

---

## Question 2 [D1-D2]
**ID:** `CO-LEC-3-2026-W32-repaso-p7-001-MASTERY-v2`
**Bloom:** Understand
**ICFES:** Semántico
**Expected_Success:** 0.85

### Contexto
**Fragmento Teatral:**
**JUAN:** (Entrando muy asustado) ¡Mamá, mamá! Hay un monstruo bajo mi cama.
**MAMÁ:** (Sonriendo con dulzura) No es un monstruo, Juan. Solo es tu oso de peluche.

### Enunciado
¿Qué información nos dan las palabras entre paréntesis en este texto?

### Options
- [ ] A) Los nombres de los juguetes de Juan. <!-- feedback: Incorrecto. Solo se menciona un oso al final, no entre paréntesis. -->
- [x] B) Cómo se sienten los personajes o qué acciones realizan. <!-- feedback: ¡Correcto! Las acotaciones indican la emoción (asustado, sonriendo) y la acción (entrando). -->
- [ ] C) Lo que los personajes deben decir en voz muy alta. <!-- feedback: Incorrecto. Lo que se dice en voz alta es el diálogo fuera del paréntesis. -->
- [ ] D) El precio de la entrada a la obra de teatro. <!-- feedback: Incorrecto. Las acotaciones no dan información económica. -->

### Explicación Pedagógica
Repaso de textos teatrales (Semana 31). Comprender la función de las acotaciones permite al estudiante "ver" la obra en su mente mientras lee el guion.

---

## Question 3 [D2-D3]
**ID:** `CO-LEC-3-2026-W32-repaso-p7-001-MASTERY-v3`
**Bloom:** Understand
**ICFES:** Semántico
**Expected_Success:** 0.80

### Contexto
"Mi hermano es un **lince** para encontrar las llaves perdidas; las ve antes que todos nosotros".

### Enunciado
¿Qué significa que el hermano sea un "**lince**" en esta oración?

### Options
- [ ] A) Que tiene mucho pelo y orejas puntiagudas. <!-- feedback: Incorrecto. Es un uso figurado de la palabra. -->
- [x] B) Que tiene una vista muy aguda y es muy ágil encontrando cosas. <!-- feedback: ¡Correcto! Se usa el animal (famoso por su vista) para resaltar la habilidad del hermano. -->
- [ ] C) Que es un animal salvaje que vive en la casa. <!-- feedback: Incorrecto. El hermano es un ser humano. -->
- [ ] D) Que le gusta mucho cazar en el bosque. <!-- feedback: Incorrecto. La oración habla de encontrar llaves en casa. -->

### Explicación Pedagógica
Repaso de lenguaje figurado (Semana 30). Identificar la cualidad compartida entre el animal y la persona en una metáfora desarrolla el pensamiento analógico.

---

## Question 4 [D2-D3]
**ID:** `CO-LEC-3-2026-W32-repaso-p7-001-MASTERY-v4`
**Bloom:** Understand
**ICFES:** Semántico
**Expected_Success:** 0.75

### Contexto
"Tengo una hormiguita en la panza
que me está haciendo bailar.
Ella no tiene vergüenza,
ella no para de andar".

### Enunciado
¿Cuántos **versos** tiene la estrofa anterior?

### Options
- [ ] A) 2 versos. <!-- feedback: Incorrecto. Cuenta cada una de las líneas del fragmento. -->
- [x] B) 4 versos. <!-- feedback: ¡Correcto! Hay cuatro líneas de texto, por lo tanto, cuatro versos. -->
- [ ] C) 1 verso. <!-- feedback: Incorrecto. El conjunto es una estrofa, pero tiene varias líneas. -->
- [ ] D) 8 versos. <!-- feedback: Incorrecto. Estás contando de más. Solo hay cuatro líneas escritas. -->

### Explicación Pedagógica
Repaso de estructura poética (Semana 29). Contar versos es una habilidad técnica básica que refuerza la noción de unidad en la poesía.

---

## Question 5 [D3-D4]
**ID:** `CO-LEC-3-2026-W32-repaso-p7-001-MASTERY-v5`
**Bloom:** Apply
**ICFES:** Semántico
**Expected_Success:** 0.70

### Contexto
"El pueblo entero estaba de **fiesta** porque el río por fin traía agua limpia".

### Enunciado
Si quisiéramos decir lo mismo usando **lenguaje figurado**, ¿cuál de estas opciones elegirías?

### Options
- [ ] A) La gente estaba muy contenta por el agua. <!-- feedback: Incorrecto. Este es lenguaje literal, dice lo mismo de forma sencilla. -->
- [ ] B) El agua del río ya no estaba sucia ni contaminada. <!-- feedback: Incorrecto. Sigue siendo lenguaje literal explicativo. -->
- [x] C) El pueblo entero tenía el corazón saltando de alegría. <!-- feedback: ¡Correcto! La expresión "corazón saltando de alegría" es una metáfora de la felicidad. -->
- [ ] D) El río Magdalena es muy largo y pasa por muchos pueblos. <!-- feedback: Incorrecto. Este es un dato informativo que no tiene que ver con la emoción de la fiesta. -->

### Explicación Pedagógica
Transformar lenguaje literal en figurado requiere creatividad y comprensión del sentido de las emociones. El estudiante debe elegir la opción que usa imágenes literarias.

---

## Question 6 [D3-D4]
**ID:** `CO-LEC-3-2026-W32-repaso-p7-001-MASTERY-v6`
**Bloom:** Apply
**ICFES:** Semántico
**Expected_Success:** 0.65

### Contexto
Imagina que estás escribiendo una obra de teatro sobre un astronauta que llega a la Luna.

### Enunciado
¿Cuál sería la **acotación** más adecuada para el momento en que el astronauta baja de la nave?

### Options
- [ ] A) ASTRONAUTA: Por fin he llegado a este lugar tan blanco. <!-- feedback: Incorrecto. Esto es un diálogo, no una instrucción de actuación. -->
- [x] B) (Baja lentamente la escalera y da un pequeño salto sobre el suelo gris) <!-- feedback: ¡Correcto! Es una instrucción de movimiento físico y ambiente que guía al actor. -->
- [ ] C) Los astronautas usan trajes espaciales muy pesados. <!-- feedback: Incorrecto. Esto es un dato informativo, no una acotación teatral. -->
- [ ] D) ¿Habrá extraterrestres en esta luna? <!-- feedback: Incorrecto. Es una pregunta que hace parte del pensamiento o diálogo. -->

### Explicación Pedagógica
Repaso de textos teatrales (Semana 31). Diferenciar entre lo que se dice (diálogo) y lo que se hace (acotación) es la clave para la creación y lectura de guiones dramáticos.

---

## Question 7 [D4-D5]
**ID:** `CO-LEC-3-2026-W32-repaso-p7-001-MASTERY-v7`
**Bloom:** Analyze
**ICFES:** Semántico
**Expected_Success:** 0.55

### Contexto
"Las perlas de tu boca brillan cuando ríes".

### Enunciado
¿Qué término real está sustituyendo la palabra "**perlas**" en este verso?

### Options
- [ ] A) Los ojos. <!-- feedback: Incorrecto. Los ojos no están en la boca. -->
- [ ] B) Las orejas. <!-- feedback: Incorrecto. No brillan al reír ni están en la boca. -->
- [x] C) Los dientes. <!-- feedback: ¡Correcto! Se comparan los dientes blancos y brillantes con perlas preciosas. -->
- [ ] D) Las palabras. <!-- feedback: Incorrecto. Las palabras se oyen, pero no son objetos que "brillan" literalmente. -->

### Explicación Pedagógica
Analizar metáforas clásicas ayuda al estudiante a entender cómo se embellece el lenguaje literario buscando semejanzas físicas entre objetos de la naturaleza y el cuerpo humano.

---

## Question 8 [D4-D5]
**ID:** `CO-LEC-3-2026-W32-repaso-p7-001-MASTERY-v8`
**Bloom:** Evaluate
**ICFES:** Crítico
**Expected_Success:** 0.50

### Contexto
Un autor escribe este poema:
"El televisor me mira,
yo miro al televisor.
Él me cuenta muchas cosas,
yo le presto mi atención".

### Enunciado
¿Qué recurso literario está usando el autor al decir que el "**televisor lo mira**"?

### Options
- [ ] A) Metáfora, porque compara el televisor con un ojo. <!-- feedback: Incorrecto. No hay una comparación directa de objetos. -->
- [x] B) Personificación, porque le da una acción humana (mirar) a un objeto inanimado. <!-- feedback: ¡Correcto! Dar cualidades de seres vivos a objetos es propio de la personificación. -->
- [ ] C) Rima, porque mirar suena como televisor. <!-- feedback: Incorrecto. Esas palabras no riman entre sí. -->
- [ ] D) Noticia, porque nos informa que el televisor funciona. <!-- feedback: Incorrecto. El lenguaje es poético e imaginativo, no informativo. -->

### Explicación Pedagógica
Evaluar el uso de figuras literarias complejas como la personificación permite al estudiante comprender cómo se anima la realidad en la literatura infantil.


[//]: # (QUALITY_REVIEW)
[//]: # (STATUS: LEGACY - FREE USE)
[//]: # (SCORE: N/A - Pre-QR era)
