import os

# Lectura Critica P1
LC_P1 = r"""---
id: "CO-LC-10-2026-P1-comprehensive-001-MASTERY"
country: "colombia"
grado: 10
asignatura: "lectura-critica"
tema: "literatura-universal-intro, critica-textual"
periodo: 1
protocol_version: "5.2"
bundle_index: 1
bundle_size: 20
alignment: "DBA MEN + Pre-ICFES"
modern_context: true
distractor_profile: "plausible_peer_set"
---
# Bundle MASTERY: Literatura Universal Intro, Critica Textual (P1)

## Question 1 (D3)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v1`
**Bloom:** Remember | **ICFES:** Identificar y entender contenidos locales
**Context:** En clase de espanol en el Colegio La Presentation de Manizales, se estudia la literatura clasica universal.
**Enunciado:** Cual de las siguientes obras fue escrita por Homero?
**Options:**
- [ ] A) La Divina Comedia <!-- feedback: Incorrecto. La Divina Comedia fue escrita por Dante Alighieri en el siglo XIV. -->
- [x] B) La Iliada <!-- feedback: Correcto. La Iliada y la Odisea son las dos grandes epopeyas atribuidas a Homero, de la Antigua Grecia. -->
- [ ] C) El Quijote <!-- feedback: Incorrecto. Don Quijote de la Mancha fue escrito por Miguel de Cervantes Saavedra en 1605. -->
- [ ] D) Hamlet <!-- feedback: Incorrecto. Hamlet fue escrito por William Shakespeare en el siglo XVII. -->

## Question 2 (D3)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v2`
**Bloom:** Remember | **ICFES:** Identificar y entender contenidos locales
**Context:** Se analiza un texto narrativo en clase.
**Enunciado:** La parte de la narracion donde se presenta el conflicto principal se llama:
**Options:**
- [ ] A) Introduccion <!-- feedback: Incorrecto. La introduccion presenta personajes y contexto. -->
- [x] B) Nudo <!-- feedback: Correcto. El nudo es la parte donde se desarrolla el conflicto principal de la historia. -->
- [ ] C) Desenlace <!-- feedback: Incorrecto. El desenlace es la resolucion del conflicto. -->
- [ ] D) Epilogo <!-- feedback: Incorrecto. El epilogo es una seccion final que cierra la historia. -->

## Question 3 (D4)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v3`
**Bloom:** Understand | **ICFES:** Identificar y entender contenidos locales
**Context:** Texto: "El Quijote creia firmemente que los molinos de viento eran gigantes descomunales. Con su lanza en ristre, arremetio contra ellos, sin importarle las advertencias de Sancho."
**Enunciado:** Que figura literaria predomina en este fragmento?
**Options:**
- [ ] A) Metafora <!-- feedback: Incorrecto. No hay una comparacion implicita. -->
- [ ] B) Hiperbole <!-- feedback: Incorrecto. Aunque puede haber exageracion, la figura principal es otra. -->
- [x] C) Ironia dramatica <!-- feedback: Correcto. El lector sabe que son molinos, pero don Quijote cree que son gigantes. Hay discrepancia entre lo que sabe el lector y lo que sabe el personaje. -->
- [ ] D) Personificacion <!-- feedback: Incorrecto. No se atribuyen cualidades humanas a objetos inanimados. -->

## Question 4 (D4)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v4`
**Bloom:** Understand | **ICFES:** Comprender el sentido global del texto
**Context:** Se lee el siguiente parrafo de un articulo: "Las redes sociales han transformado la comunicacion interpersonal. Mientras que antes la comunicacion era mayoritariamente sincronica y presencial, hoy predomina la asincronica y mediada por pantallas."
**Enunciado:** Segun el texto, cual es el principal cambio en la comunicacion?
**Options:**
- [ ] A) La comunicacion ahora es mas cara. <!-- feedback: Incorrecto. El texto no menciona costos. -->
- [x] B) Paso de ser sincronica/presencial a asincronica/mediada. <!-- feedback: Correcto. El texto contrasta explicitamente la comunicacion anterior (sincronica y presencial) con la actual (asincronica y mediada por pantallas). -->
- [ ] C) Ahora todos se comunican mas. <!-- feedback: Incorrecto. El texto habla de cambios en la forma, no en la cantidad. -->
- [ ] D) La comunicacion presencial ha desaparecido. <!-- feedback: Incorrecto. El texto dice "predomina", no desaparece. -->

## Question 5 (D5)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v5`
**Bloom:** Apply | **ICFES:** Comprender el sentido global del texto
**Context:** Texto: "El racionalismo cartesiano postula que la razon es la unica fuente de conocimiento valido. 'Pienso, luego existo' resume esta postura."
**Enunciado:** Si aplicamos esta postura al analisis de un mito griego, que conclusion se derivaria?
**Options:**
- [ ] A) El mito es valido porque explica fenomenos naturales. <!-- feedback: Incorrecto. Desde el racionalismo, el mito no es conocimiento racional. -->
- [x] B) El mito no seria conocimiento valido porque no se basa en la razon. <!-- feedback: Correcto. Para el racionalismo cartesiano, solo el conocimiento derivado de la razon es valido. Los mitos pertenecen al ambito de la tradicion o la fe, no de la razon. -->
- [ ] C) El mito es superior a la razon. <!-- feedback: Incorrecto. El racionalismo defiende lo contrario. -->
- [ ] D) La razon y el mito son complementarios. <!-- feedback: Incorrecto. Descartes separa radicalmente la razon de otras formas de conocimiento. -->

## Question 6 (D5)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v6`
**Bloom:** Analyze | **ICFES:** Reflexionar sobre el contenido
**Context:** Texto: "La tragedia griega, desde Esquilo hasta Euripides, explora la tension entre el destino (moira) y la libertad humana. Edipo Rey es el ejemplo mas claro: el heroe intenta escapar de su destino y, al hacerlo, lo cumple."
**Enunciado:** Cual es la tesis central del texto sobre la tragedia griega?
**Options:**
- [ ] A) Los heroes griegos siempre triunfan. <!-- feedback: Incorrecto. En la tragedia los heroes suelen caer. -->
- [ ] B) El destino no existe en la cultura griega. <!-- feedback: Incorrecto. El texto dice explicitamente que hay tension entre destino y libertad. -->
- [x] C) La tragedia explora la paradoja entre el destino y la voluntad humana. <!-- feedback: Correcto. El texto propone que la tragedia griega gira en torno a la tension entre el destino inexorable y la libertad de eleccion del heroe. -->
- [ ] D) Edipo Rey es la unica tragedia griega importante. <!-- feedback: Incorrecto. El texto menciona a varios autores. -->

## Question 7 (D5)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v7`
**Bloom:** Apply | **ICFES:** Identificar y entender contenidos locales
**Context:** Un estudiante analiza el siguiente fragmento de "Cien anos de soledad": "El mundo era tan reciente, que muchas cosas carecian de nombre, y para mencionarlas habia que senalarlas con el dedo."
**Enunciado:** Este fragmento es un ejemplo de:
**Options:**
- [x] A) Realismo magico. <!-- feedback: Correcto. Gabriel Garcia Marquez combina una realidad primigenia con elementos fantasticos (el mundo reciente con cosas sin nombre) en un estilo que caracteriza al realismo magico. -->
- [ ] B) Literatura gotica. <!-- feedback: Incorrecto. No hay elementos de terror o oscuridad. -->
- [ ] C) Neoclasicismo. <!-- feedback: Incorrecto. El neoclasicismo se caracteriza por la mesura y las reglas clasicas. -->
- [ ] D) Vanguardismo. <!-- feedback: Incorrecto. Aunque rompe convenciones, el estilo de Garcia Marquez se clasifica como realismo magico. -->

## Question 8 (D6)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v8`
**Bloom:** Analyze | **ICFES:** Reflexionar sobre el contenido
**Context:** Texto: "El narrador de La Odisea cuenta las aventuras de Ulises en tercera persona omnisciente. Sin embargo, en varios cantos, Ulises mismo narra sus propias historias a los feacios, convirtiendose en narrador en primera persona."
**Enunciado:** Que efecto produce este cambio de narrador?
**Options:**
- [ ] A) Confunde al lector sin proposito. <!-- feedback: Incorrecto. El cambio tiene un proposito narrativo claro. -->
- [ ] B) Hace que la historia sea menos confiable. <!-- feedback: Incorrecto. No necesariamente. -->
- [x] C) Permite contrastar la vision externa con la perspectiva subjetiva del heroe. <!-- feedback: Correcto. La alternancia entre narrador omnisciente y la voz de Ulises enriquece la narracion al ofrecer perspectivas complementarias. -->
- [ ] D) Reduce la tension dramatica. <!-- feedback: Incorrecto. Al contrario, puede aumentarla al hacer mas intima la narracion. -->

## Question 9 (D6)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v9`
**Bloom:** Apply | **ICFES:** Comprender el sentido global del texto
**Context:** Texto: "El mito de la caverna de Platon describe a prisioneros que solo ven sombras en la pared y creen que esa es la realidad. Uno de ellos escapa, ve el mundo real, y al regresar, los demas no le creen."
**Enunciado:** Cual es la ensenanza principal del mito?
**Options:**
- [ ] A) No se debe escapar de la prision. <!-- feedback: Incorrecto. El mensaje es filosofico, no literal. -->
- [ ] B) Las sombras son la unica realidad. <!-- feedback: Incorrecto. Las sombras representan la ignorancia. -->
- [x] C) El conocimiento requiere salir de la zona de confort y enfrentar el rechazo. <!-- feedback: Correcto. La alegoria representa el viaje del conocimiento: salir de la ignorancia, ver la verdad y la dificultad de compartirla. -->
- [ ] D) La filosofia es peligrosa. <!-- feedback: Incorrecto. Platon defiende el valor de la filosofia. -->

## Question 10 (D6)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v10`
**Bloom:** Understand | **ICFES:** Identificar y entender contenidos locales
**Context:** Se analiza una noticia: "El Congreso de la Republica aprobo en segundo debate el proyecto de ley que regula la inteligencia artificial en Colombia, con 85 votos a favor y 32 en contra."
**Enunciado:** En el texto, "segundo debate" corresponde a una etapa del:
**Options:**
- [ ] A) Proceso electoral. <!-- feedback: Incorrecto. El Congreso no esta en elecciones. -->
- [x] B) Proceso legislativo. <!-- feedback: Correcto. El proceso legislativo en Colombia consta de cuatro debates: dos en cada camara (Senado y Camara). "Segundo debate" indica que paso por una camara y ahora va a la otra. -->
- [ ] C) Proceso judicial. <!-- feedback: Incorrecto. El Congreso legisla, no juzga. -->
- [ ] D) Proceso administrativo. <!-- feedback: Incorrecto. No es un tramite administrativo. -->

## Question 11 (D7)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v11`
**Bloom:** Analyze | **ICFES:** Reflexionar sobre el contenido
**Context:** Texto: "El Bullying o acoso escolar tiene consecuencias psicologicas profundas en las victimas. Estudios recientes muestran que los efectos pueden persistir hasta la edad adulta, manifestandose en ansiedad, depresion y baja autoestima."
**Enunciado:** Que tipo de argumento utiliza principalmente el autor?
**Options:**
- [ ] A) Argumento de autoridad. <!-- feedback: Parcialmente correcto, pero el texto cita "estudios recientes" sin especificar. -->
- [ ] B) Argumento emotivo. <!-- feedback: Incorrecto. El tono es informativo, no emotivo. -->
- [x] C) Argumento factual (basado en datos). <!-- feedback: Correcto. El autor se apoya en estudios (datos de investigacion) para sustentar su afirmacion sobre los efectos del bullying. -->
- [ ] D) Argumento logico deductivo. <!-- feedback: Incorrecto. No hay una estructura de premisa-conclusion logica formal. -->

## Question 12 (D7)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v12`
**Bloom:** Evaluate | **ICFES:** Reflexionar sobre el contenido
**Context:** Un editorial periodistico afirma: "La jornada unica escolar en Colombia mejoraria la calidad educativa, como ha sucedido en paises como Finlandia y Corea del Sur."
**Enunciado:** Cual es una debilidad de este argumento?
**Options:**
- [ ] A) Utiliza datos falsos. <!-- feedback: Incorrecto. No sabemos si los datos son falsos. -->
- [x] B) Establece una relacion causal sin considerar diferencias contextuales. <!-- feedback: Correcto. El argumento comete la falacia de post hoc ergo propter hoc o de comparacion inadecuada: lo que funciona en Finlandia (con otro contexto cultural, economico y social) no necesariamente funciona en Colombia. -->
- [ ] C) Es muy emotivo. <!-- feedback: Incorrecto. El argumento no es emotivo, es comparativo. -->
- [ ] D) Carece de opinion. <!-- feedback: Incorrecto. Es un editorial, que por definicion expresa opinion. -->

## Question 13 (D7)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v13`
**Bloom:** Evaluate | **ICFES:** Reflexionar sobre el contenido
**Context:** Lectura de "La metamorfosis" de Kafka: "Al despertar Gregorio Samsa una manana, tras un sueno intranquilo, se encontro en su cama convertido en un monstruoso insecto."
**Enunciado:** Que funcion cumple el tono cotidiano y realista para describir un hecho fantastico?
**Options:**
- [ ] A) Hace que la historia sea increible. <!-- feedback: Incorrecto. El tono realista hace lo contrario. -->
- [ ] B) No tiene funcion especifica. <!-- feedback: Incorrecto. Es una tecnica narrativa fundamental. -->
- [x] C) Acentua el absurdo al tratar lo extraordinario como normal. <!-- feedback: Correcto. Kafka utiliza un tono completamente realista y burocratico para describir un hecho imposible, lo que potencia el efecto de extranamiento. -->
- [ ] D) Distrae al lector del hecho fantastico. <!-- feedback: Incorrecto. La juxtaposicion es deliberada. -->

## Question 14 (D8)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v14`
**Bloom:** Evaluate | **ICFES:** Reflexionar sobre el contenido
**Context:** Texto: "Algunos criticos sostienen que la literatura contemporanea ha perdido su capacidad de transformacion social, pues se enfoca demasiado en la subjetividad individual. Sin embargo, obras como 'Los ejrcitos' de Evelio Rosero demuestran que la narrativa actual si logra denunciar la violencia estructural."
**Enunciado:** Que estrategia argumentativa usa el autor del texto?
**Options:**
- [ ] A) Cita textual. <!-- feedback: Incorrecto. No hay cita directa. -->
- [x] B) Refutacion mediante contraejemplo. <!-- feedback: Correcto. El autor presenta una critica y la refuta ofreciendo un contraejemplo concreto (la obra de Rosero) que demuestra lo contrario. -->
- [ ] C) Apelacion a la tradicion. <!-- feedback: Incorrecto. No apela a la tradicion. -->
- [ ] D) Generalizacion apresurada. <!-- feedback: Incorrecto. El texto evita generalizar al ofrecer un ejemplo especifico. -->

## Question 15 (D8)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v15`
**Bloom:** Evaluate | **ICFES:** Reflexionar sobre el contenido
**Context:** Analisis de la "Antigona" de Sofocles. Creonte castiga a Antigona por enterrar a su hermano, desobedeciendo la ley del estado. Antigona defiende que las leyes divinas estan por encima de las humanas.
**Enunciado:** El conflicto central de la obra representa:
**Options:**
- [ ] A) La lucha entre el bien y el mal. <!-- feedback: Incorrecto. Ambos personajes tienen argumentos validos. -->
- [x] B) La tension entre la ley moral individual y la ley civil. <!-- feedback: Correcto. Antigona representa la ley divina/moral no escrita, mientras Creonte representa la ley del estado. Este dilema es el nucleo filosofico de la tragedia. -->
- [ ] C) La lucha de clases en la antigua Grecia. <!-- feedback: Incorrecto. No es el tema principal. -->
- [ ] D) El conflicto entre hombres y mujeres. <!-- feedback: Incorrecto. Aunque hay un componente de genero, no es el conflicto central. -->

## Question 16 (D8)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v16`
**Bloom:** Analyze | **ICFES:** Identificar y entender contenidos locales
**Context:** Texto: "La novela 'Delirio' de Laura Restrepo narra la historia de un hombre que busca a su esposa en el Bogota de los anos 80, mientras el pais esta sumergido en la violencia del narcotrafico."
**Enunciado:** El contexto historico en esta novela funciona como:
**Options:**
- [ ] A) Un simple decorado sin importancia. <!-- feedback: Incorrecto. El contexto es parte integral de la obra. -->
- [ ] B) Un personaje mas, que condiciona las acciones de los protagonistas. <!-- feedback: Correcto. En la narrativa colombiana contemporanea, el contexto de violencia no es un mero fondo sino un agente activo que determina la vida de los personajes. -->
- [ ] C) Una critica exclusiva al gobierno de los 80. <!-- feedback: Incorrecto. Es mas amplio que una critica politica especifica. -->
- [ ] D) Una excusa para hablar de amor. <!-- feedback: Incorrecto. El contexto es fundamental y sustantivo. -->

## Question 17 (D9)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v17`
**Bloom:** Create | **ICFES:** Reflexionar sobre el contenido
**Context:** Un estudiante debe escribir un ensayo critico sobre dos cuentos: "El aleph" de Borges y "La noche boca arriba" de Cortazar.
**Enunciado:** Cual seria la tesis mas original para comparar ambas obras?
**Options:**
- [ ] A) Ambos cuentos son de autores latinoamericanos. <!-- feedback: Incorrecto. Demasiado superficial. -->
- [x] B) Ambos cuentos exploran la ruptura de la realidad lineal y la coexistencia de multiples dimensiones temporales y espaciales, aunque desde perspectivas distintas: Borges desde lo filosofico-metafisico y Cortazar desde lo onirico-corporal. <!-- feedback: Correcto. El Aleph muestra un punto donde coexiste todo el universo; La noche boca arriba yuxtapone dos realidades temporales. Ambos desafian la percepcion lineal. -->
- [ ] C) Borges es mejor escritor que Cortazar. <!-- feedback: Incorrecto. Una tesis no debe ser una opinion subjetiva no sustentable. -->
- [ ] D) Ambos cuentos tratan de la muerte. <!-- feedback: Incorrecto. Demasiado vago. -->

## Question 18 (D9)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v18`
**Bloom:** Evaluate | **ICFES:** Reflexionar sobre el contenido
**Context:** Texto: "La poesia de Jose Asuncion Silva, particularmente 'Nocturno', utiliza el verso libre y las repeticiones para crear un efecto musical que expresa la melancolia y la perdida."
**Enunciado:** La afirmacion de que la musicalidad del poema refuerza su contenido emocional es:
**Options:**
- [ ] A) Falsa porque la forma y el contenido son independientes. <!-- feedback: Incorrecto. En poesia, forma y contenido estan intrinsecamente ligados. -->
- [x] B) Verdadera, ya que las repeticiones y el ritmo lento imitan el latido del corazon y el lamento, intensificando la emocion. <!-- feedback: Correcto. El analisis poetico reconoce que los recursos sonoros (aliteracion, ritmo, repeticion) apoyan el significado. En el Nocturno, el ritmo pausado y las repeticiones refuerzan la atmosfera de duelo. -->
- [ ] C) Falsa porque la poesia debe evaluarse solo por su contenido semantico. <!-- feedback: Incorrecto. La poesia es una forma de arte verbal que integra sonido y significado. -->
- [ ] D) Verdadera pero irrelevante para el analisis literario. <!-- feedback: Incorrecto. Es relevante. -->

## Question 19 (D9)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v19`
**Bloom:** Create | **ICFES:** Reflexionar sobre el contenido
**Context:** Un critico literario debe recomendar una obra universal para el plan lector de grado 10 en colegios colombianos.
**Enunciado:** Cual criterio NO deberia considerarse al seleccionar la obra?
**Options:**
- [ ] A) Relevancia literaria y valor estetico. <!-- feedback: Incorrecto. Es un criterio fundamental. -->
- [ ] B) Conexion con problematicas contemporaneas. <!-- feedback: Incorrecto. Es relevante para la formacion critica. -->
- [ ] C) Accesibilidad del lenguaje y la tematica para estudiantes de 15-16 anos. <!-- feedback: Incorrecto. Es un criterio pedagogico importante. -->
- [x] D) Que la obra sea exclusivamente de autor colombiano. <!-- feedback: Correcto. Limitar la seleccion solo a autores colombianos empobrece la formacion literaria. El plan lector debe incluir literatura universal variada. -->

## Question 20 (D10)
**ID:** `CO-LC-10-2026-P1-comprehensive-001-MASTERY-v20`
**Bloom:** Evaluate | **ICFES:** Reflexionar sobre el contenido
**Context:** Texto: "La critica textual contemporanea propone que el significado de un texto no es fijo ni unico, sino que se construye en la interaccion entre el lector y el texto. Esto no significa que todas las interpretaciones sean validas: el texto impone limites a la interpretacion."
**Enunciado:** Segun el texto, cual es la postura epistemologica correcta sobre la interpretacion textual?
**Options:**
- [ ] A) Cualquier interpretacion es igualmente valida. <!-- feedback: Incorrecto. El texto dice explicitamente que "no todas las interpretaciones son validas". -->
- [ ] B) El significado es fijo y lo determina el autor. <!-- feedback: Incorrecto. El texto rechaza la idea de un significado unico. -->
- [x] C) El significado se negocia entre el lector y el texto, con restricciones textuales. <!-- feedback: Correcto. Esta es la postura del lector implicito o la hermeneutica contemporanea: el lector participa en la construccion del sentido, pero el texto establece limites. -->
- [ ] D) El texto no tiene significado. <!-- feedback: Incorrecto. El texto afirma que el significado existe pero no es fijo. -->
"""

path = r"E:\scripts-python\worldexams\questions_data\colombia\lectura-critica\grado-10\2026\periodos\CO-LC-10-2026-P1-comprehensive-001-MASTERY-bundle.md"
os.makedirs(os.path.dirname(path), exist_ok=True)
with open(path, "w", encoding="utf-8") as f:
    f.write(LC_P1)
print("Written: LC P1")
