---
id: "CO-LEC-6-2026-P1-textos-expositivos-INDEX-MASTERY-bundle"
country: "colombia"
grado: 6
asignatura: "lectura-critica"
tema: "textos-expositivos"
periodo: 1
protocol_version: "5.2"
bundle_index: 1
bundle_size: 10
alignment: "DBA MEN + Estándares Básicos de Competencias"
modern_context: true
distractor_profile: "plausible_peer_set"
calibration:
  expected_success_rate: 0.68
  discrimination_index_target: ">= 0.22"
  simulated_responses: 100
rubric_baseline: "textos expositivos, estructura informativa, propósito comunicativo, datos explícitos, organización textual"
bundle_type: "weekly"
total_questions: 10
year: 2026
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---


# Lectura Crítica G6 — Textos Expositivos

## El Oso de Anteojos: El Gigante Gentil de los Andes

*El oso de anteojos (Tremarctos ornatus) es la única especie de oso que habita en Sudamérica. En Colombia, se encuentra en la cordillera de los Andes, desde el páramo hasta el bosque nublado. Recibe su nombre por las manchas blancas o amarillentas alrededor de sus ojos, que parecen anteojos. Es un animal omnívoro: se alimenta principalmente de frutas, bromelias y raíces, aunque ocasionalmente come pequeños mamíferos e insectos. A diferencia del oso pardo, el oso de anteojos es tímido y evita el contacto con humanos. La UICN lo clasifica como especie vulnerable debido a la pérdida de su hábitat por la deforestación y el cambio climático. En Colombia, parques nacionales como el Cocuy y el Chingaza protegen a esta especie emblemática de nuestra fauna.*

---

## Question 1 [D3]

**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de contenidos locales

### Enunciado
¿Cuál es la única especie de oso que habita en Sudamérica?

### Opciones
- [x] A) El oso de anteojos. <!-- feedback: Correcto. -->
- [ ] B) El oso pardo. <!-- feedback: Incorrecto. -->
- [ ] C) El oso polar. <!-- feedback: Incorrecto. -->
- [ ] D) El oso negro. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de un dato explícito al inicio del texto expositivo.

## Question 2 [D3]

**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de contenidos locales

### Enunciado
¿Por qué se llama "oso de anteojos"?

### Opciones
- [ ] A) Porque usa gafas. <!-- feedback: Incorrecto. -->
- [x] B) Por las manchas blancas alrededor de sus ojos que parecen anteojos. <!-- feedback: Correcto. -->
- [ ] C) Porque vive en casas de cristal. <!-- feedback: Incorrecto. -->
- [ ] D) Porque tiene los ojos muy grandes. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación del origen del nombre común de una especie.

## Question 3 [D4]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
¿Cuál es el propósito principal de este texto?

### Opciones
- [ ] A) Contar una historia sobre un oso. <!-- feedback: Incorrecto. -->
- [x] B) Exponer información científica sobre el oso de anteojos. <!-- feedback: Correcto. -->
- [ ] C) Convencer al lector de que proteja los osos. <!-- feedback: Incorrecto. -->
- [ ] D) Describir un sueño del autor. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación del propósito comunicativo de un texto expositivo vs. narrativo o persuasivo.

## Question 4 [D4]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
¿Dónde habita el oso de anteojos en Colombia?

### Opciones
- [ ] A) En la selva amazónica. <!-- feedback: Incorrecto. -->
- [x] B) En la cordillera de los Andes. <!-- feedback: Correcto. -->
- [ ] C) En la costa Caribe. <!-- feedback: Incorrecto. -->
- [ ] D) En el desierto de La Guajira. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de la ubicación geográfica del hábitat del oso de anteojos.

## Question 5 [D4]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
¿Qué significa que el oso de anteojos sea omnívoro?

### Opciones
- [ ] A) Que solo come frutas. <!-- feedback: Incorrecto. -->
- [x] B) Que come plantas y animales. <!-- feedback: Correcto. -->
- [ ] C) Que solo come carne. <!-- feedback: Incorrecto. -->
- [ ] D) Que no come nada. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Comprensión de un término científico a partir del contexto.

## Question 6 [D4]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Identificación de contenidos locales

### Enunciado
¿Qué clasificación le da la UICN al oso de anteojos?

### Opciones
- [ ] A) En peligro crítico. <!-- feedback: Incorrecto. -->
- [x] B) Vulnerable. <!-- feedback: Correcto. -->
- [ ] C) Extinto en la naturaleza. <!-- feedback: Incorrecto. -->
- [ ] D) Preocupación menor. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Identificación de una categoría taxonómica de conservación.

## Question 7 [D5]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Por qué el oso de anteojos es tímido y evita el contacto con humanos?

### Opciones
- [ ] A) Porque es agresivo. <!-- feedback: Incorrecto. -->
- [x] B) Porque es una especie diferente al oso pardo, con un comportamiento natural más reservado. <!-- feedback: Correcto. -->
- [ ] C) Porque odia a las personas. <!-- feedback: Incorrecto. -->
- [ ] D) Porque está entrenado. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Inferencia sobre las diferencias de comportamiento entre especies de osos.

## Question 8 [D5]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué relación hay entre la deforestación y la situación del oso de anteojos?

### Opciones
- [ ] A) La deforestación beneficia al oso. <!-- feedback: Incorrecto. -->
- [x] B) La deforestación destruye su hábitat, contribuyendo a que sea vulnerable. <!-- feedback: Correcto. -->
- [ ] C) La deforestación no lo afecta. <!-- feedback: Incorrecto. -->
- [ ] D) La deforestación solo afecta a los árboles. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Inferencia de la relación causa-efecto entre la deforestación y el estado de conservación.

## Question 9 [D5]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué función cumplen los parques nacionales como el Cocuy y el Chingaza según el texto?

### Opciones
- [ ] A) Son lugares turísticos. <!-- feedback: Incorrecto. -->
- [x] B) Son áreas protegidas que ayudan a conservar el hábitat del oso. <!-- feedback: Correcto. -->
- [ ] C) Son ciudades importantes. <!-- feedback: Incorrecto. -->
- [ ] D) Son reservas militares. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Inferencia de la función de las áreas protegidas en la conservación de especies.

## Question 10 [D5]

**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido

### Enunciado
¿Por qué es importante que existan textos expositivos como este sobre el oso de anteojos?

### Opciones
- [ ] A) Para entretener a los lectores. <!-- feedback: Incorrecto. -->
- [x] B) Para informar y crear conciencia sobre una especie vulnerable de nuestra fauna. <!-- feedback: Correcto. -->
- [ ] C) Para vender boletos al zoológico. <!-- feedback: Incorrecto. -->
- [ ] D) Para asustar a los niños. <!-- feedback: Incorrecto. -->

### Explicación Pedagógica
Reflexión sobre la importancia de los textos expositivos para la divulgación y conservación.

### Explicación Pedagógica Final
Bundle sobre textos expositivos con enfoque en fauna colombiana. Desarrolla habilidades de identificación de datos explícitos, comprensión del propósito comunicativo, inferencia de relaciones ecológicas y reflexión crítica sobre el valor de la información científica.
