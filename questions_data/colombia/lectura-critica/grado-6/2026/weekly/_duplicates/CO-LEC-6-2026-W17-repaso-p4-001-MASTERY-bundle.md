---
id: "CO-LEC-6-2026-W17-repaso-p4-001-MASTERY-bundle"
country: "colombia"
grado: 6
asignatura: "lectura-critica"
tema: "repaso-p4"
week: "W17"
protocol_version: "5.2"
bundle_index: 1
bundle_size: 10
alignment: "DBA MEN + Estándares Básicos de Competencias"
modern_context: true
distractor_profile: "plausible_peer_set"
calibration:
  expected_success_rate: 0.65
  discrimination_index_target: ">= 0.22"
  simulated_responses: 100
rubric_baseline: "repaso periodo 4, integración de habilidades, escenarios literarios, descripción de ambientes, comparación de textos, propósitos comunicativos, contraste de perspectivas, tipos de narrador"
bundle_type: "weekly"
total_questions: 10
year: 2026
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---


# Lectura Crítica G6 — Repaso del Periodo 4

## Dos Mundos, una Misma Ciudad

### Texto 1: "La Biblioteca del Barrio" (Fragmento narrativo)

*La biblioteca del barrio La Candelaria no era grande, pero para Martín era el lugar más fascinante del mundo. Quedaba en una casona colonial con paredes amarillas y ventanas de madera verde. Al entrar, el silencio envolvía todo como una manta suave. El olor a papel viejo y a café de la tienda de al lado se mezclaban en el aire.*

*Martín conocía cada rincón. En la primera sala estaban los libros de ciencias; al fondo, los de literatura infantil. Pero su lugar favorito era la última mesa junto a la ventana, donde la luz entraba justo para leer sin lastimarse los ojos. Allí había leído veinte libros en un año, y allí planeaba leer veinte más.*

*La bibliotecaria, doña Luz, siempre le guardaba los libros nuevos. —Martín, llegó el que estabas esperando —le decía con una voz apenas susurrada, como si compartiera un secreto. Y Martín sonreía, tomaba el libro y se sentía el niño más afortunado del mundo.*

### Texto 2: "Bibliotecas Públicas en Colombia" (Fragmento expositivo)

*Colombia cuenta con una red de más de 1.500 bibliotecas públicas distribuidas en los 32 departamentos del país. Estas bibliotecas forman parte de la Red Nacional de Bibliotecas Públicas, coordinada por el Ministerio de Cultura. Su objetivo es garantizar el acceso gratuito a la información, la lectura y la cultura para todos los ciudadanos.*

*Las bibliotecas públicas ofrecen servicios como préstamo de libros, salas de lectura, acceso a internet, talleres de escritura, clubes de lectura y actividades culturales. Muchas de ellas están ubicadas en edificios históricos o en centros culturales. Según datos del DANE, aproximadamente el 40% de los colombianos ha visitado alguna vez una biblioteca pública.*

*La Biblioteca Nacional de Colombia, fundada en 1777, es la más antigua del país y alberga más de dos millones de volúmenes. Sin embargo, en los municipios más pequeños, las bibliotecas suelen tener colecciones más reducidas pero igualmente valiosas para sus comunidades.*

---

## Question 1 [D3]

**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de tipos de texto

### Enunciado
¿Qué tipo de texto es el Texto 1?

### Opciones
- [ ] A) Un texto expositivo. <!-- feedback: Incorrecto. El Texto 1 tiene personajes (Martín, doña Luz), descripciones subjetivas y una historia. No expone datos objetivos. -->
- [x] B) Un texto narrativo (cuento). <!-- feedback: Correcto. El Texto 1 narra la experiencia de Martín en la biblioteca, con personajes, descripciones emotivas y una secuencia narrativa. -->
- [ ] C) Una noticia. <!-- feedback: Incorrecto. El Texto 1 no informa sobre un hecho reciente ni responde a las preguntas básicas del periodismo. -->
- [ ] D) Un texto instructivo. <!-- feedback: Incorrecto. No da instrucciones ni pasos a seguir. -->

### Explicación Pedagógica
Recordar las características de cada tipo de texto es fundamental. El Texto 1 es narrativo porque cuenta una historia con personajes y un lugar; el Texto 2 es expositivo porque entrega información objetiva.

## Question 2 [D3]

**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de contenidos locales

### Enunciado
Según el Texto 2, ¿cuántas bibliotecas públicas hay aproximadamente en Colombia?

### Opciones
- [ ] A) 500. <!-- feedback: Incorrecto. El texto menciona una cifra mayor. -->
- [ ] B) 32. <!-- feedback: Incorrecto. 32 son los departamentos, no las bibliotecas. -->
- [x] C) Más de 1.500. <!-- feedback: Correcto. El Texto 2 dice: "Colombia cuenta con una red de más de 1.500 bibliotecas públicas." -->
- [ ] D) 2 millones. <!-- feedback: Incorrecto. 2 millones son los volúmenes de la Biblioteca Nacional, no la cantidad de bibliotecas. -->

### Explicación Pedagógica
Identificar datos explícitos en un texto expositivo requiere leer con atención los números y cantidades. Es fácil confundir datos similares (1.500 bibliotecas vs. 2 millones de volúmenes).

## Question 3 [D4]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
¿Qué tienen en común ambos textos?

### Opciones
- [ ] A) Ambos son cuentos ficticios. <!-- feedback: Incorrecto. Solo el Texto 1 es un cuento. -->
- [ ] B) Ambos mencionan a un personaje llamado Martín. <!-- feedback: Incorrecto. Martín solo aparece en el Texto 1. -->
- [x] C) Ambos tratan sobre bibliotecas, pero desde perspectivas diferentes: el Texto 1 desde la experiencia personal y el Texto 2 desde datos objetivos. <!-- feedback: Correcto. Las bibliotecas son el tema común; lo que cambia es el enfoque: personal vs. informativo. -->
- [ ] D) Ambos están escritos para convencer al lector de visitar bibliotecas. <!-- feedback: Incorrecto. El Texto 1 no busca convencer; busca narrar. El Texto 2 informa, no persuade. -->

### Explicación Pedagógica
Identificar el tema común entre dos textos es el primer paso para compararlos. Aquí, ambos hablan de bibliotecas, pero con propósitos y géneros distintos.

## Question 4 [D4]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Caracterización literaria

### Enunciado
En el Texto 1, ¿qué característica del escenario de la biblioteca se destaca principalmente?

### Opciones
- [ ] A) Que es ruidosa y caótica. <!-- feedback: Incorrecto. El texto dice que "el silencio envolvía todo como una manta suave". -->
- [ ] B) Que está en un centro comercial. <!-- feedback: Incorrecto. La biblioteca "quedaba en una casona colonial" en La Candelaria. -->
- [x] C) Que es un lugar acogedor, silencioso y lleno de encanto, con luz natural y olor a papel. <!-- feedback: Correcto. Los detalles sensoriales (silencio, olor a papel, luz por la ventana) construyen un ambiente acogedor y tranquilo. -->
- [ ] D) Que es oscura y abandonada. <!-- feedback: Incorrecto. No hay elementos de abandono; al contrario, hay cuidado (doña Luz guarda libros nuevos, la luz entra por la ventana). -->

### Explicación Pedagógica
Los escenarios literarios se construyen con detalles sensoriales. En el Texto 1, la biblioteca es descrita como un refugio acogedor para Martín, con silencio, luz y olor a papel.

## Question 5 [D5]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué podemos inferir sobre la relación entre Martín y doña Luz en el Texto 1?

### Opciones
- [ ] A) Que no se conocen. <!-- feedback: Incorrecto. Doña Luz le guarda los libros nuevos, lo que indica que lo conoce bien. -->
- [ ] B) Que doña Luz no quiere que Martín lea. <!-- feedback: Incorrecto. Al guardarle libros, lo anima a leer. -->
- [x] C) Que hay una relación de confianza y cariño: doña Luz conoce los gustos de Martín y le guarda los libros que le interesan. <!-- feedback: Correcto. "Doña Luz siempre le guardaba los libros nuevos" y le dice "llegó el que estabas esperando" con voz de "secreto", lo que muestra cercanía y confianza. -->
- [ ] D) Que doña Luz es la mamá de Martín. <!-- feedback: Incorrecto. No hay evidencia de parentesco; doña Luz es la bibliotecaria. -->

### Explicación Pedagógica
Las relaciones entre personajes se infieren a partir de sus acciones y diálogos. Guardar libros nuevos, susurrar con complicidad y compartir secretos indican una relación cercana y afectuosa.

## Question 6 [D5]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué diferencia de enfoque hay entre el Texto 1 y el Texto 2 respecto al valor de las bibliotecas?

### Opciones
- [ ] A) El Texto 1 dice que las bibliotecas no son importantes; el Texto 2 dice que sí. <!-- feedback: Incorrecto. Ambos valoran las bibliotecas, pero de distintas maneras. -->
- [ ] B) Ambos valoran las bibliotecas de la misma forma objetiva. <!-- feedback: Incorrecto. El Texto 1 usa un enfoque subjetivo y emotivo. -->
- [x] C) El Texto 1 muestra el valor afectivo y personal de la biblioteca en la vida de un niño; el Texto 2 muestra el valor social y estadístico de las bibliotecas como instituciones. <!-- feedback: Correcto. Martín valora la biblioteca como un lugar mágico personal; el Texto 2 la valora como un servicio público con datos (1.500 bibliotecas, 40% de visitantes). -->
- [ ] D) El Texto 1 critica las bibliotecas; el Texto 2 las defiende. <!-- feedback: Incorrecto. El Texto 1 no critica; describe con cariño. El Texto 2 no defiende; solo informa. -->

### Explicación Pedagógica
Un mismo tema puede ser abordado desde lo personal y emotivo o desde lo social y estadístico. Reconocer estas diferencias es parte de la comparación crítica de textos.

## Question 7 [D5]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
En el Texto 1, la frase "el silencio envolvía todo como una manta suave" es un ejemplo de:

### Opciones
- [ ] A) Un dato estadístico. <!-- feedback: Incorrecto. No es un dato, es una figura literaria. -->
- [ ] B) Una hipérbole o exageración. <!-- feedback: Incorrecto. No exagera, compara el silencio con una manta para transmitir la sensación de cobijo. -->
- [x] C) Una comparación (símil) que personifica el silencio y lo describe como algo acogedor. <!-- feedback: Correcto. "Como una manta suave" es una comparación que transforma el silencio (abstracto) en algo tangible y acogedor. -->
- [ ] D) Una instrucción para el lector. <!-- feedback: Incorrecto. No es una instrucción. -->

### Explicación Pedagógica
Las figuras literarias como el símil enriquecen las descripciones narrativas. El Texto 2, al ser expositivo, no usa este tipo de recursos. Esta diferencia es otra pista para distinguir géneros textuales.

## Question 8 [D5]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
¿Qué información del Texto 2 ayuda a entender por qué la biblioteca del Texto 1 es valiosa para su comunidad?

### Opciones
- [ ] A) Que la Biblioteca Nacional fue fundada en 1777. <!-- feedback: Incorrecto. Ese dato no se relaciona directamente con la biblioteca del barrio La Candelaria. -->
- [ ] B) Que las bibliotecas tienen colecciones reducidas en municipios pequeños. <!-- feedback: Incorrecto. La Candelaria es un barrio de Bogotá, no un municipio pequeño. -->
- [x] C) Que las bibliotecas públicas ofrecen préstamo de libros y acceso gratuito a la cultura, lo que explica que Martín pueda leer tantos libros sin pagar. <!-- feedback: Correcto. El Texto 2 menciona servicios gratuitos como préstamo de libros, lo que contextualiza por qué Martín puede leer "veinte libros en un año". -->
- [ ] D) Que el 40% de los colombianos ha visitado una biblioteca. <!-- feedback: Incorrecto. Es un dato general que no explica el valor específico de esa biblioteca. -->

### Explicación Pedagógica
La información de un texto puede ayudar a comprender mejor el otro. Esta conexión entre textos es una forma avanzada de comprensión lectora: usar un texto como contexto para interpretar el otro.

## Question 9 [D6]

**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido

### Enunciado
¿Cuál de los dos textos sería más útil para un alcalde que quiere justificar la inversión en bibliotecas públicas ante el concejo municipal?

### Opciones
- [ ] A) El Texto 1, porque es emotivo y conmueve. <!-- feedback: Incorrecto. Aunque un texto emotivo puede ayudar, para justificar una inversión se necesitan datos objetivos. -->
- [x] B) El Texto 2, porque proporciona datos concretos (1.500 bibliotecas, alcance nacional, servicios ofrecidos) que respaldan una decisión de política pública. <!-- feedback: Correcto. Un alcalde necesita datos verificables y objetivos para justificar inversiones. El Texto 2 ofrece esos datos. -->
- [ ] C) Ambos por igual. <!-- feedback: Incorrecto. Cada texto sirve a propósitos distintos; el Texto 2 es más adecuado para fines administrativos. -->
- [ ] D) Ninguno, porque los dos son ficticios. <!-- feedback: Incorrecto. El Texto 2 no es ficticio; contiene información objetiva y verificable. -->

### Explicación Pedagógica
Evaluar la utilidad de un texto según el contexto y la necesidad del lector es una habilidad crítica. No todos los textos sirven para todos los propósitos; saber seleccionar es parte de ser un lector competente.

## Question 10 [D6]

**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido

### Enunciado
¿Qué elemento del Texto 1 NO podría aparecer en el Texto 2 sin cambiar su género y propósito?

### Opciones
- [ ] A) La mención de que las bibliotecas ofrecen préstamo de libros. <!-- feedback: Incorrecto. Eso sí podría aparecer en un texto expositivo. -->
- [ ] B) El dato de que la Biblioteca Nacional fue fundada en 1777. <!-- feedback: Incorrecto. Ese dato encaja perfectamente en un texto expositivo. -->
- [x] C) La frase "Martín sonreía, tomaba el libro y se sentía el niño más afortunado del mundo". <!-- feedback: Correcto. Esta expresión subjetiva y emotiva no encajaría en un texto expositivo que debe mantener objetividad. En el Texto 2 no hay emociones ni personajes. -->
- [ ] D) La información sobre los 32 departamentos de Colombia. <!-- feedback: Incorrecto. Ese dato encajaría perfectamente en un texto expositivo. -->

### Explicación Pedagógica
Cada género textual tiene sus propios recursos y limitaciones. Las emociones, los personajes y las descripciones subjetivas son propios de la narrativa. Los datos, las estadísticas y el tono objetivo son propios del texto expositivo. Mezclarlos inadecuadamente rompería la coherencia del género.

### Explicación Pedagógica Final
Bundle de repaso del periodo 4 que integra las habilidades trabajadas: descripción de escenarios literarios (biblioteca de La Candelaria), comparación de textos (narrativo vs. expositivo), identificación de propósitos comunicativos, análisis de recursos lingüísticos (figuras literarias vs. datos objetivos), y evaluación crítica de la utilidad de cada texto según el propósito del lector. Contexto colombiano con la Red Nacional de Bibliotecas Públicas y un barrio tradicional de Bogotá.
