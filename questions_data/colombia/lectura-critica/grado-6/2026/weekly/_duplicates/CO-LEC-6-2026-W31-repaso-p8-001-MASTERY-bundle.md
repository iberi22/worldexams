---
id: "CO-LEC-6-2026-W31-repaso-p8-001-MASTERY"
country: "colombia"
grado: 6
asignatura: "lectura-critica"
tema: "repaso-p8"
periodo: 8
week: 31
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 10
bundle_size: 10
alignment: "DBA MEN Lenguaje + Estándares Básicos Ciclo 3"
---

# Weekly Pack W31 — REPASO Periodo 8

**Grado:** 6° | **Periodo:** 8 | **Semana:** 31 | **Año:** 2026

**Temas:** Argumentación básica, punto de vista del autor, tesis y argumentos, tono y lenguaje, sesgo y parcialidad.

---

## Question 1 [D1]

**ID:** `CO-LEC-6-2026-W31-repaso-p8-001-v1`
**Bloom:** Remember
**ICFES:** Identificación de tipos de texto
**Context:** Repaso de argumentación

### Enunciado

¿Cuál es el propósito principal de un texto argumentativo?

### Options
- [ ] A) Narrar una historia con personajes y conflicto <!-- feedback: Incorrecto. Ese es el propósito de un texto narrativo. La argumentación no busca contar historias, sino defender ideas. -->
- [ ] B) Explicar un proceso paso a paso <!-- feedback: Incorrecto. Explicar un proceso es propio de textos instructivos o expositivos, no argumentativos. -->
- [x] C) Defender una opinión o postura mediante razones para convencer al lector <!-- feedback: ¡Correcto! La argumentación tiene un propósito persuasivo: el autor presenta una tesis y la respalda con argumentos para lograr que el lector adopte su punto de vista. -->
- [ ] D) Describir las características físicas de un objeto <!-- feedback: Incorrecto. Describir es propio de textos descriptivos. Aunque un texto argumentativo pueda incluir descripciones, su fin principal es convencer. -->

### Explicación Pedagógica

Recordemos que los textos se clasifican según su propósito: narrar (contar historias), describir (pintar con palabras), exponer (informar objetivamente), argumentar (convencer) e instruir (guiar acciones). La argumentación es especialmente importante en la vida cotidiana: la usamos al debatir, al escribir cartas formales, al opinar en redes sociales y al tomar decisiones en grupo.

---

## Question 2 [D1]

**ID:** `CO-LEC-6-2026-W31-repaso-p8-002-v1`
**Bloom:** Remember
**ICFES:** Identificación de elementos textuales
**Context:** Repaso de tesis

### Enunciado

En un texto argumentativo, la tesis es:

### Options
- [ ] A) El párrafo final donde se resumen los argumentos <!-- feedback: Incorrecto. Esa es la conclusión. La tesis es la idea central que se defiende y suele aparecer al inicio. -->
- [x] B) La idea principal u opinión que el autor quiere defender y demostrar <!-- feedback: ¡Correcto! La tesis es la columna vertebral del texto argumentativo. Todos los argumentos giran en torno a ella y buscan sostenerla. -->
- [ ] C) Un ejemplo que ilustra el tema <!-- feedback: Incorrecto. Los ejemplos son un tipo de argumento o recurso, no la tesis. La tesis es la postura general que se defiende. -->
- [ ] D) La lista de fuentes consultadas por el autor <!-- feedback: Incorrecto. Las fuentes son referencias bibliográficas. No son la tesis, aunque pueden usarse como respaldo de los argumentos. -->

### Explicación Pedagógica

Diferenciar tesis de argumentos y ejemplos es fundamental. La tesis responde a "¿qué opina el autor?". Los argumentos responden a "¿por qué lo cree?". Los ejemplos responden a "¿puedes dar un caso concreto?". Una buena estrategia es subrayar la oración que contiene la opinión principal del autor y luego identificar qué oraciones la respaldan.

---

## Question 3 [D2]

**ID:** `CO-LEC-6-2026-W31-repaso-p8-003-v1`
**Bloom:** Understand
**ICFES:** Comprensión de argumentos
**Context:** Identificar argumentos en un texto

### Enunciado

> "La bicicleta debería ser el medio de transporte principal en las ciudades colombianas. Primero, porque no contamina el ambiente y ayuda a reducir las emisiones de CO2. Segundo, porque es un excelente ejercicio que mejora la salud cardiovascular. Tercero, porque es económica: no gasta gasolina ni requiere costoso mantenimiento."

¿Cuántos argumentos presenta el autor?

### Options
- [ ] A) Uno: que la bicicleta es buena para la salud <!-- feedback: Incorrecto. Ese es solo el segundo argumento. El autor presenta varias razones. -->
- [ ] B) Dos: el ambiente y la salud <!-- feedback: Incorrecto. Faltó el argumento económico (tercero). -->
- [x] C) Tres: beneficio ambiental, beneficio para la salud y beneficio económico <!-- feedback: ¡Correcto! El autor usa "Primero", "Segundo" y "Tercero" para presentar tres argumentos independientes que apoyan su tesis. -->
- [ ] D) Cuatro: incluyendo que el auto es contaminante <!-- feedback: Incorrecto. El autor no menciona autos ni compara directamente. Solo presenta tres argumentos a favor de la bicicleta. -->

### Explicación Pedagógica

Los conectores de orden (primero, segundo, finalmente, además, también) son señales que nos ayudan a identificar la estructura de un texto argumentativo. Funcionan como letreros que le indican al lector: "aquí viene un nuevo argumento". Reconocerlos facilita la comprensión de la organización del texto.

---

## Question 4 [D2]

**ID:** `CO-LEC-6-2026-W31-repaso-p8-004-v1`
**Bloom:** Understand
**ICFES:** Identificación del punto de vista del autor
**Context:** Reconocer la postura del autor

### Enunciado

> **Texto:** "Colombia es un país con un potencial turístico enorme. Sus paisajes, su biodiversidad y la calidez de su gente son incomparables. Sin embargo, la falta de inversión en infraestructura vial y la inseguridad en algunas regiones siguen siendo obstáculos que debemos superar para convertirnos en un destino turístico de primer nivel."

¿Cuál es la postura del autor sobre el turismo en Colombia?

### Options
- [ ] A) Es totalmente negativa: cree que Colombia no tiene futuro turístico <!-- feedback: Incorrecto. El autor reconoce problemas, pero también destaca el potencial del país. No es una postura totalmente negativa. -->
- [ ] B) Es neutral: solo presenta datos sin opinar <!-- feedback: Incorrecto. El autor expresa opiniones ("potencial enorme", "incomparables") y propone una dirección ("debemos superar"). No es neutral. -->
- [x] C) Es positiva pero realista: reconoce las fortalezas del país y también los problemas que enfrenta <!-- feedback: ¡Correcto! El autor valora el potencial turístico (positivo) pero reconoce los obstáculos (realista). Su postura es constructiva: no idealiza ni destruye, sino que señala áreas de mejora. -->
- [ ] D) Es indiferente: al autor no le importa el turismo <!-- feedback: Incorrecto. Si fuera indiferente, no escribiría sobre el tema ni propondría soluciones. El interés del autor es evidente. -->

### Explicación Pedagógica

El punto de vista del autor no siempre es extremo (todo bueno o todo malo). A menudo es matizado: reconoce aspectos positivos y negativos. Identificar estos matices es señal de lectura sofisticada. Un autor que solo ve el lado positivo puede ser parcial; uno que solo ve lo negativo, también. Los textos más equilibrados presentan ambas caras.

---

## Question 5 [D3]

**ID:** `CO-LEC-6-2026-W31-repaso-p8-005-v1`
**Bloom:** Understand
**ICFES:** Diferenciar hecho de opinión
**Context:** Repaso de hechos vs. opiniones

### Enunciado

> "La temperatura global ha aumentado 1.1 °C desde la era preindustrial. Creo que los gobiernos deberían tomar medidas más drásticas para frenar el cambio climático."

¿Cuál afirmación es un HECHO y cuál es una OPINIÓN?

### Options
- [ ] A) Ambas son hechos porque están basadas en información real <!-- feedback: Incorrecto. La segunda afirmación expresa una creencia ("creo que"), no un dato comprobable. -->
- [ ] B) Ambas son opiniones porque hablan de cambio climático <!-- feedback: Incorrecto. Que un tema sea debatible no convierte todos los datos en opiniones. El primer dato es verificable. -->
- [x] C) "La temperatura ha aumentado 1.1 °C" es un hecho; "los gobiernos deberían tomar medidas" es una opinión <!-- feedback: ¡Correcto! El primer dato se puede verificar con mediciones científicas. La segunda afirmación expresa un juicio de valor sobre lo que debería hacerse, lo cual es una opinión. -->
- [ ] D) "Gobiernos deberían tomar medidas" es un hecho; el aumento de temperatura es una opinión <!-- feedback: Incorrecto. Es al revés. La temperatura es medible (hecho); lo que "deberían" hacer los gobiernos es una opinión. -->

### Explicación Pedagógica

Este es uno de los conceptos más importantes de la lectura crítica. Los hechos son verificables: pueden ser comprobados con evidencia. Las opiniones expresan creencias, valores o juicios. Pistas verbales de opinión: "creo que", "debería", "es mejor", "lamentablemente", "afortunadamente". Un buen texto argumentativo combina hechos (para dar credibilidad) con opiniones (para expresar su postura).

---

## Question 6 [D3]

**ID:** `CO-LEC-6-2026-W31-repaso-p8-006-v1`
**Bloom:** Understand
**ICFES:** Identificación de sesgo
**Context:** Texto con sesgo evidente

### Enunciado

> "Los agricultores colombianos son los héroes anónimos de nuestra nación. Mientras los políticos pierden el tiempo en discusiones, los campesinos trabajan de sol a sol para llevar comida a nuestra mesa. Sin ellos, este país no sería nada."

¿Qué sesgo tiene este fragmento?

### Options
- [ ] A) No tiene sesgo; es objetivo porque describe la realidad del campo <!-- feedback: Incorrecto. El texto claramente favorece a los agricultores y descalifica a los políticos. El lenguaje no es neutral. -->
- [x] B) Tiene un sesgo positivo hacia los agricultores y negativo hacia los políticos, sin presentar evidencia de ninguna de las dos posturas <!-- feedback: ¡Correcto! El autor idealiza a los agricultores ("héroes anónimos") y generaliza sobre los políticos ("pierden el tiempo"), sin datos que respalden ninguna afirmación. -->
- [ ] C) Tiene un sesgo negativo hacia los agricultores porque dice que trabajan demasiado <!-- feedback: Incorrecto. Decir que trabajan "de sol a sol" es una forma de elogiarlos, no de criticarlos. El sesgo es positivo hacia ellos. -->
- [ ] D) Es neutral porque no usa datos específicos <!-- feedback: Incorrecto. La ausencia de datos no hace que un texto sea neutral. De hecho, sin datos es más fácil que el sesgo se imponga. -->

### Explicación Pedagógica

El sesgo puede ser positivo o negativo. En este caso, el sesgo es doble: positivo hacia los agricultores y negativo hacia los políticos. El texto usa palabras como "héroes anónimos" (carga emocional positiva) versus "pierden el tiempo" (carga emocional negativa). Un texto equilibrado reconocería tanto el valor del trabajo campesino como la complejidad de la labor política.

---

## Question 7 [D4]

**ID:** `CO-LEC-6-2026-W31-repaso-p8-007-v1`
**Bloom:** Apply
**ICFES:** Aplicación de argumentación
**Context:** Elegir el argumento más sólido

### Enunciado

Quieres convencer a tu colegio de crear una huerta escolar. ¿Cuál de los siguientes argumentos sería MÁS efectivo?

### Options
- [ ] A) "Las huertas son bonitas y divertidas" <!-- feedback: Incorrecto. Es un argumento superficial basado en gustos personales. No es convincente para tomar una decisión institucional. -->
- [x] B) "La huerta escolar serviría como laboratorio vivo para las clases de ciencias, mejoraría la alimentación de los estudiantes y fomentaría el trabajo en equipo" <!-- feedback: ¡Correcto! Este argumento presenta beneficios concretos en tres áreas: académica (laboratorio para ciencias), salud (alimentación) y social (trabajo en equipo). Es completo y persuasivo. -->
- [ ] C) "Otros colegios tienen huerta y a los estudiantes les gusta" <!-- feedback: Incorrecto. Es un argumento débil porque se basa en la popularidad, no en beneficios demostrables. Además, "que les guste" es subjetivo. -->
- [ ] D) "El rector debería hacer caso a los estudiantes" <!-- feedback: Incorrecto. Este es un argumento de autoridad mal planteado. No da razones, solo exige. Puede sonar impositivo y no persuade. -->

### Explicación Pedagógica

Los argumentos más efectivos son aquellos que: 1) presentan beneficios concretos y variados, 2) se dirigen a los intereses de la audiencia (en este caso, el colegio valora lo académico, la salud y la convivencia), 3) son específicos en lugar de vagos, y 4) evitan la imposición. Un buen argumento no exige, sino que invita a ver los beneficios.

---

## Question 8 [D5]

**ID:** `CO-LEC-6-2026-W31-repaso-p8-008-v1`
**Bloom:** Analyze
**ICFES:** Análisis del tono del autor
**Context:** Identificar el tono predominante

### Enunciado

> **Fragmento:** "Es verdaderamente lamentable que, en pleno siglo XXI, todavía existan niños en Colombia que no tienen acceso a Internet. Mientras el mundo avanza hacia la inteligencia artificial y la robótica, miles de estudiantes colombianos siguen sin poder conectarse. ¿Acaso no es esto una vergüenza nacional?"

¿Qué tono predomina en este fragmento?

### Options
- [ ] A) Humorístico <!-- feedback: Incorrecto. El tema es serio y el autor no usa chistes ni ironía cómica. -->
- [x] B) Indignado y crítico <!-- feedback: ¡Correcto! Palabras como "lamentable", "vergüenza nacional" y el uso de una pregunta retórica revelan indignación. El autor está molesto por la situación y critica la falta de acceso a Internet. -->
- [ ] C) Neutral e informativo <!-- feedback: Incorrecto. El texto no es neutral. Expresa una emoción fuerte (indignación) y busca provocar esa misma emoción en el lector. -->
- [ ] D) Alegre y optimista <!-- feedback: Incorrecto. El texto describe una situación negativa (falta de acceso) con tono de denuncia. No hay alegría ni optimismo en el mensaje. -->

### Explicación Pedagógica

El tono revela la actitud emocional del autor hacia el tema. Para identificarlo, pregúntate: ¿qué emoción transmite este texto? ¿Enojo? ¿Alegría? ¿Tristeza? ¿Sarcasmo? ¿Seriedad? Las preguntas retóricas, las exclamaciones y las palabras con carga emocional son pistas importantes. El tono indignado busca movilizar al lector, no solo informarle.

---

## Question 9 [D5]

**ID:** `CO-LEC-6-2026-W31-repaso-p8-009-v1`
**Bloom:** Analyze
**ICFES:** Identificación de estructura argumentativa
**Context:** Reconocer partes del texto argumentativo

### Enunciado

> (1) Últimamente se ha discutido mucho sobre el uso de uniforme escolar. (2) En mi opinión, el uniforme debería ser obligatorio en todos los colegios. (3) Por un lado, elimina las diferencias económicas entre estudiantes, ya que todos visten igual. (4) Por otro lado, facilita la identificación de los estudiantes dentro y fuera del colegio, lo que mejora la seguridad. (5) En conclusión, el uniforme escolar promueve la igualdad y la seguridad, dos valores fundamentales en la educación.

¿Qué función cumple cada oración?

### Options
- [ ] A) (1) Tesis, (2) Conclusión, (3) Argumento, (4) Argumento, (5) Contexto <!-- feedback: Incorrecto. (1) es contexto, no tesis. (2) es la tesis. (5) es la conclusión. -->
- [ ] B) (1) Contexto, (2) Argumento, (3) Tesis, (4) Argumento, (5) Conclusión <!-- feedback: Incorrecto. (2) no es argumento, es la tesis. (3) y (4) son los argumentos. -->
- [x] C) (1) Contexto, (2) Tesis, (3) Argumento, (4) Argumento, (5) Conclusión <!-- feedback: ¡Correcto! (1) presenta el tema en discusión. (2) es la tesis (postura del autor). (3) y (4) son argumentos que la apoyan. (5) es la conclusión que sintetiza y refuerza la tesis. -->
- [ ] D) (1) Introducción, (2) Argumento, (3) Argumento, (4) Conclusión, (5) Tesis <!-- feedback: Incorrecto. (2) no es argumento sino tesis. (4) no es conclusión sino argumento. (5) es conclusión, no tesis. -->

### Explicación Pedagógica

Reconocer la estructura de un texto argumentativo es como armar un rompecabezas: cada pieza tiene un lugar y una función. La secuencia típica es: contexto (presenta el tema) → tesis (postura) → argumentos (razones) → conclusión (cierre). Al dominar esta estructura, podemos analizar cualquier texto argumentativo con mayor facilidad y también escribir mejores argumentaciones.

---

## Question 10 [D6]

**ID:** `CO-LEC-6-2026-W31-repaso-p8-010-v1`
**Bloom:** Evaluate
**ICFES:** Valoración crítica del punto de vista
**Context:** Evaluar credibilidad y punto de vista

### Enunciado

Lee la siguiente situación:

> Un artículo en redes sociales dice: "Comer solo frutas durante una semana limpia el cuerpo de todas las toxinas. Cientos de personas ya lo han probado y han visto resultados increíbles. ¡No esperes más para transformar tu salud!"

¿Qué problema tiene este texto desde el punto de vista de la lectura crítica?

### Options
- [ ] A) Que recomienda frutas, lo cual es malo para la salud <!-- feedback: Incorrecto. Las frutas no son malas para la salud. El problema no es la recomendación en sí, sino cómo se presenta. -->
- [x] B) Que hace afirmaciones sin sustento científico ("limpia toxinas"), usa testimonios vagos ("cientos de personas") y lenguaje exagerado ("resultados increíbles") para persuadir sin evidencia <!-- feedback: ¡Correcto! El texto carece de evidencia verificable, apela a testimonios anónimos y usa exageraciones. Esto se llama "argumento basado en falsa autoridad y testimonios no verificables". -->
- [ ] C) Que es muy corto y no da suficientes detalles <!-- feedback: Incorrecto. Un texto corto puede ser válido si la información es precisa. El problema es la falta de evidencia, no la extensión. -->
- [ ] D) Que usar frutas es caro y no todos pueden comprarlas <!-- feedback: Incorrecto. El texto no dice nada sobre el costo. Este es un argumento que el lector está añadiendo, no está en el texto. No debemos añadir información que no aparece. -->

### Explicación Pedagógica

Evaluar críticamente un texto implica preguntarse: 1) ¿Qué evidencias respaldan estas afirmaciones? 2) ¿Quién escribió esto y qué credibilidad tiene? 3) ¿Usa lenguaje exagerado para manipular emociones? 4) ¿Hay fuentes verificables o solo testimonios anónimos? Los textos de dudosa credibilidad suelen usar promesas milagrosas, urgencia ("no esperes más") y falta de evidencia científica. Esta habilidad es esencial en la era de la desinformación.

---
