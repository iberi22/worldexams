---
id: "CO-LEC-6-2026-W24-repaso-p6-001-MASTERY-bundle"
country: "colombia"
grado: 6
asignatura: "lectura-critica"
tema: "repaso-p6"
periodo: 6
week: 24
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 10
bundle_size: 10
alignment: "DBA MEN Lenguaje + Estándares Básicos Ciclo 3"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---


# Weekly Pack W24 — REPASO Periodo 6

**Grado:** 6° | **Periodo:** 6 | **Semana:** 24 | **Año:** 2026

**Temas:** Textos argumentativos, tesis y argumentos, opinión y persuasión, inferencia textual, causa y efecto en textos.

---

## Question 1 [D1]

**ID:** `CO-LEC-6-2026-W24-repaso-p6-001-v1`
**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de textos argumentativos
**Contexto:** Repaso de tipos de texto

### Enunciado

¿Cuál de las siguientes situaciones corresponde a un texto ARGUMENTATIVO?

### Opciones
- [ ] A) Un cuento sobre un niño que viaja por Colombia en globo <!-- feedback: Incorrecto. Un cuento es un texto NARRATIVO, no argumentativo. Cuenta una historia. -->
- [ ] B) Una receta para preparar bandeja paisa <!-- feedback: Incorrecto. Una receta es un texto INSTRUCTIVO. Explica pasos para hacer algo. -->
- [ ] C) Un artículo que describe los volcanes de Colombia <!-- feedback: Incorrecto. Un artículo descriptivo es un texto EXPOSITIVO/INFORMATIVO. Su propósito es describir, no convencer. -->
- [x] D) Un editorial que defiende por qué se debe reciclar en las escuelas <!-- feedback: ¡Correcto! Un editorial defiende una postura y busca convencer al lector. Usa argumentos para apoyar una tesis. Es un texto argumentativo. -->

### Explicación Pedagógica

Los textos argumentativos tienen como propósito convencer o persuadir al lector sobre una idea. Se caracterizan por tener una TESIS (idea que se defiende) y ARGUMENTOS (razones que la apoyan). Ejemplos: editoriales, columnas de opinión, discursos políticos, debates. No confundir con textos expositivos (informan) o narrativos (cuentan historias).

---

## Question 2 [D1]

**ID:** `CO-LEC-6-2026-W24-repaso-p6-002-v1`
**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de tesis
**Contexto:** Repaso: qué es la tesis

### Enunciado

En un texto argumentativo, ¿qué es la TESIS?

### Opciones
- [ ] A) El cuento o historia que se narra al inicio <!-- feedback: Incorrecto. Una tesis no es una historia narrativa. Los textos argumentativos pueden incluir anécdotas, pero la tesis es la idea central que se defiende. -->
- [x] B) La idea principal que el autor defiende con argumentos <!-- feedback: ¡Correcto! La tesis es la postura del autor frente a un tema. Es lo que el autor quiere demostrar. Todo el texto gira en torno a defenderla. -->
- [ ] C) La lista de fuentes que usó el autor <!-- feedback: Incorrecto. Las fuentes son referencias que apoyan los argumentos, pero no son la tesis. La tesis es la idea, no las fuentes. -->
- [ ] D) El título del texto <!-- feedback: Incorrecto. El título puede anticipar el tema, pero no es la tesis. La tesis es una afirmación que el autor buscará probar. -->

### Explicación Pedagógica

Para identificar la tesis, pregúntate: ¿qué es lo que el autor quiere demostrar? ¿Cuál es su postura? La tesis suele aparecer al inicio o al final del texto argumentativo. Frases como "estoy convencido de que", "considero que", "es necesario" suelen anunciar la tesis.

---

## Question 3 [D2]

**ID:** `CO-LEC-6-2026-W24-repaso-p6-003-v1`
**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Diferenciar hecho de opinión
**Contexto:** Repaso: hecho vs opinión

### Enunciado

> Colombia tiene una extensión de 1.141.748 km². Es el país más hermoso de Sudamérica.

¿Cuál de las dos afirmaciones es una OPINIÓN?

### Opciones
- [ ] A) "Colombia tiene una extensión de 1.141.748 km²" <!-- feedback: Incorrecto. Esa es una afirmación verificable. Se puede comprobar consultando datos geográficos oficiales. Es un HECHO. -->
- [ ] B) "Colombia tiene una extensión" <!-- feedback: Incorrecto. Eso es un hecho comprobable. Todos los países tienen una extensión territorial. -->
- [x] C) "Es el país más hermoso de Sudamérica" <!-- feedback: ¡Correcto! "Más hermoso" es una apreciación subjetiva. Depende del gusto de cada persona. No se puede comprobar objetivamente. Es una OPINIÓN. -->
- [ ] D) Ninguna, ambas son hechos comprobables <!-- feedback: Incorrecto. La primera es un hecho, pero la segunda ("más hermoso") no se puede medir ni comprobar. Es una opinión. -->

### Explicación Pedagógica

Los HECHOS son afirmaciones que se pueden verificar (medir, contar, consultar). Las OPINIONES son juicios subjetivos que expresan gustos, creencias o valoraciones. Palabras como "hermoso", "mejor", "debería", "injusto" suelen indicar opiniones. Para distinguirlas, pregúntate: ¿esto se puede comprobar con datos?

---

## Question 4 [D2]

**ID:** `CO-LEC-6-2026-W24-repaso-p6-004-v1`
**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión de argumentos
**Contexto:** Repaso: identificar argumentos

### Enunciado

> **Tesis:** Los estudiantes deben tener recreo de 30 minutos.
>
> **Argumento:** Porque necesitan tiempo para jugar, socializar y descansar la mente después de clases largas.

¿Qué función cumple el argumento en relación con la tesis?

### Opciones
- [ ] A) Contradecir la tesis <!-- feedback: Incorrecto. El argumento no contradice la tesis, sino que la apoya. Dice "porque", que indica una razón a favor. -->
- [ ] B) Preguntar si la tesis es cierta <!-- feedback: Incorrecto. El argumento no es una pregunta. Es una afirmación que da razones. -->
- [x] C) Apoyar la tesis dando razones <!-- feedback: ¡Correcto! El argumento responde a la pregunta ¿POR QUÉ? la tesis es válida. "Porque necesitan tiempo para jugar..." son las razones que apoyan que el recreo debe ser de 30 minutos. -->
- [ ] D) Describir cómo es el recreo actualmente <!-- feedback: Incorrecto. El argumento no describe la situación actual, sino que da razones para cambiar o mantener algo. -->

### Explicación Pedagógica

Los argumentos son las razones que el autor da para convencernos de que su tesis es válida. Sin argumentos, una tesis es solo una opinión. Los argumentos responden a la pregunta "¿por qué?". Cuando leas un texto argumentativo, separa mentalmente la tesis (qué defiende) de los argumentos (por qué lo defiende).

---

## Question 5 [D3]

**ID:** `CO-LEC-6-2026-W24-repaso-p6-005-v1`
**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Inferencia básica
**Contexto:** Repaso: inferir información implícita

### Enunciado

> **En la heladería**
>
> María entró a la heladería y pidió una bola de helado de arequipe. El vendedor le dijo: "Se nos acabó el arequipe hace una hora". María frunció el ceño. "¿Qué les queda?", preguntó. "Vainilla, fresa y chocolate", respondió el vendedor. María suspiró y pidió chocolate.

¿Cómo se sintió María cuando le dijeron que no había arequipe?

### Opciones
- [ ] A) Alegre, porque prefería chocolate <!-- feedback: Incorrecto. Si estuviera alegre, no habría fruncido el ceño ni suspirado. Sus reacciones indican lo contrario. -->
- [x] B) Decepcionada, porque quería arequipe <!-- feedback: ¡Correcto! "Frunció el ceño" y "suspiró" son señales de decepción. Además, pidió el helado de arequipe primero, lo que indica que era su preferido. -->
- [ ] C) Enojada con el vendedor <!-- feedback: Incorrecto. Aunque frunció el ceño, no hay evidencia de enojo contra el vendedor. El texto no dice que lo haya regañado o reclamado. -->
- [ ] D) Indiferente, como si no le importara <!-- feedback: Incorrecto. Fruncir el ceño y suspirar muestran que sí le importó. La indiferencia se expresaría con un "está bien, no importa" sin reacciones emocionales. -->

### Explicación Pedagógica

Las emociones se infieren de las reacciones físicas y verbales de las personas. Fruncir el ceño indica molestia o decepción. Suspirar puede indicar resignación o frustración. Prestar atención a estas señales nos ayuda a entender cómo se sienten los personajes, aunque no lo digan explícitamente.

---

## Question 6 [D4]

**ID:** `CO-LEC-6-2026-W24-repaso-p6-006-v1`
**Bloom:** Apply
**Expected_Success:** 0.75
**ICFES:** Causa-efecto aplicado
**Contexto:** Repaso: identificar causa y efecto

### Enunciado

> **El reciclaje en el colegio**
>
> El colegio San José decidió implementar un programa de reciclaje. Pusieron canecas de diferentes colores y capacitaron a los estudiantes. Al principio, muchos estudiantes confundían los colores. Por eso, los profesores colocaron dibujos en cada caneca para mostrar qué tipo de residuo iba en cada una.

¿Cuál fue la CAUSA de que los profesores colocaran dibujos en las canecas?

### Opciones
- [ ] A) Que el colegio decidiera implementar el reciclaje <!-- feedback: Incorrecto. Esa fue la causa inicial de todo el programa, pero no la causa específica de poner dibujos. Primero pusieron canecas, luego vieron el problema. -->
- [ ] B) Que hubiera canecas de diferentes colores <!-- feedback: Incorrecto. Las canecas de colores existían desde el inicio. El problema fue que los estudiantes las confundían. -->
- [x] C) Que los estudiantes confundían los colores de las canecas <!-- feedback: ¡Correcto! El texto dice: "Al principio, muchos estudiantes confundían los colores. Por eso, los profesores colocaron dibujos". La confusión (causa) llevó a la solución de los dibujos (efecto). -->
- [ ] D) Que los profesores quisieran decorar las canecas <!-- feedback: Incorrecto. No hay evidencia de que la decoración fuera el motivo. La razón fue práctica: resolver la confusión de los estudiantes. -->

### Explicación Pedagógica

Para identificar la causa de una acción, busca qué problema la motivó. "Por eso", "por esta razón", "debido a" son conectores que señalan que lo que viene antes es la causa y lo que sigue es la consecuencia. Aquí, el problema (confusión) causó la solución (dibujos).

---

## Question 7 [D4]

**ID:** `CO-LEC-6-2026-W24-repaso-p6-007-v1`
**Bloom:** Apply
**Expected_Success:** 0.75
**ICFES:** Persuasión y audiencia
**Contexto:** Repaso: identificar a quién va dirigido un texto

### Enunciado

> **Carta a los padres de familia**
>
> Queridos padres: Los invitamos a la primera reunión del año escolar. Hablaremos sobre el rendimiento académico de sus hijos y las actividades extracurriculares. Su asistencia es muy importante para el desarrollo educativo de los niños. Los esperamos el viernes 15 a las 6:00 p. m. en el auditorio del colegio.

¿A qué audiencia va dirigido este texto persuasivo?

### Opciones
- [ ] A) A los estudiantes del colegio <!-- feedback: Incorrecto. La carta habla de "sus hijos" y "los niños" en tercera persona, lo que indica que los destinatarios no son los estudiantes sino sus padres. -->
- [x] B) A los padres y acudientes de los estudiantes <!-- feedback: ¡Correcto! La carta comienza con "Queridos padres" y menciona "el rendimiento académico de sus hijos". Está claramente dirigida a los padres de familia. -->
- [ ] C) A los profesores del colegio <!-- feedback: Incorrecto. Los profesores serían los organizadores de la reunión, no los destinatarios. La carta está escrita por el colegio para los padres. -->
- [ ] D) A la comunidad del barrio <!-- feedback: Incorrecto. La carta se dirige específicamente a los padres del colegio, no a toda la comunidad. El tema es escolar, no comunitario. -->

### Explicación Pedagógica

Identificar la audiencia es clave para entender un texto persuasivo. El lenguaje, el tono y el contenido se adaptan según a quién va dirigido. Aquí, el tratamiento formal ("Queridos padres"), el tema escolar y la mención de "sus hijos" indican claramente que la audiencia son los padres de familia.

---

## Question 8 [D5]

**ID:** `CO-LEC-6-2026-W24-repaso-p6-008-v1`
**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Argumentos: suficiencia y pertinencia
**Contexto:** Repaso: evaluar si un argumento es adecuado

### Enunciado

**Tesis:** Los estudiantes de sexto grado deberían aprender a programar computadores.

**Argumento 1:** La programación es una habilidad útil para el futuro.
**Argumento 2:** En otros países ya enseñan programación desde primaria.
**Argumento 3:** A Juan, un estudiante de sexto, no le gusta la programación.

¿Cuál de los argumentos NO es pertinente para apoyar la tesis?

### Opciones
- [ ] A) El Argumento 1 <!-- feedback: Incorrecto. "Útil para el futuro" es una razón válida. Apoya la tesis porque muestra un beneficio de aprender programación. -->
- [ ] B) El Argumento 2 <!-- feedback: Incorrecto. "En otros países ya lo hacen" es un argumento válido. Usa el ejemplo de otros lugares como evidencia de que es posible y beneficioso. -->
- [x] C) El Argumento 3 <!-- feedback: ¡Correcto! Que a Juan no le guste la programación no es un argumento en contra ni a favor de la tesis. Es una opinión personal de un estudiante, no una razón para decidir si todos deben aprenderla. -->
- [ ] D) Todos son pertinentes <!-- feedback: Incorrecto. El Argumento 3 no es pertinente. Una opinión individual no es un argumento válido para decidir una política educativa general. -->

### Explicación Pedagógica

Un argumento pertinente es aquel que tiene relación directa con la tesis y aporta información relevante para apoyarla o refutarla. Las opiniones personales de individuos ("a Juan no le gusta") no son argumentos sólidos para decisiones generales. Los buenos argumentos usan datos, ejemplos, causas o principios generales.

---

## Question 9 [D6]

**ID:** `CO-LEC-6-2026-W24-repaso-p6-009-v1`
**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Comparación de argumentos
**Contexto:** Repaso: contrastar dos textos argumentativos

### Enunciado

**Texto A:** "Los perros deben vivir en casas con patio, no en apartamentos. Necesitan espacio para correr y jugar. Un perro encerrado en un apartamento pequeño se estresa y puede volverse agresivo."

**Texto B:** "Los perros pueden vivir felices en apartamentos si sus dueños los sacan a pasear tres veces al día y les proporcionan juguetes y estimulación. Lo importante no es el espacio, sino el tiempo y la atención que reciben."

¿En qué se diferencian principalmente las tesis de estos dos textos?

### Opciones
- [ ] A) El Texto A habla de perros grandes y el Texto B de perros pequeños <!-- feedback: Incorrecto. Los textos no especifican el tamaño de los perros. Esa diferencia no está en los textos. -->
- [x] B) El Texto A sostiene que los perros necesitan patio, mientras que el Texto B sostiene que pueden vivir en apartamentos si reciben atención suficiente <!-- feedback: ¡Correcto! La tesis de A es "los perros deben vivir en casas con patio". La tesis de B es "pueden vivir en apartamentos si reciben atención". Son posturas opuestas sobre el mismo tema. -->
- [ ] C) El Texto A defiende a los perros y el Texto B defiende a los dueños <!-- feedback: Incorrecto. Ambos textos se centran en el bienestar del perro, no en los dueños. -->
- [ ] D) No se diferencian, ambos dicen lo mismo <!-- feedback: Incorrecto. Dicen lo opuesto. A dice que los perros necesitan patio, B dice que no es necesario si hay atención. Son tesis contrarias. -->

### Explicación Pedagógica

Comparar tesis de dos textos sobre el mismo tema ayuda a desarrollar pensamiento crítico. La tesis de A se basa en el espacio físico; la de B se basa en la atención y el cuidado. Reconocer posturas opuestas es el primer paso para formarse una opinión propia informada.

---

## Question 10 [D7]

**ID:** `CO-LEC-6-2026-W24-repaso-p6-010-v1`
**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Evaluación de la calidad del texto argumentativo
**Contexto:** Repaso: juzgar la efectividad de un argumento

### Enunciado

Un estudiante escribió:

> "No deberían vender comida chatarra en la tienda escolar. La comida chatarra es mala para la salud. Todos lo saben."

¿Qué debilidad tiene este argumento?

### Opciones
- [ ] A) La tesis no está clara <!-- feedback: Incorrecto. La tesis es clara: "No deberían vender comida chatarra". Está explícita desde la primera oración. -->
- [ ] B) El argumento es demasiado largo y confuso <!-- feedback: Incorrecto. El argumento es corto, no confuso. El problema no es la extensión. -->
- [x] C) El argumento es muy general, no ofrece evidencia concreta y apela a lo que "todos saben" sin demostrarlo <!-- feedback: ¡Correcto! "Es mala para la salud" es muy vago. No dice cómo afecta la salud, ni cita datos o ejemplos. "Todos lo saben" es una falacia de apelación a la mayoría. Un buen argumento necesita evidencia específica. -->
- [ ] D) La ortografía es incorrecta <!-- feedback: Incorrecto. El texto está bien escrito ortográficamente. El problema es de contenido y calidad argumentativa, no de forma. -->

### Explicación Pedagógica

Un argumento débil se caracteriza por: generalidades sin sustento ("es malo"), apelaciones a lo que "todos saben" o "todo el mundo dice" (falacia ad populum), y falta de evidencia concreta. Para mejorar este argumento, habría que agregar datos: "contiene altos niveles de azúcar que se asocian con obesidad infantil, y en Colombia el 24% de los niños tiene sobrepeso".
---
