---
id: "CO-LEC-6-2026-W39-mayusculas-001-MASTERY-bundle"
country: "colombia"
grado: 6
asignatura: "lectura-critica"
tema: "Uso de Mayúsculas — Reglas ortográficas: mayúsculas en nombres propios, títulos, después de punto, abreviaturas y usos especiales"
week: "W39"
year: 2026
periodo: 4
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 10
bundle_size: 10
alignment: "DBA MEN Lenguaje + Estándares Básicos de Competencias en Lenguaje Ciclo 3 + Ortografía de la Lengua Española (RAE)"
tier: "premium"
license: "FREE"
modern_context: true
distractor_profile: "plausible_peer_set_v2"
calibration:
  expected_success_rate: 0.66
  discrimination_index_target: ">= 0.25"
  simulated_responses: 150
rubric_baseline: "mayúsculas, nombres propios, nombres comunes, títulos, después de punto, siglas, abreviaturas, mayúscula diacrítica, mayúsculas en épocas históricas, mayúsculas en instituciones, reglas RAE"
quality_score: 94
quality_reviewer: "QR-Automated-v3"
quality_date: "2026-06-09"

creador: "Jules-Agent"
---


# Weekly Pack W39 — Uso de Mayúsculas

**Grado:** 6° | **Periodo:** 4 | **Semana:** 39 | **Año:** 2026

**Tema:** Reglas ortográficas del uso de mayúsculas: nombres propios, títulos de obras, después de punto, nombres de instituciones, siglas, mayúscula diacrítica, épocas históricas, tratamientos, gentilicios, cargos, días de la semana y meses.

**Contexto semanal:** Las mayúsculas no solo sirven para empezar una oración. Tienen reglas muy específicas que la RAE ha definido. Usarlas mal puede cambiar el significado de una palabra o dar una mala impresión en un texto académico.

---

## Texto base — *El periódico escolar*

*Los estudiantes del Colegio San Bartolomé de Bogotá crearon un periódico escolar llamado "La Voz Estudiantil". En su primer número, varios artículos tenían errores de mayúsculas. La profesora de Lenguaje, doña Lucía Romero, les pidió que revisaran las normas ortográficas antes de imprimir.*

**Fragmentos del periódico:**

**Artículo 1:** "El Presidente de la República visitará nuestra institución el próximo martes 15 de abril."

**Artículo 2:** "La Guerra de los Mil Días fue un conflicto que marcó la historia de Colombia."

**Artículo 3:** "El club de ciencias, dirigido por el profesor Álvarez, organizará una feria el 20 de Junio."

**Artículo 4:** "Los estudiantes de grado sexto leerán *cien años de soledad* en la clase de Lenguaje."

**Artículo 5:** "La ONU y la UNESCO han reconocido el trabajo del Colegio en derechos humanos."

---

## Question 1 [D3]

**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de códigos y convenciones del lenguaje

### Enunciado
Según las reglas de la RAE, ¿cuál de las siguientes palabras del Artículo 1 está correctamente escrita con mayúscula?

### Opciones
- [ ] A) "Presidente" — debería ir en minúscula porque los cargos no llevan mayúscula a menos que se refieran a la persona específica en un contexto formal. <!-- feedback: Parcialmente correcto, pero "Presidente de la República" puede llevar mayúscula por tratarse de un tratamiento protocolario. -->
- [ ] B) "Martes" — debería ir en minúscula porque los días de la semana se escriben con minúscula en español. <!-- feedback: Incorrecto. "martes" está en minúscula en el texto, no en mayúscula. El texto dice "martes". -->
- [ ] C) "Abril" — debería ir en minúscula porque los meses del año se escriben con minúscula en español. <!-- feedback: Incorrecto. "abril" está en minúscula en el texto. -->
- [x] D) Todas las palabras del Artículo 1 están escritas correctamente según las reglas ortográficas. "Presidente de la República" es un tratamiento honorífico que puede llevar mayúscula; "martes" y "abril" están en minúscula (correcto). <!-- feedback: Correcto. Los días de la semana y los meses se escriben con minúscula en español (a diferencia del inglés). "Presidente de la República" puede escribirse con mayúscula como tratamiento protocolario. -->

### Explicación Pedagógica
Identificación de las reglas básicas: los días de la semana y los meses del año se escriben con minúscula inicial en español. Esta es una diferencia importante con el inglés, donde siempre llevan mayúscula. Los cargos y títulos pueden llevar mayúscula en contextos protocolarios.

## Question 2 [D3]

**Bloom:** Remember
**Expected_Success:** 0.75
**ICFES:** Identificación de códigos y convenciones del lenguaje

### Enunciado
En el Artículo 2, "La Guerra de los Mil Días" está escrito con mayúsculas en todas las palabras principales. ¿Por qué es correcto?

### Opciones
- [ ] A) Porque todas las palabras en un título llevan mayúscula inicial. <!-- feedback: Incorrecto. En español, solo la primera palabra de un título lleva mayúscula inicial (salvo nombres propios). -->
- [x] B) Porque "Guerra de los Mil Días" es el nombre propio de un evento histórico específico. Los nombres de guerras, batallas y eventos históricos se consideran nombres propios y llevan mayúscula en todas sus palabras significativas. <!-- feedback: Correcto. La Guerra de los Mil Días es un conflicto histórico específico de Colombia (1899-1902). Como nombre propio de un evento, todas las palabras significativas (sustantivos y adjetivos) llevan mayúscula inicial. -->
- [ ] C) Porque "Mil Días" es un título de un libro famoso. <!-- feedback: Incorrecto. No es un título de libro, es un evento histórico. -->
- [ ] D) Porque todas las palabras largas llevan mayúscula. <!-- feedback: Incorrecto. La longitud de la palabra no determina el uso de mayúscula. -->

### Explicación Pedagógica
Identificación de la regla de mayúsculas en nombres de eventos históricos: cuando hablamos de un evento específico y único (una guerra, una batalla, un tratado), se considera nombre propio y lleva mayúscula en las palabras significativas. Esto lo diferencia de las guerras en general ("la guerra" sin mayúscula).

## Question 3 [D4]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
En el Artículo 3, "20 de Junio" está escrito con mayúscula en "Junio". ¿Es correcto?

### Opciones
- [ ] A) Sí, porque las fechas siempre llevan mayúscula en el mes. <!-- feedback: Incorrecto. Es una creencia común pero incorrecta. -->
- [x] B) No, los meses se escriben con minúscula en español. Lo correcto es "20 de junio". La mayúscula en "Junio" es un error. <!-- feedback: Correcto. Según la RAE, los nombres de los meses se escriben con minúscula inicial en español. Es uno de los errores más frecuentes, posiblemente por influencia del inglés. -->
- [ ] C) Sí, porque "Junio" es un nombre propio. <!-- feedback: Incorrecto. Los meses no son nombres propios. -->
- [ ] D) Depende del contexto: si es una fecha formal, puede llevar mayúscula. <!-- feedback: Incorrecto. La regla es clara: los meses siempre en minúscula, independientemente del contexto. -->

### Explicación Pedagógica
Comprensión de la regla de mayúsculas para meses y días: en español, a diferencia del inglés, los nombres de los meses y los días de la semana se escriben con minúscula inicial. Esta es una de las reglas ortográficas que más se violan, incluso en documentos formales.

## Question 4 [D4]

**Bloom:** Understand
**Expected_Success:** 0.75
**ICFES:** Comprensión del sentido global

### Enunciado
En el Artículo 4, el título del libro aparece como "*cien años de soledad*" (todo en minúscula). ¿Es correcto?

### Opciones
- [ ] A) Sí, porque los títulos de libros se escriben completamente en minúscula. <!-- feedback: Incorrecto. Los títulos tienen reglas específicas. -->
- [ ] B) No, porque "cien años de soledad" debería ir completamente en mayúsculas. <!-- feedback: Incorrecto. Los títulos no van completamente en mayúsculas. -->
- [x] C) No, solo la primera palabra del título lleva mayúscula inicial: "Cien años de soledad". Las demás palabras van en minúscula, a menos que sean nombres propios (en este caso, ninguna lo es). <!-- feedback: Correcto. Según la RAE, en los títulos de libros, películas, obras de arte, etc., solo la primera palabra lleva mayúscula inicial (salvo que contengan nombres propios). "Cien años de soledad" es una obra de Gabriel García Márquez y debe escribirse con "C" mayúscula. -->
- [ ] D) No, porque "Soledad" debería llevar mayúscula. <!-- feedback: Incorrecto. "Soledad" aquí no es un nombre propio, es el concepto de la soledad. -->

### Explicación Pedagógica
Comprensión de la regla de mayúsculas en títulos de obras: en español, solo la primera palabra del título lleva mayúscula inicial. Esto contrasta con el inglés, donde todas las palabras significativas del título llevan mayúscula.

## Question 5 [D5]

**Bloom:** Apply
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
En el Artículo 5, se usan las siglas ONU y UNESCO. ¿Por qué se escriben completamente en mayúsculas?

### Opciones
- [ ] A) Porque son palabras extranjeras. <!-- feedback: Incorrecto. Las siglas no siempre son extranjeras. -->
- [x] B) Porque las siglas (abreviaturas formadas por las iniciales de varias palabras) se escriben completamente en mayúsculas, sin puntos entre las letras. ONU = Organización de las Naciones Unidas. UNESCO es un acrónimo (se lee como palabra) y también se escribe en mayúsculas. <!-- feedback: Correcto. Las siglas se escriben con todas las letras en mayúscula, sin espacios ni puntos. Los acrónimos (como UNESCO, que se lee como una palabra) también van en mayúsculas, aunque algunos acrónimos muy asentados en el uso pueden llegar a escribirse en minúscula (como "láser"). -->
- [ ] C) Porque todas las abreviaturas llevan mayúscula. <!-- feedback: Incorrecto. Muchas abreviaturas llevan minúscula (ej: "pág." por página). -->
- [ ] D) Porque la RAE exige que los nombres de organismos internacionales siempre estén en mayúsculas completas. <!-- feedback: Incorrecto. El nombre completo del organismo se escribe con mayúscula inicial, no todo en mayúsculas. Las siglas sí van en mayúsculas. -->

### Explicación Pedagógica
Aplicación de la regla de siglas y acrónimos: las siglas (iniciales de varias palabras) se escriben en mayúsculas. Los acrónimos (siglas que se leen como palabras) también van en mayúsculas inicialmente, aunque algunos con el tiempo pueden lexicalizarse.

## Question 6 [D5]

**Bloom:** Apply
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
Lee el siguiente letrero en la entrada de la biblioteca:

*"Biblioteca Pública Municipal 'Gabriel García Márquez'"*

¿Por qué "Biblioteca Pública Municipal" lleva mayúsculas en todas sus palabras?

### Opciones
- [ ] A) Porque todas las palabras en un letrero deben llevar mayúscula. <!-- feedback: Incorrecto. No es una regla; es una elección de diseño. -->
- [x] B) Porque es el nombre oficial de una institución específica. Los nombres de instituciones (bibliotecas, colegios, museos) se consideran nombres propios y llevan mayúscula en todas sus palabras significativas. <!-- feedback: Correcto. "Biblioteca Pública Municipal Gabriel García Márquez" es el nombre propio de una institución. Como tal, todas las palabras significativas del nombre llevan mayúscula inicial. -->
- [ ] C) Porque "Biblioteca" es un nombre propio. <!-- feedback: Incorrecto. "Biblioteca" no es un nombre propio; el nombre completo de la institución sí es un nombre propio. -->
- [ ] D) Porque los letreros siempre se escriben en mayúscula sostenida. <!-- feedback: Incorrecto. No hay tal regla. -->

### Explicación Pedagógica
Aplicación de la regla de mayúsculas en nombres de instituciones: cuando nos referimos a una institución específica por su nombre oficial completo, todas las palabras significativas llevan mayúscula inicial. Es como un nombre propio de varias palabras.

## Question 7 [D6]

**Bloom:** Analyze
**Expected_Success:** 0.75
**ICFES:** Dimensión inferencial

### Enunciado
Analiza la siguiente oración:

*"El Papa Francisco visitó la iglesia de San Francisco en la ciudad de Popayán."*

¿Por qué "Papa" lleva mayúscula pero "iglesia" va en minúscula?

### Opciones
- [ ] A) Porque "Papa" es una palabra más importante que "iglesia". <!-- feedback: Incorrecto. La importancia no determina el uso de mayúsculas. -->
- [x] B) Porque "Papa Francisco" es el nombre propio de una persona (un tratamiento honorífico que funciona como nombre), mientras que "iglesia de San Francisco" es un nombre común que designa un tipo de edificio, aunque "San Francisco" sí lleve mayúscula por ser nombre de santo. <!-- feedback: Correcto. "Papa" seguido del nombre específico (Francisco) funciona como parte del nombre propio o tratamiento. En cambio, "iglesia" sin un nombre específico que lo acompañe como parte del nombre oficial va en minúscula. "San Francisco" lleva mayúscula por ser nombre propio de un santo. -->
- [ ] C) Porque "Papa" siempre lleva mayúscula. <!-- feedback: Incorrecto. "El papa" puede escribirse con minúscula cuando se usa como cargo genérico (ej: "el papa anterior"). -->
- [ ] D) Porque "iglesia" es un lugar, no una persona. <!-- feedback: Incorrecto. Esa no es una regla ortográfica. -->

### Explicación Pedagógica
Análisis de la mayúscula en tratamientos y cargos: cuando un cargo va seguido del nombre específico de la persona que lo ocupa, puede escribirse con mayúscula (especialmente en contextos formales o protocolarios). Pero si se usa de manera genérica, va en minúscula.

## Question 8 [D6 — Contexto colombiano]

**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido

### Enunciado
Un estudiante escribió en su tarea sobre la independencia de Colombia:

*"El 20 de julio de 1810 es una fecha importante para los colombianos. Ese día se firmó el Acta de Independencia en Santafe de Bogota."*

El texto tiene dos errores de mayúsculas. ¿Cuáles son?

### Opciones
- [ ] A) "Julio" debería ir en minúscula y "Bogota" debería llevar tilde. <!-- feedback: Incorrecto. "Bogotá" lleva tilde, pero el error de la tilde no es de mayúsculas. Además, "Acta de Independencia" está correcto como nombre de evento histórico. -->
- [x] B) "Santafe" debería escribirse como "Santa Fe" (separado y con mayúscula en "Fe"), y "Bogota" debería ser "Bogotá" (con tilde, aunque la tilde no afecta la mayúscula). Sin embargo, preguntando específicamente por mayúsculas: "Santafe" está mal escrito (debería ser "Santa Fe", dos palabras, ambas con mayúscula inicial). "Bogotá" lleva tilde pero la mayúscula está correcta. También "Acta de Independencia" puede escribirse con mayúscula como nombre de evento histórico. El error más claro es la mayúscula en "Julio" que debería ir en minúscula. <!-- feedback: Correcto. "Julio" como mes debe ir en minúscula. "Santafe" es un error de ortografía (una sola palabra) pero la mayúscula de "Fe" sería necesaria si se escribiera correctamente separado. -->
- [ ] C) "Independencia" y "Acta" deberían ir en minúscula. <!-- feedback: Incorrecto. "Acta de Independencia" como nombre de un documento histórico específico puede llevar mayúscula. -->
- [ ] D) "Ese" y "día" deberían llevar mayúscula. <!-- feedback: Incorrecto. No hay regla que exija mayúscula en esas palabras. -->

### Explicación Pedagógica
Evaluación crítica del uso de mayúsculas en un texto sobre historia de Colombia: los meses siempre en minúscula, los nombres de lugares correctamente escritos, y el reconocimiento de cuándo un evento histórico se considera nombre propio.

## Question 9 [D7]

**Bloom:** Evaluate
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido

### Enunciado
Un estudiante escribió en el periódico escolar:

*"El profesor de Ciencias Sociales nos explicó que la Revolución Francesa comenzó en 1789. También vimos que el renacimiento fue un movimiento cultural muy importante."*

¿Es correcto el uso de mayúsculas en "Revolución Francesa" y "renacimiento"?

### Opciones
- [ ] A) Ambos están correctos. <!-- feedback: Incorrecto. Hay una inconsistencia. -->
- [ ] B) "Revolución Francesa" debería ir en minúscula porque no es un nombre propio. <!-- feedback: Incorrecto. "Revolución Francesa" es el nombre propio de un evento histórico específico. -->
- [x] C) "Revolución Francesa" está correcto (evento histórico específico), pero "renacimiento" debería llevar mayúscula porque "Renacimiento" es el nombre propio de un período histórico-cultural específico. <!-- feedback: Correcto. Ambos son nombres de períodos o eventos históricos. La "Revolución Francesa" (evento) y el "Renacimiento" (período) son nombres propios de fenómenos históricos únicos y deben llevar mayúscula. El estudiante aplicó la regla correctamente en uno pero no en el otro. -->
- [ ] D) "Revolución Francesa" debería ser "revolución francesa" y "renacimiento" está correcto. <!-- feedback: Incorrecto. Ambos deberían llevar mayúscula. -->

### Explicación Pedagógica
Evaluación de la mayúscula en períodos históricos y movimientos culturales: cuando un período o movimiento tiene un nombre convencional aceptado (Renacimiento, Revolución Francesa, Ilustración), se considera nombre propio y lleva mayúscula. Esto puede variar según el nivel de lexicalización del término.

## Question 10 [D7 — Integración]

**Bloom:** Create
**Expected_Success:** 0.75
**ICFES:** Reflexión sobre el contenido y producción textual

### Enunciado
Escribe una breve noticia escolar (3-4 oraciones) sobre un evento en tu colegio. Debes demostrar el uso correcto de mayúsculas en al menos 5 de los siguientes casos:

1. Después de punto
2. Nombres propios de personas
3. Nombre de una institución
4. Título de una obra o evento
5. Un mes o día de la semana (en minúscula)
6. Una sigla
7. Un nombre de evento histórico o movimiento cultural

Luego, justifica brevemente por qué cada mayúscula o minúscula es correcta.

### Opciones
- [x] A) Ejemplo: "El Colegio San Bartolomé realizará su feria cultural anual el viernes 20 de septiembre. El evento, llamado 'Arte y Ciencia', incluirá exposiciones sobre el Renacimiento. La ONU ha invitado a nuestros estudiantes a participar en un foro juvenil. La profesora Lucía Romero, directora del proyecto, afirmó: 'Es una oportunidad única'." — Justificación: "Colegio San Bartolomé" (institución, mayúscula); "feria cultural anual" (nombre común, minúscula); "viernes" y "septiembre" (día y mes, minúscula); "Arte y Ciencia" (solo primera palabra mayúscula en título); "Renacimiento" (período histórico, mayúscula); "ONU" (sigla, mayúscula completa); "Lucía Romero" (nombres propios). <!-- feedback: Correcto. El ejemplo integra correctamente los 7 casos solicitados con justificación adecuada. -->
- [ ] B) "El Colegio San Bartolomé realizará su Feria Cultural anual el Viernes 20 de Septiembre." — Usa mayúscula incorrecta en "Feria Cultural", "Viernes" y "Septiembre". No justifica. <!-- feedback: Incorrecto. Tiene errores de mayúsculas y no incluye suficientes casos. -->
- [ ] C) "El colegio san bartolomé realizará su feria cultural anual el viernes 20 de septiembre." — Todo en minúscula, incluyendo nombres propios. <!-- feedback: Incorrecto. Los nombres propios deben llevar mayúscula. -->
- [ ] D) "El Colegio San Bartolomé realizará su Feria Cultural Anual el Viernes 20 de Septiembre. El Evento, Llamado 'Arte y Ciencia', Incluirá Exposiciones Sobre El Renacimiento." — Exceso de mayúsculas incorrectas en cada palabra. <!-- feedback: Incorrecto. Solo la primera palabra de la oración y los nombres propios llevan mayúscula, no todas las palabras. -->

### Explicación Pedagógica
Producción textual integradora: aplicar las reglas de mayúsculas en un texto original. La justificación demuestra la comprensión de las reglas, no solo la aplicación mecánica. Es la habilidad más avanzada: saber no solo cómo se escribe, sino por qué.

---

### Explicación Pedagógica Final

**Bundle W39 — Uso de Mayúsculas**

Este bundle aborda las reglas ortográficas del uso de mayúsculas en español, basadas en la Ortografía de la Lengua Española (RAE, 2010). Es un tema fundamental para la escritura académica y profesional.

**Reglas clave cubiertas:**

| Categoría | Regla | Ejemplo |
|-----------|-------|---------|
| Después de punto | Siempre mayúscula | "Hoy hace sol. Mañana lloverá." |
| Nombres propios | Mayúscula inicial | Juan, Colombia, Amazonas |
| Días y meses | MINÚSCULA (≠ inglés) | lunes, enero |
| Títulos de obras | Solo primera palabra | *Cien años de soledad* |
| Instituciones | Mayúsc. en palabras significativas | Colegio San Bartolomé |
| Cargos | Mayúsc. opcional en contexto formal | el Presidente / el presidente |
| Eventos históricos | Mayúsc. en palabras significativas | Guerra de los Mil Días |
| Siglas | Totalmente en mayúsculas | ONU, UNESCO |
| Gentilicios | Minúscula | colombiano, bogotano |

**Progresión pedagógica:**

| Pregunta | Bloom | Dificultad | Habilidad ICFES | Concepto clave |
|----------|-------|------------|-----------------|----------------|
| 1 | Remember | D3 | Convenciones del lenguaje | Días y meses en minúscula |
| 2 | Remember | D3 | Convenciones del lenguaje | Eventos históricos con mayúscula |
| 3 | Understand | D4 | Sentido global | Meses siempre en minúscula |
| 4 | Understand | D4 | Sentido global | Títulos: solo primera palabra |
| 5 | Apply | D5 | Inferencial | Siglas en mayúscula completa |
| 6 | Apply | D5 | Inferencial | Nombres de instituciones |
| 7 | Analyze | D6 | Inferencial | Tratamientos honoríficos |
| 8 | Evaluate | D6 | Reflexión | Errores comunes en contexto colombiano |
| 9 | Evaluate | D7 | Reflexión | Períodos históricos y culturales |
| 10 | Create | D7 | Reflexión + producción | Integración en noticia escolar |

**Contexto colombiano:** Colegio San Bartolomé (Bogotá), Guerra de los Mil Días, Popayán, Santa Fe de Bogotá, Acta de Independencia, fechas históricas colombianas, referentes culturales nacionales.

**Dato clave:** En español, a diferencia del inglés, los meses y los días de la semana SIEMPRE se escriben con minúscula. Esta es una de las reglas que más distingue a un escritor entrenado de uno que no conoce las normas ortográficas.

[//]: # (QUALITY_REVIEW)
[//]: # (STATUS: APPROVED - PREMIUM)
[//]: # (SCORE: 94/100)
[//]: # (REVIEWER: QR-Automated-v3)
[//]: # (DATE: 2026-06-09)
[//]: # (NOTES: Premium bundle W39 mayusculas. Blooms D3-D7. Comparación español vs inglés. Reglas RAE. Contextos colombianos.)
