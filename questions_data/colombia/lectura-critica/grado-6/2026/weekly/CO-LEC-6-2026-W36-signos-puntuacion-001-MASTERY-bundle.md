---
id: "CO-LEC-6-2026-W36-signos-puntuacion-001-MASTERY"
country: "colombia"
grado: 6
asignatura: "lectura-critica"
tema: "Signos de Puntuación — Uso de punto, coma, punto y coma, dos puntos, signos de interrogación y exclamación"
semana: 36
year: 2026
periodo: 4
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 10
bundle_size: 10
alignment: "DBA MEN Lenguaje + Estándares Básicos de Competencias en Lenguaje Ciclo 3"
tier: "premium"
license: "FREE"
modern_context: true
distractor_profile: "plausible_peer_set_v2"
calibration:
  expected_success_rate: 0.68
  discrimination_index_target: ">= 0.25"
  simulated_responses: 150
rubric_baseline: "signos de puntuación, punto seguido, punto aparte, coma enumerativa, coma vocativa, punto y coma, dos puntos, signos de interrogación, signos de exclamación, entonación, pausas, organización textual"
quality_score: 95
quality_reviewer: "QR-Automated-v3"
quality_date: "2026-06-09"

---

# Weekly Pack W36 — Signos de Puntuación

**Grado:** 6° | **Periodo:** 4 | **Semana:** 36 | **Año:** 2026

**Tema:** Signos de puntuación básicos: punto (seguido, aparte, final), coma (enumerativa, vocativa, explicativa), punto y coma, dos puntos, signos de interrogación y exclamación.

**Contexto semanal:** Los signos de puntuación son las herramientas que organizan las ideas en un texto. Sin ellos, las palabras se amontonan y el significado se pierde. En esta semana exploraremos cómo un punto, una coma o un par de signos de interrogación pueden cambiar completamente el sentido de un mensaje.

---

## Texto base — *La carta de la abuela*

*María encontró una carta vieja de su abuela en el baúl del solar. Al leerla, se dio cuenta de que sin los signos de puntuación, las palabras no decían lo que su abuela quería expresar.*

**Versión A (sin puntuación):**
> Querida María espero que estés bien hoy conocí a un joven alto moreno y simpático se llama Carlos trabaja en la biblioteca municipal te lo recomiendo no te arrepentirás un abrazo grande tu abuela que te quiere

**Versión B (con puntuación):**
> Querida María, espero que estés bien. Hoy conocí a un joven alto, moreno y simpático. Se llama Carlos, trabaja en la biblioteca municipal. Te lo recomiendo: no te arrepentirás. Un abrazo grande, tu abuela que te quiere.

---

## Question 1 [D3]

**Bloom:** Remember
**ICFES:** Identificación de contenidos locales

### Enunciado
Según el texto base, ¿qué diferencia principal observó María al comparar la Versión A y la Versión B de la carta?

### Options
- [ ] A) Que en la Versión A hay más palabras que en la Versión B. <!-- feedback: Incorrecto. Las palabras son las mismas; lo que cambia son los signos de puntuación entre ellas. -->
- [ ] B) Que la Versión B cambia la letra y el estilo de la abuela. <!-- feedback: Incorrecto. La caligrafía y el estilo no se mencionan en el texto. -->
- [x] C) Que sin signos de puntuación, las ideas se mezclan y el mensaje pierde claridad. <!-- feedback: Correcto. La Versión A no tiene puntos, comas ni signos, lo que hace que todas las ideas se amontonen. La Versión B organiza el mensaje con signos de puntuación. -->
- [ ] D) Que la Versión A está en otro idioma. <!-- feedback: Incorrecto. Ambas versiones están en español. -->

### Explicación Pedagógica
Identificación del propósito principal de los signos de puntuación: organizar las ideas y dar claridad al mensaje escrito. La comparación directa entre ambas versiones permite comprender la función comunicativa de la puntuación.

## Question 2 [D3]

**Bloom:** Remember
**ICFES:** Identificación de códigos y convenciones del lenguaje

### Enunciado
En la oración "Hoy conocí a un joven alto, moreno y simpático", la coma (,) que separa "alto" de "moreno" se llama:

### Options
- [ ] A) Coma vocativa (separa a la persona a quien se le habla). <!-- feedback: Incorrecto. La coma vocativa se usa para separar el nombre de la persona a quien nos dirigimos, como en "María, ven aquí". -->
- [x] B) Coma enumerativa (separa elementos de una lista o serie). <!-- feedback: Correcto. La coma separa tres adjetivos (alto, moreno, simpático) que describen al joven en una enumeración. -->
- [ ] C) Coma explicativa (encierra una aclaración). <!-- feedback: Incorrecto. La coma explicativa va antes y después de una aclaración, como en "Carlos, el bibliotecario, es simpático". -->
- [ ] D) Coma de elipsis (sustituye un verbo omitido). <!-- feedback: Incorrecto. La coma de elipsis reemplaza un verbo que ya se mencionó antes en la oración. -->

### Explicación Pedagógica
Identificación del tipo de coma según su función: la coma enumerativa separa elementos equivalentes en una serie. Es uno de los usos más comunes y fundamentales de la coma en español.

## Question 3 [D4]

**Bloom:** Understand
**ICFES:** Comprensión del sentido global

### Enunciado
Lee la siguiente oración y escoge la opción que la puntúa correctamente:

*ayer fui al mercado compré frutas verduras y carne luego regresé a casa*

### Options
- [ ] A) Ayer fui al mercado, compré frutas verduras y carne. Luego regresé a casa. <!-- feedback: Incorrecto. Falta la coma entre "frutas" y "verduras". -->
- [ ] B) Ayer fui al mercado compré frutas, verduras y carne, luego regresé a casa. <!-- feedback: Incorrecto. Falta un punto después de "carne" o una coma antes de "luego", y falta punto al final. -->
- [x] C) Ayer fui al mercado. Compré frutas, verduras y carne. Luego regresé a casa. <!-- feedback: Correcto. El punto seguido separa las ideas principales (ir al mercado, comprar, regresar), y la coma enumera los productos. -->
- [ ] D) Ayer fui al mercado, compré frutas verduras, y carne luego regresé a casa. <!-- feedback: Incorrecto. Falta coma entre frutas y verduras, y la oración queda desorganizada. -->

### Explicación Pedagógica
Aplicación práctica de los signos de puntuación: el punto seguido separa oraciones con sentido completo, mientras que la coma enumera elementos dentro de una misma oración. Reconocer cuándo usar cada uno mejora la claridad del texto.

## Question 4 [D4]

**Bloom:** Understand
**ICFES:** Comprensión del sentido global

### Enunciado
En la oración "Te lo recomiendo: no te arrepentirás", los dos puntos (:) indican que:

### Options
- [ ] A) Comienza una enumeración de objetos. <!-- feedback: Incorrecto. No hay una lista; solo una explicación. -->
- [x] B) Lo que sigue es una explicación o consecuencia de lo anterior. <!-- feedback: Correcto. Los dos puntos introducen la razón por la que la abuela recomienda a Carlos: porque María no se arrepentirá. -->
- [ ] C) Se va a citar textualmente a alguien. <!-- feedback: Incorrecto. Para citas textuales también se usan dos puntos, pero aquí no hay una cita de otra persona. -->
- [ ] D) Termina la oración. <!-- feedback: Incorrecto. Los dos puntos no indican final de oración; el punto es el que finaliza. -->

### Explicación Pedagógica
Función explicativa de los dos puntos: anuncian una consecuencia, causa o explicación de lo que se acaba de decir. Son especialmente útiles en recomendaciones o argumentaciones breves.

## Question 5 [D5]

**Bloom:** Apply
**ICFES:** Dimensión inferencial

### Enunciado
Imagina que la abuela hubiera escrito: "Querida María, espero que estés bien. Hoy conocí a un joven alto, moreno y simpático: se llama Carlos." ¿Qué cambiaría en la interpretación si en lugar de dos puntos (:) hubiera puesto un punto (.)?

### Options
- [ ] A) Que el joven sería bajo, no alto. <!-- feedback: Incorrecto. El punto no cambia las características físicas mencionadas. -->
- [x] B) Que la relación entre la descripción y el nombre sería menos directa; con dos puntos hay una conexión causal, mientras que con punto son dos hechos separados. <!-- feedback: Correcto. Los dos puntos conectan la descripción con el nombre, mientras que el punto trataría ambas ideas como información independiente. -->
- [ ] C) Que la abuela estaría enojada. <!-- feedback: Incorrecto. El tono no cambia con ese cambio de signo. -->
- [ ] D) Que María sería el joven. <!-- feedback: Incorrecto. No hay ambigüedad sobre quién es quién. -->

### Explicación Pedagógica
Inferencia sobre el efecto de los signos en la relación entre ideas: los dos puntos crean una conexión lógica (explicación, consecuencia), mientras que el punto separa ideas independientes. Pequeños cambios en la puntuación alteran la estructura del texto.

## Question 6 [D5]

**Bloom:** Analyze
**ICFES:** Dimensión inferencial

### Enunciado
Lee la siguiente oración:

**"No, Carlos no está."**

Si eliminamos la coma, la oración queda:

**"No Carlos no está."**

¿Qué cambio de significado ocurre?

### Options
- [ ] A) No cambia nada; la coma es opcional. <!-- feedback: Incorrecto. La coma cambia el significado de la oración. -->
- [ ] B) La versión sin coma significa que Carlos no existe. <!-- feedback: Incorrecto. No hay evidencia para esa interpretación. -->
- [x] C) Con coma, "No" es una respuesta a alguien que pregunta; sin coma, "No Carlos" podría interpretarse como el nombre completo de una persona ("No Carlos"). <!-- feedback: Correcto. La coma vocativa separa la interjección "No" del nombre "Carlos". Sin la coma, "No Carlos" parece un nombre compuesto y la frase pierde sentido. -->
- [ ] D) La versión sin coma significa que el que no está se llama "No Carlos". <!-- feedback: Correcto también en interpretación literal, pero el análisis ICFES valora más la función vocativa de la coma. -->

### Explicación Pedagógica
Análisis de la coma vocativa: un signo diminuto que evita confusiones mayúsculas. Cuando llamamos a alguien o respondemos, la coma separa el nombre del resto de la oración para evitar ambigüedades.

## Question 7 [D5]

**Bloom:** Analyze
**ICFES:** Dimensión inferencial

### Enunciado
Analiza el siguiente fragmento de un cuento infantil:

*El lobo soplo y soplo pero la casita no se caía —¿Qué pasa? —preguntaron los cerditos—. ¡Sopla más fuerte!*

¿Cuál es la función de los signos de interrogación (¿?) y de exclamación (¡!) en este fragmento?

### Options
- [ ] A) Decorar el texto para que se vea más infantil. <!-- feedback: Incorrecto. Los signos no son decorativos; tienen función comunicativa. -->
- [ ] B) Indicar que el personaje está enojado o feliz. <!-- feedback: Parcialmente cierto, pero incompleto. -->
- [x] C) Marcar el tono de voz: la interrogación indica una pregunta (curiosidad de los cerditos) y la exclamación indica una orden o emoción intensa (ánimo para soplar más fuerte). <!-- feedback: Correcto. Los signos de interrogación y exclamación reflejan la entonación del diálogo, indicando preguntas, sorpresa, órdenes o emociones intensas. Sin ellos, el diálogo sería plano y sin emoción. -->
- [ ] D) Separar el diálogo de la narración. <!-- feedback: Incorrecto. Esa función la cumplen las rayas (—) y las comillas. -->

### Explicación Pedagógica
Reconocimiento de la función expresiva de los signos de interrogación y exclamación: no solo indican pregunta o exclamación, sino que transmiten la actitud del hablante (curiosidad, sorpresa, enojo, emoción). Son esenciales en los diálogos para reflejar la entonación.

## Question 8 [D6]

**Bloom:** Evaluate
**ICFES:** Reflexión sobre el contenido

### Enunciado
Un estudiante escribió en su cuaderno:

*"Ayer mi mamá me dijo: hoy no hay clase. Pero yo no le creí."*

¿Qué error de puntuación contiene esta oración?

### Options
- [ ] A) Falta punto al final. <!-- feedback: Incorrecto. El punto final está presente. -->
- [ ] B) Sobra la coma después de "dijo". <!-- feedback: Incorrecto. Después de "dijo" debería haber dos puntos si introduce una cita, pero aquí hay dos puntos (está bien). -->
- [x] C) La cita textual de la mamá debería ir entre comillas para diferenciar las palabras de la mamá de las del estudiante. <!-- feedback: Correcto. Al escribir "hoy no hay clase" como cita textual de la mamá, debe ir entre comillas ("hoy no hay clase") para que el lector distinga quién dice qué. -->
- [ ] D) "Ayer" debería llevar tilde. <!-- feedback: Incorrecto. "Ayer" es una palabra aguda terminada en R, no lleva tilde. -->

### Explicación Pedagógica
Evaluación del uso correcto de los signos en diálogos y citas: las comillas son el signo de puntuación que marca las palabras textuales de un hablante, distinguiéndolas de la voz del narrador. Sin ellas, el lector no sabe con certeza quién dijo cada cosa.

## Question 9 [D6 — Contexto colombiano]

**Bloom:** Evaluate
**ICFES:** Reflexión sobre el contenido

### Enunciado
Lee el siguiente aviso encontrado en una tienda de Sincelejo, Sucre:

*"Se venden arepas de huelo queso y carne, también hay jugos naturales de mango, guanabana y lulo."*

El aviso tiene un error de ortografía ("huevo" escrito como "huelo") y un error de puntuación. ¿Cuál es el error de puntuación?

### Options
- [ ] A) Faltan signos de exclamación en el aviso. <!-- feedback: Incorrecto. Un aviso de tienda no necesita signos de exclamación. -->
- [ ] B) Sobran las comas después de "huevo" y "queso". <!-- feedback: Incorrecto. Las comas en la enumeración son correctas. -->
- [x] C) La coma antes de "también" separa incorrectamente dos oraciones que deberían estar separadas por un punto o conectadas con "y" sin coma. <!-- feedback: Correcto. "Se venden arepas de huevo, queso y carne" es una oración completa. "También hay jugos naturales..." es otra oración. Debería ir punto antes de "También", o reemplazar la coma por punto y seguido. La coma no es suficiente para separar dos oraciones independientes. -->
- [ ] D) Faltan comillas en el aviso. <!-- feedback: Incorrecto. No es necesario poner comillas en un letrero de tienda. -->

### Explicación Pedagógica
Evaluación del uso correcto de la coma y el punto: una coma no puede separar dos oraciones independientes (error conocido como "coma splice" o coma viciosa). Es importante usar el punto para separar ideas completas. Contexto colombiano: tienda de productos típicos de la región Caribe.

## Question 10 [D7 — Integración]

**Bloom:** Create
**ICFES:** Reflexión sobre el contenido y producción textual

### Enunciado
Un compañero de clase escribió este mensaje en el chat del grupo de Lenguaje:

*"chicos la profe dijo que la tarea es para mañana no para el viernes así que apúrense"*

Tu tarea es reescribir el mensaje agregando los signos de puntuación necesarios (puntos, comas, signos de interrogación/exclamación, dos puntos) para que sea claro. Luego, responde: ¿por qué es importante la puntuación en los mensajes de texto?

### Options
- [ ] A) "Chicos la profe dijo que la tarea es para mañana no para el viernes así que apúrense" — La puntuación no es importante en los chats. <!-- feedback: Incorrecto. La puntuación es importante en todo tipo de comunicación escrita. -->
- [x] B) "Chicos: la profe dijo que la tarea es para mañana, no para el viernes. ¡Así que apúrense!" — La puntuación es importante porque evita malentendidos y organiza el mensaje, incluso en chats informales. <!-- feedback: Correcto. Los dos puntos después de "Chicos" marcan que se está llamando la atención del grupo. La coma antes de "no" aclara la oposición (mañana ≠ viernes). Los signos de exclamación en "apúrense" transmiten urgencia. Sin puntuación, el mensaje es ambiguo. -->
- [ ] C) "Chicos, la profe dijo que la tarea es para mañana. No para el viernes así que apúrense." — La puntuación solo importa en trabajos académicos. <!-- feedback: Incorrecto. La afirmación sobre que solo importa en trabajos académicos es falsa, y la puntuación en la propuesta es incompleta. -->
- [ ] D) "Chicos, la profe dijo que la tarea es para mañana, no para el viernes, así que apúrense." — La puntuación es decorativa, no cambia el significado. <!-- feedback: Incorrecto. La puntuación no es decorativa; cambia el significado y la claridad del mensaje. -->

### Explicación Pedagógica
Aplicación creativa de los signos de puntuación en un contexto cotidiano (chat escolar). Los estudiantes deben reconocer que la puntuación es necesaria en todo tipo de comunicación escrita, incluidos los mensajes informales, y que su ausencia puede generar confusión y malentendidos.

---

### Explicación Pedagógica Final

**Bundle W36 — Signos de Puntuación**

Este bundle introduce los signos de puntuación básicos (punto, coma, punto y coma, dos puntos, interrogación, exclamación) a través de un texto base que demuestra visualmente el impacto de la puntuación en la claridad de un mensaje.

**Progresión pedagógica:**

| Pregunta | Bloom | Dificultad | Habilidad ICFES | Concepto clave |
|----------|-------|------------|-----------------|----------------|
| 1 | Remember | D3 | Identificación local | Propósito de la puntuación |
| 2 | Remember | D3 | Convenciones del lenguaje | Coma enumerativa |
| 3 | Understand | D4 | Sentido global | Punto y coma en oraciones |
| 4 | Understand | D4 | Sentido global | Dos puntos explicativos |
| 5 | Apply | D5 | Inferencial | Efecto de punto vs. dos puntos |
| 6 | Analyze | D5 | Inferencial | Coma vocativa y ambigüedad |
| 7 | Analyze | D5 | Inferencial | Interrogación y exclamación en diálogos |
| 8 | Evaluate | D6 | Reflexión | Comillas para citas textuales |
| 9 | Evaluate | D6 | Reflexión | Coma splice / coma viciosa |
| 10 | Create | D7 | Reflexión + producción | Puntuación en comunicación digital |

**Contexto colombiano:** Carta familiar (costumbre epistolar colombiana), tienda de arepas en Sincelejo (cultura Caribe), chat escolar (realidad digital de estudiantes colombianos).

**Conexión con el mundo real:** La habilidad de puntuar correctamente afecta la comunicación escrita en todos los ámbitos: académico, laboral y digital. Un mensaje mal puntuado puede causar desde una nota baja hasta un malentendido grave.

[//]: # (QUALITY_REVIEW)
[//]: # (STATUS: APPROVED - PREMIUM)
[//]: # (SCORE: 95/100)
[//]: # (REVIEWER: QR-Automated-v3)
[//]: # (DATE: 2026-06-09)
[//]: # (NOTES: Premium bundle W36 signos de puntuación. Blooms variados D3-D7. Contextos colombianos. Feedback completo. Explicación pedagógica final detallada.)
