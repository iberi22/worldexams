---
id: "PY-CIE-11-2026-W05-tema-w05-001-MASTERY-bundle"
country: "paraguay"
grado: 11
asignatura: "ciencias-naturales"
tema: "tema-w05"
periodo: "weekly"
week: "W05"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "MEC Paraguay"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# MASTERY Bundle - Ciencias Naturales: División celular: mitosis y meiosis (W05)

Este bundle cubre de manera exhaustiva el tema de División celular: mitosis y meiosis para el Grado 11 (3° Curso de la Educación Media) en Paraguay, alineado con los programas oficiales del MEC Paraguay.

---

## Question 1 [D3]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v1
**Bloom:** Remember
**EJE:** Genética y Reproducción
**Expected_Success:** 0.85
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Encarnación de Encarnación, Liz y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿En qué etapa de la mitosis las cromátidas hermanas se separan de manera simultánea y migran hacia polos opuestos?

### Opciones
- [x] A) Anafase <!-- feedback: ¡Correcto! Anafase responde perfectamente a la base teórica correspondiente. -->
- [ ] B) La dineína y la cinesina <!-- feedback: Incorrecto. La dineína y la cinesina no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) Las histonas nucleares H2A, H2B, H3 y H4 <!-- feedback: Incorrecto. Las histonas nucleares H2A, H2B, H3 y H4 es un concepto diferente de la unidad temática. -->
- [ ] D) Quiasma <!-- feedback: Incorrecto. Quiasma describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Anafase constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 2 [D3]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v2
**Bloom:** Understand
**EJE:** Genética y Reproducción
**Expected_Success:** 0.82
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional de Pilar de Pilar, Joaquín y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿En qué subfase del ciclo celular somático eucariota ocurre la duplicación del ADN cromosómico?

### Opciones
- [x] A) Fase S <!-- feedback: ¡Correcto! Fase S responde perfectamente a la base teórica correspondiente. -->
- [ ] B) El complejo promotor de la anafase (APC/C) <!-- feedback: Incorrecto. El complejo promotor de la anafase (APC/C) no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) La dineína y la cinesina <!-- feedback: Incorrecto. La dineína y la cinesina es un concepto diferente de la unidad temática. -->
- [ ] D) Telofase <!-- feedback: Incorrecto. Telofase describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Fase S constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 3 [D3]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v3
**Bloom:** Remember
**EJE:** Genética y Reproducción
**Expected_Success:** 0.79
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Encarnación de Encarnación, Sofía y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué estructura proteica en el centrómero de los cromosomas sirve como punto de anclaje para los microtúbulos del huso?

### Opciones
- [x] A) El cinetocoro <!-- feedback: ¡Correcto! El cinetocoro responde perfectamente a la base teórica correspondiente. -->
- [ ] B) La dineína y la cinesina <!-- feedback: Incorrecto. La dineína y la cinesina no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) La meiosis <!-- feedback: Incorrecto. La meiosis es un concepto diferente de la unidad temática. -->
- [ ] D) La fisión binaria <!-- feedback: Incorrecto. La fisión binaria describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de El cinetocoro constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 4 [D3]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v4
**Bloom:** Understand
**EJE:** Genética y Reproducción
**Expected_Success:** 0.76
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional del Alto Paraná de Ciudad del Este, Valentina y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué proceso de división nuclear mitótica mantiene rigurosamente constante la ploidía genómica en las células hijas?

### Opciones
- [x] A) La mitosis <!-- feedback: ¡Correcto! La mitosis responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Fase S <!-- feedback: Incorrecto. Fase S no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) Las neuronas maduras <!-- feedback: Incorrecto. Las neuronas maduras es un concepto diferente de la unidad temática. -->
- [ ] D) Zigoteno <!-- feedback: Incorrecto. Zigoteno describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de La mitosis constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 5 [D5]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v5
**Bloom:** Apply
**EJE:** Genética y Reproducción
**Expected_Success:** 0.73
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Encarnación de Encarnación, Jorge y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué proceso divisional nuclear especializado rinde cuatro células haploides a partir de una célula progenitora diploide?

### Opciones
- [x] A) La meiosis <!-- feedback: ¡Correcto! La meiosis responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Fase G1 <!-- feedback: Incorrecto. Fase G1 no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) La dineína y la cinesina <!-- feedback: Incorrecto. La dineína y la cinesina es un concepto diferente de la unidad temática. -->
- [ ] D) El cinetocoro <!-- feedback: Incorrecto. El cinetocoro describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de La meiosis constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 6 [D5]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v6
**Bloom:** Understand
**EJE:** Genética y Reproducción
**Expected_Success:** 0.70
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional de Luque de Luque, Benjamín y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Durante qué fase mitótica se vuelve a formar la envoltura nuclear alrededor de los cromosomas descondensados?

### Opciones
- [x] A) Telofase <!-- feedback: ¡Correcto! Telofase responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Fase G1 <!-- feedback: Incorrecto. Fase G1 no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) Zigoteno <!-- feedback: Incorrecto. Zigoteno es un concepto diferente de la unidad temática. -->
- [ ] D) La dineína y la cinesina <!-- feedback: Incorrecto. La dineína y la cinesina describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Telofase constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 7 [D5]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v7
**Bloom:** Apply
**EJE:** Genética y Reproducción
**Expected_Success:** 0.67
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional de Luque de Luque, Valentina y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué subfase de la profase I de la meiosis se caracteriza por el apareamiento y sinapsis de cromosomas homólogos?

### Opciones
- [x] A) Zigoteno <!-- feedback: ¡Correcto! Zigoteno responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Anafase <!-- feedback: Incorrecto. Anafase no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) La fisión binaria <!-- feedback: Incorrecto. La fisión binaria es un concepto diferente de la unidad temática. -->
- [ ] D) La dineína y la cinesina <!-- feedback: Incorrecto. La dineína y la cinesina describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Zigoteno constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 8 [D5]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v8
**Bloom:** Understand
**EJE:** Genética y Reproducción
**Expected_Success:** 0.64
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Dr. Pedro P. Peña de Coronel Oviedo, Sebastián y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Cómo se denomina la unión visible de cromátidas no hermanas donde se ha producido un entrecruzamiento de ADN?

### Opciones
- [x] A) Quiasma <!-- feedback: ¡Correcto! Quiasma responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Fase S <!-- feedback: Incorrecto. Fase S no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) La mitosis <!-- feedback: Incorrecto. La mitosis es un concepto diferente de la unidad temática. -->
- [ ] D) Metafase I <!-- feedback: Incorrecto. Metafase I describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Quiasma constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 9 [D5]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v9
**Bloom:** Apply
**EJE:** Genética y Reproducción
**Expected_Success:** 0.61
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional de la Capital de Asunción, Joaquín y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿En qué fase de la meiosis I los pares de cromosomas homólogos se alinean de manera aleatoria en el ecuador celular?

### Opciones
- [x] A) Metafase I <!-- feedback: ¡Correcto! Metafase I responde perfectamente a la base teórica correspondiente. -->
- [ ] B) La mitosis <!-- feedback: Incorrecto. La mitosis no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) El complejo promotor de la anafase (APC/C) <!-- feedback: Incorrecto. El complejo promotor de la anafase (APC/C) es un concepto diferente de la unidad temática. -->
- [ ] D) Las neuronas maduras <!-- feedback: Incorrecto. Las neuronas maduras describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Metafase I constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 10 [D5]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v10
**Bloom:** Understand
**EJE:** Genética y Reproducción
**Expected_Success:** 0.58
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Dr. Raúl Peña de Caacupé, Sofía y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué proteína motora interactúa con los microtúbulos cinetocóricos para mover los cromosomas durante la anafase?

### Opciones
- [x] A) La dineína y la cinesina <!-- feedback: ¡Correcto! La dineína y la cinesina responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Meiosis II <!-- feedback: Incorrecto. Meiosis II no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) Fase S <!-- feedback: Incorrecto. Fase S es un concepto diferente de la unidad temática. -->
- [ ] D) El fragmoplasto <!-- feedback: Incorrecto. El fragmoplasto describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de La dineína y la cinesina constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 11 [D7]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v11
**Bloom:** Analyze
**EJE:** Genética y Reproducción
**Expected_Success:** 0.55
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Dr. Raúl Peña de Caacupé, Liz y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué molécula del ciclo celular, regulada por ciclinas, controla el paso de la fase G2 a la fase M?

### Opciones
- [x] A) La quinasa dependiente de ciclina (CDK) <!-- feedback: ¡Correcto! La quinasa dependiente de ciclina (CDK) responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Zigoteno <!-- feedback: Incorrecto. Zigoteno no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) Meiosis II <!-- feedback: Incorrecto. Meiosis II es un concepto diferente de la unidad temática. -->
- [ ] D) La fisión binaria <!-- feedback: Incorrecto. La fisión binaria describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de La quinasa dependiente de ciclina (CDK) constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 12 [D7]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v12
**Bloom:** Apply
**EJE:** Genética y Reproducción
**Expected_Success:** 0.52
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Encarnación de Encarnación, Gladys y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿En qué etapa de la interfase la célula crece, sintetiza proteínas y acumula organelos sin duplicar su material genético?

### Opciones
- [x] A) Fase G1 <!-- feedback: ¡Correcto! Fase G1 responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Las neuronas maduras <!-- feedback: Incorrecto. Las neuronas maduras no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) Los microtúbulos polares <!-- feedback: Incorrecto. Los microtúbulos polares es un concepto diferente de la unidad temática. -->
- [ ] D) Metafase I <!-- feedback: Incorrecto. Metafase I describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Fase G1 constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 13 [D7]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v13
**Bloom:** Analyze
**EJE:** Genética y Reproducción
**Expected_Success:** 0.49
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Dr. Pedro P. Peña de Coronel Oviedo, Joaquín y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué tipo de células somáticas humanas entran en una fase de latencia permanente llamada G0 sin dividirse más?

### Opciones
- [x] A) Las neuronas maduras <!-- feedback: ¡Correcto! Las neuronas maduras responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Fase G1 <!-- feedback: Incorrecto. Fase G1 no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) Fase S <!-- feedback: Incorrecto. Fase S es un concepto diferente de la unidad temática. -->
- [ ] D) La fisión binaria <!-- feedback: Incorrecto. La fisión binaria describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Las neuronas maduras constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 14 [D7]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v14
**Bloom:** Apply
**EJE:** Genética y Reproducción
**Expected_Success:** 0.46
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional de la Capital de Asunción, Joaquín y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué estructura citoesquelética se encarga de realizar la citocinesis en las células de las raíces de las plantas?

### Opciones
- [x] A) El fragmoplasto <!-- feedback: ¡Correcto! El fragmoplasto responde perfectamente a la base teórica correspondiente. -->
- [ ] B) La quinasa dependiente de ciclina (CDK) <!-- feedback: Incorrecto. La quinasa dependiente de ciclina (CDK) no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) La dineína y la cinesina <!-- feedback: Incorrecto. La dineína y la cinesina es un concepto diferente de la unidad temática. -->
- [ ] D) La mitosis <!-- feedback: Incorrecto. La mitosis describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de El fragmoplasto constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 15 [D7]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v15
**Bloom:** Analyze
**EJE:** Genética y Reproducción
**Expected_Success:** 0.43
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional de Luque de Luque, Sofía y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué tipo de división celular bacteriana asexual duplica el ADN circular y divide la célula en dos de forma equitativa?

### Opciones
- [x] A) La fisión binaria <!-- feedback: ¡Correcto! La fisión binaria responde perfectamente a la base teórica correspondiente. -->
- [ ] B) El fragmoplasto <!-- feedback: Incorrecto. El fragmoplasto no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) La quinasa dependiente de ciclina (CDK) <!-- feedback: Incorrecto. La quinasa dependiente de ciclina (CDK) es un concepto diferente de la unidad temática. -->
- [ ] D) Anafase <!-- feedback: Incorrecto. Anafase describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de La fisión binaria constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 16 [D7]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v16
**Bloom:** Apply
**EJE:** Genética y Reproducción
**Expected_Success:** 0.40
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Ortiz Guerrero de Villarrica, Camila y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Durante qué división meiótica (I o II) se separan físicamente las cromátidas hermanas sin recombinación previa?

### Opciones
- [x] A) Meiosis II <!-- feedback: ¡Correcto! Meiosis II responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Zigoteno <!-- feedback: Incorrecto. Zigoteno no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) Las histonas nucleares H2A, H2B, H3 y H4 <!-- feedback: Incorrecto. Las histonas nucleares H2A, H2B, H3 y H4 es un concepto diferente de la unidad temática. -->
- [ ] D) Fase S <!-- feedback: Incorrecto. Fase S describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Meiosis II constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 17 [D9]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v17
**Bloom:** Evaluate
**EJE:** Genética y Reproducción
**Expected_Success:** 0.37
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Encarnación de Encarnación, Santiago y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué anomalía en la segregación cromosómica durante la anafase meiótica puede ocasionar una trisomía en el cigoto?

### Opciones
- [x] A) La no disyunción cromosómica <!-- feedback: ¡Correcto! La no disyunción cromosómica responde perfectamente a la base teórica correspondiente. -->
- [ ] B) La mitosis <!-- feedback: Incorrecto. La mitosis no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) El complejo promotor de la anafase (APC/C) <!-- feedback: Incorrecto. El complejo promotor de la anafase (APC/C) es un concepto diferente de la unidad temática. -->
- [ ] D) Fase S <!-- feedback: Incorrecto. Fase S describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de La no disyunción cromosómica constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 18 [D9]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v18
**Bloom:** Analyze
**EJE:** Genética y Reproducción
**Expected_Success:** 0.34
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional del Alto Paraná de Ciudad del Este, Jorge y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué filamento microtubular especial no se ancla a ningún cromosoma pero estabiliza el huso de un polo a otro?

### Opciones
- [x] A) Los microtúbulos polares <!-- feedback: ¡Correcto! Los microtúbulos polares responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Fase G1 <!-- feedback: Incorrecto. Fase G1 no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) La dineína y la cinesina <!-- feedback: Incorrecto. La dineína y la cinesina es un concepto diferente de la unidad temática. -->
- [ ] D) Anafase <!-- feedback: Incorrecto. Anafase describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Los microtúbulos polares constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 19 [D9]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v19
**Bloom:** Evaluate
**EJE:** Genética y Reproducción
**Expected_Success:** 0.31
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Dr. Pedro P. Peña de Coronel Oviedo, Liz y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué tipo de proteínas histónicas forman el núcleo del nucleosoma alrededor del cual se enrolla el ADN cromosómico?

### Opciones
- [x] A) Las histonas nucleares H2A, H2B, H3 y H4 <!-- feedback: ¡Correcto! Las histonas nucleares H2A, H2B, H3 y H4 responde perfectamente a la base teórica correspondiente. -->
- [ ] B) La meiosis <!-- feedback: Incorrecto. La meiosis no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) Fase G1 <!-- feedback: Incorrecto. Fase G1 es un concepto diferente de la unidad temática. -->
- [ ] D) El fragmoplasto <!-- feedback: Incorrecto. El fragmoplasto describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de Las histonas nucleares H2A, H2B, H3 y H4 constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.

---

## Question 20 [D9]
**ID:** PY-CIE-11-2026-W05-tema-w05-001-MASTERY-v20
**Bloom:** Analyze
**EJE:** Genética y Reproducción
**Expected_Success:** 0.28
**Contexto:** Durante una actividad de laboratorio en el Colegio Nacional Dr. Pedro P. Peña de Coronel Oviedo, Ramón y su grupo investigan sobre División celular: mitosis y meiosis.

### Enunciado
¿Qué complejo multiproteico degrada selectivamente las ciclinas promoviendo la salida de la mitosis celular?

### Opciones
- [x] A) El complejo promotor de la anafase (APC/C) <!-- feedback: ¡Correcto! El complejo promotor de la anafase (APC/C) responde perfectamente a la base teórica correspondiente. -->
- [ ] B) Quiasma <!-- feedback: Incorrecto. Quiasma no se relaciona directamente con esta pregunta o describe otra función. -->
- [ ] C) El fragmoplasto <!-- feedback: Incorrecto. El fragmoplasto es un concepto diferente de la unidad temática. -->
- [ ] D) Las histonas nucleares H2A, H2B, H3 y H4 <!-- feedback: Incorrecto. Las histonas nucleares H2A, H2B, H3 y H4 describe un proceso o componente distinto. -->

### Explicacion Pedagogica
El concepto de El complejo promotor de la anafase (APC/C) constituye un fundamento esencial para comprender los procesos analizados en la unidad de División celular: mitosis y meiosis.
