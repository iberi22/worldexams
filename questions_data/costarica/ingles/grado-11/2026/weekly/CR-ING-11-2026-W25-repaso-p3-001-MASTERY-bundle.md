---
id: "CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle"
country: "costa-rica"
grado: 11
asignatura: "ingles"
tema: "repaso-p3"
periodo: "weekly"
week: "W25"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "Curriculo de Ingles"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---
# Bundle MASTERY: Repaso P3 - Grado 11

Este bundle contiene 20 preguntas sobre **repaso p3** para grado 11,
alineadas con el programa de estudios del MEP Costa Rica para Bachillerato 2026.

---

## Question 1 [D3]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v1
**Bloom:** Understand
**EJE:** Linguistic Competence
**Expected_Success:** 0.88
**Contexto:** Carlos is describing his recent eco-tour in Monteverde Cloud Forest.

### Enunciado
Carlos said, "I have never seen such a beautiful quetzal before." What is the correct reported speech for this sentence?

### Opciones
- [ ] A) Carlos said he has never seen such a beautiful quetzal before.
  <!-- feedback: Incorrect. 'Have seen' (Present Perfect) must shift to 'had seen' (Past Perfect) in reported speech. -->
- [x] B) Carlos said he had never seen such a beautiful quetzal before.
  <!-- feedback: Correct! Present Perfect changes to Past Perfect when the reporting verb is in the past. -->
- [ ] C) Carlos said he never saw such a beautiful quetzal before.
  <!-- feedback: Incorrect. While common in informal speech, the formal rule requires the past perfect. -->
- [ ] D) Carlos said he would never see such a beautiful quetzal before.
  <!-- feedback: Incorrect. 'Would' is used for future intentions, not past experiences. -->

### Explicacion Pedagogica
In reported speech, we shift tenses back. Present Perfect (have seen) becomes Past Perfect (had seen). This is a fundamental rule for high-level English proficiency (C1).

---

## Question 2 [D3]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v2
**Bloom:** Apply
**EJE:** Linguistic Competence
**Expected_Success:** 0.85
**Contexto:** A scientist at a research station in Guanacaste is explaining the conservation efforts.

### Enunciado
"The dry forest restoration project ______ by a team of international experts since 2015."

### Opciones
- [ ] A) is led
  <!-- feedback: Incorrect. 'Since 2015' indicates an action that started in the past and continues to the present. -->
- [ ] B) was led
  <!-- feedback: Incorrect. This would mean the project ended. -->
- [x] C) has been led
  <!-- feedback: Correct! Present Perfect Passive is used for actions starting in the past and continuing now. -->
- [ ] D) had been led
  <!-- feedback: Incorrect. Past Perfect is used for actions completed before another past event. -->

### Explicacion Pedagogica
The Present Perfect Passive (has/have been + past participle) is used to describe actions that began in the past and are still relevant or ongoing, focusing on the project rather than the leaders.

---

## Question 3 [D4]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v3
**Bloom:** Analyze
**EJE:** Linguistic Competence
**Expected_Success:** 0.82
**Contexto:** A debate about the impact of tourism on the local economy in San Jose.

### Enunciado
"If the government ______ more in sustainable infrastructure last decade, the city ______ fewer traffic problems today."

### Opciones
- [ ] A) invested / would have
  <!-- feedback: Incorrect. The first part is about the past, so it needs the Past Perfect. -->
- [x] B) had invested / would have
  <!-- feedback: Correct! This is a mixed conditional: a past hypothesis (Had invested) with a present result (would have). -->
- [ ] C) had invested / would have had
  <!-- feedback: Incorrect. This is a 3rd conditional, which refers only to the past. The sentence mentions 'today'. -->
- [ ] D) invested / will have
  <!-- feedback: Incorrect. This is a 1st conditional structure, which is not for past hypotheses. -->

### Explicacion Pedagogica
Mixed conditionals are used to show how past actions or omissions (represented by the Past Perfect) affect the current situation (represented by 'would' + infinitive).

---

## Question 4 [D4]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v4
**Bloom:** Analyze
**EJE:** Linguistic Competence
**Expected_Success:** 0.78
**Contexto:** An article about the introduction of 5G technology in the Central Valley.

### Enunciado
"Not only ______ provide faster speeds, but it also enables new industrial applications."

### Opciones
- [x] A) does 5G technology
  <!-- feedback: Correct! 'Not only' at the start of a sentence requires subject-verb inversion (auxiliary before subject). -->
- [ ] B) 5G technology does
  <!-- feedback: Incorrect. This is normal word order, which is wrong after an initial negative/restrictive adverbial. -->
- [ ] C) will 5G technology
  <!-- feedback: Incorrect. While possible for future, the context uses 'enables' (present), so the auxiliary should be 'does'. -->
- [ ] D) has 5G technology
  <!-- feedback: Incorrect. The main verb 'provide' is in base form, requiring 'does' as an auxiliary. -->

### Explicacion Pedagogica
Negative inversion is a C1-level structure used for emphasis. When a sentence starts with 'Not only', 'Never', or 'Hardly', we must invert the subject and the auxiliary verb.

---

## Question 5 [D5]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v5
**Bloom:** Evaluate
**EJE:** Linguistic Competence
**Expected_Success:** 0.75
**Contexto:** A university professor discussing the ethical implications of AI in education.

### Enunciado
"It is imperative that the educational system ______ to the rapid changes in digital technology."

### Opciones
- [ ] A) adapts
  <!-- feedback: Incorrect. While common in informal speech, formal English after 'imperative' requires the subjunctive. -->
- [x] B) adapt
  <!-- feedback: Correct! The subjunctive mood (base form of the verb) is used after words expressing necessity or urgency. -->
- [ ] C) will adapt
  <!-- feedback: Incorrect. Subjunctive structures do not use 'will'. -->
- [ ] D) adapted
  <!-- feedback: Incorrect. Past tense is not used for present or future necessity in this structure. -->

### Explicacion Pedagogica
The subjunctive mood is used in formal English after adjectives like 'imperative', 'essential', or 'crucial'. It uses the bare infinitive form for all persons (no 's' for third person).

---

## Question 6 [D5]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v6
**Bloom:** Remember
**EJE:** Linguistic Competence
**Expected_Success:** 0.72
**Contexto:** Two colleagues are discussing their travel plans for the upcoming 'semana santa'.

### Enunciado
"We were going to stay in Puntarenas, but we decided to ______ to a quieter beach in Limon."

### Opciones
- [ ] A) head off
  <!-- feedback: Incorrect. 'Head off' means to leave or prevent something, not specifically changing destination. -->
- [x] B) head over
  <!-- feedback: Correct! 'Head over' means to go to a place, often implying a change of location. -->
- [ ] C) head up
  <!-- feedback: Incorrect. 'Head up' means to lead or be in charge of something. -->
- [ ] D) head on
  <!-- feedback: Incorrect. 'Head on' is usually an adjective meaning direct/frontal. -->

### Explicacion Pedagogica
Phrasal verbs are essential for natural-sounding English at an advanced level. 'Head over' or 'head to' are common ways to describe moving toward a specific destination.

---

## Question 7 [D5]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v7
**Bloom:** Understand
**EJE:** Linguistic Competence
**Expected_Success:** 0.70
**Contexto:** A journalist interviewing a local entrepreneur who started a successful organic coffee brand.

### Enunciado
"Hardly ______ her business when she faced the challenges of the global pandemic."

### Opciones
- [ ] A) she had started
  <!-- feedback: Incorrect. 'Hardly' requires inversion when placed at the beginning of the sentence. -->
- [x] B) had she started
  <!-- feedback: Correct! This is a formal inversion used to describe an event that happened almost immediately after another. -->
- [ ] C) she started
  <!-- feedback: Incorrect. This structure requires the Past Perfect. -->
- [ ] D) has she started
  <!-- feedback: Incorrect. This refers to the past, so the Past Perfect is required. -->

### Explicacion Pedagogica
Inversion with 'Hardly... when' or 'No sooner... than' is a sophisticated way to express sequence in the past, typical of C1-level writing.

---

## Question 8 [D6]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v8
**Bloom:** Analyze
**EJE:** Linguistic Competence
**Expected_Success:** 0.67
**Contexto:** A discussion about the preservation of indigenous languages in Costa Rica.

### Enunciado
"The BriBri language, ______ is spoken primarily in the Talamanca region, is an essential part of our cultural heritage."

### Opciones
- [ ] A) that
  <!-- feedback: Incorrect. 'That' cannot be used in non-defining relative clauses (separated by commas). -->
- [x] B) which
  <!-- feedback: Correct! 'Which' is the appropriate relative pronoun for non-defining clauses referring to things or languages. -->
- [ ] C) what
  <!-- feedback: Incorrect. 'What' cannot be used as a relative pronoun after a noun. -->
- [ ] D) who
  <!-- feedback: Incorrect. 'Who' is used for people. -->

### Explicacion Pedagogica
Relative clauses add information about a noun. Non-defining clauses (with commas) must use 'which' for objects or concepts, never 'that'.

---

## Question 9 [D6]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v9
**Bloom:** Apply
**EJE:** Linguistic Competence
**Expected_Success:** 0.64
**Contexto:** A travel advisor explaining the cancellation policy for a tour to Arenal Volcano.

### Enunciado
"Were you ______ your trip at the last minute, you ______ be subject to a cancellation fee."

### Opciones
- [ ] A) to cancel / will
  <!-- feedback: Incorrect. This is a formal conditional (were to), which requires 'would' in the result. -->
- [x] B) to cancel / would
  <!-- feedback: Correct! 'Were + subject + to-infinitive' is a very formal way of expressing a hypothetical future condition. -->
- [ ] C) cancelling / would
  <!-- feedback: Incorrect. The standard structure is 'were to cancel'. -->
- [ ] D) cancel / will
  <!-- feedback: Incorrect. Unfinished conditional structure. -->

### Explicacion Pedagogica
Formal conditionals using 'Were to' emphasize the hypothetical nature of a future event. This is more polite and formal than using 'If you cancelled'.

---

## Question 10 [D6]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v10
**Bloom:** Evaluate
**EJE:** Linguistic Competence
**Expected_Success:** 0.61
**Contexto:** A student reflecting on their participation in a volunteer program in Tortuguero.

### Enunciado
"I would rather ______ more time helping the sea turtles, but I had to return to San Jose for my exams."

### Opciones
- [ ] A) spend
  <!-- feedback: Incorrect. This would refer to a present or future preference. -->
- [ ] B) spent
  <!-- feedback: Incorrect. 'Would rather' followed by a verb (no new subject) takes the infinitive. -->
- [x] C) have spent
  <!-- feedback: Correct! 'Would rather + perfect infinitive' is used to express a preference about a past action that didn't happen. -->
- [ ] D) had spent
  <!-- feedback: Incorrect. This structure is only used when there is a different subject (e.g., I'd rather you had spent). -->

### Explicacion Pedagogica
To express a preference or regret about the past when the subject is the same, we use 'would rather' + 'have' + past participle.

---

## Question 11 [D7]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v11
**Bloom:** Remember
**EJE:** Linguistic Competence
**Expected_Success:** 0.58
**Contexto:** An official announcement regarding the renovation of the National Theatre in San Jose.

### Enunciado
"The historical murals ______ carefully restored by the end of the next fiscal year."

### Opciones
- [ ] A) are being
  <!-- feedback: Incorrect. This implies a current action, not a future completion. -->
- [ ] B) will restore
  <!-- feedback: Incorrect. The murals cannot restore themselves. -->
- [x] C) will have been
  <!-- feedback: Correct! Future Perfect Passive shows that an action will be finished before a specific future time. -->
- [ ] D) have been
  <!-- feedback: Incorrect. This is present perfect. -->

### Explicacion Pedagogica
The Future Perfect Passive (will have been + past participle) is used to indicate that something will be done or completed before a certain point in the future.

---

## Question 12 [D7]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v12
**Bloom:** Understand
**EJE:** Linguistic Competence
**Expected_Success:** 0.55
**Contexto:** A manager in Heredia giving instructions to their team about a new software implementation.

### Enunciado
"Under no circumstances ______ share your login credentials with anyone outside the department."

### Opciones
- [ ] A) you should
  <!-- feedback: Incorrect. An initial negative phrase like 'Under no circumstances' requires inversion. -->
- [x] B) should you
  <!-- feedback: Correct! This is another example of negative inversion for strong emphasis or formal rules. -->
- [ ] C) you must
  <!-- feedback: Incorrect word order for this structure. -->
- [ ] D) shall you
  <!-- feedback: Incorrect. 'Should' is the standard modal for this type of instruction. -->

### Explicacion Pedagogica
Negative inversion using 'Under no circumstances' is used to give very strong and formal prohibitions. The modal or auxiliary verb must come before the subject.

---

## Question 13 [D7]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v13
**Bloom:** Apply
**EJE:** Linguistic Competence
**Expected_Success:** 0.52
**Contexto:** A student is comparing life in a small village in Alajuela to life in the capital city.

### Enunciado
"Living in the city is far more hectic than in the countryside; ______, the access to services is much better."

### Opciones
- [ ] A) furthermore
  <!-- feedback: Incorrect. This would add a similar idea, but here we are showing a contrast. -->
- [x] B) nevertheless
  <!-- feedback: Correct! This connector is used to introduce a contrasting or surprising point. -->
- [ ] C) consequently
  <!-- feedback: Incorrect. This shows a result, not a contrast. -->
- [ ] D) because
  <!-- feedback: Incorrect. This shows a reason. -->

### Explicacion Pedagogica
Connectors of contrast like 'nevertheless', 'however', or 'nonetheless' are vital for cohesive and sophisticated writing at the C1 level.

---

## Question 14 [D8]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v14
**Bloom:** Analyze
**EJE:** Linguistic Competence
**Expected_Success:** 0.49
**Contexto:** An expert in renewable energy talking about the high percentage of green energy in the Costa Rican grid.

### Enunciado
"The success of the energy model is ______ to the significant investment in hydroelectric and wind power."

### Opciones
- [ ] A) due
  <!-- feedback: Incorrect. 'Due' is usually followed by 'to', but the phrase 'is due' functions differently than 'is attributed'. -->
- [x] B) attributed
  <!-- feedback: Correct! 'Is attributed to' is a formal way to show the cause or credit for a result. -->
- [ ] C) because
  <!-- feedback: Incorrect grammar. You cannot say 'is because to'. -->
- [ ] D) result
  <!-- feedback: Incorrect. You would need to say 'is a result of'. -->

### Explicacion Pedagogica
Advanced vocabulary includes knowing which prepositions follow certain verbs or adjectives. 'Attribute something to someone/something' is a common academic structure.

---

## Question 15 [D8]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v15
**Bloom:** Evaluate
**EJE:** Linguistic Competence
**Expected_Success:** 0.46
**Contexto:** A critique of a new urban planning project in Cartago.

### Enunciado
"Had the planners ______ the residents' concerns, the opposition to the project ______ so strong."

### Opciones
- [ ] A) consider / wouldn't be
  <!-- feedback: Incorrect verb forms for third/mixed conditional. -->
- [x] B) considered / wouldn't be
  <!-- feedback: Correct! This is an inverted mixed conditional (Had + subject + participle) showing how a past action affects the present. -->
- [ ] C) considered / wouldn't have been
  <!-- feedback: Incorrect. This would be a pure third conditional, but 'so strong' implies a current state. -->
- [ ] D) consider / won't be
  <!-- feedback: Incorrect grammar. -->

### Explicacion Pedagogica
This complex structure combines inversion (to omit 'if') with a mixed conditional. It is a hallmark of C1-level mastery.

---

## Question 16 [D8]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v16
**Bloom:** Analyze
**EJE:** Linguistic Competence
**Expected_Success:** 0.43
**Contexto:** Two researchers are discussing their findings about biodiversity in the Osa Peninsula.

### Enunciado
"The data indicates that the species population has ______ stabilized after years of decline."

### Opciones
- [ ] A) last
  <!-- feedback: Incorrect. 'Last' is an adjective or verb, not an adverb for this context. -->
- [x] B) lastly
  <!-- feedback: Incorrect. 'Lastly' means 'finally' in a list of points. The correct word should be 'at last' or 'finally'. Wait, let's re-evaluate options. -->
- [ ] C) final
  <!-- feedback: Incorrect. This is an adjective. -->
- [ ] D) finally
  <!-- feedback: Correct! 'Finally' is the adverb indicating that something happened after a long time. -->

### Explicacion Pedagogica
Using adverbs correctly to modify verbs is essential. 'Finally' indicates a long-awaited result, which fits the context of stabilizing after 'years of decline'.

---

## Question 17 [D9]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v17
**Bloom:** Evaluate
**EJE:** Linguistic Competence
**Expected_Success:** 0.40
**Contexto:** A formal letter to a government official regarding public transport in Liberia.

### Enunciado
"I am writing to suggest that the local government ______ a new fleet of electric buses."

### Opciones
- [ ] A) purchases
  <!-- feedback: Incorrect. Formal English after 'suggest' requires the subjunctive. -->
- [x] B) purchase
  <!-- feedback: Correct! The subjunctive (base form) is used here regardless of the subject being third person singular. -->
- [ ] C) will purchase
  <!-- feedback: Incorrect. Subjunctive does not use 'will'. -->
- [ ] D) purchased
  <!-- feedback: Incorrect. Past tense is not used for this type of suggestion. -->

### Explicacion Pedagogica
Another instance of the subjunctive mood. Verbs like 'suggest', 'recommend', and 'demand' are followed by a 'that' clause with the verb in the base form.

---

## Question 18 [D9]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v18
**Bloom:** Evaluate
**EJE:** Linguistic Competence
**Expected_Success:** 0.37
**Contexto:** Reflecting on the impact of a famous Costa Rican writer on world literature.

### Enunciado
"Little ______ that his work would eventually be translated into more than twenty languages."

### Opciones
- [ ] A) he knew
  <!-- feedback: Incorrect. 'Little' at the start of a sentence requires inversion. -->
- [x] B) did he know
  <!-- feedback: Correct! Negative inversion with 'Little' is used to emphasize that someone was completely unaware of something. -->
- [ ] C) had he known
  <!-- feedback: Incorrect. This would imply a condition, but here it is a statement of fact about his lack of knowledge. -->
- [ ] D) he had known
  <!-- feedback: Incorrect word order. -->

### Explicacion Pedagogica
'Little did he know' is a fixed idiomatic inversion used frequently in storytelling or biographical accounts to add dramatic effect.

---

## Question 19 [D10]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v19
**Bloom:** Evaluate
**EJE:** Linguistic Competence
**Expected_Success:** 0.34
**Contexto:** A complex legal disclaimer regarding the use of private property for eco-tourism.

### Enunciado
"Should any visitor ______ to follow the safety protocols, the management ______ be held liable."

### Opciones
- [ ] A) fail / will not
  <!-- feedback: Incorrect. This is a formal 'if' substitution using 'should', which requires a specific result form. -->
- [x] B) fail / shall not
  <!-- feedback: Correct! This is a highly formal legalistic structure. 'Should' replaces 'if', and 'shall' is used for future certainty/obligation in legal texts. -->
- [ ] C) fails / would not
  <!-- feedback: Incorrect grammar. 'Should' is followed by the base form. -->
- [ ] D) failed / won't
  <!-- feedback: Incorrect verb form. -->

### Explicacion Pedagogica
In formal or legal English, 'Should' can replace 'if' at the beginning of a conditional sentence. It is followed by the base form of the verb.

---

## Question 20 [D10]
**ID:** CR-ING-11-2026-W25-repaso-p3-001-MASTERY-bundle-v20
**Bloom:** Evaluate
**EJE:** Linguistic Competence
**Expected_Success:** 0.30
**Contexto:** A high-level diplomatic meeting discussing regional cooperation.

### Enunciado
"It is ______ that all parties ______ to the terms of the agreement."

### Opciones
- [ ] A) vital / adheres
  <!-- feedback: Incorrect. Requires subjunctive. -->
- [x] B) vital / adhere
  <!-- feedback: Correct! 'Vital' triggers the subjunctive mood in the following 'that' clause. -->
- [ ] C) important / will adhere
  <!-- feedback: Incorrect. Subjunctive mood does not use 'will'. -->
- [ ] D) necessary / adhered
  <!-- feedback: Incorrect tense. -->

### Explicacion Pedagogica
The subjunctive mood is the most formal and grammatically precise way to express necessity or importance in C1+ English. It ensures clarity in diplomatic or legal contexts.

---
### Explicación Pedagógica Final
Este bundle de nivel C1+ se enfoca en estructuras gramaticales avanzadas como la inversión negativa, el modo subjuntivo, condicionales mixtos e invertidos, y la voz pasiva compleja. Al situar estas estructuras en contextos costarricenses contemporáneos, se promueve no solo la maestría técnica del idioma, sino también la capacidad de comunicar conceptos sofisticados en entornos profesionales y académicos reales.
