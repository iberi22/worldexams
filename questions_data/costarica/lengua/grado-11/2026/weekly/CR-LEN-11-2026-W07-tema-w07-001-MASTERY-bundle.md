---
id: "CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle"
country: "costarica"
grado: 11
asignatura: "lengua"
tema: "tema-w07"
periodo: "weekly"
week: "W07"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "MEP Costa Rica Bachillerato 2026"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
bundle_index: 1
---

# Bundle MASTERY: Géneros Literarios: Lírica - Grado 11

Este bundle contiene 20 preguntas sobre **géneros literarios: lírica** para grado 11,
alineadas con el programa de estudios del MEP Costa Rica para Bachillerato 2026.

---
## Question 1 [D3]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v1
**Bloom:** Remember
**EJE:** Análisis literario
**Expected_Success:** 0.85
**Contexto:** Durante un recital de poesía en San José, se analizan los componentes del género lírico.

### Enunciado
¿Cuál de las siguientes características define de manera primordial al género lírico frente a los demás géneros literarios?

### Opciones
- [x] A) La expresión subjetiva de sentimientos, emociones y estados de ánimo de un yo lírico, generalmente estructurada en versos.
  <!-- feedback: Correcto. El género lírico se centra en la interioridad afectiva y expresiva del poeta. -->
- [ ] B) La representación física de acciones cómicas destinadas a representarse en un escenario del Teatro Nacional.
  <!-- feedback: Incorrecto. Esto caracteriza al género dramático o teatral. -->
- [ ] C) La narración cronológica de acontecimientos bélicos protagonizados por héroes históricos de Alajuela.
  <!-- feedback: Incorrecto. Esto define al género narrativo épico o novela histórica. -->
- [ ] D) La redacción de informes técnicos sobre el cultivo de café en Cartago.
  <!-- feedback: Incorrecto. Corresponde a la tipología expositiva de divulgación técnica. -->

### Explicacion Pedagogica
La lírica se define por la manifestación de la intimidad afectiva del sujeto creador, sirviéndose del verso, el ritmo y figuras de construcción.

---

## Question 2 [D3]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v2
**Bloom:** Understand
**EJE:** Análisis literario
**Expected_Success:** 0.82
**Contexto:** Sofía estudia un poema del célebre escritor costarricense Jorge Debravo, el poeta de los humildes y de la justicia social.

### Enunciado
¿Quién es el 'yo lírico' en un texto poético?

### Opciones
- [ ] A) La persona de carne y hueso que compró el libro en una librería de Cartago.
  <!-- feedback: Incorrecto. Este es el lector real externo, no una entidad interna del poema. -->
- [x] B) La voz o sujeto de enunciación ficticio creado por el autor que expresa sus sentimientos dentro del poema.
  <!-- feedback: Correcto. El yo lírico es la instancia enunciativa interna de la poesía, análoga al narrador en la narrativa. -->
- [ ] C) El funcionario del MEP encargado de certificar las calificaciones de Español.
  <!-- feedback: Incorrecto. Es un actor administrativo externo que no interviene en la voz artística del poema. -->
- [ ] D) El antagonista que busca destruir la felicidad del protagonista en la obra dramática.
  <!-- feedback: Incorrecto. Esto pertenece al género de la narrativa o del teatro, no de la lírica. -->

### Explicacion Pedagogica
El yo lírico es la máscara o voz literaria creada por el poeta para enunciar de forma subjetiva el universo emocional del poema.

---

## Question 3 [D4]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v3
**Bloom:** Understand
**EJE:** Análisis de textos
**Expected_Success:** 0.80
**Contexto:** Javier analiza la estructura rítmica de una estrofa poética escrita en versos de arte menor.

### Enunciado
¿Cómo se define formalmente la métrica en la lírica tradicional española y costarricense?

### Opciones
- [ ] A) La cantidad de páginas físicas impresas que tiene un poemario de San José.
  <!-- feedback: Incorrecto. La longitud del libro físico es una variable comercial, no una medida métrica del verso. -->
- [x] B) La medida silábica de los versos, determinada por la acentuación y la aplicación de licencias poéticas como la sinalefa.
  <!-- feedback: Correcto. La métrica estudia la estructura rítmica y la cantidad de sílabas métricas de los versos. -->
- [ ] C) La cantidad de personajes que mueren al final de un capítulo dramático en Cartago.
  <!-- feedback: Incorrecto. Esto pertenece a la estructura del argumento de la novela o tragedia teatral. -->
- [ ] D) Las reglas de ortografía de acentuación dictadas oficialmente por el MEP.
  <!-- feedback: Incorrecto. Aunque relacionadas, las reglas métricas poéticas son licencias estéticas del arte lírico clásicas. -->

### Explicacion Pedagogica
La métrica mide la duración silábica de los versos, modulando la sonoridad rítmica mediante la sinalefa, la diéresis, la sinéresis y la ley del acento final.

---

## Question 4 [D4]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v4
**Bloom:** Apply
**EJE:** Análisis literario
**Expected_Success:** 0.78
**Contexto:** María analiza una estrofa poética costarricense y nota que las vocales de las palabras finales coinciden a partir de la última vocal acentuada.

### Enunciado
¿Qué tipo de rima se presenta cuando únicamente coinciden los sonidos vocálicos al final de los versos?

### Opciones
- [x] A) Rima asonante.
  <!-- feedback: Correcto. En la rima asonante coinciden solamente las vocales a partir del último acento tónico del verso. -->
- [ ] B) Rima consonante.
  <!-- feedback: Incorrecto. En la rima consonante coinciden de forma exacta todos los sonidos (vocales y consonantes). -->
- [ ] C) Rima libre o verso blanco.
  <!-- feedback: Incorrecto. Los versos libres o blancos carecen de coincidencia de rima sonora al final. -->
- [ ] D) Rima pareada cruzada.
  <!-- feedback: Incorrecto. Describe la disposición u orden estructural de los versos, no la coincidencia fonética de sonidos. -->

### Explicacion Pedagogica
La rima asonante o imperfecta es aquella donde hay identidad fónica exclusivamente de carácter vocálico al final de los versos.

---

## Question 5 [D5]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v5
**Bloom:** Apply
**EJE:** Análisis literario
**Expected_Success:** 0.75
**Contexto:** Andrey desea componer un cuarteto tradicional utilizando rima consonante.

### Enunciado
¿Cuál de las siguientes opciones describe de forma exacta la distribución de la rima en un cuarteto clásico de arte mayor?

### Opciones
- [ ] A) El primer verso rima con el segundo, y el tercero rima con el cuarto (AABB).
  <!-- feedback: Incorrecto. Esta distribución describe un pareado doble o estrofa de rima gemela. -->
- [x] B) El primer verso rima con el cuarto, y el segundo rima con el tercero (ABBA).
  <!-- feedback: Correcto. Es la estructura de rima abrazada clásica del cuarteto endecasílabo. -->
- [ ] C) El primer verso rima con el tercero, y el segundo queda suelto (ABAC).
  <!-- feedback: Incorrecto. Es un esquema asimétrico ajeno al cuarteto consonante clásico de arte mayor. -->
- [ ] D) Todos los versos terminan con la misma palabra de forma tautológica (AAAA).
  <!-- feedback: Incorrecto. Esto constituye una repetición viciosa que carece del dinamismo musical del cuarteto. -->

### Explicacion Pedagogica
El cuarteto clásico es una estrofa de cuatro versos endecasílabos con rima consonante de esquema abrazado (ABBA).

---

## Question 6 [D5]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v6
**Bloom:** Apply
**EJE:** Análisis de textos
**Expected_Success:** 0.73
**Contexto:** Ana analiza el poema 'Nosotros los hombres' de Jorge Debravo, el cual proclama la solidaridad y la hermandad humana.

### Enunciado
¿Qué figura de construcción o sintáctica se presenta al repetir de forma deliberada una o varias palabras al inicio de versos consecutivos?

### Opciones
- [ ] A) Hipérbaton (alteración del orden de la oración).
  <!-- feedback: Incorrecto. El hipérbaton altera el orden lógico sintáctico tradicional de las oraciones. -->
- [x] B) Anáfora.
  <!-- feedback: Correcto. La anáfora es la repetición de palabras al inicio de versos sucesivos para dar ritmo y énfasis al poema. -->
- [ ] C) Aliteración.
  <!-- feedback: Incorrecto. Consiste en la repetición constante de un mismo sonido consonántico a lo largo del verso. -->
- [ ] D) Asíndeton.
  <!-- feedback: Incorrecto. Es la eliminación de las conjunciones copulativas coordinantes para acelerar el verso. -->

### Explicacion Pedagogica
La anáfora es un recurso de repetición estilística que genera sonoridad, paralelismo sintáctico y acentúa el mensaje emocional lírico.

---

## Question 7 [D5]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v7
**Bloom:** Apply
**EJE:** Análisis literario
**Expected_Success:** 0.70
**Contexto:** Para un proyecto en Alajuela, Javier compone un poema dedicado a la carreta típica costarricense.

### Enunciado
Si desea emplear la personificación o prosopopeya, ¿cuál de los siguientes versos representa mejor este recurso retórico?

### Opciones
- [ ] A) La carreta de madera avanza lentamente jalada por dos bueyes robustos.
  <!-- feedback: Incorrecto. Es una descripción literal y fáctica de una acción cotidiana del campo, sin personificación. -->
- [x] B) La carreta cansada lloraba sus penas de madera sobre el pedregoso camino de Cartago.
  <!-- feedback: Correcto. Atribuye cualidades humanas ('cansada', 'lloraba sus penas') a un objeto inanimado (la carreta). -->
- [ ] C) La rueda gira y gira como un círculo perfecto pintado de alegres colores.
  <!-- feedback: Incorrecto. Es un símil de comparación directa ('como un círculo perfecto'), no una personificación. -->
- [ ] D) Qué hermosa es la carreta típica costarricense declarada símbolo nacional por el MEP.
  <!-- feedback: Incorrecto. Expresa una opinión directa y fáctica sin recurrir a tropos de personificación. -->

### Explicacion Pedagogica
La personificación atribuye rasgos, sentimientos, voces o conductas eminentemente humanas a seres inanimados o animales de forma poética.

---

## Question 8 [D6]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v8
**Bloom:** Apply
**EJE:** Análisis de textos
**Expected_Success:** 0.68
**Contexto:** Un poema del Valle Central dice:
'En el silencio de la tarde gris,
solo se escuchaba el leve susurro de las hojas al viento.'

### Enunciado
¿Qué figura de construcción de carácter fonético se manifiesta en la repetición intencionada del sonido silbante /s/ para imitar el ruido del viento?

### Opciones
- [ ] A) Anáfora de palabras al inicio de cada estrofa.
  <!-- feedback: Incorrecto. Consiste en la repetición de vocablos al comienzo del verso, no de sonidos consonánticos aislados. -->
- [x] B) Aliteración.
  <!-- feedback: Correcto. La aliteración repite de forma deliberada un fonema para evocar acústicamente una sensación física o un sonido de la naturaleza. -->
- [ ] C) Hipérbole de exageración dramática costarricense.
  <!-- feedback: Incorrecto. La hipérbole deforma la realidad por exageración, no tiene un fin mimético sonoro fonético. -->
- [ ] D) Polisíndeton por acumulación excesiva de conjunciones.
  <!-- feedback: Incorrecto. Consiste en repetir conjunciones como 'y' para dar lentitud, no en reiterar sonidos de consonantes. -->

### Explicacion Pedagogica
La aliteración persigue la imitación fónica de sensaciones físicas de la naturaleza (como el viento o el agua) mediante la repetición de fonemas afines.

---

## Question 9 [D6]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v9
**Bloom:** Apply
**EJE:** Análisis literario
**Expected_Success:** 0.65
**Contexto:** María analiza la siguiente estrofa poética de un autor costarricense contemporáneo:
'Tus labios son pétalos de orquídea húmeda,
que guardan el rocío fresco de la mañana de Heredia.'

### Enunciado
¿Qué tropo poético fundamental se utiliza al identificar de forma directa un elemento real (labios) con uno imaginario (pétalos) sin nexos comparativos?

### Opciones
- [ ] A) Símil utilizando el nexo de comparación 'como'.
  <!-- feedback: Incorrecto. El símil requiere un nexo comparativo explícito (como, tal cual, parece). Aquí la identificación es directa. -->
- [x] B) Metáfora.
  <!-- feedback: Correcto. La metáfora identifica directamente un término real con uno imaginario basándose en una relación de semejanza. -->
- [ ] C) Antítesis por oposición de conceptos en la asamblea.
  <!-- feedback: Incorrecto. La antítesis contrapone ideas contrarias, no une dos elementos afines de forma directa. -->
- [ ] D) Sinécdoque de la parte por el todo del país.
  <!-- feedback: Incorrecto. Consiste en designar un objeto con el nombre de otro debido a una relación de inclusión física de la parte o el todo. -->

### Explicacion Pedagogica
La metáfora pura o impura asocia el plano real con la dimensión figurada de forma directa, enriqueciendo la carga expresiva del poema.

---

## Question 10 [D6]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v10
**Bloom:** Understand
**EJE:** Análisis de textos
**Expected_Success:** 0.62
**Contexto:** Durante un recital en Cartago, un estudiante de Español estudia el concepto de 'licencia poética'.

### Enunciado
¿Qué fenómeno métrico ocurre cuando se une la vocal final de una palabra con la vocal inicial de la siguiente para formar una sola sílaba métrica?

### Opciones
- [ ] A) Hiato poético de separación silábica.
  <!-- feedback: Incorrecto. El hiato es precisamente la ruptura voluntaria de la sinalefa, separando las vocales en sílabas distintas. -->
- [x] B) Sinalefa.
  <!-- feedback: Correcto. La sinalefa es la licencia métrica fundamental que fusiona acústica y silábicamente las vocales limítrofes de palabras adyacentes. -->
- [ ] C) Sinéresis de vocales internas en la palabra.
  <!-- feedback: Incorrecto. Consiste en unir dos vocales que forman hiato dentro de una misma palabra, no entre palabras distintas. -->
- [ ] D) Diéresis por colocación de puntos sobre la vocal.
  <!-- feedback: Incorrecto. Consiste en separar dos vocales que forman diptongo dentro de una palabra para sumar una sílaba métrica. -->

### Explicacion Pedagogica
La sinalefa es el recurso métrico natural del idioma español que permite fluidificar la sonoridad del verso unificando vocales fronterizas de vocablos contiguos.

---

## Question 11 [D7]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v11
**Bloom:** Analyze
**EJE:** Análisis literario
**Expected_Success:** 0.58
**Contexto:** Lea con atención los siguientes versos del poeta de Cartago Jorge Debravo:
'Soy un hombre sencillo que trabaja de sol a sol,
y que lleva en las manos la áspera caricia de la tierra.
No tengo más riquezas que este viento de las montañas
y esta sed infinita de justicia que me quema la sangre.'

### Enunciado
¿Qué actitud lírica y temple de ánimo se manifiestan de forma predominante en el fragmento anterior?

### Opciones
- [x] A) Una actitud de comunión íntima con el campesinado humilde y un temple de ánimo de ferviente compromiso ético y anhelo de justicia social.
  <!-- feedback: Correcto. El fragmento resalta la humildad del peón ('hombre sencillo', 'manos ásperas') y proclama un anhelo de justicia de tono apasionado y ético. -->
- [ ] B) Un desprecio altivo hacia las labores agrícolas costarricenses y la vida campestre de Cartago.
  <!-- feedback: Incorrecto. Exalta su condición de hombre sencillo conectado con la tierra con un profundo sentido de dignidad, sin desprecios. -->
- [ ] C) Una descripción puramente estadística de los salarios devengados en las montañas por cosechar café.
  <!-- feedback: Incorrecto. Mantiene un registro expresivo lírico de carácter existencial y social, ajeno a informes económicos matemáticos. -->
- [ ] D) La narración mítica de un viaje interplanetario del yo lírico por encima de las nubes del Volcán Irazú.
  <!-- feedback: Incorrecto. Aborda la realidad humana de la vida agraria y la sed de justicia social del campesino real del país. -->

### Explicacion Pedagogica
La obra de Jorge Debravo destaca por su lirismo de hondo compromiso humano, dignificando el trabajo rústico frente al sufrimiento y la injusticia social.

---

## Question 12 [D7]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v12
**Bloom:** Analyze
**EJE:** Análisis literario
**Expected_Success:** 0.55
**Contexto:** Considere los siguientes versos de una poesía costarricense contemporánea:
'El silencio es un pozo de agua amarga en la provincia,
donde caen las horas lentas de la tarde de Liberia.'

### Enunciado
¿Qué relación analógica y simbólica se establece en la metáfora principal de estos versos?

### Opciones
- [ ] A) Identifica el silencio de Liberia con el ciclo geológico de la lluvia de Guanacaste.
  <!-- feedback: Incorrecto. Es un tropo de carácter existencial íntimo y anímico, no un informe hidrológico geológico. -->
- [x] B) Asocia el silencio con un pozo hondo de agua amarga para simbolizar la pesadez existencial, la desolación y la amargura del fluir del tiempo en el pueblo.
  <!-- feedback: Correcto. El pozo y el agua amarga metaforizan el encierro emocional, la soledad y la tristeza del paso lento de las horas del personaje. -->
- [ ] C) Anuncia que el agua potable del pozo de Liberia está contaminada químicamente por pesticidas.
  <!-- feedback: Incorrecto. Se trata de un poema con fines estéticos y expresivos subjetivos, no un reporte de contaminación sanitaria. -->
- [ ] D) Propone al MEP la abolición del uso de pozos de agua en las escuelas públicas rurales.
  <!-- feedback: Incorrecto. Carece de vinculación con la administración educativa o infraestructura escolar de secundaria del país. -->

### Explicacion Pedagogica
La metáfora existencial proyecta estados de ánimo sobre los objetos físicos, traduciendo el vacío espiritual y la soledad de la tarde provincial en un pozo hondo de agua amarga.

---

## Question 13 [D7]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v13
**Bloom:** Analyze
**EJE:** Análisis de textos
**Expected_Success:** 0.52
**Contexto:** Lea atentamente la siguiente estrofa:
'Del monte en la ladera verde y umbría,
corren las aguas limpias con gran porfía.'

### Enunciado
¿Qué figura de construcción sintáctica se manifiesta de forma evidente al alterar el orden lógico gramatical tradicional de la primera oración?

### Opciones
- [ ] A) Anáfora de palabras al inicio del verso.
  <!-- feedback: Incorrecto. No hay repetición de palabras en versos sucesivos. -->
- [x] B) Hipérbaton.
  <!-- feedback: Correcto. El hipérbaton altera el orden lógico regular de la sintaxis (sujeto, verbo, complementos) con fines rítmicos o de rima poética. -->
- [ ] C) Asíndeton por omisión de nexos copulativos.
  <!-- feedback: Incorrecto. No omite conjunciones necesarias; altera la disposición de las palabras. -->
- [ ] D) Metáfora pura del monte de adobes de la provincia.
  <!-- feedback: Incorrecto. Es una alteración sintáctica de estructura formal de la oración, no un tropo de comparación semántica. -->

### Explicacion Pedagogica
El hipérbaton permite modificar la estructura natural de la frase en español para acomodar la métrica, la rima y acentuar la cadencia del poema.

---

## Question 14 [D8]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v14
**Bloom:** Analyze
**EJE:** Comprensión lectora
**Expected_Success:** 0.48
**Contexto:** Analice la estructura formal de los siguientes versos:
'¡Oh bosque sagrado de Guanacaste!
Tu verde follaje es techo del cansado peón,
y tu sombra fresca es caricia de la tarde.'

### Enunciado
¿Qué actitud lírica adopta el yo lírico en esta composición poética costarricense?

### Opciones
- [ ] A) Actitud enunciativa de relato objetivo e impersonal de hechos.
  <!-- feedback: Incorrecto. La actitud enunciativa describe la realidad de forma distante en tercera persona gramatical, ajena a la invocación directa. -->
- [x] B) Actitud apostrófica o apelativa, al dirigirse de forma directa en segunda persona gramatical a un receptor (el bosque sagrado).
  <!-- feedback: Correcto. El uso de la invocación ('¡Oh bosque...!', 'Tu verde...') es típico de la actitud apostrófica poética. -->
- [ ] C) Actitud carmínica o de la canción pura exenta de interlocutor.
  <!-- feedback: Incorrecto. Aunque expresa sentimientos, la presencia explícita de la interpelación en segunda persona define la actitud apostrófica. -->
- [ ] D) Actitud dramática teatral destinada a la comedia municipal.
  <!-- feedback: Incorrecto. Es una poesía lírica y no un libreto de teatro cómico para actores. -->

### Explicacion Pedagogica
La actitud apostrófica o de apelación se define por la interpelación directa del yo lírico a un 'tú' (humano, divino o de la naturaleza), imprimiendo un tono vehemente o de ruego al poema.

---

## Question 15 [D8]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v15
**Bloom:** Evaluate
**EJE:** Análisis de textos
**Expected_Success:** 0.45
**Contexto:** Un crítico costarricense analiza un soneto escrito por un autor alajuelense de secundaria. El soneto tiene versos de catorce sílabas (alejandrinos), pero la última estrofa rompe abruptamente la métrica con dos versos de cinco sílabas que riman de forma imperfecta con palabras vulgares coloquiales del Valle Central.

### Enunciado
Al evaluar la consistencia formal y el rigor estético de esta composición clásica, ¿cuál es el dictamen académico más fundamentado?

### Opciones
- [ ] A) Es una muestra magistral de vanguardismo absoluto que mejora sustancialmente la armonía tradicional del soneto clásico.
  <!-- feedback: Incorrecto. En las composiciones clásicas como el soneto, romper de forma caprichosa la métrica y el registro sin justificación deforma la armonía formal de la estrofa. -->
- [x] B) Representa una grave inconsistencia formal que destruye el ritmo, la musicalidad y la estructura estricta del soneto clásico, revelando falta de dominio de la métrica y del registro poético culto.
  <!-- feedback: Correcto. El soneto exige catorce versos endecasílabos de rima consonante; romper la simetría métrica con versos cortos incongruentes desbarata la estrofa. -->
- [ ] C) Demuestra que el estudiante utilizó oraciones impersonales con pasiva refleja de forma incorrecta.
  <!-- feedback: Incorrecto. La métrica poética y la rima son ajenas a las reglas de la pasiva refleja gramatical expositiva. -->
- [ ] D) Es perfectamente válido porque el MEP prohíbe el uso de versos endecasílabos en todos los colegios de secundaria del país.
  <!-- feedback: Incorrecto. El MEP no prohíbe ningún tipo de metro; al contrario, promueve el estudio formal de la métrica clásica española. -->

### Explicacion Pedagogica
El soneto es la estructura lírica cerrada clásica por excelencia que exige perfección métrica, rima consonante abrazada y alternancia silábica rigurosa para conservar su sonoridad.

---

## Question 16 [D8]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v16
**Bloom:** Evaluate
**EJE:** Análisis literario
**Expected_Success:** 0.42
**Contexto:** Considere los siguientes versos de un poema escolar que describe el Volcán Poás:
'El volcán es un dragón furioso que ruge de rabia,
escupiendo fuego de fuego y cenizas de cenizas de azufre.'

### Enunciado
Al evaluar la calidad expresiva de las metáforas y repeticiones de esta estrofa, ¿cuál es su principal debilidad estilística?

### Opciones
- [ ] A) El uso de la personificación referida a un accidente de relieve geográfico de Alajuela.
  <!-- feedback: Incorrecto. La personificación de volcanes es un recurso clásico valioso y no constituye una debilidad de estilo. -->
- [x] B) La redundancia léxica vacía e ineficaz en los versos ('fuego de fuego', 'cenizas de cenizas') que empobrece la musicalidad y la riqueza expresiva del poema.
  <!-- feedback: Correcto. Repetir palabras idénticas de forma sucesiva sin un propósito estético justificado cansa la audición e indica pobreza de vocabulario poético. -->
- [ ] C) La ausencia de datos matemáticos exactos sobre la temperatura de la laguna del cráter.
  <!-- feedback: Incorrecto. El lenguaje poético lírico no requiere de precisiones científicas de laboratorios químicos o termómetros. -->
- [ ] D) La mención del volcán de la provincia de Alajuela de forma directa.
  <!-- feedback: Incorrecto. Mencionar un lugar geográfico costarricense emblemático enriquece el arraigo cultural del poema. -->

### Explicacion Pedagogica
La lírica de calidad evita repeticiones viciosas redundantes, optando por la variedad de vocabulario, la cadencia rítmica y la precisión de sus tropos y metáforas.

---

## Question 17 [D9]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v17
**Bloom:** Evaluate
**EJE:** Análisis de textos
**Expected_Success:** 0.38
**Contexto:** Lea el siguiente fragmento de un poema dedicado al cultivo de caña de azúcar en San Carlos:
'El machete del peón corta el viento como un rayo de plata,
y la caña herida llora lágrimas de dulce savia en el suelo.'

### Enunciado
¿Qué valor expresivo aporta el uso de la personificación en el segundo verso de esta composición?

### Opciones
- [ ] A) Anuncia que las plantas de caña tienen sentimientos de dolor biológico que les impide crecer saludables.
  <!-- feedback: Incorrecto. Es un tropo estético-literario poético; las plantas carecen de sentimientos biológicos literales. -->
- [ ] B) Instruye detalladamente en la física de los cortes metálicos industriales de caña.
  <!-- feedback: Incorrecto. No provee datos técnicos de ingeniería agroindustrial; su fin es de creación artística. -->
- [x] C) Dota de sensibilidad dramática, belleza y dolor estético al trabajo de la zafra, conmoviendo al lector mediante la humanización de la naturaleza.
  <!-- feedback: Correcto. Humanizar la caña herida que llora savia dulcifica y sacraliza poéticamente el esfuerzo físico del peón en el agro. -->
- [ ] D) Exige la prohibición de cosechar caña manualmente en la provincia de Alajuela.
  <!-- feedback: Incorrecto. No tiene propósitos legislativos de reforma laboral o de prohibiciones agrícolas estatales. -->

### Explicacion Pedagogica
La personificación poética de la naturaleza herida dignifica y dota de dramatismo estético la cotidianidad del trabajo agrícola en la lírica nacional.

---

## Question 18 [D9]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v18
**Bloom:** Evaluate
**EJE:** Análisis literario
**Expected_Success:** 0.35
**Contexto:** Analice este fragmento de un poema contemporáneo escrito en San José:
'El reloj de la pared devora mis segundos de juventud con su tictac de hierro,
convirtiéndome en una hoja seca que el viento del tiempo arrastra hacia la fosa de adobes.'

### Enunciado
¿Qué visión existencial del tiempo se proyecta con fuerza poética en la metáfora principal del fragmento anterior?

### Opciones
- [ ] A) La celebración gozosa de la longevidad y el envejecimiento de la población costarricense.
  <!-- feedback: Incorrecto. El tono es de angustia y asfixia frente al paso destructor del tiempo, no de gozo o celebración. -->
- [x] B) La visión trágica y dolorosa del paso inexorable del tiempo (tempus fugit) que consume de forma implacable la vida humana y arrastra al sujeto hacia la muerte inevitable.
  <!-- feedback: Correcto. El reloj de hierro que devora segundos y la hoja seca arrastrada metaforizan de forma exacta el dolor de la fugacidad existencial humana. -->
- [ ] C) La exigencia de colocar relojes de adobes en todas las plazas públicas del país.
  <!-- feedback: Incorrecto. El reloj es un símbolo poético de la fugacidad del tiempo, no una propuesta de decoración urbana municipal. -->
- [ ] D) La descripción física objetiva de las oscilaciones pendulares de un reloj mecánico de pared de Cartago.
  <!-- feedback: Incorrecto. Es un análisis lírico subjetivo íntimo existencial de un estado de ánimo, no un manual técnico de relojería física. -->

### Explicacion Pedagogica
El tropo de la hoja seca y el tiempo destructor (cronófago) representa el tópico clásico de la fugacidad de la vida humana, traduciendo la angustia existencial del yo lírico frente a la finitud de la carne.

---

## Question 19 [D10]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v19
**Bloom:** Understand
**EJE:** Análisis de textos
**Expected_Success:** 0.32
**Contexto:** Un grupo de estudiantes de undécimo año repasa los componentes históricos de la lírica costarricense en su colegio de San José.

### Enunciado
¿Cuál es el nombre de la figura de construcción lírica que consiste en suprimir deliberadamente las conjunciones copulativas ordinarias (como 'y') entre términos sucesivos para dar dinamismo, rapidez y vértigo al verso?

### Opciones
- [ ] A) Polisíndeton.
  <!-- feedback: Incorrecto. El polisíndeton es lo opuesto: repite de forma excesiva las conjunciones copulativas ('y', 'e', 'ni') para dar lentitud y pesadez al poema. -->
- [x] B) Asíndeton.
  <!-- feedback: Correcto. El asíndeton elimina las conjunciones ('y', 'e', 'ni') sustituyéndolas por comas para agilizar la sonoridad rítmica del verso. -->
- [ ] C) Aliteración fonética de la provincia.
  <!-- feedback: Incorrecto. Reitera un sonido de letra aislada, no elimina conjunciones coordinantes sintácticas. -->
- [ ] D) Metáfora de adobes costumbrista.
  <!-- feedback: Incorrecto. Es una figura sintáctica de construcción de la frase, no un tropo de traslación de significado semántico. -->

### Explicacion Pedagogica
El asíndeton despoja al verso de ataduras sintácticas coordinantes, acelerando el fluir del poema mediante la sucesión vertiginosa de vocablos.

---

## Question 20 [D10]
**ID:** CR-LEN-11-2026-W07-tema-w07-001-MASTERY-bundle-v20
**Bloom:** Remember
**EJE:** Análisis literario
**Expected_Success:** 0.30
**Contexto:** Durante un foro sobre poesía clásica en el Liceo de Heredia, se define el concepto de 'estrofa'.

### Enunciado
¿En qué consiste una estrofa en la estructura formal del género lírico tradicional?

### Opciones
- [ ] A) En la cantidad de capítulos narrativos que tiene un libro impreso costarricense.
  <!-- feedback: Incorrecto. Corresponde a la división secuencial de la narrativa (novelas o crónicas), no de la poesía lírica. -->
- [x] B) En un conjunto de versos agrupados conforme a un esquema fijo de rima, métrica y ritmo en el poema.
  <!-- feedback: Correcto. Define con rigor formal a la estrofa clásica como unidad rítmica estructural del poema. -->
- [ ] C) En el diario íntimo de un personaje que se siente triste en San José.
  <!-- feedback: Incorrecto. Esto puede ser un componente temático de un diario de ficción, no la definición formal de una estrofa poética. -->
- [ ] D) En el catálogo de normas de disciplina vigentes en las aulas del colegio públicas.
  <!-- feedback: Incorrecto. Carece de sentido en el estudio formal de las estructuras de la teoría poética de la lírica. -->

### Explicacion Pedagogica
La estrofa organiza métrica y rítmicamente el fluir del poema, sirviendo como andamiaje armónico de la expresión lírica tradicional.
