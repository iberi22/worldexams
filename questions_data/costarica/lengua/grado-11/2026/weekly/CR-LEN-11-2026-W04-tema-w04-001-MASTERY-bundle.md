---
id: "CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle"
country: "costarica"
grado: 11
asignatura: "lengua"
tema: "tema-w04"
periodo: "weekly"
week: "W04"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "MEP Costa Rica Bachillerato 2026"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: Textos Descriptivos - Grado 11

Este bundle contiene 20 preguntas sobre **textos descriptivos** para grado 11,
alineadas con el programa de estudios del MEP Costa Rica para Bachillerato 2026.

---
## Question 1 [D3]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v1
**Bloom:** Remember
**ICFES:** Comprensión lectora
**Expected_Success:** 0.85
**Contexto:** Durante una práctica de redacción en Cartago, Sofía analiza las características del texto descriptivo.

### Enunciado
¿Cuál es el recurso lingüístico fundamental que caracteriza de forma primordial a la tipología descriptiva?

### Opciones
- [x] A) La abundancia de adjetivos calificativos y sintagmas preposicionales que detallan cualidades de un objeto.
  <!-- feedback: Correcto. La descripción se apoya fuertemente en sustantivos y adjetivos para detallar rasgos de personas, objetos o lugares. -->
- [ ] B) El empleo continuo de conectores conclusivos de causa y efecto en oraciones impersonales.
  <!-- feedback: Incorrecto. Esto caracteriza a la tipología argumentativa o expositiva. -->
- [ ] C) La acumulación de verbos de movimiento conjugados en pretérito perfecto simple de indicativo.
  <!-- feedback: Incorrecto. Esto acelera la acción del género narrativo, no la descripción pausada. -->
- [ ] D) La presencia constante de diálogos entre personajes de ficción costarricenses.
  <!-- feedback: Incorrecto. Corresponde al estilo dialógico de la narrativa o del drama. -->

### Explicacion Pedagogica
La descripción es eminentemente nominalista y se sustenta en la adjetivación para caracterizar cualidades físicas o psicológicas.

---

## Question 2 [D3]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v2
**Bloom:** Understand
**ICFES:** Análisis literario
**Expected_Success:** 0.82
**Contexto:** Javier lee una novela donde se describe minuciosamente el carácter moral, los valores y las virtudes del sabio don Carlos.

### Enunciado
¿Qué tipo de descripción literaria se realiza cuando se retratan únicamente los rasgos internos e intelectuales de un personaje?

### Opciones
- [ ] A) Prosopografía (descripción de rasgos físicos).
  <!-- feedback: Incorrecto. La prosopografía se enfoca de manera exclusiva en las características externas y fisonomía del personaje. -->
- [x] B) Etopeya (descripción de rasgos morales y de conducta).
  <!-- feedback: Correcto. La etopeya retrata la personalidad, la moral, las costumbres y el carácter de un individuo. -->
- [ ] C) Retrato (fusión balanceada de rasgos externos e internos).
  <!-- feedback: Incorrecto. El retrato amalgama de forma equilibrada rasgos físicos y conductuales. -->
- [ ] D) Topografía (descripción pormenorizada del relieve del terreno).
  <!-- feedback: Incorrecto. Describe las características espaciales de un paisaje, no personas. -->

### Explicacion Pedagogica
La etopeya es la figura descriptiva centrada en la caracterización psicológica, moral y conductual de un ser o personaje.

---

## Question 3 [D4]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v3
**Bloom:** Understand
**ICFES:** Comprensión lectora
**Expected_Success:** 0.80
**Contexto:** Andrey debe realizar una descripción técnica y objetiva sobre los componentes minerales del suelo del Volcán Irazú.

### Enunciado
¿Qué cualidad es indispensable para que una descripción cumpla adecuadamente un propósito de carácter científico y técnico?

### Opciones
- [ ] A) La libre expresión de las emociones de admiración estética que provoca el volcán.
  <!-- feedback: Incorrecto. La descripción técnica exige una neutralidad libre de emociones personales. -->
- [x] B) La objetividad rigurosa, la precisión léxica y el predominio de un tono denotativo e informativo libre de valoraciones.
  <!-- feedback: Correcto. Las descripciones técnicas se basan en datos empíricos objetivos explicados con exactitud de vocabulario. -->
- [ ] C) El uso constante de metáforas poéticas referidas a las cenizas como cabelleras grises.
  <!-- feedback: Incorrecto. Esto pertenece a la descripción literaria subjetiva, no a la científica técnica. -->
- [ ] D) La redacción del informe utilizando la variedad lingüística rural o voseo informal costarricense.
  <!-- feedback: Incorrecto. Se debe usar la variante formal estándar para asegurar el carácter académico científico. -->

### Explicacion Pedagogica
La descripción técnica persigue representar la realidad de forma fidedigna y verificable, haciendo uso de un lenguaje denotativo, preciso y científico.

---

## Question 4 [D4]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v4
**Bloom:** Apply
**ICFES:** Comprensión lectora
**Expected_Success:** 0.78
**Contexto:** María redacta una carta describiendo detalladamente la plaza central de la ciudad de Heredia tal y como era hace un siglo.

### Enunciado
¿Qué tipo de descripción literaria espacial se efectúa al caracterizar un paisaje o entorno físico inanimado?

### Opciones
- [x] A) Topografía.
  <!-- feedback: Correcto. La topografía es la descripción de un lugar, paisaje o espacio geográfico determinado. -->
- [ ] B) Cronografía.
  <!-- feedback: Incorrecto. La cronografía es la descripción de un tiempo o época histórica, no un lugar físico. -->
- [ ] C) Prosopografía.
  <!-- feedback: Incorrecto. Describe el físico de una persona, no un espacio geográfico. -->
- [ ] D) Etopeya.
  <!-- feedback: Incorrecto. Retrata el carácter de un individuo, no un paisaje o plaza. -->

### Explicacion Pedagogica
La topografía es el recurso que permite pintar con palabras la fisonomía, el relieve y la disposición de los objetos en un entorno geográfico o urbano.

---

## Question 5 [D5]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v5
**Bloom:** Apply
**ICFES:** Análisis literario
**Expected_Success:** 0.75
**Contexto:** En un cuento sobre San José de finales del siglo XIX, un fragmento describe las costumbres coloniales durante la época del cultivo del café.

### Enunciado
¿Qué figura descriptiva se emplea cuando el autor se esmera en caracterizar con palabras una época, un período histórico o un marco de tiempo?

### Opciones
- [ ] A) Topografía.
  <!-- feedback: Incorrecto. Describe un espacio geográfico físico o relieve, no el tiempo histórico. -->
- [ ] B) Etopeya.
  <!-- feedback: Incorrecto. Se enfoca en el carácter de una persona, no en el contexto temporal. -->
- [x] C) Cronografía.
  <!-- feedback: Correcto. La cronografía es la descripción de una época, un tiempo histórico, una estación del año o un momento del día. -->
- [ ] D) Prosopografía.
  <!-- feedback: Incorrecto. Detalla los rasgos físicos de seres humanos o animales. -->

### Explicacion Pedagogica
La cronografía es la descripción que contextualiza temporalmente una historia pintando con palabras las características singulares de un período o época histórica.

---

## Question 6 [D5]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v6
**Bloom:** Apply
**ICFES:** Sintaxis
**Expected_Success:** 0.73
**Contexto:** Ana redacta una prosopografía del héroe nacional Juan Santamaría para un acto cívico en Alajuela.

### Enunciado
¿Cuál de las siguientes oraciones representa adecuadamente esta figura descriptiva física?

### Opciones
- [ ] A) Juan Santamaría poseía un corazón noble lleno de valentía y un infinito amor por Costa Rica.
  <!-- feedback: Incorrecto. Retrata cualidades internas y morales (etopeya), no rasgos físicos externos. -->
- [x] B) El joven alajuelense era de tez morena, espaldas anchas, mirada firme y cabello rizado oscuro.
  <!-- feedback: Correcto. Describe fielmente las características físicas del héroe, lo que constituye una prosopografía. -->
- [ ] C) La Batalla de Rivas ocurrió el once de abril de mil ochocientos cincuenta y seis en Nicaragua.
  <!-- feedback: Incorrecto. Es un hecho fáctico histórico narrado, no una descripción física de una persona. -->
- [ ] D) Ojalá todos los jóvenes de undécimo año recuerden con orgullo el heroico sacrificio del tambor.
  <!-- feedback: Incorrecto. Es una expresión desiderativa y moral de opinión, no prosopográfica. -->

### Explicacion Pedagogica
La prosopografía se limita a la descripción física, corporal y externa de una persona o ser vivo.

---

## Question 7 [D5]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v7
**Bloom:** Apply
**ICFES:** Análisis literario
**Expected_Success:** 0.70
**Contexto:** Para un proyecto literario, Andrey escribe una caricatura literaria de un personaje codicioso que vive en Liberia.

### Enunciado
¿Qué recurso es característico en el diseño de una caricatura descriptiva literaria?

### Opciones
- [ ] A) La medición milimétrica exacta de la estatura real del personaje.
  <!-- feedback: Incorrecto. Esto es propio de un peritaje forense o médico, no de la literatura artística. -->
- [x] B) La deformación y exageración humorística o grotesca de los rasgos físicos y morales más notables de la persona.
  <!-- feedback: Correcto. La caricatura literaria distorsiona la realidad hiperbolizando los defectos para lograr un efecto cómico o crítico. -->
- [ ] C) La traducción exacta del texto descriptivo utilizando el latín clásico.
  <!-- feedback: Incorrecto. El uso del latín es ajeno a la naturaleza humorística de la caricatura costarricense moderna. -->
- [ ] D) La abolición total de todo tipo de tropos y figuras literarias en el cuento.
  <!-- feedback: Incorrecto. Al contrario, la caricatura literaria rebosa de figuras como la hipérbole y el símil. -->

### Explicacion Pedagogica
La caricatura literiva exagera satíricamente los rasgos físicos o éticos de un personaje, empleando comparaciones insólitas e hipérboles expresivas.

---

## Question 8 [D6]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v8
**Bloom:** Apply
**ICFES:** Comprensión lectora
**Expected_Success:** 0.68
**Contexto:** Javier elabora una guía turística describiendo los senderos del Parque Nacional Manuel Antonio.

### Enunciado
¿Cuál es el orden organizativo espacial más lógico y eficaz para presentar una descripción topográfica de un sendero forestal?

### Opciones
- [ ] A) Presentar las conclusiones económicas del parque y luego las del MEP.
  <!-- feedback: Incorrecto. Las variables de presupuestos escolares son ajenas a una guía de naturaleza para senderistas. -->
- [x] B) Organizar la descripción siguiendo una dirección espacial coherente (por ejemplo: de la entrada al mirador, o de abajo hacia arriba).
  <!-- feedback: Correcto. Seguir una secuencia espacial (izquierda a derecha, norte a sur, inicio a fin) facilita que el lector construya la imagen mental del trayecto. -->
- [ ] C) Enumerar los nombres de los científicos por orden de fecha de nacimiento.
  <!-- feedback: Incorrecto. Esto organiza datos de biografías, pero desorienta espacialmente sobre el sendero físico. -->
- [ ] D) Redactar el texto de atrás hacia adelante empezando por los signos de puntuación finales.
  <!-- feedback: Incorrecto. Destruye la coherencia elemental del idioma español. -->

### Explicacion Pedagogica
La topografía requiere una progresión espacial ordenada que asista a la imaginación visual del receptor, estructurando los elementos de forma secuencial.

---

## Question 9 [D6]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v9
**Bloom:** Apply
**ICFES:** Semántica
**Expected_Success:** 0.65
**Contexto:** María realiza una descripción literaria de un atardecer de verano en las playas de Guanacaste.

### Enunciado
¿Cuál de las siguientes oraciones utiliza términos que apelan de mejor manera a la sinestesia sensorial?

### Opciones
- [ ] A) El sol es una estrella amarilla gigante que calienta la arena de la playa diariamente.
  <!-- feedback: Incorrecto. Es una oración descriptiva llana de carácter astronómico básico, no una sinestesia sensorial. -->
- [x] B) El horizonte exhalaba un viento de aroma ambarino y el cielo se tiñó de una cálida melodía de colores.
  <!-- feedback: Correcto. Mezcla sensaciones de distintos sentidos de forma poética (aroma ambarino - olfato/vista, melodía de colores - oído/vista), logrando una sinestesia. -->
- [ ] C) El agua del mar posee gran cantidad de cloruro de sodio disuelto en cada litro.
  <!-- feedback: Incorrecto. Es un hecho químico fáctico puramente científico, no una sinestesia poética. -->
- [ ] D) La temperatura descendió tres grados Celsius conforme se ocultaba el sol en el oeste.
  <!-- feedback: Incorrecto. Es una medición física objetiva propia de un informe meteorológico. -->

### Explicacion Pedagogica
La sinestesia es un tropo literario de gran valor descriptivo que asocia sensaciones procedentes de diferentes dominios sensoriales (oído, vista, olfato, gusto, tacto) en una sola expresión artística.

---

## Question 10 [D6]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v10
**Bloom:** Understand
**ICFES:** Análisis literario
**Expected_Success:** 0.62
**Contexto:** Durante una mesa redonda de Español en el Liceo de San José, se define el concepto de 'autorretrato'.

### Enunciado
¿En qué consiste esta modalidad de descripción literaria?

### Opciones
- [ ] A) En la pintura al óleo que realiza un artista costarricense en su taller de San José.
  <!-- feedback: Incorrecto. Esto corresponde al ámbito de las artes plásticas, no al análisis textual literario. -->
- [x] B) En la descripción detallada de sí mismo que hace el propio autor, amalgamando rasgos físicos y de carácter.
  <!-- feedback: Correcto. El autorretrato es la descripción en primera persona de la propia fisonomía y personalidad (prosopografía y etopeya unidas). -->
- [ ] C) En una descripción científica objetiva de una cámara fotográfica digital costarricense.
  <!-- feedback: Incorrecto. Es la descripción de un objeto inanimado ajeno al sujeto mismo de la creación escritural. -->
- [ ] D) En el diario de viaje de un explorador que detalla la fauna silvestre de Puntarenas.
  <!-- feedback: Incorrecto. Eso es una crónica de viajes o una descripción biológica topográfica. -->

### Explicacion Pedagogica
El autorretrato literario unifica la prosopografía y la etopeya bajo una mirada introspectiva y descriptiva del propio escritor sobre sí mismo.

---

## Question 11 [D7]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v11
**Bloom:** Analyze
**ICFES:** Análisis literario
**Expected_Success:** 0.58
**Contexto:** Lea atentamente el siguiente fragmento descriptivo:
'Don Joaquín era un anciano de andar enérgico, de espaldas encorvadas por los años y de mirada viva que escudriñaba a su interlocutor tras unos anteojos de plata. Su voz, grave y serena, transmitía una paciencia infinita, propia de quien ha vivido todas las batallas del campo costarricense.'

### Enunciado
¿Cómo se clasifica de forma precisa la descripción literaria del fragmento anterior?

### Opciones
- [x] A) Se trata de un retrato literario, pues combina de manera equilibrada la descripción de rasgos físicos y la caracterización moral y psicológica del personaje.
  <!-- feedback: Correcto. Detalla rasgos físicos (andar, espaldas, mirada, voz) e internos (paciencia, serena, carácter), configurando un retrato. -->
- [ ] B) Es una etopeya pura porque excluye cualquier mención a la vestimenta, fisonomía o rasgos biológicos externos del personaje.
  <!-- feedback: Incorrecto. El texto abunda en rasgos físicos (espaldas, andar, mirada, ojos), por lo que no es una etopeya pura. -->
- [ ] C) Corresponde a una topografía detallada sobre las colinas pedregosas donde don Joaquín solía sembrar caña.
  <!-- feedback: Incorrecto. Se enfoca en un personaje humano, no describe paisajes o colinas. -->
- [ ] D) Es una caricatura burlesca que ridiculiza los rasgos físicos y la vejez del anciano de Cartago.
  <!-- feedback: Incorrecto. El fragmento trata al personaje con respeto y tono solemne, no hay parodia, burla o deformación grotesca. -->

### Explicacion Pedagogica
El retrato literario es una síntesis descriptiva integral que fusiona la apariencia exterior del individuo con su intimidad psicológica y espiritual.

---

## Question 12 [D7]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v12
**Bloom:** Analyze
**ICFES:** Comprensión lectora
**Expected_Success:** 0.55
**Contexto:** Analice el siguiente fragmento de una descripción del Volcán Arenal:
'La imponente mole cónica del volcán Arenal se alzaba majestuosa recortando su silueta perfecta sobre un cielo cargado de nubes plomizas. De su cúspide brotaba un lento hilo de vapor grisáceo, un recordatorio silencioso de la inmensa fuerza encerrada en sus entrañas de basalto.'

### Enunciado
¿Qué intención estética y expresiva predomina en la selección léxica del fragmento anterior?

### Opciones
- [ ] A) La intención de recopilar datos numéricos precisos sobre la altitud e inclinación de la mole volcánica.
  <!-- feedback: Incorrecto. No presenta datos numéricos, porcentajes o mediciones de altitud. -->
- [x] B) La intención de evocar el misterio, la imponencia y el poder telúrico del volcán mediante un lenguaje lírico sumamente evocador y descriptivo.
  <!-- feedback: Correcto. El uso de adjetivos evocadores ('imponente mole', 'majestuosa', 'silencio', 'entrañas de basalto') busca crear impacto emocional y admiración estética en el lector. -->
- [ ] C) La intención de instruir sobre las vías de evacuación en caso de una erupción en Alajuela.
  <!-- feedback: Incorrecto. No provee directrices de seguridad ni planes de emergencia escolar. -->
- [ ] D) La intención de descalificar los programas turísticos que visitan la provincia de Guanacaste.
  <!-- feedback: Incorrecto. El fragmento alaba la belleza geográfica de la región y no emite juicios sobre turismo o finanzas. -->

### Explicacion Pedagogica
La descripción literaria subjetiva se sirve de la adjetivación expresiva y de la personificación metafórica para impregnar la escena de valores sensoriales y afectivos.

---

## Question 13 [D7]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v13
**Bloom:** Analyze
**ICFES:** Pragmática
**Expected_Success:** 0.52
**Contexto:** En una novela costumbrista ambientada en Alajuela, se describe minuciosamente el proceso de elaboración del dulce de caña en un viejo trapiche de bueyes.

### Enunciado
¿De qué manera esta descripción procesal enriquece el realismo cultural de la obra?

### Opciones
- [ ] A) Anula los conflictos de los personajes reduciendo el libro a un manual de repostería del MEP.
  <!-- feedback: Incorrecto. El pasaje enriquece el marco cultural de fondo sin anular el desarrollo dramático de la trama. -->
- [x] B) Involucra al lector en las tradiciones productivas y la cotidianidad rural costarricense, aportando verosimilitud histórica y color local a la trama.
  <!-- feedback: Correcto. Detallar oficios y tecnologías tradicionales ayuda a preservar la identidad colectiva y dota de realismo a la ficción. -->
- [ ] C) Sustituye por completo los capítulos del nudo dramático para acelerar el desenlace de la historia.
  <!-- feedback: Incorrecto. La descripción de costumbres rurales ambienta, no suprime las acciones principales del relato. -->
- [ ] D) Instruye científicamente sobre la composición molecular de la sacarosa agroindustrial.
  <!-- feedback: Incorrecto. Tiene un propósito estético y folclórico, no es un informe químico de laboratorio. -->

### Explicacion Pedagogica
La descripción de costumbres y oficios tradicionales costarricenses consolida la verosimilitud del contexto, estrechando el vínculo de identidad con la obra.

---

## Question 14 [D8]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v14
**Bloom:** Analyze
**ICFES:** Comprensión lectora
**Expected_Success:** 0.48
**Contexto:** Lea con atención el siguiente pasaje descriptivo:
'El mercado central de San José era un laberinto bullicioso de aromas que se entrelazaban con el griterío de los vendedores. El perfume de los granos de café recién molidos chocaba con el olor húmedo del cilantro de coyote y la acidez de las frutas maduras apiladas en los tramos de madera.'

### Enunciado
¿Qué recurso perceptivo predomina en la construcción espacial de este fragmento urbano?

### Opciones
- [ ] A) La descripción de formas geométricas exactas y medidas métricas de los pasillos del mercado central.
  <!-- feedback: Incorrecto. No se mencionan dimensiones, distancias o metros lineales. -->
- [x] B) El uso de la percepción olfativa y auditiva para caracterizar el dinamismo, la vitalidad y la diversidad del ambiente comercial josefino.
  <!-- feedback: Correcto. Enfatiza olores (café, cilantro, frutas) y sonidos (griterío, bullicio) para que el lector reconstruya la atmósfera sensorial del mercado. -->
- [ ] C) La enumeración cronológica de los decretos municipales costarricenses de sanidad del mercado.
  <!-- feedback: Incorrecto. Es un fragmento literario descriptivo, no legal o normativo municipal. -->
- [ ] D) La descripción de las especies de aves silvestres que anidan en el techo del edificio patrimonial.
  <!-- feedback: Incorrecto. No se hace mención de aves o ecología de nidos en el fragmento. -->

### Explicacion Pedagogica
La construcción espacial literaria suele trascender lo puramente visual, incorporando impresiones olfativas y sonoras que sumergen sensitivamente al receptor en la escena.

---

## Question 15 [D8]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v15
**Bloom:** Evaluate
**ICFES:** Análisis crítico
**Expected_Success:** 0.45
**Contexto:** Un jurado costarricense evalúa la descripción física de un personaje de fantasía en una antología juvenil elaborada en Heredia. El pasaje dice:
'La criatura era sumamente gigantesca y poseía unos enormes brazos increíblemente grandes que terminaban en unas manos gigantes con unos dedos colosales.'

### Enunciado
Al evaluar la calidad estilística y retórica de esta descripción, ¿cuál es el principal defecto de redacción presente?

### Opciones
- [ ] A) La falta de mención del cantón específico de Heredia donde habita la criatura.
  <!-- feedback: Incorrecto. El lugar geográfico es opcional en la literatura fantástica y no constituye un defecto de redacción. -->
- [x] B) La redundancia léxica extrema e ineficaz (pleonasmo y repetición de sinónimos de tamaño: 'gigantesca', 'enormes', 'grandes', 'gigantes', 'colosales') que empobrece la riqueza del vocabulario descriptivo.
  <!-- feedback: Correcto. Repetir vocablos de significado idéntico de forma continua denota falta de recursos lingüísticos y cansa la lectura. -->
- [ ] C) Que la criatura fantástica no posee rasgos humanos tradicionales ni valores morales de Costa Rica.
  <!-- feedback: Incorrecto. Los seres fantásticos no requieren poseer rasgos humanos para estar bien construidos narrativamente. -->
- [ ] D) La ausencia de rimas consonantes al final de cada adjetivo calificativo de la frase.
  <!-- feedback: Incorrecto. La prosa descriptiva literaria no tiene la exigencia formal de rimar al final de las oraciones. -->

### Explicacion Pedagogica
La descripción de calidad evita la repetición viciosa de sinónimos absolutos de tamaño o cualidad, prefiriendo la precisión, los símiles originales y la variedad léxica.

---

## Question 16 [D8]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v16
**Bloom:** Evaluate
**ICFES:** Análisis crítico
**Expected_Success:** 0.42
**Contexto:** Lea la siguiente descripción de un hotel turístico ubicado en el Caribe costarricense:
'El hotel ofrece cabañas de madera rústica rodeadas de un exuberante jardín tropical habitado por monos congos y tucanes, brindando una experiencia inmersiva de total tranquilidad frente al mar.'

### Enunciado
¿Cómo se califica la coherencia pragmática de este texto de carácter descriptivo-publicitario?

### Opciones
- [ ] A) Incoherente porque los monos congos son animales que gritan muy fuerte y arruinan la tranquilidad prometida.
  <!-- feedback: Incorrecto. En el contexto de turismo ecológico, los congos y tucanes simbolizan naturaleza idílica y armonía, no molestia auditiva. -->
- [x] B) Altamente coherente, ya que selecciona estratégicamente rasgos exóticos agradables del entorno ecológico de Limón para construir una imagen de paraíso natural que atraiga al consumidor de forma persuasiva.
  <!-- feedback: Correcto. El texto publicitario destaca la flora y fauna amigables para crear una representación idealizada del destino vacacional. -->
- [ ] C) Poco sólida ya que debió redactarse utilizando fórmulas algebraicas y ecuaciones de balances hoteleros.
  <!-- feedback: Incorrecto. Un folleto descriptivo turístico no es un informe contable o financiero matemático. -->
- [ ] D) Inválida debido a que el MEP costarricense prohíbe la propaganda de hoteles comerciales en las aulas.
  <!-- feedback: Incorrecto. Se evalúa el texto descriptivo como tipología textual y herramienta de comunicación, lo cual es de gran valor curricular. -->

### Explicacion Pedagogica
La descripción de carácter publicitario ejerce una selección sesgada y laudatoria de los elementos del espacio, buscando generar un impacto de atracción o seducción comercial en el destinatario.

---

## Question 17 [D9]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v17
**Bloom:** Evaluate
**ICFES:** Análisis crítico
**Expected_Success:** 0.38
**Contexto:** En un artículo sobre la historia del arte costarricense, un ensayista escribe:
'El pintor herediano esculpió un paisaje invernal donde plasmó las copas nevadas de las montañas josefinas y el gélido viento que congelaba las aguas de la laguna.'

### Enunciado
Al analizar la veracidad espacial y cultural de esta descripción realista costarricense, ¿cuál es el principal anacronismo o incongruencia contextual presente?

### Opciones
- [ ] A) La mención de la provincia de San José como un territorio con montañas de relieve pronunciado.
  <!-- feedback: Incorrecto. San José es un territorio montañoso (Cerro de la Muerte, etc.), lo cual es geográficamente real. -->
- [ ] B) La falta de precisión técnica al no detallar el tipo de pintura al óleo utilizada por el artista de Heredia.
  <!-- feedback: Incorrecto. No se requiere detallar el tipo de pintura química exacta para comprender la escena descrita. -->
- [x] C) La representación de un invierno con paisajes de nieve y lagunas congeladas en el Valle Central de Costa Rica, lo cual es ajeno al clima tropical del país.
  <!-- feedback: Correcto. En Costa Rica, el 'invierno' equivale a la estación lluviosa de clima tropical, por lo que las montañas nevadas y lagunas congeladas son incongruentes con la geografía realista nacional. -->
- [ ] D) Sostener que un pintor herediano de Alajuela pueda realizar paisajes de la provincia de San José.
  <!-- feedback: Incorrecto. Un artista puede pintar cualquier paisaje de su país o del extranjero independientemente de su lugar de nacimiento. -->

### Explicacion Pedagogica
La literatura realista y costumbrista exige coherencia con las variables ecológicas y geográficas del territorio nacional que pretende retratar fidedignamente.

---

## Question 18 [D9]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v18
**Bloom:** Evaluate
**ICFES:** Análisis crítico
**Expected_Success:** 0.35
**Contexto:** Considere el siguiente fragmento descriptivo literario escrito por un estudiante de Alajuela:
'La gata de don José era tan ágil como una tortuga cansada y tan silenciosa como una comparsa del carnaval de Limón, lo que fascinaba a todos los vecinos del cantón.'

### Enunciado
¿Qué figura de pensamiento humorística predomina en esta curiosa descripción de la mascota?

### Opciones
- [ ] A) Símil de carácter solemne y exaltación de las cualidades biológicas felinas.
  <!-- feedback: Incorrecto. El símil compara a un felino ágil con una tortuga cansada y silenciosa con un carnaval ruidoso, lo cual es paradójico y cómico, no solemne. -->
- [x] B) La ironía humorística basada en la comparación incongruente de cualidades opuestas (agilidad-tortuga, silencio-comparsa de Limón) para provocar extrañeza y jocosidad.
  <!-- feedback: Correcto. El texto juega irónicamente comparando la agilidad y el silencio del gato con cosas caracterizadas precisamente por la lentitud y el ruido estridente. -->
- [ ] C) Una topografía detallada sobre las playas de la costa limonense durante los desfiles anuales.
  <!-- feedback: Incorrecto. El texto describe una mascota felina, no el relieve geográfico de la costa de Limón. -->
- [ ] D) Una cronografía exacta del desarrollo de los felinos costarricenses desde el siglo diecinueve.
  <!-- feedback: Incorrecto. No describe épocas históricas o tiempo, describe rasgos irónicos de un animal. -->

### Explicacion Pedagogica
La ironía descriptiva asocia rasgos contrapuestos de manera intencional, logrando una distorsión semántica lúdica y expresiva que descoloca las expectativas del lector.

---

## Question 19 [D10]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v19
**Bloom:** Understand
**ICFES:** Comprensión lectora
**Expected_Success:** 0.32
**Contexto:** Durante una asamblea de Español en Cartago, se analiza la función de los adjetivos especificativos frente a los adjetivos explicativos (epítetos) en los textos descriptivos.

### Enunciado
¿Cuál es la función semántica de un epíteto en la descripción literaria tradicional?

### Opciones
- [ ] A) Restringir la definición científica del sustantivo para evitar interpretaciones.
  <!-- feedback: Incorrecto. El epíteto no busca restricción científica, busca realzar estéticamente. -->
- [x] B) Destacar una cualidad intrínseca o natural del sustantivo con fines puramente estéticos y ornamentales (como 'blanca nieve' o 'manso buey').
  <!-- feedback: Correcto. El epíteto es un adjetivo explicativo que resalta características inherentes de forma estético-literaria. -->
- [ ] C) Sustituir el uso de verbos conjugados en la oración principal.
  <!-- feedback: Incorrecto. El adjetivo modifica al sustantivo, jamás reemplaza la función sintáctica del núcleo verbal. -->
- [ ] D) Señalar la procedencia de los personajes de la novela histórica costarricense.
  <!-- feedback: Incorrecto. Señalar la procedencia se realiza con adjetivos gentilicios, no con epítetos. -->

### Explicacion Pedagogica
El epíteto es un recurso estilístico clave de la descripción poética y clásica que enfatiza propiedades notorias o consustanciales al objeto sin delimitarlo de forma restrictiva.

---

## Question 20 [D10]
**ID:** CR-LEN-11-2026-W04-tema-w04-001-MASTERY-bundle-v20
**Bloom:** Remember
**ICFES:** Comprensión lectora
**Expected_Success:** 0.30
**Contexto:** Un grupo de estudiantes costarricenses estudia los diferentes enfoques de la tipología descriptiva para las Pruebas Nacionales del MEP.

### Enunciado
¿Cómo se denomina a la descripción literaria donde el sujeto describe un paisaje en movimiento, por ejemplo, contemplado desde la ventana del tren al Caribe?

### Opciones
- [ ] A) Topografía estática.
  <!-- feedback: Incorrecto. La topografía estática asume que el observador y el paisaje están inmóviles durante el proceso descriptivo. -->
- [x] B) Descripción cinematográfica o topografía dinámica.
  <!-- feedback: Correcto. Ocurre cuando el observador se desplaza, registrando la variación constante de las formas en el espacio geográfico conforme avanza. -->
- [ ] C) Prosopografía en relieve.
  <!-- feedback: Incorrecto. Esto se refiere a rasgos humanos físicos y no al desplazamiento espacial frente a un paisaje natural. -->
- [ ] D) Cronografía de secuencia.
  <!-- feedback: Incorrecto. Es la descripción del transcurrir temporal o de épocas, no del movimiento geográfico observable. -->

### Explicacion Pedagogica
La topografía dinámica o descripción cinematográfica capta la realidad espacial en movimiento, proyectando una secuencia fluida de sensaciones visuales cambiantes.
