---
id: "CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle"
country: "costarica"
grado: 11
asignatura: "lengua"
tema: "tema-w02"
periodo: "weekly"
week: "W02"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "MEP Costa Rica Bachillerato 2026"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
---

# Bundle MASTERY: Textos Expositivos - Grado 11

Este bundle contiene 20 preguntas sobre **textos expositivos** para grado 11,
alineadas con el programa de estudios del MEP Costa Rica para Bachillerato 2026.

---
## Question 1 [D3]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v1
**Bloom:** Remember
**EJE:** Comprensión lectora
**Expected_Success:** 0.85
**Contexto:** En un folleto informativo del Ministerio de Salud en San José se explican las medidas de prevención contra el dengue.

### Enunciado
¿Cuál es el propósito primordial de un texto expositivo?

### Opciones
- [x] A) Informar, explicar y transmitir datos objetivos y conceptos claros sobre un tema determinado de manera ordenada.
  <!-- feedback: Correcto. El texto expositivo busca informar y hacer comprender un tema de forma rigurosa. -->
- [ ] B) Persuadir al lector sobre una ideología política específica mediante argumentos afectivos.
  <!-- feedback: Incorrecto. Esto caracteriza a los textos argumentativos. -->
- [ ] C) Relatar hazañas de personajes costarricenses históricos empleando un lenguaje lírico.
  <!-- feedback: Incorrecto. Eso es propio de textos narrativos o poéticos. -->
- [ ] D) Instruir paso a paso en el ensamblaje de un dispositivo tecnológico nacional.
  <!-- feedback: Incorrecto. Eso corresponde a un texto de tipo instructivo. -->

### Explicacion Pedagogica
La función principal de la tipología expositiva es la transmisión objetiva de información y conocimientos detallados sobre un asunto específico.

---

## Question 2 [D3]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v2
**Bloom:** Understand
**EJE:** Comprensión lectora
**Expected_Success:** 0.82
**Contexto:** Durante un taller de ciencias en Cartago, Javier analiza un informe sobre la actividad geotérmica de los volcanes de la cordillera.

### Enunciado
¿Qué rasgo lingüístico es característico de los textos expositivos académicos?

### Opciones
- [ ] A) La preeminencia de adjetivos valorativos que expresen la opinión del redactor.
  <!-- feedback: Incorrecto. Los textos expositivos deben ser objetivos y neutrales. -->
- [x] B) El predominio de la función referencial, la tercera persona gramatical y un léxico denotativo y preciso.
  <!-- feedback: Correcto. Estas marcas lingüísticas garantizan la neutralidad y claridad conceptual. -->
- [ ] C) El uso continuo del voseo informal costarricense en oraciones exclamativas.
  <!-- feedback: Incorrecto. Se prefiere la formalidad de la tercera persona para mantener la distancia objetiva. -->
- [ ] D) La inclusión de diálogos directos entre el investigador y los objetos de estudio.
  <!-- feedback: Incorrecto. La objetividad expositiva evita la estructura dialógica narrativa. -->

### Explicacion Pedagogica
Para transmitir información objetiva de manera rigurosa, se emplea la función referencial de la lengua y un vocabulario preciso y denotativo.

---

## Question 3 [D4]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v3
**Bloom:** Understand
**EJE:** Análisis crítico
**Expected_Success:** 0.80
**Contexto:** Un grupo de estudiantes lee un texto de divulgación científica sobre la protección de la biodiversidad marina en la Isla del Coco.

### Enunciado
¿Cuál de las siguientes estructuras organizativas es común en los textos expositivos?

### Opciones
- [ ] A) Introducción lírica, nudo dramático, clímax poético y desenlace abierto.
  <!-- feedback: Incorrecto. Esta estructura corresponde al género narrativo literario. -->
- [x] B) Presentación de una problemática o concepto, desarrollo explicativo mediante subtemas y síntesis final.
  <!-- feedback: Correcto. Es la estructura clásica (introducción, desarrollo, conclusión) orientada a la explicación. -->
- [ ] C) Exordio oratorio, planteamiento de tesis, cuerpo de refutaciones y epílogo persuasivo.
  <!-- feedback: Incorrecto. Esta es la estructura argumentativa clásica. -->
- [ ] D) Instrucciones de uso, advertencias de seguridad y cláusulas de garantía legal.
  <!-- feedback: Incorrecto. Es propio de manuales de uso y textos jurídicos. -->

### Explicacion Pedagogica
La macroestructura expositiva organiza el saber partiendo de una introducción conceptual, seguida de un desglose explicativo y una síntesis.

---

## Question 4 [D4]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v4
**Bloom:** Apply
**EJE:** Comprensión lectora
**Expected_Success:** 0.78
**Contexto:** Para una tarea en el Liceo de Heredia, Sofía lee un texto que compara las fuentes de energía solar y eólica en Guanacaste.

### Enunciado
¿Qué tipo de estructura expositiva se utiliza principalmente cuando se destacan semejanzas y diferencias entre dos recursos?

### Opciones
- [x] A) Estructura de comparación o contraste.
  <!-- feedback: Correcto. Organiza la información estableciendo similitudes y diferencias entre conceptos, objetos o procesos. -->
- [ ] B) Estructura cronológica pura o secuencia temporal estricta.
  <!-- feedback: Incorrecto. No se enfoca en el ordenamiento temporal de sucesos históricos. -->
- [ ] C) Estructura de causa-efecto lineal única.
  <!-- feedback: Incorrecto. Aunque puede haber relaciones causales, la comparación es la predominante. -->
- [ ] D) Estructura descriptiva espacial de la zona.
  <!-- feedback: Incorrecto. No describe la disposición física u orientación espacial del terreno. -->

### Explicacion Pedagogica
La organización por comparación y contraste permite confrontar dos o más elementos para detallar sus afinidades y discrepancias.

---

## Question 5 [D5]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v5
**Bloom:** Apply
**EJE:** Sintaxis
**Expected_Success:** 0.75
**Contexto:** Andrey redacta una explicación técnica sobre el tratamiento de aguas residuales en las cercanías del río Tempisque.

### Enunciado
¿Cuál de las siguientes oraciones utiliza una construcción impersonal adecuada para mantener la objetividad del escrito?

### Opciones
- [ ] A) Nosotros creemos que las plantas de filtrado funcionan muy bien.
  <!-- feedback: Incorrecto. Utiliza la primera persona del plural, perdiendo objetividad. -->
- [ ] B) Yo vi que las aguas residuales se limpian en lagunas de estabilización.
  <!-- feedback: Incorrecto. El pronombre de primera persona singular 'yo' introduce un tono subjetivo. -->
- [x] C) Se procesa el caudal recolectado mediante un sistema de filtros biológicos de alta eficiencia.
  <!-- feedback: Correcto. El uso de la pasiva refleja ('Se procesa') anula la presencia del sujeto individual, favoreciendo la impersonalidad. -->
- [ ] D) Los costarricenses tenemos que cuidar todas nuestras hermosas cuencas.
  <!-- feedback: Incorrecto. Utiliza un tono apelativo y emocional de primera persona plural. -->

### Explicacion Pedagogica
Las construcciones impersonales y de pasiva refleja son recursos clave en los textos expositivos para centrar la atención en el proceso y no en el agente.

---

## Question 6 [D5]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v6
**Bloom:** Apply
**EJE:** Comprensión lectora
**Expected_Success:** 0.73
**Contexto:** Un artículo explicativo detalla que el asfalto de la autopista General Cañas se deteriora por la combinación de lluvias intensas y exceso de peso vehicular.

### Enunciado
¿Qué tipo de ordenación de ideas predomina en un párrafo con estas características?

### Opciones
- [ ] A) Ordenación descriptiva de rasgos físicos del terreno.
  <!-- feedback: Incorrecto. No describe colores, formas o materiales fijos de la autopista. -->
- [ ] B) Ordenación analógica con otros sistemas de transporte en Centroamérica.
  <!-- feedback: Incorrecto. No compara la General Cañas con otras carreteras regionales. -->
- [x] C) Ordenación de causa y efecto.
  <!-- feedback: Correcto. Identifica los factores desencadenantes (lluvia, exceso de peso) y su consecuencia directa (deterioro vial). -->
- [ ] D) Ordenación jerárquica de cargos políticos municipales.
  <!-- feedback: Incorrecto. No clasifica la importancia de funcionarios viales costarricenses. -->

### Explicacion Pedagogica
La estructura de causa-efecto se enfoca en exponer las razones que originan un fenómeno determinado y los efectos resultantes observables.

---

## Question 7 [D5]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v7
**Bloom:** Apply
**EJE:** Semántica
**Expected_Success:** 0.70
**Contexto:** María prepara una presentación sobre la producción de café de especialidad en la zona de Los Santos.

### Enunciado
¿Cuál de los siguientes términos representa un tecnicismo adecuado para un informe expositivo del sector cafetalero?

### Opciones
- [ ] A) Cafecito delicioso de la tarde.
  <!-- feedback: Incorrecto. Es una expresión coloquial y afectiva, inadecuada para un texto técnico. -->
- [x] B) Organoléptico, referido a las propiedades físicas y sensoriales del grano.
  <!-- feedback: Correcto. Es un tecnicismo agronómico y gastronómico propio del análisis especializado de calidad. -->
- [ ] C) Chorreador tradicional de madera.
  <!-- feedback: Incorrecto. Es un término costumbrista costarricense, no un tecnicismo del proceso agroindustrial. -->
- [ ] D) Taza humeante y agradable.
  <!-- feedback: Incorrecto. Es un sintagma descriptivo literario, no un vocablo especializado técnico. -->

### Explicacion Pedagogica
Los tecnicismos son vocablos especializados propios de una disciplina o sector profesional que garantizan la precisión conceptual del texto.

---

## Question 8 [D6]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v8
**Bloom:** Apply
**EJE:** Pragmática
**Expected_Success:** 0.68
**Contexto:** En el sitio web de la municipalidad de Liberia se publica una guía explicativa para el manejo de residuos sólidos.

### Enunciado
Para facilitar la lectura rápida de los ciudadanos, ¿qué recurso tipográfico y estructural es idóneo?

### Opciones
- [ ] A) La redacción del texto en un único párrafo masivo sin subdivisiones ni espacios.
  <!-- feedback: Incorrecto. Dificulta la lectura rápida y comprensión de los conceptos clave. -->
- [x] B) El empleo de subtítulos claros, viñetas organizadoras y negritas en conceptos clave.
  <!-- feedback: Correcto. Ayuda a segmentar la información jerárquicamente, mejorando la legibilidad. -->
- [ ] C) La transcripción de la ley de residuos en letra cursiva.
  <!-- feedback: Incorrecto. Resulta poco práctico y atenta contra la accesibilidad digital del sitio municipal. -->
- [ ] D) La inclusión de poemas laudatorios sobre el aseo de las calles.
  <!-- feedback: Incorrecto. Pertenece al género lírico y no aporta claridad funcional al manejo de residuos. -->

### Explicacion Pedagogica
Los subtítulos, listas con viñetas y relieves tipográficos son recursos organizadores visuales indispensables en la divulgación expositiva.

---

## Question 9 [D6]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v9
**Bloom:** Apply
**EJE:** Sintaxis
**Expected_Success:** 0.65
**Contexto:** Javier debe transformar un párrafo narrativo sobre la fauna de Manuel Antonio en uno puramente expositivo.

### Enunciado
¿Cuál de las siguientes adaptaciones cumple mejor con el propósito expositivo?

### Opciones
- [ ] A) Caminando por el sendero, vi un mono cariblanco que me miró fijamente y me asustó bastante.
  <!-- feedback: Incorrecto. Es un relato vivencial anecdótico y en primera persona. -->
- [x] B) El mono cariblanco (Cebus capucinus) es un primate omnívoro nativo de América Central, caracterizado por su pelaje blanco alrededor del rostro.
  <!-- feedback: Correcto. Adopta una descripción biológica objetiva, rigurosa y en tercera persona con vocabulario denotativo. -->
- [ ] C) Qué maravillosos son los animalitos que saltan alegres de rama en rama en nuestro amado parque nacional.
  <!-- feedback: Incorrecto. Expresa emociones personales y utiliza diminutivos afectivos coloquiales. -->
- [ ] D) ¡Ojalá todos los turistas respeten a los monos de Manuel Antonio cuando visiten el país!
  <!-- feedback: Incorrecto. Es una oración desiderativa y exclamativa, no expositiva. -->

### Explicacion Pedagogica
La reescritura expositiva requiere eliminar la perspectiva subjetiva, los juicios de valor y las anécdotas personales para enfocarse en descripciones de forma objetiva.

---

## Question 10 [D6]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v10
**Bloom:** Understand
**EJE:** Comprensión lectora
**Expected_Success:** 0.62
**Contexto:** Durante una investigación escolar en Alajuela, se analiza un folleto expositivo sobre el ciclo de vida del sapo dorado de Monteverde.

### Enunciado
¿Cuál es la función del glosario que suele incluirse al final de este tipo de textos técnicos?

### Opciones
- [ ] A) Ofrecer una lista de agradecimientos a las autoridades del MEP.
  <!-- feedback: Incorrecto. Eso corresponde a la sección de agradecimientos o dedicatoria. -->
- [x] B) Definir términos especializados o tecnicismos poco comunes utilizados en el cuerpo de la exposición.
  <!-- feedback: Correcto. Permite aclarar el significado preciso de los vocablos para facilitar la comprensión del lector general. -->
- [ ] C) Mostrar imágenes estéticas del ecosistema de bosque nuboso.
  <!-- feedback: Incorrecto. Eso es propio de una galería fotográfica o apéndice visual. -->
- [ ] D) Presentar el índice cronológico de las publicaciones de la escuela.
  <!-- feedback: Incorrecto. Corresponde al índice general o bibliografía del autor. -->

### Explicacion Pedagogica
El glosario proporciona definiciones exactas de conceptos técnicos de la materia tratada, apoyando la asimilación conceptual del texto.

---

## Question 11 [D7]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v11
**Bloom:** Analyze
**EJE:** Análisis crítico
**Expected_Success:** 0.58
**Contexto:** Lea detenidamente el siguiente fragmento de un artículo de divulgación científica:
'El cambio climático ha elevado la temperatura superficial del mar en el Golfo de Nicoya. Este calentamiento altera el metabolismo de las especies marinas locales, reduciendo la disponibilidad de oxígeno e impactando severamente la pesca artesanal de las comunidades de Puntarenas.'

### Enunciado
¿Cómo se organiza la secuencia lógica de ideas expuestas en el fragmento?

### Opciones
- [x] A) Presentación de una causa global, descripción del mecanismo biológico intermedio y exposición de las consecuencias socioeconómicas locales.
  <!-- feedback: Correcto. El texto va de la causa (cambio climático/temperatura) al mecanismo biológico (metabolismo/oxígeno) y concluye con las consecuencias socioeconómicas (pesca artesanal de Puntarenas). -->
- [ ] B) Comparación exhaustiva entre los ecosistemas acuáticos del Golfo de Nicoya y los del Caribe limonense.
  <!-- feedback: Incorrecto. No se realiza ninguna comparación con el Caribe en el fragmento. -->
- [ ] C) Exposición cronológica de las regulaciones de pesca vigentes en Costa Rica desde el siglo pasado.
  <!-- feedback: Incorrecto. El texto describe una relación ecológica física y económica actual, no leyes históricas. -->
- [ ] D) Defensa de una tesis política sobre la privatización de los muelles de la provincia puntarenense.
  <!-- feedback: Incorrecto. Es un texto científico de divulgación objetiva, no un manifiesto político argumentativo. -->

### Explicacion Pedagogica
La lógica interna progresa de forma descendente, enlazando una causa físico-química de gran escala con efectos biológicos y repercusiones productivas humanas locales.

---

## Question 12 [D7]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v12
**Bloom:** Analyze
**EJE:** Comprensión lectora
**Expected_Success:** 0.55
**Contexto:** Considere el siguiente fragmento de un informe escolar sobre hidroelectricidad en Costa Rica:
'El Complejo Hidroeléctrico Arenal genera una parte importante de la electricidad del país. A diferencia de las plantas térmicas, la generación hidroeléctrica no quema combustibles fósiles, reduciendo sustancialmente la emisión de dióxido de carbono al ambiente.'

### Enunciado
¿Qué recurso explicativo se utiliza de forma explícita en el fragmento para esclarecer el funcionamiento del complejo?

### Opciones
- [ ] A) La definición etimológica de la palabra 'hidroelectricidad' según la Real Academia.
  <!-- feedback: Incorrecto. No se hace mención del origen de la palabra ni de diccionarios. -->
- [x] B) La oposición o contraste directo con un sistema alternativo de generación energética.
  <!-- feedback: Correcto. Contrasta la energía hidroeléctrica con las 'plantas térmicas' para resaltar los beneficios ecológicos de la primera. -->
- [ ] C) La narración de una jornada laboral de los técnicos de la planta de Arenal.
  <!-- feedback: Incorrecto. No narra ninguna anécdota personal ni laboral. -->
- [ ] D) La inclusión de una metáfora poética sobre el agua como espejo del alma de la provincia.
  <!-- feedback: Incorrecto. El fragmento mantiene un tono expositivo objetivo e impersonal sin metáforas poéticas. -->

### Explicacion Pedagogica
El contraste con sistemas térmicos permite clarificar los atributos singulares y positivos del objeto de estudio de forma comparativa.

---

## Question 13 [D7]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v13
**Bloom:** Analyze
**EJE:** Pragmática
**Expected_Success:** 0.52
**Contexto:** Un texto expositivo sobre las características arquitectónicas de la Basílica de Cartago incluye un plano detallado con referencias numéricas de cada sección estructural.

### Enunciado
¿Qué relación se establece entre el plano visual y el texto escrito continuo?

### Opciones
- [ ] A) El plano contradice las descripciones históricas hechas por los ensayistas.
  <!-- feedback: Incorrecto. La infografía apoya la veracidad del texto escrito, no lo contradice. -->
- [x] B) El plano cumple una función complementaria e ilustrativa, facilitando la visualización espacial de los elementos técnicos explicados.
  <!-- feedback: Correcto. Las imágenes técnicas y esquemas son complementos informativos vitales que asisten a la explicación verbal. -->
- [ ] C) El plano sustituye por completo la necesidad de leer el texto descriptivo.
  <!-- feedback: Incorrecto. El plano requiere de la explicación textual para contextualizar sus datos. -->
- [ ] D) El plano es puramente decorativo y carece de valor informativo real en la asamblea.
  <!-- feedback: Incorrecto. Posee un alto valor informativo al detallar espacialmente la estructura arquitectónica. -->

### Explicacion Pedagogica
La combinación de recursos visuales e información textual escrita optimiza la transmisión del saber descriptivo y técnico en la tipología expositiva.

---

## Question 14 [D8]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v14
**Bloom:** Analyze
**EJE:** Comprensión lectora
**Expected_Success:** 0.48
**Contexto:** En una monografía explicativa de undécimo año sobre el cultivo hidropónico en Alajuela, se lee:
'La hidroponía permite cultivar hortalizas sin suelo agrícola. Las raíces reciben una solución nutritiva equilibrada disuelta en agua, la cual contiene todos los elementos químicos esenciales para el desarrollo de la planta.'

### Enunciado
¿Cuál es el concepto clave que se está definiendo operativamente en este fragmento?

### Opciones
- [ ] A) La escasez de agua potable en las llanuras del norte de Alajuela.
  <!-- feedback: Incorrecto. El texto aborda el cultivo hidropónico en general, no una problemática de escasez de agua de la provincia. -->
- [x] B) La hidroponía como técnica de cultivo basada en soluciones nutritivas en lugar de tierra féil.
  <!-- feedback: Correcto. El texto define operativamente qué es y cómo funciona el cultivo sin suelo denominado hidroponía. -->
- [ ] C) La composición molecular de los nutrientes orgánicos costarricenses.
  <!-- feedback: Incorrecto. Menciona de forma general 'todos los elementos químicos', pero no detalla fórmulas de nutrientes específicos. -->
- [ ] D) La historia de la agricultura tradicional costarricense antes de la tecnología industrial.
  <!-- feedback: Incorrecto. Se enfoca en una tecnología moderna de cultivo, no en su historia tradicional. -->

### Explicacion Pedagogica
La definición operativa introduce un concepto técnico y explica de inmediato su funcionamiento práctico dentro del marco explicativo del texto.

---

## Question 15 [D8]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v15
**Bloom:** Evaluate
**EJE:** Análisis crítico
**Expected_Success:** 0.45
**Contexto:** Un estudiante costarricense consulta un sitio web de edición abierta para realizar una investigación sobre la biodiversidad del Parque Nacional Corcovado y encuentra la siguiente afirmación:
'Corcovado es increíble y tiene un montón de animales súper lindos y raros que a todos los turistas costarricenses les encanta fotografiar los fines de semana.'

### Enunciado
Al evaluar este fragmento para su uso como fuente en un informe expositivo académico, ¿cuál es su principal deficiencia?

### Opciones
- [ ] A) La falta de mención del cantón específico donde se localiza el parque nacional.
  <!-- feedback: Incorrecto. Aunque útil, no es la principal deficiencia de estilo o estructura académica del texto. -->
- [x] B) El uso de adjetivos de carácter subjetivo y coloquialismos ('increíble', 'súper lindos', 'raros') que comprometen la objetividad y el rigor científico necesarios.
  <!-- feedback: Correcto. Los de naturaleza académica deben evitar la adjetivación emocional y valorativa en pos del rigor formal científico. -->
- [ ] C) Que el autor no utilizó oraciones escritas con rima consonante tradicional.
  <!-- feedback: Incorrecto. Las monografías científicas deben evitar rimas y lirismos. -->
- [ ] D) La exclusión de datos climáticos sobre la temperatura de la Península de Osa.
  <!-- feedback: Incorrecto. Se puede hablar de fauna sin clima; la falla es el tono informal y subjetivo. -->

### Explicacion Pedagogica
Un texto expositivo riguroso debe prescindir de valoraciones subjetivas e imprecisiones coloquiales, adoptando un vocabulario objetivo, técnico y neutral.

---

## Question 16 [D8]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v16
**Bloom:** Evaluate
**EJE:** Análisis crítico
**Expected_Success:** 0.42
**Contexto:** Considere el siguiente fragmento de una enciclopedia costarricense sobre el río Grande de Tárcoles:
'El río Grande de Tárcoles presenta altos niveles de contaminación orgánica e industrial debido a que recibe las descargas de los sistemas de alcantarillado de la Gran Área Metropolitana (GAM).'

### Enunciado
¿Qué criterio de validez científica respalda de mejor manera la solidez expositiva de este enunciado?

### Opciones
- [ ] A) La inclusión de leyendas mágicas costarricenses referidas a los cocodrilos que habitan el puente del río Tárcoles.
  <!-- feedback: Incorrecto. Las leyendas aportan valor folclórico, no rigor expositivo o científico. -->
- [x] B) La correlación causal clara y lógica entre la ubicación geográfica de la GAM y el vertido directo de residuos en la cuenca.
  <!-- feedback: Correcto. Explica la contaminación mediante una relación física y demográfica comprobable (causa y efecto). -->
- [ ] C) La opinión personal expresada por un grupo de pescadores artesanales en sus redes de Puntarenas.
  <!-- feedback: Incorrecto. Las opiniones personales informales en redes sociales no constituyen criterios de validez científica formal. -->
- [ ] D) La redacción del párrafo utilizando figuras literarias complejas como el hipérbaton.
  <!-- feedback: Incorrecto. La ornamentación literaria dificulta el propósito expositivo de claridad y precisión de la información. -->

### Explicacion Pedagogica
La validez científica de un texto expositivo se sustenta en explicaciones lógicas de causalidad física respaldadas por la geografía y la ecología observables.

---

## Question 17 [D9]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v17
**Bloom:** Evaluate
**EJE:** Pragmática
**Expected_Success:** 0.38
**Contexto:** En un artículo escolar sobre la historia del Teatro Nacional de Costa Rica, un estudiante escribe el siguiente enunciado:
'La construcción del Teatro Nacional comenzó a finales del siglo XIX, financiada principalmente por un impuesto a la exportación de café, lo que impulsó las artes costarricenses.'

### Enunciado
¿Qué valor aporta este enunciado para la comprensión histórica del tema expuesto?

### Opciones
- [ ] A) Aporta una tesis de la abolición del café en las montañas de San José.
  <!-- feedback: Incorrecto. No propone abolir el café, explica el origen de los fondos de construcción del teatro. -->
- [ ] B) Proporciona una descripción estética de la fachada de mármol del edificio.
  <!-- feedback: Incorrecto. Explica un proceso de financiamiento socioeconómico, no un rasgo físico estético. -->
- [x] C) Explica la estrecha relación de interdependencia económica entre el desarrollo agrícola cafetalero y el fomento de la infraestructura cultural nacional.
  <!-- feedback: Correcto. Provee un contexto histórico explicativo fundamental para comprender cómo se gestó la obra arquitectónica. -->
- [ ] D) Critica la falta de inversión en los teatros regionales de la provincia de Alajuela.
  <!-- feedback: Incorrecto. Se circunscribe a datos del siglo XIX and no emite juicios de valor sobre presupuestos contemporáneos. -->

### Explicacion Pedagogica
El enunciado enriquece el texto expositivo al proveer una explicación causal socioeconómica que ayuda a entender los factores históricos de un hito cultural costarricense.

---

## Question 18 [D9]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v18
**Bloom:** Evaluate
**EJE:** Análisis crítico
**Expected_Success:** 0.35
**Contexto:** Un docente costarricense evalúa la coherencia interna de una monografía técnica de undécimo año sobre el manejo del cultivo de caña de azúcar en la provincia de Guanacaste. El estudiante concluye su desarrollo con la siguiente frase:
'Por lo tanto, la caña de azúcar es un cultivo fascinante porque me recuerda las vacaciones de mi infancia en San Carlos y el dulce sabor del trapiche.'

### Enunciado
¿Cuál es el principal fallo metodológico y estructural en la conclusión del estudiante?

### Opciones
- [ ] A) No proponer un aumento del salario de los trabajadores de los trapiches.
  <!-- feedback: Incorrecto. Las monografías técnicas no tienen la obligación de proponer reformas salariales de cañeros. -->
- [x] B) Romper de forma abrupta el registro objetivo e impersonal del trabajo académico introduciendo vivencias subjetivas, anécdotas infantiles y juicios personales de agrado.
  <!-- feedback: Correcto. La irrupción de la primera persona singular ('me recuerda') y recuerdos nostálgicos anula la objetividad académica requerida en una monografía. -->
- [ ] C) No citar textualmente leyes vigentes de la Liga Agrícola Industrial de la Caña.
  <!-- feedback: Incorrecto. No es estrictamente obligatorio citar leyes específicas en una frase de síntesis técnica general. -->
- [ ] D) La mención de San Carlos, puesto que la monografía se enfocaba únicamente en Guanacaste.
  <!-- feedback: Incorrecto. Aunque es un cambio regional, la falla metodológica fundamental es el cambio de un tono objetivo a uno subjetivo de agrado. -->

### Explicacion Pedagogica
La conclusión de un escrito expositivo formal debe condensar de forma objetiva los hallazgos principales, evitando digresiones personales que anulen el rigor científico.

---

## Question 19 [D10]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v19
**Bloom:** Understand
**EJE:** Comprensión lectora
**Expected_Success:** 0.32
**Contexto:** Durante un taller de Español sobre tipologías textuales en Cartago, se debate sobre la función de las notas al pie de página en las monografías expositivas.

### Enunciado
¿Cuál es el propósito instrumental de este recurso tipográfico?

### Opciones
- [ ] A) Permitir al autor expresar poesías breves inspiradas en el tema.
  <!-- feedback: Incorrecto. No se utilizan para decoraciones líricas o poéticas ajenas a la ciencia. -->
- [x] B) Aportar aclaraciones marginales, precisar fuentes bibliográficas o ampliar información secundaria sin interrumpir la fluidez del texto principal.
  <!-- feedback: Correcto. Es la función académica del aparato de notas al pie. -->
- [ ] C) Enumerar los nombres de los estudiantes reprobados en el curso.
  <!-- feedback: Incorrecto. Su función es puramente informativa sobre las fuentes académicas citadas. -->
- [ ] D) Sustituir por completo las conclusiones generales del informe monográfico.
  <!-- feedback: Incorrecto. Las conclusiones constituyen un apartado autónomo e indispensable al final del trabajo. -->

### Explicacion Pedagogica
Las notas al pie de página sirven para documentar la procedencia de los datos o desarrollar digresiones explicativas colaterales de valor informativo.

---

## Question 20 [D10]
**ID:** CR-LEN-11-2026-W02-tema-w02-001-MASTERY-bundle-v20
**Bloom:** Remember
**EJE:** Comprensión lectora
**Expected_Success:** 0.30
**Contexto:** Un grupo de estudiantes de Heredia repasa la estructura de los textos de divulgación científica para preparar su examen de Bachillerato MEP.

### Enunciado
¿Qué característica formal define a un texto de divulgación frente a uno especializado?

### Opciones
- [ ] A) La obligatoriedad de escribirse enteramente en inglés académico estadounidense.
  <!-- feedback: Incorrecto. Pueden redactarse perfectamente en español costarricense u otros idiomas oficiales. -->
- [x] B) La adaptación de conceptos y terminología compleja a un lenguaje accesible, claro y didáctico para un público receptor no experto.
  <!-- feedback: Correcto. La divulgación tiende puentes comunicativos simplificando el lenguaje sin perder rigor de la información. -->
- [ ] C) El uso exclusivo de un lenguaje cifrado comprensible únicamente por científicos de universidades.
  <!-- feedback: Incorrecto. Esto describiría un artículo especializado cerrado de investigación, no un texto divulgativo. -->
- [ ] D) La ausencia total de datos fácticos o explicaciones lógicas sobre la naturaleza del país.
  <!-- feedback: Incorrecto. El texto divulgativo está repleto de explicaciones y datos, aunque presentados de manera didáctica. -->

### Explicacion Pedagogica
La divulgación científica persigue democratizar el conocimiento adaptando tecnicismos y estructuras conceptuales complejas para que resulten asequibles a la población general.
