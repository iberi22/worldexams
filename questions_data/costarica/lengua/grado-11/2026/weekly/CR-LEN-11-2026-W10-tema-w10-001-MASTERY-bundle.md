---
id: "CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle"
country: "costarica"
grado: 11
asignatura: "lengua"
tema: "tema-w10"
periodo: "weekly"
week: "W10"
year: 2026
bundle_type: "weekly"
protocol_version: "5.2"
total_questions: 20
bundle_size: 20
alignment: "MEP Costa Rica Bachillerato 2026"
license: "FREE"
tier: "legacy"
creador: "Jules-Agent"
bundle_index: 1
---

# Bundle MASTERY: Figuras Literarias - Grado 11

Este bundle contiene 20 preguntas sobre **figuras literarias** para grado 11,
alineadas con el programa de estudios del MEP Costa Rica para Bachillerato 2026.

---
## Question 1 [D3]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v1
**Bloom:** Remember
**EJE:** Análisis literario
**Expected_Success:** 0.85
**Contexto:** Durante una sesión de Español en el Liceo de Heredia, los estudiantes repasan los tropos y las figuras literarias.

### Enunciado
¿Cuál de las siguientes definiciones describe formalmente a la figura literaria de la metáfora frente al símil?

### Opciones
- [x] A) La identificación directa de un término real con uno imaginario debido a una relación de semejanza, sin utilizar nexos comparativos.
  <!-- feedback: Correcto. La metáfora es la traslación de sentido directa sin nexos explícitos. -->
- [ ] B) La comparación explícita de dos elementos utilizando nexos comparativos como 'como', 'parece' o 'semeja'.
  <!-- feedback: Incorrecto. Esta definición formal corresponde al símil o comparación. -->
- [ ] C) La exageración desmesurada de las cualidades de un personaje de Alajuela.
  <!-- feedback: Incorrecto. Esto constituye una definición formal de la hipérbole. -->
- [ ] D) La repetición constante de sonidos en un mismo verso costarricense.
  <!-- feedback: Incorrecto. Esto define a la aliteración poética, no a la metáfora. -->

### Explicacion Pedagogica
La metáfora es el tropo conceptual por excelencia que unifica el plano real e imaginario de forma directa basándose en analogías estéticas.

---

## Question 2 [D3]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v2
**Bloom:** Understand
**EJE:** Análisis literario
**Expected_Success:** 0.82
**Contexto:** Sofía lee un poema costarricense dedicado al atardecer en las playas de Puntarenas y encuentra el verso: 'El sol parecía una moneda de oro encendida que caía lentamente en el agua amarga'.

### Enunciado
¿Qué figura literaria se manifiesta en este verso poético?

### Opciones
- [ ] A) Anáfora de palabras al inicio del verso.
  <!-- feedback: Incorrecto. Consiste en la repetición de palabras en versos sucesivos, ausente en este enunciado. -->
- [x] B) Símil o comparación.
  <!-- feedback: Correcto. El uso del nexo comparativo 'parecía' establece de forma explícita la relación de semejanza entre el sol y la moneda. -->
- [ ] C) Hipérbaton por alteración sintáctica en Heredia.
  <!-- feedback: Incorrecto. El enunciado mantiene una estructura sintáctica lineal natural en español costarricense. -->
- [ ] D) Asíndeton por omisión de conjunciones copulativas coordinantes.
  <!-- feedback: Incorrecto. No suprime conjunciones de forma sistemática para acelerar el ritmo poético. -->

### Explicacion Pedagogica
El símil o comparación exige la presencia de un nexo verbal o gramatical comparativo que conecte lógicamente el plano real con la dimensión figurada.

---

## Question 3 [D4]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v3
**Bloom:** Understand
**EJE:** Análisis de textos
**Expected_Success:** 0.80
**Contexto:** Javier analiza un fragmento literario donde se describe el viento de las montañas de Cartago de la siguiente manera: 'El viento silbaba canciones tristes entre las maderas de adobes de la casa'.

### Enunciado
¿Qué recurso poético se emplea cuando se le atribuyen conductas y sentimientos humanos al viento?

### Opciones
- [ ] A) Metáfora de adobes costumbrista de la provincia.
  <!-- feedback: Incorrecto. La metáfora asocia un término real con uno figurado; atribuir conducta es personificación. -->
- [x] B) Personificación o prosopopeya.
  <!-- feedback: Correcto. Consiste en conceder rasgos, acciones o sentimientos humanos (silbar canciones tristes) a elementos de la naturaleza u objetos inanimados. -->
- [ ] C) Símil utilizando el nexo comparativo 'como'.
  <!-- feedback: Incorrecto. No hay nexos comparativos de semejanza explícitos en el enunciado de Cartago. -->
- [ ] D) Aliteración fonética de la letra /s/ en la asamblea.
  <!-- feedback: Incorrecto. Aunque hay sonidos de /s/, la figura conceptual dominante es la atribución de conducta humana. -->

### Explicacion Pedagogica
La personificación o prosopopeya es el tropo de pensamiento que humaniza el entorno inanimado, dotando de dinamismo vital a la naturaleza en la literatura.

---

## Question 4 [D4]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v4
**Bloom:** Apply
**EJE:** Análisis literario
**Expected_Success:** 0.78
**Contexto:** María desea componer un poema exagerando la cantidad de lluvia que cae en la provincia de Limón durante el invierno.

### Enunciado
¿Cuál de las siguientes oraciones utiliza de mejor manera el tropo de la hipérbole?

### Opciones
- [ ] A) En la provincia de Limón llueve copiosamente durante varios días de la estación lluviosa.
  <!-- feedback: Incorrecto. Es un enunciado puramente fáctico descriptivo, libre de exageraciones o tropos poéticos. -->
- [x] B) Llovieron océanos enteros sobre Limón, inundando hasta el último rincón de la provincia con cascadas celestiales.
  <!-- feedback: Correcto. El uso de 'océanos enteros' es una exageración desmesurada (hipérbole) con fines expresivos estéticos. -->
- [ ] C) La lluvia caribeña caía como finas lágrimas sobre las hojas de banano.
  <!-- feedback: Incorrecto. Es un símil comparativo poético ('como finas lágrimas'), no una hipérbole de exageración. -->
- [ ] D) El Ministerio de Educación Pública emitió una alerta por el temporal.
  <!-- feedback: Incorrecto. Es un hecho fáctico de carácter periodístico o informativo de seguridad del MEP. -->

### Explicacion Pedagogica
La hipérbole deforma de manera intencional y desmesurada la realidad para intensificar el efecto dramático de la escena.

---

## Question 5 [D5]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v5
**Bloom:** Apply
**EJE:** Análisis literario
**Expected_Success:** 0.75
**Contexto:** Andrey lee una estrofa poética costarricense y analiza la figura de la 'antítesis'.

### Enunciado
¿En qué consiste la figura de la antítesis en la poesía clásica?

### Opciones
- [ ] A) En la repetición de un mismo sustantivo al inicio de cada verso.
  <!-- feedback: Incorrecto. Esto define a la anáfora poética, no a la antítesis. -->
- [x] B) En la contraposición de dos palabras, conceptos o ideas de significado opuesto en una misma estrofa para resaltar un contraste.
  <!-- feedback: Correcto. La antítesis confronta términos contrapuestos de forma lógica y poética (como 'luz' y 'sombra', o 'dulce' y 'amargo'). -->
- [ ] C) En el uso de construcciones impersonales con pasiva refleja.
  <!-- feedback: Incorrecto. Esto caracteriza a la sintaxis del texto expositivo académico, ajeno a figuras poéticas líricas. -->
- [ ] D) En la supresión de signos de puntuación del libreto de teatro.
  <!-- feedback: Incorrecto. Se trata de un recurso de puntuación, no de contraposición semántica lírica. -->

### Explicacion Pedagogica
La antítesis realza la expresión lírica al aproximar conceptos opuestos, estructurando un conflicto semántico de gran belleza en el poema.

---

## Question 6 [D5]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v6
**Bloom:** Apply
**EJE:** Análisis de textos
**Expected_Success:** 0.73
**Contexto:** Ana analiza el verso de una poesía dedicada al campesino costarricense: 'La áspera caricia de tus manos, labriego sencillo...'

### Enunciado
¿Qué figura basada en la contradicción de dos términos aparentemente incompatibles en un mismo sintagma se manifiesta en este verso poético?

### Opciones
- [ ] A) Hipérbaton sintáctico del Valle Central de la provincia.
  <!-- feedback: Incorrecto. El hipérbaton altera el orden de las palabras, ausente en este sintagma nominal directo. -->
- [x] B) Oxímoron.
  <!-- feedback: Correcto. El oxímoron une dos conceptos contradictorios en un mismo sintagma ('áspera' y 'caricia' se contradicen), creando un nuevo significado poético. -->
- [ ] C) Anáfora de Cartago.
  <!-- feedback: Incorrecto. Consiste en la repetición de palabras en versos sucesivos, no en la contradicción de un sintagma. -->
- [ ] D) Epíteto del buey cansado.
  <!-- feedback: Incorrecto. El epíteto es un adjetivo explicativo que destaca una cualidad natural obvia (blanca nieve), no un término contradictorio. -->

### Explicacion Pedagogica
El oxímoron es una figura que une de forma directa términos opuestos en un mismo sintagma oracional, obligando al lector a un esfuerzo de síntesis semántica.

---

## Question 7 [D5]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v7
**Bloom:** Apply
**EJE:** Análisis de textos
**Expected_Success:** 0.70
**Contexto:** Javier compone un poema sobre la noche estrellada en el Volcán Poás de Alajuela.

### Enunciado
Si desea incorporar un epíteto, ¿cuál de las siguientes opciones representa mejor esta figura?

### Opciones
- [ ] A) La noche de Alajuela estaba despejada y se podían ver las constelaciones.
  <!-- feedback: Incorrecto. Es una descripción de carácter fáctico denotativo, sin epítetos. -->
- [x] B) La oscura noche nos envolvía mientras contemplábamos las brillantes estrellas del firmamento.
  <!-- feedback: Correcto. 'Oscura noche' y 'brillantes estrellas' utilizan adjetivos explicativos (epítetos) que destacan cualidades inherentes naturales de los sustantivos. -->
- [ ] C) La noche parecía un manto de adobes oscuros de la provincia.
  <!-- feedback: Incorrecto. Es un símil poético comparativo explícito con un nexo ('parecía'), no un epíteto descriptivo. -->
- [ ] D) El guardaparques nos solicitó abrigarnos bien debido al viento del cráter.
  <!-- feedback: Incorrecto. Es un enunciado descriptivo de conducta real, libre de epítetos poéticos. -->

### Explicacion Pedagogica
El epíteto es un adjetivo calificativo que explicita una cualidad obvia y natural de la materia con una finalidad estética, ornamental y estilística.

---

## Question 8 [D6]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v8
**Bloom:** Apply
**EJE:** Análisis de textos
**Expected_Success:** 0.68
**Contexto:** Durante un lector en San José, se lee el verso: 'Su voz era un manantial de agua fresca que calmaba la sed del alma de Heredia.'

### Enunciado
¿Qué tipo de metáfora se efectúa en este de la provincia al identificar 'su voz' con un 'manantial' utilizando el verbo 'ser'?

### Opciones
- [ ] A) Metáfora pura de la zafra.
  <!-- feedback: Incorrecto. La metáfora pura omite el plano real (A), dejando únicamente el término imaginario (B). Aquí 'su voz' está explícito. -->
- [x] B) Metáfora impura (de identificación directa del plano real A con el imaginario B a través del verbo 'ser': A es B).
  <!-- feedback: Correcto. El verso une de forma directa 'su voz' (A) con 'manantial' (B) a través de la cópula verbal 'era' (A es B). -->
- [ ] C) Símil poético de la provincia de Alajuela.
  <!-- feedback: Incorrecto. No utiliza nexos gramaticales comparativos de semejanza explícitos como 'como' o 'semeja'. -->
- [ ] D) Hipérbole de las municipalidades del país.
  <!-- feedback: Incorrecto. No constituye una exageración de gran escala; es una correspondencia de significado. -->

### Explicacion Pedagogica
La metáfora impura asocia de forma directa los dos planos en la oración gramatical, sirviéndose convencionalmente del verbo copulativo 'ser'.

---

## Question 9 [D6]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v9
**Bloom:** Apply
**EJE:** Análisis de textos
**Expected_Success:** 0.65
**Contexto:** María analiza un poema del costarricense Jorge Debravo dedicado al dolor del obrero de Cartago.

### Enunciado
¿Qué figura se presenta en el verso: 'El hierro del arado hiere la tierra herida con amargura'?

### Opciones
- [ ] A) Asíndeton por omisión de conjunciones.
  <!-- feedback: Incorrecto. No hay supresión de conjunciones coordinantes sintácticas en la frase. -->
- [x] B) Derivación o políptoton (acumulación de palabras con la misma raíz léxica: 'hierro', 'hiere', 'herida') con fines rítmicos.
  <!-- feedback: Correcto. Utiliza términos derivados de la misma familia de palabras ('herir', 'herida', 'hierro') para potenciar la fuerza fónica y semántica del dolor en el verso. -->
- [ ] C) Asíndeton de la provincia de Alajuela.
  <!-- feedback: Incorrecto. Corresponde a la supresión de nexos sintácticos, ausente en este enunciado directo de Cartago. -->
- [ ] D) Símil utilizando el nexo de comparación 'como'.
  <!-- feedback: Incorrecto. No hay nexos comparativos gramaticales de semejanza explícitos. -->

### Explicacion Pedagogica
La derivación o políptoton reitera vocablos de un mismo origen etimológico en la frase, intensificando el dramatismo rítmico de la lírica.

---

## Question 10 [D6]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v10
**Bloom:** Understand
**EJE:** Análisis de textos
**Expected_Success:** 0.62
**Contexto:** Durante una asamblea de Español en Cartago, un estudiante analiza la figura de la 'sinécdoque'.

### Enunciado
¿En qué consiste la sinécdoque como tropo en el análisis textual literario?

### Opciones
- [ ] A) En la contraposición lógica de dos ideas opuestas en una estrofa poética.
  <!-- feedback: Incorrecto. Esto constituye la definición formal de la antítesis, no de la sinécdoque. -->
- [x] B) En designar un elemento con el nombre de otro a partir de una relación de inclusión de carácter físico, por ejemplo, mencionando la parte para referirse al todo.
  <!-- feedback: Correcto. Describe con rigor la transferencia semántica por inclusión de la sinécdoque. -->
- [ ] C) En atribuir cualidades y conductas humanas a seres de la naturaleza u objetos inanimados.
  <!-- feedback: Incorrecto. Esto es la definición técnica de la personificación. -->
- [ ] D) En la imitación de un ruido mediante fonemas en el verso.
  <!-- feedback: Incorrecto. Esto constituye la definición formal de la aliteración, no de la sinécdoque. -->

### Explicacion Pedagogica
La sinécdoque es un tropo semántico de gran valor descriptivo que sustituye vocablos basándose en relaciones de contigüidad física o cuantitativa.

---

## Question 11 [D7]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v11
**Bloom:** Analyze
**EJE:** Análisis de textos
**Expected_Success:** 0.58
**Contexto:** Lea atentamente el siguiente fragmento lírico:
'Tus ojos de noche oscura alumbran el camino del viajero;
tus manos de viento fresco carician mi frente en el Valle Central;
tu voz de arroyo limpio es música de paz que duerme las penas.'

### Enunciado
¿Qué figura de construcción sintáctica de repetición y paralelismo organiza la estructura de este poema?

### Opciones
- [x] A) La anáfora y el paralelismo sintáctico, al repetir la misma estructura gramatical e iniciar cada verso con la misma palabra de pertenencia.
  <!-- feedback: Correcto. El poema se organiza mediante la repetición de palabras al inicio ('tus', 'tus', 'tu') y repite una misma estructura gramatical en los tres versos, logrando paralelismo. -->
- [ ] B) El hipérbaton por alteración del orden de la oración en la provincia de Alajuela.
  <!-- feedback: Incorrecto. El orden sintáctico se mantiene lineal, natural e idéntico en los tres versos, sin hipérbaton. -->
- [ ] C) El asíndeton por omisión de comas de la provincia.
  <!-- feedback: Incorrecto. No hay omisión de nexos copulativos coordinantes; se estructura mediante oraciones completas. -->
- [ ] D) La caricatura de los rasgos físicos del cansado viajero.
  <!-- feedback: Incorrecto. El poema es lírico, de tono afectivo, amoroso y pacífico, libre de parodias burlescas de deformación. -->

### Explicacion Pedagogica
La anáfora combinada con el paralelismo sintáctico dota al poema de una gran simetría y ritmo, acentuando de forma cadenciosa los atributos del sujeto.

---

## Question 12 [D7]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v12
**Bloom:** Analyze
**EJE:** Análisis literario
**Expected_Success:** 0.55
**Contexto:** Considere los siguientes versos de un poema de Cartago:
'El cráter del volcán bosteza humo perezoso,
y la montaña herida sueña con el agua limpia del río.'

### Enunciado
¿Qué figuras de pensamiento de carácter descriptivo configuran la atmósfera de estos versos?

### Opciones
- [ ] A) El hipérbaton y el asíndeton de la provincia de San José.
  <!-- feedback: Incorrecto. El orden oracional es directo y no hay omisión de conjunciones copulativas coordinantes. -->
- [x] B) La personificación o prosopopeya, al atribuir conductas y sentimientos netamente humanos al cráter y a la montaña.
  <!-- feedback: Correcto. Humaniza los accidentes geográficos dándoles vida, lo cual es personificación. -->
- [ ] C) La antítesis entre el humo del volcán y los adobes de la casa.
  <!-- feedback: Incorrecto. No hay una confrontación de conceptos opuestos directos en el enunciado. -->
- [ ] D) La sinécdoque de la parte por el todo referida al cultivo de café.
  <!-- feedback: Incorrecto. No hay sustitución cuantitativa de la parte por el todo de términos agrícolas. -->

### Explicacion Pedagogica
La personificación de la geología volcánica costarricense impregna al paisaje de sensibilidad humana, transformando el relieve físico en un escenario íntimo.

---

## Question 13 [D7]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v13
**Bloom:** Analyze
**EJE:** Análisis de textos
**Expected_Success:** 0.52
**Contexto:** Lea atentamente el siguiente enunciado poético de Heredia:
'El guerrero cayó en el campo de combate como un roble herido, que se desploma con un sordo quejido de madera.'

### Enunciado
¿Cómo operan el símil y la metáfora en la construcción de la muerte del guerrero en este fragmento?

### Opciones
- [ ] A) La muerte del guerrero se analiza de forma cómica ridiculizando su fisonomía física.
  <!-- feedback: Incorrecto. El tono es de respeto, solemnidad y dolor trágico; no hay burlas ni caricaturas cómicas. -->
- [x] B) Establecen una relación de semejanza explícita ('como un roble herido') que asocia la fuerza y dignidad del combatiente con la caída del árbol, magnificando la tragedia de su muerte.
  <!-- feedback: Correcto. El símil del roble dota al guerrero de atributos de fortaleza, nobleza y grandeza de alma ante su fin inevitable. -->
- [ ] C) La metáfora del roble herido instruye sobre las especies forestales de la provincia.
  <!-- feedback: Incorrecto. El fin es estético-literario poético; no busca dar lecciones forestales o botánicas. -->
- [ ] D) Exige la prohibición de podar árboles en la provincia de Heredia.
  <!-- feedback: Incorrecto. Carece de vinculación con normativas de conservación forestal municipal o prohibiciones. -->

### Explicacion Pedagogica
El símil de carácter orgánico (el roble) sacraliza y dignifica de forma trágica la caída del héroe, asociando la muerte humana con la destrucción de la grandeza natural.

---

## Question 14 [D8]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v14
**Bloom:** Analyze
**EJE:** Análisis de textos
**Expected_Success:** 0.48
**Contexto:** Considere el verso de un poema escrito en Liberia:
'Guanacaste, tierra de sol ardiente,
donde el colón es oro en las manos del peón sencillo...'

### Enunciado
¿Qué tropo poético se efectúa al identificar de forma directa al 'colón' con el 'oro'?

### Opciones
- [ ] A) Símil utilizando el nexo de comparación 'semeja'.
  <!-- feedback: Incorrecto. No utiliza nexos gramaticales comparativos de semejanza explícitos; la correspondencia es de identificación directa. -->
- [x] B) Metáfora de equivalencia de identificación del plano real (colón) con el imaginario (oro) para realzar el gran valor del trabajo rústico del labriego.
  <!-- feedback: Correcto. Asocia de forma directa el colón costarricense con el oro para dignificar económicamente y dar alto valor moral al jornalero. -->
- [ ] C) Sinécdoque de la parte por el todo de los bancos de la capital.
  <!-- feedback: Incorrecto. No hay relaciones cuantitativas de inclusión de parte-todo con el sistema bancario. -->
- [ ] D) La ironía de la moneda de colones costarricense emitida por el MEP.
  <!-- feedback: Incorrecto. El tono es de elogio, orgullo e identidad, no de sátira, desprecio o burla del signo monetario. -->

### Explicacion Pedagogica
La metáfora monetaria (colón-oro) dignifica y exalta el fruto del esfuerzo físico del campesino costarricense, confiriéndole la máxima nobleza material y moral de la riqueza agraria nacional.

---

## Question 15 [D8]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v15
**Bloom:** Evaluate
**EJE:** Análisis de textos
**Expected_Success:** 0.45
**Contexto:** Un crítico costarricense analiza una composición lírica de Heredia dedicada al cuidado del agua. Uno de sus versos dice:
'El agua clara limpia de agua dulce es un caudal puro de agua de lluvia de la provincia de Heredia.'

### Enunciado
Al evaluar la consistencia estilística y la riqueza retórica de este verso, ¿cuál es la principal flaqueza de redacción que presenta?

### Opciones
- [ ] A) El uso del epíteto 'agua dulce' para referirse a los ríos.
  <!-- feedback: Incorrecto. Emplear adjetivos explicativos correctos no constituye un defecto de redacción por sí solo. -->
- [x] B) La redundancia léxica extrema y la pobreza de vocabulario poético al repetir la palabra 'agua' de forma continua e ineficaz sin ritmo ni justificación estética.
  <!-- feedback: Correcto. La reiteración de vocablos de forma burda en un solo verso denota falta de recursos lingüísticos poéticos. -->
- [ ] C) La falta de inclusión de datos sobre la turbidez emitidos por el AyA.
  <!-- feedback: Incorrecto. La poesía lírica no requiere de informes de laboratorios químicos o mediciones de sanidad. -->
- [ ] D) Sostener que el agua dulce fluye por la provincia de Heredia.
  <!-- feedback: Incorrecto. Heredia posee cuencas hídricas reales y ríos de agua dulce de gran belleza natural; es geográficamente real. -->

### Explicacion Pedagogica
La creación lírica de calidad exige evitar redundancias viciosas, prefiriendo la cadencia musical, la variedad léxica, el rigor de la métrica y la precisión de sus metáforas.

---

## Question 16 [D8]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v16
**Bloom:** Evaluate
**EJE:** Análisis de herencia
**Expected_Success:** 0.42
**Contexto:** Considere los siguientes versos de un poema de corte amoroso-existencial costarricense:
'Tu sonrisa brillante es un destello de sol en la tarde gris de Alajuela;
tu mirada de cielo azul disipa los negros nubarrones de mi tristeza existencial.'

### Enunciado
¿Cómo se califica la coherencia semántica de las metáforas de esta estrofa?

### Opciones
- [ ] A) Incoherente debido a que Alajuela es una provincia calurosa y no posee días nublados.
  <!-- feedback: Incorrecto. Alajuela experimenta días nublados reales durante el invierno de clima tropical, lo cual es geográficamente real. -->
- [x] B) Altamente coherentemente, ya que utiliza de forma sistemática la contraposición semántica entre elementos de luz (sonrisa brillante, destello de sol, cielo azul) frente a elementos de oscuridad (tarde gris, negros nubarrones) para expresar con belleza lírica el efecto del ser amado.
  <!-- feedback: Correcto. Describe la perfecta armonía estructural y el sentido de las antítesis de luz y sombra en la poesía amorosa. -->
- [ ] C) Poco sólida ya que debió redactarse utilizando el voseo de la provincia.
  <!-- feedback: Incorrecto. La lírica seria puede redactarse perfectamente utilizando el pronombre de tratamiento de respeto o variantes estándar. -->
- [ ] D) Inválida debido a que el MEP prohíbe la redacción de poemas de amor en los colegios.
  <!-- feedback: Incorrecto. El MEP no prohíbe las temáticas amorosas clásicas en la creación poética escolar, las promueve como parte del desarrollo del lenguaje. -->

### Explicacion Pedagogica
La consistencia semántica en la lírica amorosa tradicional se consolida mediante la estructuración armónica de tropos polares de luz y sombra que traducen de forma estética los estados emocionales del yo lírico.

---

## Question 17 [D9]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v17
**Bloom:** Evaluate
**EJE:** Análisis de herencia
**Expected_Success:** 0.38
**Contexto:** Considere los siguientes versos de un poema costumbrista dedicado a las leyendas de Cartago:
'El Cadejos, perro de ojos de fuego encendido, corre silencioso como un gato de adobes por el potrero oscuro.'

### Enunciado
Al analizar la coherencia estilística de la comparación 'silencioso como un gato de adobes', ¿cuál es su valor y consistencia estética?

### Opciones
- [ ] A) Inconsistente ya que los adobes de la provincia de Cartago no se mueven ni tienen gatos de arcilla.
  <!-- feedback: Incorrecto. Es un tropo poético de color local, no una descripción científica física de adobes. -->
- [x] B) Es una comparación de gran color local costarricense que integra la rústica arquitectura tradicional (los adobes) con el sigilo del animal para caracterizar el andar misterioso de la criatura mítica.
  <!-- feedback: Correcto. El 'gato de adobes' evoca una imagen rural típica costarricense de gran valor identitario costumbrista. -->
- [ ] C) Demuestra que el autor posee un gran manejo de los antibióticos del perro doméstico.
  <!-- feedback: Incorrecto. Es un análisis estético de figuras retóricas, libre de variables de veterinaria o medicina animal. -->
- [ ] D) Exige la demolición de todas las casas tradicionales coloniales del país.
  <!-- feedback: Incorrecto. No tiene propósitos legislativos o de demolición de la provincia; el fin es de color local. -->

### Explicacion Pedagogica
La literatura costumbrista de Costa Rica enriquece sus comparaciones y símiles valiéndose de elementos cotidianos de la vida agraria (como los adobes o las carretas), logrando un arraigo estético.

---

## Question 18 [D9]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v18
**Bloom:** Evaluate
**EJE:** Análisis de herencia
**Expected_Success:** 0.35
**Contexto:** Analice este fragmento de un poema contemporáneo escrito en San José:
'El reloj de la pared devora mis segundos de juventud con su tictac de hierro,
convirtiéndome en una hoja seca que el viento del tiempo arrastra hacia la fosa de adobes.'

### Enunciado
¿Qué visión existencial del tiempo se proyecta con fuerza poética en la metáfora principal del fragmento anterior?

### Opciones
- [ ] A) La celebración gozosa de la longevidad y el envejecimiento de la población costarricense.
  <!-- feedback: Incorrecto. El tono es de angustia y asfixia frente al paso destructor del tiempo, no de gozo o celebración. -->
- [x] B) La visión trágica y dolorosa del paso inexorable del tiempo (tempus fugit) que consume de forma implacable la vida humana y arrastra al sujeto hacia la muerte inevitable.
  <!-- feedback: Correcto. El reloj de hierro que devora segundos y la hoja seca arrastrada metaforizan de forma exacta el dolor de la fugacidad existencial humana. -->
- [ ] C) La exigencia de colocar relojes de adobes en todas las plazas públicas del país.
  <!-- feedback: Incorrecto. El reloj es un símbolo poético de la fugacidad del tiempo, no una propuesta de decoración urbana municipal. -->
- [ ] D) La descripción física de las oscilaciones pendulares de un reloj mecánico de la provincia.
  <!-- feedback: Incorrecto. Es un análisis lírico de la provincia. -->

### Explicacion Pedagogica
El tropo de la hoja seca y el tiempo destructor (cronófago) representa el tópico clásico de la fugacidad de la vida humana, traduciendo la angustia existencial del yo lírico frente a la finitud de la carne.

---

## Question 19 [D10]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v19
**Bloom:** Understand
**EJE:** Análisis de textos
**Expected_Success:** 0.32
**Contexto:** Un grupo de estudiantes de undécimo año de Heredia repasa la clasificación de las figuras de construcción.

### Enunciado
¿Cuál es el nombre de la figura sintáctica que consiste en repetir de forma deliberada las conjunciones copulativas (como 'y') en una estrofa poética para dar lentitud al ritmo del verso?

### Opciones
- [ ] A) Asíndeton por supresión de conjunciones sintácticas.
  <!-- feedback: Incorrecto. El asíndeton hace lo contrario: elimina las conjunciones para dar rapidez y velocidad al verso. -->
- [x] B) Polisíndeton.
  <!-- feedback: Correcto. El polisíndeton acumula conjunciones coordinantes de forma voluntaria para dotar de pesadez y lentitud expresiva al poema. -->
- [ ] C) Anáfora al inicio del verso de la provincia.
  <!-- feedback: Incorrecto. Reitera vocablos al comienzo de versos consecutivos, no conjunciones copulativas a lo largo de la frase. -->
- [ ] D) Metáfora del buey cansado de Cartago.
  <!-- feedback: Incorrecto. Es un tropo semántico de correspondencia, no una figura de construcción sintáctica oracional de conjunciones. -->

### Explicacion Pedagogica
El polisíndeton retarda de forma deliberada el fluir sintáctico del poema, confiriéndole solemnidad y acentuando de forma individual cada uno de los elementos de la serie.

---

## Question 20 [D10]
**ID:** CR-LEN-11-2026-W10-tema-w10-001-MASTERY-bundle-v20
**Bloom:** Remember
**EJE:** Análisis de textos
**Expected_Success:** 0.30
**Contexto:** Durante una asamblea de Español en Alajuela, se repasa la definición de la figura del 'hipérbaton'.

### Enunciado
¿En qué consiste el hipérbaton como figura de construcción en el idioma español?

### Opciones
- [ ] A) En la exageración de los rasgos psicológicos de un campesino.
  <!-- feedback: Incorrecto. Esto constituye la definición de la etopeya de exageración o caricatura moral de carácter retórico. -->
- [x] B) En la alteración intencionada de la disposición regular u orden sintáctico lógico de las palabras de la oración (sujeto, verbo, complementos) en el escrito.
  <!-- feedback: Correcto. El hipérbaton fractura la estructura lineal lógica del idioma con fines rítmicos poéticos clásicos. -->
- [ ] C) En la descripción de las playas de la provincia de Puntarenas.
  <!-- feedback: Incorrecto. Se trata de un recurso de geografía descriptiva (topografía), no de una alteración sintáctica oracional formal. -->
- [ ] D) En la abolición de las clases de literatura por decretos del MEP.
  <!-- feedback: Incorrecto. Es un recurso lingüístico estético literario formal clásico de la lengua, ajeno a normativas burocráticas. -->

### Explicacion Pedagogica
El hipérbaton rompe la secuencia lógica regular sintáctica del español, emulando la estructura clásica de la sintaxis latina clásica con una alta finalidad de sonoridad métrica y distinción literaria.
