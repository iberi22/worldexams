# [Ola C3.03] feat-cuentos-a-nieve-osa — Nieve la osa polar y la promesa del hielo

> Wave C3 — Cuentos 01-05 (content). No `jules` label yet (dispatch protocol: create
> without label, verify islands, then label in one loop). Merge order: 03/5 | Risk: LOW | Effort: Medium 3-5h

---

## 1. Current State (MEASURABLE)

- File: `questions_data/cuentos/nieve-osa-hielo/cuento.md` DOES NOT EXIST (`ls` 0 files) — greenfield issue.
...
Create `questions_data/cuentos/nieve-osa-hielo/cuento.md` with the FULL story text in neutral
...
**Frontmatter (exact keys):** `slug: "nieve-osa-hielo"`, `titulo`, `edad: "4-5"`,
`idioma: "es-neutro"`, `eje: "animales"`, `habitat: "tundra"`, `valor: "responsabilidad"`,
...
1) El hielo donde juega Nieve se adelgaza
2) Glauco el buho explica: el mundo se calienta cuando desperdiciamos
3) Nieve promete cuidar
4) Apaga la luz de la cueva (ahorro) y recoge restos
5) Ensena a Pipo la foca a cuidar tambien
6) Tormenta: se refugian y comparten calor
7) El hielo aguanta el invierno
8) Moraleja: lo pequeno que cuidas, te cuida
...
### Pregunta 1 — Que le pasaba al hielo? (correcta: se adelgazaba / se derretia)
### Pregunta 2 — Que prometio Nieve? (correcta: cuidar / no desperdiciar)
### Pregunta 3 — Que puedes cuidar tu en casa? (correcta: abierta guiada al ahorro — luz, agua; acepta la respuesta de cuidado concreta)

**Art:** 2-4 character SVGs in `personajes/` (signature color #BFD9E8; viewBox
...
1. search: "polar bear cub facts for kids simple"
2. search: "tundra arctic habitat animals children"
3. search: "ice melts water cycle explanation preschool"
4. search: "aurora borealis simple explanation kids"
5. search: "neutral Spanish escritura infantil consejos"

## 4. Agent Session Prompt

"Before writing, please:
1. Research the topics above and note 3-5 verified facts usable for ages 4-5.
2. Plan Nieve (osezna blanca, firma #BFD9E8), Pipo (foca), Glauco (buho sabio amable) + 8 ice scenes (icebergs, aurora, cueva, tormenta suave, nunca aterradora).
2. Read `docs/CUENTOS/00_BIBLIA.md` (this cuento's beats + quiz), `docs/CUENTOS/01_DIRECCION_ARTE.md` (palette, signature color #BFD9E8), and `docs/CUENTOS/02_COPYRIGHT_Y_FORMATO.md` (format v1 + veto list).
...
- `docs/CUENTOS/01_DIRECCION_ARTE.md` §§2-4 → palette (#FDF6EC bg, #3A2E2A ink), signature color #BFD9E8, scene spec (800×450, 3 planes, ≤60 nodes), piezas reuse.
- `questions_data/cuentos/LICENSE-CONTENT.md` → legal header text.

## 6. Acceptance Criteria (COMMAND-VERIFIABLE)

Validator: `node saberparatodos/scripts/validate_cuentos.js questions_data/cuentos/nieve-osa-hielo/cuento.md` (planned in C1.02 — **if the file does not exist yet, the fallback grep checks below are authoritative** and the agent must note `validator-missing-fallback-used` in the PR body).

- [ ] `test -f questions_data/cuentos/nieve-osa-hielo/cuento.md && head -1 questions_data/cuentos/nieve-osa-hielo/cuento.md | grep -c "© 2026 SaberParaTodos"` == 1
- [ ] `grep -c '^slug: "nieve-osa-hielo"' questions_data/cuentos/nieve-osa-hielo/cuento.md` == 1 && `grep -c 'license: "PROPRIETARY-FREE-READ"' questions_data/cuentos/nieve-osa-hielo/cuento.md` == 1 && `grep -c '^paginas: 8' questions_data/cuentos/nieve-osa-hielo/cuento.md` == 1
- [ ] `grep -c '^## Pagina' questions_data/cuentos/nieve-osa-hielo/cuento.md` == 8
- [ ] Every page 30-80 words: `awk '/^## Pagina/{if(n)print n,w; n=1; w=0; next} /^## Quiz/{print n,w; exit} /^!\[/{next} /^#/{next} /./{w+=NF}' questions_data/cuentos/nieve-osa-hielo/cuento.md` → all 8 counts in [30,80]
- [ ] `grep -c '^### Pregunta' questions_data/cuentos/nieve-osa-hielo/cuento.md` == 3 && `grep -c '^- \[ \]' questions_data/cuentos/nieve-osa-hielo/cuento.md` == 6 && `grep -c '^- \[x\]' questions_data/cuentos/nieve-osa-hielo/cuento.md` == 3 && `grep -c '<!-- feedback:' questions_data/cuentos/nieve-osa-hielo/cuento.md` == 9 && `grep -c '^### Explicacion' questions_data/cuentos/nieve-osa-hielo/cuento.md` == 1
- [ ] `grep -c '!\[alt:' questions_data/cuentos/nieve-osa-hielo/cuento.md` == 8 (alt text on every scene)
- [ ] Characters: `ls questions_data/cuentos/nieve-osa-hielo/personajes/*.svg | wc -l` in 2..4 && scenes: `ls questions_data/cuentos/nieve-osa-hielo/escenas/*.svg | wc -l` == 8
- [ ] `find questions_data/cuentos/nieve-osa-hielo/personajes -name '*.svg' -size +25k | wc -l` == 0 && `find questions_data/cuentos/nieve-osa-hielo/escenas -name '*.svg' -size +60k | wc -l` == 0
- [ ] `grep -ril 'base64\|<image' questions_data/cuentos/nieve-osa-hielo/ | wc -l` == 0 (pure vector, per 01_DIRECCION_ARTE.md §7) && every SVG: `head -1 <f> | grep -c '© 2026 SaberParaTodos'` == 1 && `grep -L 'viewBox' questions_data/cuentos/nieve-osa-hielo/*/*.svg | wc -l` == 0
- [ ] Neutral-Spanish veto list: `grep -rEic "pesos|dólares|dolares|euros|soles|quetzal|guaraní|guarani|lempira|córdoba|cordoba|balboa|colón|colon|vos|tenés|tenes|hacé|hace|mirá|mira|che|parce|tío|compa|chido|chévere|chevere|bac[aá]n" questions_data/cuentos/nieve-osa-hielo/cuento.md` == 0 (see 02_COPYRIGHT_Y_FORMATO.md §2)
- [ ] BR-03: `grep -ric 'swal\|\$swal\|karma\|telemet' questions_data/cuentos/nieve-osa-hielo/ | wc -l` == 0 (zero $SWAL tokens, karma, telemetry in kids flows)

## 7. Files to Modify

| File | Current State | Change | Risk |
|------|--------------|--------|------|
| `questions_data/cuentos/nieve-osa-hielo/cuento.md` | NEW | Full story 8 pages 30-80 words/page + quiz 3Q x 3 options + Explicacion | LOW |
| `questions_data/cuentos/nieve-osa-hielo/personajes/nieve.svg` | NEW | Osezna blanca, firma #BFD9E8, common eyes, < 25 KB | LOW |
| `questions_data/cuentos/nieve-osa-hielo/personajes/pipo.svg` | NEW | Foca Pipo, same pattern, < 25 KB | LOW |
| `questions_data/cuentos/nieve-osa-hielo/personajes/glauco.svg` | NEW | Buho Glauco sabio y amable, same pattern, < 25 KB | LOW |
| `questions_data/cuentos/nieve-osa-hielo/escenas/p1-*.svg` … `p8-*.svg` | NEW (8) | Hielo/aurora/cueva, 800x450, reuse piezas, < 60 KB each | LOW |
...
- [ ] `git diff --stat HEAD` is NOT empty; the PR MUST contain `questions_data/cuentos/nieve-osa-hielo/cuento.md` in the diff (`git show HEAD --name-only | grep -c 'cuentos/nieve-osa-hielo/cuento.md'` >= 1)
- [ ] All SVGs listed: `git show HEAD --name-only | grep -c 'cuentos/nieve-osa-hielo/.*\.svg'` >= 10 (characters + 8 scenes)
- [ ] `wc -l questions_data/cuentos/nieve-osa-hielo/cuento.md` >= 60 (full story + quiz, never a stub)
...
```bash
node saberparatodos/scripts/validate_cuentos.js questions_data/cuentos/nieve-osa-hielo/cuento.md 2>&1 | tail -5 || echo "VALIDATOR-MISSING-USING-FALLBACK-GREPS"
grep -c '^## Pagina' questions_data/cuentos/nieve-osa-hielo/cuento.md  # expect 8
grep -c '^- \[x\]' questions_data/cuentos/nieve-osa-hielo/cuento.md  # expect 3
grep -c '<!-- feedback:' questions_data/cuentos/nieve-osa-hielo/cuento.md  # expect 9
grep -rEic "pesos|dólares|dolares|euros|soles|quetzal|guaraní|guarani|lempira|córdoba|cordoba|balboa|colón|colon|vos|tenés|tenes|hacé|hace|mirá|mira|che|parce|tío|compa|chido|chévere|chevere|bac[aá]n" questions_data/cuentos/nieve-osa-hielo/cuento.md  # expect 0
grep -ril 'base64\|<image' questions_data/cuentos/nieve-osa-hielo/ | wc -l  # expect 0
find questions_data/cuentos/nieve-osa-hielo -name '*.svg' | wc -l  # expect >= 10
find questions_data/cuentos/nieve-osa-hielo/personajes -name '*.svg' -size +25k | wc -l  # expect 0
```
